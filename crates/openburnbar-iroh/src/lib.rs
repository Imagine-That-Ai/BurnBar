...
mod tests {
    ...
    fn concurrent_handshake_cap_blocks_when_slots_are_exhausted() {
        ...
        let Ok(p1) = admission.handshake_slots.try_acquire() else {
            panic!("first of two configured handshake permits must be acquirable");
        };
        let Ok(p2) = admission.handshake_slots.try_acquire() else {
            panic!("second of two configured handshake permits must be acquirable");
        };
