...
- **MDASH-006:** Path-bound AAD is not enforced for current `sealedPayload`/`sealedText` writes — `validCloudSealedPayload` accepts a global AAD or any regex-matching AAD, and readers fall back to the global branch. A malicious same-user client can transplant/replay ciphertext across docs/fields/collections that are claimed to be path-bound. Fix before claiming "binds AES-GCM payloads to uid+collection+doc+field" without caveat.
...
- **MDASH-005 / MDASH-013:** iOS now pins the host key (post-pairing substitution refused), but the safety-number confirmation gate is default-off so first-contact remains TOFU. **Android has no host-key pin at all** and fetches the key from Firestore cache, reopening the cloud-substitution/MITM vector on Android. Do not claim the control channel is pinned across platforms.
...
- **MDASH-007:** `users/{uid}/devices` is owner-writable with no schema allowlist — a malicious same-user client can inject attacker-controlled push tokens and receive agent-reply notifications / call invites.
- **MDASH-008:** Mission/agent-import `claimedBy` is spoofable via any trusted macOS escrow device ID — a same-user client can claim and execute missions as the Mac without Mac-signed proof.
- **MDASH-009:** Initial `cloud_vault_state/current` creation is client-writable — a same-user client can race first-setup and lock the legitimate device out of CloudVault.
- **MDASH-024:** `cloud_vault_key_wrappers` allows direct client writes without writer/source binding, enabling wrapper tampering/DoS within the same account.
...
- **MDASH-026:** VoIP/call push forwards an unbounded caller `displayName` to APNs/FCM — metadata leakage beyond the already-accepted caller-name caveat.
- **MDASH-034:** Agent-reply push deep link encodes `threadId`, exposing a stable conversation identifier to push providers.
