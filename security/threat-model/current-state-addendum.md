...
Additional implementation pass: the current worktree now hard-gates dangerous CLI autonomy flags behind fresh local-auth proof in every build, fixes the unrestricted-shell reauth cadence, wires mobile security-control sources into the Xcode target, and passes focused macOS/mobile/Android/server/security test slices. This does not prove production deployment.
...
| Dangerous agent autonomy (`T-TOOL-02`, `T-AI-07`) | Trusted/YOLO grant paths could emit dangerous external CLI flags without local-auth proof in non-distribution builds; reauth cadence could remain stuck requiring proof after a successful proof. | `AgentLens/Services/CLIBridge/AgentSecurityPolicy.swift`; `AgentLensTests/Active/AgentSecurityPolicyTests.swift`; `AgentLensTests/Active/CLIBridgeTests.swift`; focused `xcodebuild test` macOS security slice. | Dangerous CLI flags now require fresh local-auth proof in every build; without proof the CLI falls back to sandboxed/approval mode. The reauth cadence now gates first use and then permits only the configured bounded action window before the next proof. | The product path that mints and passes fresh local-auth proof must remain explicit and auditable. External CLI agents still cannot be fully interposed once delegated. |
...
1. Keep `--dangerously-skip-permissions`, `--dangerously-bypass-approvals-and-sandbox`, and `runShellUnrestricted` fail-closed unless a fresh local-auth proof is present; wire any product path that mints that proof to explicit, auditable user intent.
