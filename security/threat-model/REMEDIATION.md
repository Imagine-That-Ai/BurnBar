...
- `T-TOOL-02`: `AgentDistributionGate.allowsDangerousAutonomyFlag` now fails closed in every build, including debug/developer builds and even with fresh local-auth proof. Fresh local auth gates OpenBurnBar broker actions, not vendor process bypass flags. The per-N unrestricted-shell reauth cadence now distinguishes "no proof yet" from "proof recorded, zero actions consumed," so the first action is gated and a successful proof opens a bounded action window.
- `T-TOOL-10`: the restricted `shell_run` Seatbelt profile now uses default-deny operations, explicitly withholds the network operation family, confines writes to the workspace, and denies file data under the user's home directory by default except for the active workspace and narrowly scoped toolchain/cache roots needed by developer workflows.
