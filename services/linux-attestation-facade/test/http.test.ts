...
import { fixture, MemoryEnrollments, MemoryObjects, MemoryState } from "./helpers.js";
...
const registrar: RegistrarClient = { async begin() { return { activationBlob: "blob" }; }, async activate() {} };

describe("HTTP boundaries", () => {
  it("requires a Firebase bearer token before public request parsing", async () => {
    const service = new IngressService(new MemoryState(), new MemoryObjects(), new MemoryEnrollments(), registrar, { maxEvidenceBytes: 32, uploadTtlMillis: 1000, enrollmentLeaseMillis: 75_000 });
  ...
  it("rejects unknown public JSON fields and oversized bodies", async () => {
    const service = new IngressService(new MemoryState(), new MemoryObjects(), new MemoryEnrollments(), registrar, { maxEvidenceBytes: 32, uploadTtlMillis: 1000, enrollmentLeaseMillis: 75_000 });
    const base = await start(createIngressServer(service, userAuth, { jsonBodyLimit: 256, evidenceBodyLimit: 32 }));
    const response = await fetch(`${base}/v1/evidence-uploads`, {
      method: "POST", headers: { authorization: "Bearer valid-user-token", "content-type": "application/json" },
  ...
  it("returns retryable sanitized errors for Firestore and GCS outages", async () => {
    ...
    const createService = new IngressService(new UnavailableState(), new MemoryObjects(), new MemoryEnrollments(), registrar, { maxEvidenceBytes: 32, uploadTtlMillis: 1000, enrollmentLeaseMillis: 75_000 });
    ...
    const uploadService = new IngressService(state, new UnavailableObjects(), new MemoryEnrollments(), registrar, { maxEvidenceBytes: 32, uploadTtlMillis: 1000, enrollmentLeaseMillis: 75_000 }, () => 1_000, () => "upload-1");
    await uploadService.createUpload("user-1", { appId: "app", deviceId: "device", challengeId: "challenge", releaseDigestSha256: "a".repeat(64), expectedSha256: "ee8250fb76e094b34b471f13a73dbbe51d1ae142e9df59d7c0d31ec20f0a0a8e", expectedSize: 8 });
