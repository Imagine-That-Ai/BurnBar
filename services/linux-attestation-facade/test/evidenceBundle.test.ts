...
function header(bundle: Buffer): {
  ...
  return {
    length,
    value: JSON.parse(bundle.subarray(12, 12 + length).toString()) as {
      schemaVersion: number;
      records: Array<{
        kind: string;
        offset: number;
...
function rewriteCanonical(
  bundle: Buffer,
  mutate: (value: ReturnType<typeof header>["value"]) => void,
): Buffer {
  ...
  return Buffer.concat([
    ...
    bytes,
    bundle.subarray(12 + parsed.length),
  ]);
}
...
describe("OBBATST1 parser", () => {
  ...
  it("rejects a noncanonical JSON header even when it is valid JSON", () => {
    ...
    const bytes = Buffer.concat([
      ...
      noncanonical,
      original.subarray(12 + parsed.length),
    ]);
    assert.throws(() => parseEvidenceBundle(bytes), PublicError);
  });
  ...
  it("rejects deeply nested untrusted headers without recursive canonicalization", () => {
    ...
    const headerBytes = Buffer.from(
      `{"records":[${nested},{},{},{}],"schemaVersion":1}`,
    );
...
describe("installed manifest verification", () => {
  it("accepts a valid signed manifest bound to the challenge", () => {
    ...
    verifyInstalledManifest(
      parsed,
      data.challenge,
      policy.releaseManifestPublicKeyPem,
    );
  });
  ...
  it("rejects detached signature and binding mismatches", async () => {
    ...
    assert.throws(
      () =>
        verifyInstalledManifest(
          { ...parsed, installedManifestSignature: signature },
          data.challenge,
          policy.releaseManifestPublicKeyPem,
        ),
      PublicError,
    ...
    const noNewlineSignature = sign(
      null,
      withoutNewline,
      releaseKeys.privateKey,
    );
    const noNewlineChallenge = structuredClone(data.challenge);
    noNewlineChallenge.releaseDigestSha256 = (
      await import("../src/validation.js")
    ).sha256(withoutNewline);
    assert.throws(
      () =>
        verifyInstalledManifest(
          {
            ...parsed,
            installedManifest: withoutNewline,
            installedManifestSignature: noNewlineSignature,
          },
          noNewlineChallenge,
          policy.releaseManifestPublicKeyPem,
        ),
  ...
  it("rejects malformed authorized clients, file entries, and inventory roots after valid signing", async () => {
    const cases: Array<(manifest: Record<string, unknown>) => void> = [
      (manifest) => {
        (
          manifest.authorizedClients as Array<Record<string, unknown>>
        )[0]!.mode = 0o777;
      },
      (manifest) => {
        (manifest.files as Array<Record<string, unknown>>)[0]!.extra = true;
      },
      (manifest) => {
        manifest.installedFilesRootSha256 = "f".repeat(64);
      },
    ];
      ...
      assert.throws(
        () =>
          verifyInstalledManifest(
            {
              ...parsed,
              installedManifest: manifestBytes,
              installedManifestSignature: signature,
            },
            data.challenge,
            policy.releaseManifestPublicKeyPem,
          ),
