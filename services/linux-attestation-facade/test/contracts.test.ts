...
import {
  parseVerifyRequest,
  quoteQualifyingDataSha256,
} from "../src/contracts.js";
import { PublicError } from "../src/errors.js";
import { fixture } from "./helpers.js";

describe("strict verifier contract", () => {
  it("accepts the exact protocol contract", () =>
    assert.deepEqual(parseVerifyRequest(fixture().request), fixture().request));
  it("rejects additional properties at every boundary", () => {
    const request = fixture().request as unknown as Record<string, unknown>;
    request.extra = true;
    assert.throws(
      () => parseVerifyRequest(request),
      (error: unknown) =>
        error instanceof PublicError && error.code === "bad_request",
    );
  });
  it("rejects uppercase digests instead of normalizing them", () => {
    const request = fixture().request;
    request.evidence.upload.sha256 =
      request.evidence.upload.sha256.toUpperCase();
    assert.throws(
      () => parseVerifyRequest(request),
      (error: unknown) =>
        error instanceof PublicError && error.code === "bad_request",
  ...
  it("derives quote qualifying data from the exact nine-field NUL-delimited contract", async () => {
    ...
    const fields = [
      Buffer.from("openburnbar.linux.tpm-quote.v1"),
      Buffer.from(challenge.challenge, "base64url"),
      Buffer.from(challenge.appId),
      Buffer.from(challenge.deviceId),
      Buffer.from(challenge.appVersion),
      Buffer.from(challenge.architecture),
      Buffer.from(challenge.releaseDigestSha256),
      Buffer.from(challenge.policyId),
      Buffer.from(challenge.attestationKind),
    ];
    ...
    assert.equal(
      quoteQualifyingDataSha256(challenge),
      (await import("../src/validation.js")).sha256(expected),
    );
  ...
  it("mirrors Packet A broker labels and quote PCR fields", () => {
    ...
    assert.equal(
      parseVerifyRequest(request).evidence.quote.quotePcrValuesBase64,
      request.evidence.quote.quotePcrValuesBase64,
    );
  ...
  it("rejects noncanonical and broker-oversized standard base64 quote fields", () => {
    ...
    oversized.evidence.quote.quotePcrValuesBase64 =
      Buffer.alloc(12_289).toString("base64");
  ...
  it("rejects evidence metadata above the ingress descriptor ceiling", () => {
    const request = fixture().request;
    request.evidence.upload.size = 16 * 1024 * 1024 + 1;
    assert.throws(() => parseVerifyRequest(request), PublicError);

    const bundle = fixture().request;
    bundle.evidence.evidenceBundle.byteLength = 16 * 1024 * 1024 + 1;
    assert.throws(() => parseVerifyRequest(bundle), PublicError);
  });
});
