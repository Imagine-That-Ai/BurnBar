...
import type { AttestationPolicy, EnrollmentRecord, EnrollmentStore, EvidenceObjectStore, KeylimeResult, KeylimeVerifier, PolicyStore, UploadRecord, UploadStateStore, VerdictSigner } from "../src/ports.js";
...
export class MemoryEnrollments implements EnrollmentStore {
  ...
  async deletePending(uid: string, deviceId: string, agentId: string): Promise<void> {
    this.records = this.records.filter(value => value.uid !== uid || value.deviceId !== deviceId || value.agentId !== agentId || value.active);
  }
  async activate(uid: string, deviceId: string, agentId: string): Promise<void> {
    const record = this.records.find(value => value.uid === uid && value.deviceId === deviceId && value.agentId === agentId && !value.active);
    if (record === undefined) throw new PublicError(409, "conflict", "Enrollment state changed");
    record.active = true;
