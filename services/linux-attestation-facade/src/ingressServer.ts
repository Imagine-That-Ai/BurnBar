...
import { ingressTicketFromRequest } from "./ingressTicket.js";
import type { UserAuthenticator } from "./ports.js";
import { bearerToken, empty, handleError, json, readExactBody, readJson } from "./http.js";
  ...
  return createServer(async (request, response) => {
    try {
      ...
      if (request.method === "POST" && url.pathname === "/v1/enroll/begin") {
        const ticket = ingressTicketFromRequest(request);
        return json(response, 200, await service.beginEnrollment(identity.uid, ticket, parseBeginEnrollment(await readJson(request, options.jsonBodyLimit))));
      }
      ...
      if (request.method === "POST" && url.pathname === "/v1/evidence-uploads") {
        const ticket = ingressTicketFromRequest(request);
        return json(response, 201, await service.createUpload(identity.uid, ticket, parseCreateUpload(await readJson(request, options.jsonBodyLimit))));
      }
      const uploadMatch = /^\/v1\/evidence-uploads\/([A-Za-z0-9._:-]+)$/.exec(url.pathname);
      if (request.method === "PUT" && uploadMatch?.[1] !== undefined) {
        const claim = await service.beginUploadAttempt(identity.uid, uploadMatch[1]);
        const bytes = await readExactBody(request, claim.expectedSize, options.evidenceBodyLimit);
        return json(response, 200, await service.upload(identity.uid, uploadMatch[1], bytes, claim));
      }
