...
export function parseBeginEnrollment(value: unknown): {
  deviceId: string;
  ...
  exactKeys(
    source,
    ["deviceId", "ekCertificateBase64", "ekTpmBase64", "akTpmBase64"],
    "request",
  );
  return {
    deviceId: brokerLabel(source.deviceId, "deviceId"),
    ekCertificateBase64: base64(
      source.ekCertificateBase64,
      "ekCertificateBase64",
      128 * 1024,
    ),
...
export function parseCompleteEnrollment(value: unknown): {
  deviceId: string;
  ...
  return {
    deviceId: brokerLabel(source.deviceId, "deviceId"),
    activationProof: base64(
      source.activationProof,
      "activationProof",
      128 * 1024,
    ),
  };
