...
const challengeKeys = ["uid", "appId", "deviceId", "appVersion", "architecture", "releaseDigestSha256", "policyId", "attestationKind", "challengeId", "challenge", "expiresAtMillis", "protocolVersion"] as const;
...
export function parseChallenge(value: unknown): AttestationChallenge {
  ...
  if (source.attestationKind !== ATTESTATION_KIND || source.protocolVersion !== PROTOCOL_VERSION) {
  ...
  if (Buffer.from(challenge, "base64url").byteLength !== 32) throw new PublicError(400, "bad_request", "challenge.challenge is invalid");
  return {
    ...
    architecture: (() => {
      const architecture = brokerLabel(source.architecture, "challenge.architecture");
      if (architecture !== "aarch64" && architecture !== "x86_64") throw new PublicError(400, "bad_request", "challenge.architecture is invalid");
      return architecture;
    })(),
    releaseDigestSha256: sha256Hex(source.releaseDigestSha256, "challenge.releaseDigestSha256"),
    ...
    challenge,
    expiresAtMillis: integer(source.expiresAtMillis, "challenge.expiresAtMillis", 1, Number.MAX_SAFE_INTEGER),
    protocolVersion: PROTOCOL_VERSION,
  };
}

export function quoteQualifyingDataSha256(challenge: AttestationChallenge): string {
  const fields = [
  ...
  return sha256(Buffer.concat(fields.flatMap((field, index) => index === fields.length - 1 ? [field] : [field, separator])));
}

export function parseEvidenceReceipt(value: unknown): EvidenceReceipt {
  const source = object(value, "evidence.upload");
  exactKeys(source, ["uploadId", "generation", "sha256", "size"], "evidence.upload");
  return {
...
export function parseVerifyRequest(value: unknown): VerifyRequest {
  ...
  exactKeys(evidence, ["schemaVersion", "quote", "evidenceBundle", "upload"], "evidence");
  if (evidence.schemaVersion !== 1) throw new PublicError(400, "bad_request", "Unsupported evidence schema");
  const quote = object(evidence.quote, "evidence.quote");
  exactKeys(quote, ["schemaVersion", "deviceId", "quoteAttestationBase64", "quoteSignatureBase64", "quotePcrValuesBase64", "pcrBank", "pcrSelection", "qualifyingDataSha256"], "evidence.quote");
  if (quote.schemaVersion !== 1 || quote.pcrBank !== "sha256" || !Array.isArray(quote.pcrSelection) || quote.pcrSelection.length !== 5 || quote.pcrSelection.some((value, index) => value !== [0, 2, 4, 7, 10][index])) {
  ...
  const quoteEvidence: QuoteEvidence = {
    ...
    quoteAttestationBase64: base64(quote.quoteAttestationBase64, "evidence.quote.quoteAttestationBase64", 12_288),
    quoteSignatureBase64: base64(quote.quoteSignatureBase64, "evidence.quote.quoteSignatureBase64", 3_072),
    quotePcrValuesBase64: base64(quote.quotePcrValuesBase64, "evidence.quote.quotePcrValuesBase64", 12_288),
    pcrBank: "sha256",
    pcrSelection: [0, 2, 4, 7, 10],
    qualifyingDataSha256: sha256Hex(quote.qualifyingDataSha256, "evidence.quote.qualifyingDataSha256"),
  };
  const bundle = object(evidence.evidenceBundle, "evidence.evidenceBundle");
  exactKeys(bundle, ["descriptorIndex", "format", "byteLength", "sha256"], "evidence.evidenceBundle");
  if (bundle.descriptorIndex !== 0 || bundle.format !== "openburnbar_tpm_evidence_bundle_v1") throw new PublicError(400, "bad_request", "Unsupported evidence bundle");
  return {
...
export function canonicalVerdict(verdict: SignedVerdict): string {
  return [
    verdict.v, verdict.issuer, verdict.audience, verdict.decision, verdict.uid,
    verdict.appId, verdict.deviceId, verdict.appVersion, verdict.architecture,
    verdict.releaseDigestSha256, verdict.policyId, verdict.attestationKind,
    verdict.challengeId, verdict.challengeHashSha256, verdict.trustClass,
    verdict.verifierReceiptHash, verdict.attestedAtMillis, verdict.expiresAtMillis,
  ].join("\n");
}
