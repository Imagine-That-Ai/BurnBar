...
export function parseEvidenceBundle(bytes: Buffer): ParsedEvidenceBundle {
  if (bytes.byteLength > MAX_BUNDLE_BYTES || bytes.byteLength < PREFIX_BYTES + 2 || !bytes.subarray(0, 8).equals(MAGIC)) return invalidBundle();
  const headerLength = bytes.readUInt32BE(8);
  if (headerLength < 2 || headerLength > 16_384 || PREFIX_BYTES + headerLength >= bytes.byteLength) return invalidBundle();
  ...
  if (header.schemaVersion !== 1 || !Array.isArray(header.records) || header.records.length !== 4) return invalidBundle();
  let expectedOffset = PREFIX_BYTES + headerLength;
  const bodies: Buffer[] = [];
  for (let index = 0; index < KINDS.length; index += 1) {
    const record = object(header.records[index], "evidence bundle record");
    exactKeys(record, ["kind", "offset", "byteLength", "sha256"], "evidence bundle record");
    if (record.kind !== KINDS[index]) return invalidBundle();
    const offset = integer(record.offset, "record.offset", PREFIX_BYTES, MAX_BUNDLE_BYTES - 1);
    const byteLength = integer(record.byteLength, "record.byteLength", 1, MAX_BUNDLE_BYTES);
    const digest = sha256Hex(record.sha256, "record.sha256");
    if (offset !== expectedOffset || offset + byteLength > bytes.byteLength) return invalidBundle();
  ...
  const [imaLog, uefiLog, installedManifest, installedManifestSignature] = bodies;
  if (imaLog === undefined || uefiLog === undefined || installedManifest === undefined || installedManifestSignature === undefined) return invalidBundle();
  return { imaLog, uefiLog, installedManifest, installedManifestSignature };
}

export function verifyInstalledManifest(bundle: ParsedEvidenceBundle, challenge: AttestationChallenge, publicKeyPem: string): void {
  if (bundle.installedManifestSignature.byteLength !== 64 || sha256(bundle.installedManifest) !== challenge.releaseDigestSha256) return invalidManifest();
  let key;
  try {
    key = createPublicKey(publicKeyPem);
    if (key.asymmetricKeyType !== "ed25519" || !verify(null, bundle.installedManifest, key, bundle.installedManifestSignature)) return invalidManifest();
  } catch {
  ...
  let text: string;
  try {
    text = new TextDecoder("utf-8", { fatal: true }).decode(bundle.installedManifest);
    parsed = JSON.parse(text) as unknown;
  } catch {
    return invalidManifest();
  }
  ...
  const required = ["schemaVersion", "product", "appId", "firebaseAppId", "packageVersion", "gitCommit", "packageArchitecture", "packageFormat", "packageName", "policyId", "brokerProtocolVersion", "installedFilesRootSha256", "authorizedClients", "files"];
  exactKeys(manifest, required, "installed manifest");
  const authorizedClients = Array.isArray(manifest.authorizedClients) ? manifest.authorizedClients : [];
  ...
  const valid = manifest.schemaVersion === 1
    ...
    && /^1:[0-9]+:web:[A-Za-z0-9]+$/.test(string(manifest.firebaseAppId, "manifest.firebaseAppId", 160))
    && manifest.firebaseAppId === challenge.appId
    && /^(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)$/.test(string(manifest.packageVersion, "manifest.packageVersion", 80))
    && manifest.packageVersion === challenge.appVersion
    && (manifest.packageArchitecture === "aarch64" || manifest.packageArchitecture === "x86_64")
    ...
    && string(manifest.policyId, "manifest.policyId", 128) === challenge.policyId
    && manifest.brokerProtocolVersion === 2
    && /^[a-f0-9]{40}$/.test(String(manifest.gitCommit))
    && sha256Hex(manifest.installedFilesRootSha256, "manifest.installedFilesRootSha256") === filesRoot;
  if (!valid) return invalidManifest();
}

interface AuthorizedDaemon { path: string; sha256: string; ownerUid: number; ownerGid: number; mode: number }
...
function validateAuthorizedClients(values: unknown[]): AuthorizedDaemon {
  ...
  exactKeys(value, ["role", "path", "sha256", "ownerUid", "ownerGid", "mode"], "manifest.authorizedClients[0]");
  ...
  if (value.role !== "daemon" || daemon.path !== "/usr/bin/openburnbar-daemon") return invalidManifest();
  return daemon;
}
...
function validateFiles(values: unknown[], daemon: AuthorizedDaemon): string {
  ...
  for (const [index, raw] of values.entries()) {
    ...
    if (!path.startsWith("/usr/") || path.includes("\0") || paths.has(path)) return invalidManifest();
    paths.add(path);
    ...
    if (type === "file") {
      exactKeys(value, ["path", "type", "sha256", "size", "mode", "uid", "gid"], label);
      const digest = sha256Hex(value.sha256, "file sha256");
      const size = integer(value.size, "file size", 0, 2_147_483_648);
      rootRecords.push(`${path}\0file\0${digest}\0${String(size)}\0${mode}\0${String(uid)}\0${String(gid)}`);
      if (path === daemon.path) {
        daemonMatches += 1;
        if (digest !== daemon.sha256 || size === 0 || size > 512 * 1024 * 1024 || mode !== daemon.mode.toString(8).padStart(4, "0") || uid !== daemon.ownerUid || gid !== daemon.ownerGid) return invalidManifest();
      }
      ...
      rootRecords.push(`${path}\0symlink\0${target}\0${mode}\0${String(uid)}\0${String(gid)}`);
    } else {
      return invalidManifest();
    }
  }
  if (daemonMatches !== 1) return invalidManifest();
  rootRecords.sort((left, right) => Buffer.compare(Buffer.from(left), Buffer.from(right)));
