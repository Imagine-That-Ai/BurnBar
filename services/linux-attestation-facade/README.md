...
Verifier: fixed private `KEYLIME_VERIFIER_URL`, `EVIDENCE_BUCKET`, `VERIFIER_OIDC_AUDIENCE`, `VERIFIER_CALLER_SERVICE_ACCOUNT`, `KMS_SIGNING_KEY_VERSION`, `VERDICT_ISSUER`, `VERDICT_AUDIENCE`, optional `EVIDENCE_MAX_BYTES` (16 MiB and never configurable above 16 MiB), optional `VERDICT_TTL_MILLIS`, and optional `VERIFICATION_LEASE_MILLIS` (75 seconds by default and longer than the Functions request budget).
