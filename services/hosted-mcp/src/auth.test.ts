...
test("verifies Ed25519 bearer tokens and rejects legacy HMAC when public key is configured", () => {
  ...
  try {
    ...
    const hmacToken = mintDevelopmentToken(
      { ...claims, jti: "jti-hmac" },
      "legacy-secret",
    );
    assert.throws(
      () => verifyBearerToken(`Bearer ${hmacToken}`),
      /Legacy HMAC/,
    );
  } finally {
...
test("cursor signing rejects tampering and scope mismatch", () => {
  ...
  assert.equal(
    verifyCursor(cursor, "u1", "burnbar_search_conversations").offset,
    10,
  );
  assert.throws(
    () => verifyCursor(`${cursor}x`, "u1", "burnbar_search_conversations"),
    /signature|Malformed/,
  );
  assert.throws(
    () => verifyCursor(cursor, "u2", "burnbar_search_conversations"),
    /does not match/,
  );
});
...
test("registry exposes required tool surface and redaction strips raw content", () => {
  ...
  assert.deepEqual(
    redact({ query: "secret words", nested: { body: "plaintext" } }),
    {
      query: "[REDACTED]",
      nested: { body: "[REDACTED]" },
    },
  );
  const resumeTool = tools.find(
    (tool) => tool.name === "burnbar_resume_conversation",
  );
  assert.deepEqual(resumeTool?.requiredScopes, [
    "conversation:read",
    "search:read",
  ]);
  const codeSearchTool = tools.find(
...
test("MCP metadata methods require active client, Pro entitlement, and current client scopes", async () => {
  ...
  await assert.rejects(
    () =>
      handleMcpRequest(
        dbForClient(
          { displayName: "No Pro", allowedScopes: noProClaims.scopes },
          false,
        ),
        noProClaims,
        {
