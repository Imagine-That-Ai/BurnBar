...
function fixtureRollup(): UsageRollup {
  ...
  return {
    ...
    modelSummaries: [
      { model: "claude-opus-4.6", provider: "anthropic", label: "claude-opus-4.6", requests: 2_247, tokens: Math.round(totalTokens * 0.34), cost: 512.2 },
      { model: "gpt-5.3-codex", provider: "openai", label: "gpt-5.3-codex", requests: 1_874, tokens: Math.round(totalTokens * 0.24), cost: 301.5 },
      { model: "claude-sonnet-4.6", provider: "anthropic", label: "claude-sonnet-4.6", requests: 1_565, tokens: Math.round(totalTokens * 0.24), cost: 300.2 },
      { model: "kimi-k2.5", provider: "moonshot", label: "kimi-k2.5", requests: 981, tokens: Math.round(totalTokens * 0.11), cost: 112.87 },
      { model: "gpt-5.3", provider: "openai", label: "gpt-5.3", requests: 327, tokens: Math.round(totalTokens * 0.07), cost: 57.4 },
    ],
    deviceSummaries: [
      { deviceId: "studio-macbook", label: "studio-macbook", requests: 5_102, tokens: Math.round(totalTokens * 0.81) },
      { deviceId: "home-mini", label: "home-mini", requests: 1_892, tokens: Math.round(totalTokens * 0.19) },
    ],
    accountSummaries: [
    ...
    executionSourceSummaries: [
      { sourceId: "claude-code", sourceName: "Claude Code", label: "Claude Code", totalRequests: 3_204, totalTokens: Math.round(totalTokens * 0.52), totalCost: 701.3 },
      { sourceId: "codex", sourceName: "Codex", label: "Codex", totalRequests: 2_106, totalTokens: Math.round(totalTokens * 0.27), totalCost: 342.6 },
      { sourceId: "kimi", sourceName: "Kimi Code", label: "Kimi Code", totalRequests: 981, totalTokens: Math.round(totalTokens * 0.12), totalCost: 118.4 },
      { sourceId: "cursor", sourceName: "Cursor", label: "Cursor", totalRequests: 512, totalTokens: Math.round(totalTokens * 0.06), totalCost: 84.2 },
      { sourceId: "factory", sourceName: "Factory Droid", label: "Factory Droid", totalRequests: 191, totalTokens: Math.round(totalTokens * 0.03), totalCost: 37.67 },
    ],
    comboSummaries: [
