 ...
 * website/src/lib/analytics/amplitudeTransport.ts and
 ...
 * pre-consent render ships zero Amplitude bytes and makes zero network calls.
 * Next's bundler code-splits the dynamic `import()` into its own chunk, so the
 * Amplitude code is absent from the initial page payload until opt-in.
 *
 * Hardened to macOS/website parity: autocapture fully off, no default tracking,
 * no remote-config fetch, IP/geo stripped, anonymous device id in localStorage
 * (no cookies), no user id until an authenticated member is identified (and then
 * only a hashed uid). With no key it never loads.
 */
export class AmplitudeTransport implements AnalyticsTransport {
  ...
  start(apiKey: string): void {
    ...
    if (this.amp) {
      ...
      if (this.pendingUserId !== undefined) this.amp.setUserId(this.pendingUserId);
      this.flush(this.amp);
      return;
    }
    ...
    import("@amplitude/analytics-browser")
      .then((amp) => {
        amp.init(apiKey, {
          ...
          instanceName: "openburnbar-console",
          identityStorage: "localStorage", // anonymous device id, no cookies
          optOut: false,
        });
        this.amp = amp;
        if (this.optedOut) return; // a stop() raced in before the SDK resolved
        if (this.pendingUserId !== undefined) amp.setUserId(this.pendingUserId);
        this.flush(amp);
      })
      .catch(() => {
  ...
  setUserId(id: string | undefined): void {
    ...
    if (!this.started || this.optedOut) return;
    if (this.amp) this.amp.setUserId(id);
  }
  ...
  stop(): void {
    ...
    if (this.amp) {
      ...
      this.amp.reset(); // clear the anonymous device id + user id so a future grant starts fresh
    }
  }
