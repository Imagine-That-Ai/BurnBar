/**
 * Tri-state analytics consent for the console, ported verbatim from the website
 * (website/src/lib/analytics/consent.ts), itself ported from
 * AgentLens/Services/Analytics/AnalyticsConsentStore.swift.
 *
 * `unset` and `declined` are treated identically — both keep the console dark
 * (no Amplitude module loaded, no network). Only `granted` permits egress.
 * Revoking sets `declined`, never returns to `unset`, so the first-run prompt
 * does not reappear after the user opts out.
 */
...
export const CONSENT_STORAGE_KEY = "burnbar-console-analytics-consent";
...
export class ConsentStore {
  ...
  /** True once the member has answered the prompt either way. */
