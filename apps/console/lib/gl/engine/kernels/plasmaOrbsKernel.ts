/**
 * Plasma Orbs kernel — "Liquid chrome / orb cluster" the 2026 mainstream WebGL
 * backdrop NOT already covered by the 19 existing kernels (aurora, fluid-aurora,
 * mesh, iridescence, fluid, boids, lic, halftone, etc).
 *
 * Concept: a small cluster of 5–7 noise-displaced circular "blobs" rendered with
 * analytic inverse-square metaball density (no depth peeling, no float FBO — a
 * single full-screen pass), with a soft chromatic-aberration rim, an iridescent
 * oil-slick ramp on the silhouette, and gentle film grain. The user sees slow,
 * luminous glass-like orbs drifting through a deep palette-tinted void, fusing
 * when they pass close. Pointer-inert look (the cursor subtly warps the field
 * via the host's smoothed uPointer uniform).
 *
 * Why this fills a gap: iridescence (thin-film interference) is static across the
 * screen; fluid (Navier–Stokes) is opaque ink; aurora / fluid-aurora are vertical
 * curtains; boids is an agent swarm; mesh is a four-anchor gradient slab. None of
 * these is the "few glassy orbs floating in deep space" idiom that dominates 2026
 * design-tool galleries (Framer Marketplace "Orb Background", shadcn/ui orb,
 * aura.build hero gallery).
 *
 * Pure fragment shader via {@link createShaderKernel}; no per-frame JS, no float
 * target, no sim — runs on the same set as iridescence/halftone.
 */
...
export function createPlasmaOrbsKernel(): Kernel {
