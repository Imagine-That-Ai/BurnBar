/**
 * Moiré (Quasicrystal) kernel — "Hologram".
 *
 * A breathing quasicrystal: the sum of N equally-rotated cosine waves (de Bruijn
 * dual construction) forming an aperiodic, Penrose-like interference field. Each
 * cosine is **analytically band-limited** (IQ `fcos`) — multiplied by the `sinc`
 * of its own per-pixel footprint — so the high-frequency fringes never shimmer or
 * crawl at any DPR. This is the kernel's whole engineering point: provably
 * alias-free quasicrystal interference, no supersampling.
 *
 * Breathes on a ~14s/31s irrational lung (gathers + brightens on inhale), spins
 * the basis on a slow clock (the canonical QC animation), scrubs phase radially,
 * and lenses under the pointer. Dark = additive over ink with a filmic knee
 * (antinodes bloom toward white); Light = pearl deposit, ink contour on nodes.
 * Pure fragment shader via {@link createShaderKernel}; no per-frame JS, zero new
 * uniforms (D8: single-pass, no infra).
 */
...
const int   QC_WAVES_C   = ${QC_WAVES};
const float QC_GOLDEN_C  = ${QC_GOLDEN.toFixed(10)};
const float QC_FREQ_C    = ${QC_FREQ.toFixed(1)};
...
export function createMoireKernel(): Kernel {
  return createShaderKernel({ id: "moire", label: "Moiré", body: BODY });
}
