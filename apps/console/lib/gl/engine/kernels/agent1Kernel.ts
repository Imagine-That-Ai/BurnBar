/**
 * Agent 1 kernel — "Generative Mesh Fluid".
 *
 * The most widely adopted WebGL background effect in 2026: a domain-warped
 * generative mesh that flows like liquid silk. Organic color blobs drift and
 * merge through a curl-noise wind, creating an ever-morphing gradient field
 * that reads as both structured (mesh-like color regions) and fluid (continuous
 * organic motion). This is the aesthetic that replaced static mesh gradients in
 * modern product design — living, breathing color that responds to the pointer.
 *
 * Why it trends in 2026:
 * - Apple's static mesh gradients (2021-2024) evolved into animated variants
 *   across iOS, macOS, and web design systems
 * - Fluid simulation aesthetics merged with gradient meshes for performance
 * - The "organic blob" look became the signature of premium SaaS landing pages
 * - Single-pass fragment shader = 60fps on mobile without GPU compute
 *
 * Technique: 3-layer domain-warped FBM with curl-advected anchor points.
 * Each layer is a soft radial gradient centered on a noise-driven orbit;
 * the layers blend via smooth-min (exponential blend) creating the
 * characteristic "liquid merge" look. A slow hue drift cycles through the
 * palette accents so the field never visually loops.
 *
 * Perf: ~25 snoise calls per pixel, well within the 60fps budget on
 * mid-tier Android. No float targets, no sim — pure display-pass.
 */
...
export function createAgent1Kernel(): Kernel {
  return createShaderKernel({ id: "agent1", label: "Agent 1", body: BODY });
}
