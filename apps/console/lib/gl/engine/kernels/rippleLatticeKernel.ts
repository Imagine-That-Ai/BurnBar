/**
 * Ripple Lattice kernel — a full-bleed lattice of soft accent dots that breathe
 * with a slow traveling wave, while the cursor emits concentric sonar ripples.
 *
 * A regular grid (cell ≈ `res.y/28`) is sampled in pixel space; each cell hosts
 * one soft dot whose radius breathes via a traveling sine wave keyed on the cell
 * id (`gid.x+gid.y`) and modulated by a slow `fbm` field so the breathing reads
 * organic rather than mechanical. The pointer drives two couplings: a radial
 * `exp`-falloff **ripple** (`sin(d·18 − t·2.6)`) that bulges + brightens dots and
 * radially **shoves** the sampling space outward (the concentric-sonar look), and
 * a softer **proximity** bulge that swells + lights the dots nearest the cursor.
 * Color is 100% palette-driven through `accentRamp`, and the composite branches
 * on `uTheme` — additive glow on dark ink, ink-deposit dots on light paper.
 *
 * Distinct from the existing kernels:
 *   - moire: an N-wave cosine interference lattice (no discrete dot cells, no
 *     cursor-driven radial displacement).
 *   - mesh / blobs-mesh: gradient-blended anchor fields (continuous, not a grid
 *     of discrete breathing dots).
 *   - inversion-lattice: an iterated circle-inversion fractal (self-similar ring
 *     nest, not a Cartesian dot grid).
 *   None of these is the "Cartesian dot lattice + cursor sonar ripple that shoves
 *   + brightens nearby dots" idiom.
 *
 ...
 * Modeled as a MESH cousin (structured, lattice-like). Color is 100%
 * palette-driven and theme-branched.
 */
...
export function createRippleLatticeKernel(): Kernel {
