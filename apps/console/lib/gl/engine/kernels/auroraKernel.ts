/**
 * Aurora kernel — "Living Aurora": three parallaxed luminous curtains whose
 * color is keyed to ALTITUDE (cyan base, rose/violet crown), choreographed by a
 * multi-clock breath, with a single light PILLAR that rises through the field on
 * its own ~18s clock.
 *
 * Perf spine is "shared-warp-once": the expensive IQ domain warp is computed a
 * single time (5 fbm = 20 snoise) then three cheap pure-trig curtains are
 * advected through the shared flow — depth without tripling fbm cost.
 *
 * Dark: additive over deep ink + filmic tonemap knee (cores bloom toward white)
 *   + faint airglow grain floor.
 * Light: the same field deposited SUBTRACTIVELY as a pale pearl watermark
 *   (cyan base / rose crown), never adding toward white, never dark blobs.
 * Timing is identical in both themes; only the final composite math flips.
 *
...
export function createAuroraKernel(): Kernel {
  return createShaderKernel({ id: "aurora", label: "Aurora", body: BODY });
}
