/**
 * Volumetric kernel — "Crepuscular": a per-pixel raymarcher that integrates
 * single-scattered light through an fbm density medium with Beer–Lambert
 * extinction. A slow-orbiting light source, self-shadowed by one cheap density
 * tap per step, carves the medium into drifting god-ray shafts (real shafts,
 * not a radial blur). Interleaved gradient noise jitters each ray's start so the
 * 32-step march reads as film grain instead of bands.
 *
 * Dark: additive in-scatter over deep ink + filmic knee (shafts bloom white).
 * Light: the same shaft field deposited SUBTRACTIVELY as pale luminous columns
 *        (never adding toward white, never muddy blobs). Timing identical; only
 *        the final composite flips.
 *
 * Pure fragment shader via {@link createShaderKernel}; no per-frame JS. Opts into
 * the shared scroll control block via `controls:["scroll"]` (D7) and reads D2's
 * `vec2 uScroll` (offsetPx, yMax) + `float uScrollVel` (px/frame) — no bespoke
 * uniform, no factory fork.
 */
...
export function createVolumetricKernel(): Kernel {
  return createShaderKernel({
    id: "volumetric",
    label: "Volumetric",
    body: BODY,
    // D7 opt-in → host pushes D2's shared uScroll (vec2) / uScrollVel (float) block.
    controls: ["scroll"],
  });
}
