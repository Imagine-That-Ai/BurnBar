...
const MEMBERSHIP_PORTAL_METHOD: &str = "daemon.membership.portalUrl";
...
fn membership_checkout_url() -> Result<serde_json::Value, String> {
    call_daemon_method(
        ...
        Some(serde_json::json!({
            "success_url": "https://openburnbar.com/account",
            "cancel_url": "https://openburnbar.com/account"
        })),
    )
    .map_err(membership_rpc_error)
}

// Active Stripe members manage payment details, invoices, and cancellation in
// Stripe's hosted billing portal. The daemon owns callable auth and URL minting;
// this command supplies only the public HTTPS return location.
#[tauri::command]
fn membership_portal_url() -> Result<serde_json::Value, String> {
    call_daemon_method(
        MEMBERSHIP_PORTAL_METHOD,
        Some(serde_json::json!({
            "return_url": "https://openburnbar.com/"
        })),
    )
    .map_err(membership_rpc_error)
}
