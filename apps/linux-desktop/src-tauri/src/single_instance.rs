...
pub(crate) fn acquire(directory: &Path, startup_messages: &[Message]) -> Result<Acquire, String> {
    ...
    spawn_listener(listener, sender.clone(), stop.clone())?;
    // A primary launch can be started directly by a desktop notification
    // action (for example a Reply click while no shell process is running).
    // Deep-link routes have their own one-shot startup store, but notification
    // actions carry intent that must reach the dispatcher and renderer. Queue
    // only those typed messages here; forwarding a Route as well would cause
    // the same deep link to be consumed twice.
    for message in startup_messages {
        if matches!(message, Message::NotificationAction { .. }) {
            // The receiver is returned immediately below and cannot be
            // dropped while this sender is still in scope.
            sender
                .send(message.clone())
                .map_err(|_| "single_instance_startup_message_unavailable".to_string())?;
        }
    }
    Ok(Acquire::Primary {
        guard: Guard {
...
mod tests {
    ...
    #[test]
    fn primary_launch_queues_notification_actions_for_the_dispatcher() {
        let directory = temp_directory();
        let primary = acquire(
            &directory,
            &[Message::NotificationAction {
                action: "reply".to_string(),
                payload: serde_json::json!({"notificationId":"cold-start-reply"}),
            }],
        )
        .unwrap();
        let (guard, receiver) = match primary {
