...
export function TextExpansionSurface() {
  ...
  const acknowledge = (inAppOnly: boolean, systemIMEEnabled = consent?.systemIMEEnabled === true) => {
    setConsentBusy(true);
    setVersion((v) => v + 1);
    const persist = async () => {
      if ((!inAppOnly || !systemIMEEnabled) && nativeEngineAvailable && textExpansionEngineStop) {
        try {
      ...
      try {
        await writeTextExpansionConsentPersisted({ inAppOnly, systemIMEEnabled, declinedGlobalCapture: true });
        setStorageError(null);
      } catch (error) {
  ...
  const consentRow = (
    <div className="consent-row">
      <label>
        <input
          type="checkbox"
          aria-label="In-app expansion only"
          checked={Boolean(consent?.inAppOnly)}
          disabled={persistenceBlocked || consentBusy}
          onChange={(event) => {
            acknowledge(event.currentTarget.checked);
          }}
        />
        {' Enable in-app expansion. No global key capture on Linux.'}
      </label>
      {nativeEngineAvailable && consent?.inAppOnly ? (
        <label>
          <input
            type="checkbox"
            checked={consent.systemIMEEnabled === true}
            disabled={persistenceBlocked || consentBusy}
            onChange={(event) => acknowledge(true, event.currentTarget.checked)}
          />
          {' Enable the signed IBus engine in inspectable, non-password fields. It never reads the clipboard or surrounding text.'}
        </label>
      ) : null}
    </div>
  );
