...
export const useProvidersStore = create<ProvidersState>()((set, get) => ({
  ...
  async setPreferredCredentialSlot(providerID, slotID) {
    ...
    try {
      ...
      const readback = await bridge.configUpdate({ ...snapshot, providers });
      const confirmedProvider = (readback.providers ?? []).find(
        (provider) => provider.providerID === normalizedProviderID
      );
      const confirmedSlotID = confirmedProvider?.preferredCredentialSlotID ?? '';
      if (confirmedSlotID !== normalizedSlotID) {
        throw new Error(
          normalizedSlotID
            ? `Daemon did not confirm preferred credential slot '${normalizedSlotID}'.`
            : 'Daemon did not confirm clearing the preferred credential slot.'
        );
      }
      await get().load();
