...
export const useMissionsStore = create<MissionsState>()((set, get) => ({
  ...
  async answerQuestion(id, answer, selectedOptionId) {
    const { bridge, fixtureMode } = useShellStore.getState();
    if (fixtureMode || !bridge || typeof bridge.questionAnswer !== 'function') {
      set((s) => ({
        questionById: {
          ...s.questionById,
          [id]: { pending: false, error: 'Question answering is unavailable in this shell.' }
        }
      }));
      return false;
    }
    ...
    if (!trimmed) {
      set((s) => ({
        questionById: {
          ...s.questionById,
          [id]: { pending: false, error: 'Enter an answer before submitting.' }
        }
      }));
      return false;
    }
    set((s) => ({
      questionById: { ...s.questionById, [id]: { pending: true, error: null } }
    }));
    try {
      await bridge.questionAnswer({ questionId: id, answer: trimmed, selectedOptionId });
      await get().load();
      set((s) => ({
        questionById: { ...s.questionById, [id]: { pending: false, error: null } }
      }));
      return true;
    } catch (e) {
      set((s) => ({
        questionById: {
          ...s.questionById,
          [id]: { pending: false, error: e instanceof Error ? e.message : 'Question answer failed' }
        }
      }));
      return false;
    }
