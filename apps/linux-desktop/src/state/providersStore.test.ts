...
describe('providersStore custom model lifecycle', () => {
  ...
  it('keeps the previous catalog and reports stale truth when slot readback disagrees', async () => {
    const config: ConfigSnapshot = {
      paths: { supportDir: '', socketPath: '', configDir: '', providerLogPaths: [] },
      secretServiceStatus: 'ready',
      telemetryEnabled: false,
    ...
    const configSnapshot = vi.fn(async () => config);
    const configUpdate = vi.fn(async (next: ConfigSnapshot) => ({
      ...next,
      providers: next.providers?.map((provider) => ({ ...provider, preferredCredentialSlotID: 'team' }))
    }));
    const providerCatalog = vi.fn(async () => previousCatalog);
    useShellStore.setState({
      fixtureMode: false,
      bridge: { configSnapshot, configUpdate, providerCatalog } as never
    });
    useProvidersStore.setState({ catalog: previousCatalog });

    await useProvidersStore.getState().setPreferredCredentialSlot('anthropic', 'backup');

    expect(providerCatalog).not.toHaveBeenCalled();
    expect(useProvidersStore.getState().catalog).toEqual(previousCatalog);
    expect(useProvidersStore.getState().mutationError).toBe(
      "Daemon did not confirm preferred credential slot 'backup'."
    );
  });
