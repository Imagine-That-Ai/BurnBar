...
export type PendingQuestion = {
  ...
  evidenceRefs: string[];
  suggestedOptions: PendingQuestionOption[];
  deepLink?: {
    kind: 'session_log' | 'dashboard' | 'project' | 'settings';
    targetId?: string;
    title: string;
    subtitle?: string;
  };
...
export type MissionRecord = {
  ...
  prLinkage?: MissionPRLinkageSnapshot;
  metadata?: Record<string, unknown>;
  /** Derived locally from updatedAt; separate from daemon-owned runtime health. */
  freshness?: MissionFreshness;
};
export type MissionListResult = {
  missions: MissionRecord[];
  pendingApprovals: PendingApproval[];
  /** Present on current shells; optional only for fixture/version-skew compatibility. */
...
export type AccountCloudDataExportRequest = {
  domains?: string[];
  destinationPath: string;
};
...
export type TrustedDeviceTrustState = 'pending' | 'trusted' | 'revoked';
export type TrustedDevice = {
  deviceId: string;
  displayName: string;
  platform: string;
  trustState: TrustedDeviceTrustState;
  isCurrentDevice: boolean;
  safetyFingerprint?: string;
};
export type TrustedDeviceListResult = {
  ok: boolean;
  devices: TrustedDevice[];
};
export type TrustedDeviceMutationResult = {
  ok: boolean;
  deviceId: string;
  trustState: TrustedDeviceTrustState;
...
export interface LinuxShellBridge {
  ...
  trustedDeviceApprove?(deviceId: string): Promise<TrustedDeviceMutationResult>;
  trustedDeviceRevoke?(deviceId: string): Promise<TrustedDeviceMutationResult>;
