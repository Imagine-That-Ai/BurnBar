...
import {
  ...
  mapAccountStatus,
  mapAccountCloudDataExportResult,
  mapAccountCloudDataDeletionResult,
  mapTrustedDeviceList,
  mapTrustedDeviceMutation,
...
export async function loadShellBridge(): Promise<LinuxShellBridge | null> {
  ...
  return {
    ...
    // P01/P11 — authoritative ledger projection plus bounded recent activity
    usageSummary: async () => {
    ...
    missionList: async () => {
      const [missionRaw, questionRaw] = await Promise.all([
        invoke<RawJsonValue>('mission_list'),
        invoke<RawJsonValue>('question_list')
      ]);
      return { ...mapMissionList(missionRaw), pendingQuestions: mapQuestionList(questionRaw) };
    },
    // Canonical daemon.mission.get returns packet/result/evidence detail from
    // the snapshot; daemon.mission.health owns runtime health and ordered history.
    missionGet: async (id) => {
    ...
    trustedDeviceList: async () => {
      const raw = await invoke<RawJsonValue>('trusted_device_list');
      return mapTrustedDeviceList(raw);
    },
    trustedDeviceApprove: async (deviceId) => {
      const raw = await invoke<RawJsonValue>('trusted_device_approve', { deviceId });
      return mapTrustedDeviceMutation(raw);
    },
    trustedDeviceRevoke: async (deviceId) => {
      const raw = await invoke<RawJsonValue>('trusted_device_revoke', { deviceId });
      return mapTrustedDeviceMutation(raw);
    },
    ...
    // P15 — active members use Stripe Billing Portal, never a second checkout.
    membershipPortalUrl: async () => {
      const raw = await invoke<RawJsonValue>('membership_portal_url');
