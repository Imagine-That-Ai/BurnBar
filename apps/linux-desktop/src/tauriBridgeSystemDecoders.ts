...
const TRUSTED_DEVICE_STATES = new Set<TrustedDevice['trustState']>(['pending', 'trusted', 'revoked']);

function boundedTrustedDeviceText(value: RawJsonValue, label: string, maxBytes: number): string {
  const text = str(value).trim();
  if (!text || text.length > maxBytes || /[\r\n]/u.test(text)) {
    throw new Error(`Native trusted-device ${label} is invalid.`);
  }
  return text;
}

export function mapTrustedDeviceList(raw: RawJsonValue): TrustedDeviceListResult {
  const source = obj(pick(raw, 'result') ?? raw);
  const values = arr(pick(source, 'devices'));
  if (values.length > 128) throw new Error('Native trusted-device list exceeds the safe limit.');
  const seen = new Set<string>();
  const devices: TrustedDevice[] = values.map((value) => {
    const device = obj(value);
    const deviceId = boundedTrustedDeviceText(pick(device, 'deviceID', 'deviceId', 'id'), 'ID', 160);
    if (seen.has(deviceId)) throw new Error('Native trusted-device list contains a duplicate ID.');
    seen.add(deviceId);
    const trustState = boundedTrustedDeviceText(pick(device, 'trustState', 'trust_state'), 'state', 32) as TrustedDevice['trustState'];
    if (!TRUSTED_DEVICE_STATES.has(trustState)) throw new Error('Native trusted-device state is invalid.');
    const safetyFingerprint = str(pick(device, 'safetyFingerprint', 'safety_fingerprint')).trim();
    if (safetyFingerprint.length > 256 || /[\r\n]/u.test(safetyFingerprint)) {
      throw new Error('Native trusted-device fingerprint is invalid.');
    }
    return {
      deviceId,
      displayName: boundedTrustedDeviceText(pick(device, 'displayName', 'display_name'), 'display name', 256),
      platform: boundedTrustedDeviceText(pick(device, 'platform'), 'platform', 64),
      trustState,
      isCurrentDevice: Boolean(pick(device, 'isCurrentDevice', 'is_current_device')),
      ...(safetyFingerprint ? { safetyFingerprint } : {})
    };
  });
  return { ok: Boolean(pick(source, 'ok', 'success')), devices };
}

export function mapTrustedDeviceMutation(raw: RawJsonValue): TrustedDeviceMutationResult {
  const source = obj(pick(raw, 'result') ?? raw);
  const deviceId = boundedTrustedDeviceText(pick(source, 'deviceID', 'deviceId', 'id'), 'ID', 160);
  const trustState = boundedTrustedDeviceText(pick(source, 'trustState', 'trust_state'), 'state', 32) as TrustedDeviceMutationResult['trustState'];
  if (!TRUSTED_DEVICE_STATES.has(trustState)) throw new Error('Native trusted-device state is invalid.');
  return {
    ok: Boolean(pick(source, 'ok', 'success')),
    deviceId,
    trustState,
    alreadyInState: Boolean(pick(source, 'alreadyInState', 'already_in_state'))
  };
}
...
export function mapAccountCloudDataExportResult(raw: RawJsonValue): AccountCloudDataExportResult {
  const value = obj(pick(raw, 'result') ?? raw);
  ...
  const destinationPath = requireString(
    pick(value, 'destinationPath', 'destination_path'),
    'account export.destinationPath'
  );
  if (byteCount < 0 || byteCount > 8 * 1024 * 1024 || schemaVersion !== 2) {
    throw new Error('Native account export returned invalid metadata.');
  }
  if (
    destinationPath.length > 4_096 ||
    !destinationPath.startsWith('/') ||
    destinationPath.includes('..') ||
    Array.from(destinationPath).some((character) => {
      const codePoint = character.codePointAt(0) ?? 0;
      return codePoint < 0x20 || codePoint === 0x7f;
    })
  ) {
    throw new Error('Native account export returned an unsafe destination path.');
  }
  return {
    ok: requireBoolean(pick(value, 'ok'), 'account export.ok'),
    destinationPath,
    byteCount,
    schemaVersion
  };
...
export function mapMembershipPortalUrl(raw: RawJsonValue): string {
  const value = str(pick(raw, 'url', 'portalUrl', 'portal_url'));
  if (!value) throw new Error('Daemon did not return a Stripe billing portal URL.');
  return value;
}
...
export function mapDatabaseSnapshotPath(raw: RawJsonValue): string | null {
  if (raw === null || raw === undefined) return null;
  const path = str(raw);
  if (
    !path.startsWith('/')
    ...
    throw new Error('Native database snapshot dialog returned an unsafe path.');
  }
  const segments = path.split('/');
  const filename = segments.at(-1) ?? '';
  const stem = filename.slice(0, -9);
  if (
    segments.slice(1, -1).some((segment) => segment.length === 0 || segment === '.' || segment === '..')
    || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(stem)
  ) {
