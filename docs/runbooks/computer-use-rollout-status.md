...
- **Test coverage (`swift test --filter OpenBurnBarComputerUseCoreTests`):** 63 tests green covering Phase 8 substrate (5 codec-cursor tests in `MediaPacketCodecTests`)
...
- **Test coverage:** Daemon `swift build` green; Phase 9 dispatch logic tested via the run-coordinator types being exercised by other test paths.
- **Acceptance gate remaining:**
  - [ ] `hosted_computer_use_sync` SKU live in App Store Connect
...
- **Code landed (2026-05-17):**
  - `ComputerUsePhoneControlSigner` (Ed25519 + canonical-JSON intent hashing)
...
- **Test coverage:** 6 PhoneControlSigner golden-case tests proving signature, replay, freshness, intent-hash, peer-key, payload-stability semantics
- **Acceptance gate remaining:**
...
- **Code landed (2026-05-17):**
  ...
  - `ComputerUseAuditExportWriter` `.cua` archive format (magic + per-entry SHA-256 + trailer hash; tamper detection passes)
...
- **Test coverage:** 2 ScopeLibrary tests · 3 AuditExportWriter tests · 4 OpenTimestampsClient tests
- **Acceptance gate remaining:**
