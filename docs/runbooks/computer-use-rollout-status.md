...
- **Verification (2026-05-22):**
  ...
  - `cd android && ./gradlew :app:assembleDebug --no-daemon`; installed `app-debug.apk` on Samsung `<ANDROID_SERIAL>` with `adb install -r`, launched `com.openburnbar/.MainActivity`, and confirmed it was the resumed focused activity.
  - `npm --prefix functions run test:firestore-rules`
