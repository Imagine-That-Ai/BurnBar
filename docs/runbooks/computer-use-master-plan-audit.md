...
| `swift test --package-path OpenBurnBarDaemon --filter ComputerUseRunCoordinatorTests` | 16 tests, 0 failures; browser approval requests include pre-action PNG evidence fields, and `ComputerUseService` now rejects app-owned `system` / `agent_watch` modes at session start instead of creating daemon sessions that later fail through `accessibilityTrusted: false` |
...
The current-checkout Android debug APK was installed and launched on the plugged-in physical Samsung Galaxy S24 after fixing the `QuotaStore` AndroidViewModel constructor crash.
...
Remaining work: app-owned Path C still needs the rest of the real device matrix: deny-region 12/12, direct-download notarization, and `spctl --assess`.
...
- Current-checkout Android debug APK compiles, installs, and launches on Alberto's physical Samsung Galaxy S24; the `QuotaStore` startup crash was fixed by exposing the explicit `QuotaStore(Application)` constructor required by the platform factory.
...
- Phase 11 direct-download notarized distribution and deny-region matrix. Calculator 50/50 and TextEdit 50/50 are now proven by `tools/CUClickSmoke`.
- Phase 12 live phone-to-Mac input latency, replay, tamper, and cross-device approval proof through a paired iroh session.
- Phase 12 multi-device matrix proof beyond focused physical iPhone build/install/launch, physical Android build/install/launch, and sender tests.
