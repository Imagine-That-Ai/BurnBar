...
Before attesting `C`, serialize the release train: record and temporarily dequeue unrelated `main` merge-queue entries, merge the candidate, and keep the queue clear until activation `P` completes. Re-enqueue the recorded pull requests in their original order after `P` lands. This preserves every required check; it prevents unrelated commits from entering the restricted `C..P` path set. If `main` advances after `C`, abandon that attestation and produce a new exact-main candidate rather than allowlisting incidental paths. The replacement candidate must modify a governed `domain-core.yml` push path so GitHub produces a fresh authoritative push run; a manual dispatch or empty commit is diagnostic only and cannot replace that proof.
