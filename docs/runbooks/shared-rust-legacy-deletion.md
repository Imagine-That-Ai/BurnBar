...
The `pull_request_target`-based `Domain Core Trusted Deletion Guard` comes from the default branch, checks out candidate bytes as data only, and runs only the default branch's committed evaluator. CI also supplies the trusted base SHA, `--verify-signed-evidence`, and actual deletion PR/head coordinates. Official `main` strictly requires the always-running `BurnBar CI Gate` umbrella, whose committed context inventory includes this trusted guard, the path-scoped `Domain Core PR Gate`, and the native, Android, and Windows aggregate compile gates. Requiring the umbrella rather than its components separately keeps branch protection complete without duplicating contexts or deadlocking unrelated PRs when a path-scoped component intentionally does not emit. The trusted deletion guard fails closed on:
