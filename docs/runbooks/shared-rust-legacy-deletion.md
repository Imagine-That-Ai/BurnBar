...
The `pull_request_target`-based `Domain Core Trusted Deletion Guard` comes from the default branch, checks out candidate bytes as data only, and runs only the default branch's committed evaluator. CI also supplies the trusted base SHA, `--verify-signed-evidence`, and actual deletion PR/head coordinates. Official `main` strictly requires the always-running `BurnBar CI Gate` umbrella. That umbrella also runs from trusted default-branch bytes, observes component checks on the exact pull-request or merge-group candidate, and has its workflow, evaluator, and context inventory sealed into the domain-core control-plane manifest. Its committed inventory includes this trusted guard, the path-scoped `Domain Core PR Gate`, and the native, Android, and Windows aggregate compile gates. Requiring the umbrella rather than its components separately keeps branch protection complete without duplicating contexts or deadlocking unrelated PRs when a path-scoped component intentionally does not emit. The trusted deletion guard fails closed on:
...
The protected signer and trusted deletion guard audit the live default-branch protection settings without mutating them. Official `main` must strictly require the `BurnBar CI Gate` umbrella, apply protection to administrators, require one current independent approval, dismiss stale approvals, disable force-pushes and deletions, and use an active `ALLGREEN` merge queue with no bypass actors. The merge queue makes the umbrella evaluate the exact current-base merge-group candidate rather than a stale pull-request head. The committed umbrella inventory must continue to include `Domain Core Trusted Deletion Guard`, `Domain Core PR Gate`, and the native, Android, and Windows aggregate checks; direct branch-protection duplication of those component contexts is neither required nor authoritative. Signed release artifacts are cached by immutable SHA-256 under runner-temporary storage during a gate run; attestations are separately deduplicated by their complete verification key.
