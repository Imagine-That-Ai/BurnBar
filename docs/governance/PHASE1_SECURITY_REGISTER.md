...
| P0-6 | **Closed 2026-06-13** | Privileged input prefers authenticated XPC; VirtualHID/RemoteAccess peer auth; privileged-socket red-team in `nightly-dast-sandbox.yml` (`privileged-socket-redteam-ci.sh`); RC evidence in `docs/runbooks/privileged-socket-redteam-rc.md`. | Maintain advisory red-team cadence via sandbox + RC runbook until a self-hosted privileged macOS runner can make the live-socket proof blocking again. |
| LB-1 | **Closed 2026-06-13** | Monitoring plane wiring verified by `scripts/ci/verify-phase1-security-gates.sh`: AGPL metadata decoupled in nightly/ops-confidence (`HEALTH_GATE_REQUIRE_SOURCE_METADATA=0`), extension `npm ci` in test matrix, per-lane `ops-failure-issue` dedupe, uptime definitions, privileged red-team in the sandbox nightly, health-gate availability/compliance split. | Operator: first green paged nightly dispatch + live uptime-check apply in GCP (structural debt is zero); sandbox failures are tracked separately under `lane:nightly-sandbox`. |
