...
| S-4 | Alerts & Digest — digest on/off + hour | `"On · 9:00 AM"` | `AlertSettings.dailyDigestEnabled` / `.dailyDigestHour` | **Direct**, gated on PR2 | **Latency lie until fixed.** `DailyDigestManager.requestAuthorization()` / `scheduleDigest(from:at:)` are called only at launch (`AgentLensApp+LiveServices.swift:309-315`, `OpenBurnBarStartupRecovery.swift:463-467`). PR2 lands `SettingsEffectsObserver` so **both** surfaces reschedule on change. Until then the tile renders `.degraded("Takes effect next launch")` — it never presents a switch that looks like it worked |
...
- **P1 — Daily Digest never reschedules.** `requestAuthorization()` / `scheduleDigest(from:at:)` are called only at launch (`AgentLensApp+LiveServices.swift:309-315`, `OpenBurnBarStartupRecovery.swift:463-467`). Fixed by `SettingsEffectsObserver` (§9) so both surfaces reschedule; not by putting the call in the tile, which would leave Settings inert and the two surfaces behaving differently.
