...
- **Linux desktop** (`apps/linux-desktop`) expresses liquid glass in CSS tokens (`styles/liquid-glass-tokens.css` + `styles/liquid-glass.css`) with backdrop-filter plates over the WebGL `KernelBackdrop`. The first-run onboarding wizard (`surfaces/OnboardingSurface.tsx` + `surfaces/onboarding.css`) is a single elevated glass plate (specular edge, sheen wash, capsule step rail, interactive glass actions) mirroring the macOS Hermes/onboarding `GlassCard` vocabulary — never nested glass-inside-glass.
