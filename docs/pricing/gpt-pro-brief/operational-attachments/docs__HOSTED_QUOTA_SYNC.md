...
- **Low-cost opaque database search.** Each hosted search chunk writes exact
  token posting edges and semantic posting edges. Search queries send only
  device-derived opaque hashes, read bounded posting/chunk candidates from
  Firestore, and download encrypted bodies only when a user opens a result.
- **Deep name/path recall without unbounded write cost.** Hosted search index
  v4 splits transcript text into 16 KB chunks, stores up to 1024 exact and
  keyed prefix token hashes on each chunk for database fallback matching, and
  caps per-chunk token posting edges to keep Firestore write amplification
  bounded.
- **Document-level index freshness.** Each hosted search commit is stamped with
  an opaque commit ID. Search validates matched chunks against the current
  document body hash and storage path, so per-conversation commits remain
  searchable without trusting stale posting edges.
