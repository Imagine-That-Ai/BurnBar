...
Upstream model naming (learned in Task 1): catalog model ids are globally unique, so the two gateway providers carry provider-prefixed ids (`openrouter-anthropic-claude-opus-5`, `vercel-anthropic-claude-opus-5`) whose first alias is the vendor-namespaced name the provider accepts (`anthropic/claude-opus-5`). `BurnBarProviderRoute.resolvedModelID` is the catalog id today, and the executor sends it upstream verbatim, so for `openrouter` and `vercel-ai-gateway` routes the request body's `model` must be rewritten to the catalog model's first alias, never the unique id (test: a request for `openrouter/anthropic/claude-opus-5` reaches the stub upstream with `model == "anthropic/claude-opus-5"`). The gateway's requested-model parser must also try an exact provider + alias match on the whole remainder before splitting `provider/account/model`, because the alias itself contains a slash.
