...
4. Confirm `/v1/models` advertises route-eligible models:
...
For supported local CLIs, use Settings -> Connections -> App connectors and press Connect. BurnBar reads its own live `/v1/models` catalog and writes route-eligible models into the client config automatically:
...
- Droid / Factory CLI: `customModels` / `custom_models` entries for the live advertised catalog in `~/.factory/settings.local.json`, `~/.factory/settings.json`, and `~/.factory/config.json`, using Factory's `generic-chat-completion-api` custom model provider.

## Behavior

- `/v1/models` is derived from BurnBar's live provider/account configuration and, for OpenAI-compatible providers that expose `/models`, includes every upstream-advertised model for each eligible account.
- Each model row includes provider id, account id, account label, capabilities, quota state, enabled state, route eligibility, and last error/refresh fields.
...
- `/v1/models` shows `route_eligible: true` for the model.
