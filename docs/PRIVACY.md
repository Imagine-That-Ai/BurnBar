...
| OpenRouter / MiniMax | Optional BurnBar Pro hosted LLM fallback for Intelligence Brief answers | Your Intelligence Brief question plus a privacy-bounded usage digest containing hashed project IDs, hashed inferred task-title fingerprints, aggregate token/cost totals, and quota bucket metadata — no raw transcripts, provider keys, file bodies, cleartext project folder names, or cleartext task titles | [openrouter.ai/privacy](https://openrouter.ai/privacy) / [minimax.io/privacy](https://www.minimax.io/privacy) |
