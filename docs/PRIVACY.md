...
If you turn on "Back up approved memories" in **Settings → Devices & Sync → Memory Sync**, the memories you have approved — including the ones the Memory MCP learns while you work — are replicated to your own Firestore namespace **end-to-end encrypted**. The body, kind, scope, confidence and citations of every memory are sealed on your Mac with a key held only in your Keychain and wrapped for each of your own devices; OpenBurnBar has no copy of that key and cannot decrypt any of it.

Backup alone is one-way: your Mac uploads, nothing comes back down. A second, separate opt-in under it, "Sync memories to my other devices," turns on the pull half — reading your own sealed facts back down from your other signed-in devices onto this one. Backing memory up is not the same consent as syncing it across devices: the sub-toggle defaults off even once you have already turned backup on, both live under the same Data Vault entitlement (Pro Max or Ultra) — the server's rules require it on every write, and OpenBurnBar itself requires it before the pull will issue a single read — and turning backup off stops the pull too.

Every field in a stored `memory_facts` document is on the server's own allowlist (`firestore.rules`), so a field that is not on the list fails the write outright rather than silently landing. The complete list, field by field: `uid` (your own user id — the rules already scope the whole namespace to you), `docID` (must match the document's own id), `schemaVersion` and `sourceKind` (`chat` or `agent`), `kind`, `reviewStatus` (must be `"approved"` — nothing else can ever be written), `sealedMemory` (the sealed blob — this is the one field that actually carries your memory's content, and it is opaque without your device key), `sourceRefHmacs` (keyed hashes of the sources the memory came from, not the sources themselves), `citationCount` (an integer count, not the citations), and three timestamps, `validFrom`, `updatedAt`, and `replicatedAt`. Two more fields exist only for vault-key rotation and are optional on every write: `vaultGeneration` and `rewrapJobId`. `kind` — one of `fact`, `preference`, `event`, `profile`, `relationship`, `other`, `decision`, `architecture`, `procedure`, `gotcha`, or `todo` — is the one semantic field the server can read in the clear; every other field is either an id, a hash, a count, a timestamp, or the sealed blob itself. What a document never contains, enforced by the server's own security rules rather than by client good behaviour: the memory text, the body, the citations, any embedding or vector, your tags, your entities, your metadata, or your project names and paths — the rules reject a write that includes a `text`, `body`, `citations`, `vector`, `cloakedVector`, or `embedding` field outright. Secrets you asked BurnBar to retain never leave the device at all, and neither do memories awaiting your review, memories BurnBar flagged as prompt injection, or repository knowledge.
