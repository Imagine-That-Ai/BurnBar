...
| 4 | ~~**The alert can't deep-link to quota.**~~ **Retracted 2026-08-16 — this was wrong.** `NavigationCoordinator.DashboardRoute` *does* have a `.quota` case, with a doc comment explicitly about the pre-limit alert deep-link, and `.inbox(itemID:)` alongside it. The core loop can land. | [NavigationCoordinator.swift:33–47](../AgentLens/Services/NavigationCoordinator.swift#L33) | — |
...
3. **"The alert can't deep-link to quota" was simply false** — see the retraction in §3. `DashboardRoute` has carried `.quota` and `.inbox(itemID:)` all along, each with a doc comment naming the notification path. I repeated an agent's claim into a committed doc without checking it; the check took one `sed`. Correcting the record here rather than quietly editing it, because the same discipline applies to us as to the paywall copy.
4. **Provider count drifts across lanes** — 32, 36, 37 depending on whether you count the docs matrix, the ingestion catalog, or registered parsers. `docs/PROVIDERS.md` lists **37 rows**; `contracts/provider-ingestion-catalog.json` has 31 local-parser entries. Pick one number, generate it, and never write it by hand again — the website already does this correctly and the app does not.
