...
- **Roster-tampering client.** `allow write: if false` on `team_rosters/**`, unconditionally, with no callable anywhere that accepts a membership assertion. Nine App-Check-enforced callables are the only path (`createTeam`, `inviteTeamMember`, `acceptTeamInvite`, `promoteTeamMember`, `removeTeamMember`, `rotateTeamKey`, `abandonTeamKeyGeneration`, `recordTeamRewrapComplete`, `recordTeamSlugKeyId`), each appending to an append-only audit log.
- **Client smuggling key material through the roster.** `recordTeamSlugKeyId` is the one callable that carries a value derived from a team key, and what it carries is a DIGEST: its validator admits exactly `CloudVaultCrypto.vaultKeyID`'s output (`v1_` plus 32 lowercase hex) and nothing else, so neither key bytes nor arbitrary attacker-chosen content can reach a document every member of the team reads. It is admin-only and write-once — the same fingerprint again is a no-op, a different one is refused for ever, because the slug key names every document the team has and a second one would orphan all of them.
...
**On-device serving boundary (amendment A1, 2026-09-06).** Separate from the server-facing claim above, and narrower: on a member's own Mac a team row is read into a session only where that checkout's committed `.openburnbar/project.json` names the team **and** the exact `teamProjectId` the row landed in. One predicate carries that — `memory_engine/_namespaces.py::_team_row_servable` — and after A1 it is the only thing that carries it. A1 removed the *local* project fence from team rows, because a `teamProjectId` and a local `proj_<32hex>` id are different namespaces and comparing them made every project-scoped team fact invisible to every member including its author; a second, weaker filter must not be mistakable for the boundary. Unlinking a repository stops the serving on the next call, not at the next pull, because the file is read live per call. The six negative properties (no link file serves nothing; a different `teamProjectId` serves nothing; a different team serves nothing; removing the link stops serving at once; personal rows are unaffected; the write fence and the local-write lock are unchanged) are each pinned by a named test in `tools/openburnbar-mcp/tests/test_memory_blind_sync.py`, and forcing the predicate to `True` opens every serving surface at once. A1 changes no server-visible metadata, no key handling, no roster authority and no write authority.
...
Audit note: "two members of one team derive the same document id for the same fact" is now proved end to end by `tools/openburnbar-mcp/tests/test_team_two_clone.py`, which builds two real git repositories with the same root commit and different `remote.origin.url` forms (SSH and HTTPS), asserts that `project_identity_fingerprint` genuinely splits them into two `proj_` ids, checks the same `teams.<teamId>.teamProjectId` into both, and runs two independent engines over two SQLite stores against a shared in-memory team space — no Firestore emulator. It covers the positive (one document id, one convergence key, one landing row id, a converging second contribution, untouched personal rows on both sides), the negative (a `teamProjectId` that differs between the checkouts converges nothing and each pull refuses the other's document), and a mutation (pointing the derivation at the git fingerprint turns sixteen of its twenty-five tests red). What it does not cover: CloudVault sealing, the AAD, the key ring, `firestore.rules` and the Swift pull service, which keep their own suites.

Audit note (closed): this proof found that a team fact sealed with `engineScope = "project"` landed correctly in the `teamProjectId` partition and was then withheld from every recall surface, because the engine's pre-existing per-project fence admitted a non-personal row only when its `project_id` equalled the session's — and `teamProjectId` is deliberately not any local engine project id, so only `personal`-scoped team facts ever reached a model. Amendment A1 (2026-09-06, the paragraph above) is the controller ruling that closed it: the local project fence no longer applies to team rows, and `_team_row_servable` alone decides them. The end-to-end assertion moved with the ruling — `test_a_project_scoped_team_fact_lands_and_reaches_the_checkout_that_links_it` now pins the corrected behaviour on both scopes, including that unlinking the checkout stops serving both on the very next call.
