...
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ---------------------------------------------- |
| **I1** | Every ephemeral PII/token collection (`voip_outbound`, `fcm_outbound`, `agent_notification_events`, `incoming_call_contexts`) declares a `ttl:true` field override in `firestore.indexes.json`. | F-RR09-001 / F-RR09-007 / F-RR09-008 | static gate                         |
...
| **I5** | The outbound push payload builders (`buildVoipApnsPayload`, `buildFcmCallPayload`) never include `connection_id` / `pairedDeviceId` / a real display name; Android resolves call routing from short-lived owner-scoped context keyed by the ephemeral push correlation id. | F-RR09-008 | static gate + `voipPushMetadata.test.ts`       |
