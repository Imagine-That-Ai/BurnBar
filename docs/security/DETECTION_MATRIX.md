...
| Privileged socket P0 | `privileged_socket_peer_rejected` audit events | Mac daemons | `RUN_PRIVILEGED_SOCKET_REDTEAM=1` + `scripts/ops/run-privileged-socket-redteam.sh` |
