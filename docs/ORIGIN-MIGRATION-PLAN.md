...
| `Imagine-That-Ai/imaginethat-llc` (CubeLove) | **Best cutover candidate — pending access.** | Private, web-only, no Apple signing, no macOS runners, deploys to Firebase/Cloudflare rather than to release assets. If its CI is Node-only it fits Depot. **UNKNOWN until §10-U1 is closed.** |
...
**UNKNOWN** — `deploy.yml` cannot be read from this workspace (§1.3). What follows is what the **live site proves**, which is enough to plan against.
...
**Why this is the good news:** a Firebase deploy needs `firebase deploy` with a Google credential. It does **not** need GitHub Releases, macOS runners, notarization, or a package registry. It is the one ship in the portfolio that can plausibly run entirely off GitHub.

Two paths, in preference order:

1. **Local/operator deploy with Workload Identity or a short-lived service-account token.** Fastest, no new vendor, works during any GitHub outage. Should exist as a documented break-glass path regardless of migration.
2. **Depot CI on an Origin-hosted repo.** Depot runs GitHub Actions workflow syntax (`depot ci migrate workflows` copies `.github/workflows/` into `.depot/workflows/`) and imports secrets (`depot ci migrate secrets-and-vars`). Since CubeLove is Node/web, Depot's lack of macOS is irrelevant. **This is the strongest Origin cutover candidate in the org.**
...
|---|---|---|---|
| **Seat 1 — Builder A** | Implementer | Odd-numbered packets (P1, P3, P5, …) | Review its own packet; touch a live pointer without ATC go |
| **Seat 2 — ATC** | Air traffic control | Sequencing, go/no-go on every live pointer, the ship calendar, the blocker register, incident comms | Write code on any packet it gates |
| **Seat 3 — Builder B** | Implementer | Even-numbered packets (P2, P4, P6, …) | Review its own packet; touch a live pointer without ATC go |
...
**Concurrency rule:** at most **two** packets in flight, one per builder, and **never two packets that touch the same live pointer.** The three pointers that must be serialized under ATC control:
...
Numbered `P#` = packet. Each has exactly one builder. **No step in Phase A–D changes a live pointer** except P3 and P6, which are the intended ship actions and are explicitly ATC-gated.
...
**P2 — Re-run the 1.0.35 release lane.** *(Builder B; ATC go required)*
With P1 merged, dispatch `release.yml` from the immutable `v1.0.35` tag ref per `docs/RELEASE_MACOS.md`, `promote=false`. Do **not** re-cut or move the tag.
**Done-check:** `gh release view v1.0.35` shows a published, non-latest release with the full asset set. Live pointers unchanged.

**P3 — Promote 1.0.35.** *(Builder B; ATC go required; live pointer)*
Run the documented `promote=true` path with `expected_live_macos_version=1.0.29` and the current live commit.
**Done-check:** `curl -s https://downloads.burnbar.ai/latest-macos.json` reports `1.0.35`; `https://downloads.burnbar.ai/OpenBurnBar-1.0.35-macOS.dmg` returns `200`; `bash scripts/ci/verify-public-macos-download-trust.sh` passes.

**P4 — Ship npm `openburnbar@0.1.1`.** *(Builder A)*
Not outage-blocked; the tag was simply never pushed (§3.2). Dry-run first, then push `openburnbar-npm-v0.1.1`.
...
**P5 — Ship CubeLove.** *(Builder B)*
Deploy the pending CubeLove change by whichever path is currently healthy.
**Done-check:** `curl -s https://cubelove.ai | grep release-sha` returns the intended commit SHA.

### Phase B — Take GitHub out of the customer path (still nothing migrates)

**P6 — Repoint update feeds at first-party immutable R2 URLs.** *(Builder A; live pointer; the highest-value packet in this plan)*
...
**P7 — Mirror the remaining artifacts to R2.** *(Builder B)*
...
**P8 — Give the Mac prerelease channel a non-GitHub feed.** *(Builder A)*
...
**P9 — Move the cross-run artifact handoff to R2.** *(Builder B)*
...
**P10 — Make the merge gate outage-tolerant.** *(Builder A)*
...
**P11 — Mirror build-time dependencies.** *(Builder B)*
...
**P12 — Resolve the seat/team question.** *(ATC)*
...
**P13 — Claim the codebase name.** *(ATC)*
One-time, irreversible-ish, and it defines every future Origin URL. ATC does it deliberately, after P12.
**Done-check:** `cursor.com/codebase/{owner}` resolves; `{owner}` is recorded here, replacing the placeholder.

**P14 — Mirror `BurnBar` into Origin.** *(Builder A)*
...
**P15 — Use Origin for review on one low-risk PR.** *(Builder B, reviewed by Builder A)*
...
**P16 — Attach a cloud agent and one automation.** *(Builder A)*
...
**P17 — Buildkite macOS feasibility spike.** *(Builder B; timeboxed; draft only)*
...
**P18 — Get the Origin API reference.** *(ATC)*
...
**P19 — Get a public-visibility answer.** *(ATC)*
...
**P20 — Inventory CubeLove.** *(Builder A; blocked on §10-U1)*
...
**P21 — CubeLove cutover, if and only if P17/P20 justify it.** *(Builder B)*
...
**P22 — Decide BurnBar's disposition.** *(ATC)*
Given P17/P18/P19, either schedule a cutover or record "mirror-only, revisit on <named Origin capability>". A written non-decision is a decision; an unwritten one is drift.
...
*Mitigation:* P6 makes every feed URL versioned and first-party **before** any host change. Never publish a feed whose enclosure URL can change meaning.
...
*Mitigation:* P13, ATC-only, after P12.
...
*Mitigation:* no automation against Origin's permissions or rules until P18 lands. Treat Phase D as evaluation, not as infrastructure.
...
*Mitigation:* P11. Otherwise the migration produces a false sense of independence.
...
*Mitigation:* P19 gates any BurnBar cutover. This is a licensing exposure, not merely a convenience issue — it warrants counsel review before any move.
...
*Mitigation:* P12 before P13, and before any migration is scheduled.
...
| **U2** | Whether external CI can report check runs onto an Origin PR | `cursor.com/docs/api/origin.md` returns 404 | P18 — ask `hi@cursor.com`. **This single answer determines whether Origin can ever gate a macOS product.** |
| **U3** | Whether three individual Ultra accounts can share one Origin codebase | Origin sharing is documented only in terms of Cursor **team** membership; Ultra is an individual plan | P12 — ask `hi@cursor.com`; price a Cursor Teams plan as the fallback |
| **U4** | Whether Origin will support **public** repositories | Only Internal and Private are documented | P19 |
| **U5** | Whether Buildkite can run signed, notarized macOS builds for an Origin-hosted repo | Buildkite has hosted Apple-silicon agents, but no Origin + macOS documentation exists from either vendor | P17 spike on a throwaway repo |
| **U6** | Origin SSH remote URL format | Only HTTPS is documented, though the CLI manages SSH keys (`origin ssh-key add`) | Ask, or read the green **Code** button after P13. **Do not assume a format.** |
| **U7** | Origin storage quotas, repo/file size limits, LFS | Not documented | Relevant because release evidence and vendored frameworks are large. Ask before P14. |
...
- **No Origin slug guessed.** Every Origin URL is written as `{owner}` until a human claims the name (P13).
...
Origin went early-beta today and it is a real git host with real pull requests, but it has no CI, no releases, no artifacts, no package registry, no secret storage, and no public repositories — so BurnBar, which is public, AGPL, notarized, and shipped through release assets and npm, cannot leave GitHub yet. CubeLove probably can, and is the right first migration once we can read its repo. The thing worth doing this week has almost nothing to do with Origin: our live macOS update feed is hosted on Cloudflare but points its download URLs at GitHub, so today's outage gave every updating customer a coin-flip chance of failure while our own R2 copy of the DMG sat there working. Fix that and GitHub stops being a customer-visible dependency. And we should be honest that the outage did not block 1.0.35 by itself — one of the two failing jobs was our own brittle test asserting on the text of `firestore.rules`, and that gate is still red on `main`, so the next tag fails the same way on any host we pick.
