...
The producer must also exercise the trusted-device/App Check-gated
`daemon.account.cloud_data.export` route. Use an absolute owner-selected
destination and verify that the response contains only the destination path,
byte count, and schema version. The resulting file must be owner-only schema
version 2 JSON with no raw credentials, sealed references, action proofs, or
renderer payload. An unauthenticated or untrusted request must fail closed
without creating or replacing a destination.
