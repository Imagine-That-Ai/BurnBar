...
| Text expansion | Substitute | Linux is intentionally in-app only. macOS uses a global CGEvent tap. | Safe Linux IME/fcitx/IBus-style system-wide design, no keylogger, with compositor/desktop proof. |
| Pet companion | Partial | Linux now has route/preview/tier behavior, a contained daemon-backed chat bubble, bounded file-drop staging, and a generated/searchable/persisted catalog of 114 shared entries (112 GLB-backed entries plus three legacy atlas forms, including one hybrid) with selected-form rendering (2026-08-04 source slices). macOS still has a transparent non-activating panel, offscreen 3D thumbnail-backed avatar/form selection, ambient behavior, and richer persona/local-floor chat. | Linux ambient overlay where supported, Wayland-safe fallback, click-through/topmost proof, full thumbnail/2D-form parity, behavior/persona/local-floor behavior, and installed catalog/chat/drop proof. |
