...
| Pet companion | Partial | Linux now has route/preview/tier behavior, a contained daemon-backed chat bubble, bounded file-drop staging, and a generated/searchable/persisted catalog for all 110 synced 3D pet avatars (2026-08-04 source slices). macOS still has a transparent non-activating panel, thumbnail-backed avatar/form selection including legacy 2D atlas forms, ambient behavior, and richer persona/local-floor chat. | Linux ambient overlay where supported, Wayland-safe fallback, click-through/topmost proof, thumbnail/2D-form parity, persona/local-floor behavior, and installed catalog/chat/drop proof. |
