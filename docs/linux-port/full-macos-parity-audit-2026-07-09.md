...
| Pet companion | Partial | Linux now has route/preview/tier behavior plus a contained daemon-backed chat bubble and bounded file-drop staging (2026-08-04 source slice). macOS still has a transparent non-activating panel, hover toolbar/avatar selection, ambient behavior, and richer persona/local-floor chat. | Linux ambient overlay where supported, Wayland-safe fallback, click-through/topmost proof, avatar/form selection, persona/local-floor behavior, and installed chat/drop proof. |
