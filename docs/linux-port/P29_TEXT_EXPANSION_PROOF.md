...
Deb, RPM, and AUR candidates package the executable-bound, release-signed IBus engine, its component XML, and its schema-v2 manifest. They also package the **native Fcitx5 addon** (`/usr/lib/openburnbar/fcitx5/openburnbar-fcitx5.so`, built from `packaging/linux/fcitx5-addon/` by `scripts/linux-port/build-fcitx5-addon.sh`) with its own release-signed schema-v2 manifest at `/usr/share/openburnbar/text-expansion/text-expansion-engine-fcitx5.json` and Fcitx5 registration metadata under `/usr/share/openburnbar/text-expansion/fcitx5/`. The daemon resolves signed manifests per backend, so the IBus manifest can never satisfy the Fcitx5 signature gate or vice versa. Standalone AppImage system expansion remains unavailable for both backends because an engine signed for a canonical installed identity cannot safely authenticate a transient mount path; in-app expansion is unaffected.

The package also carries `/usr/share/openburnbar/text-expansion/fcitx5-openburnbar-addon.json`, the machine-readable Fcitx5 capability contract (`status: native-addon-packaged`). It records the addon paths, the required `Fcitx5Core` headers, and the same no-global-capture, no-clipboard, no-surrounding-text, and secure-field-denial invariants as the IBus engine. `scripts/linux-port/validate-fcitx5-addon-source.mjs` fails closed if the addon source loses a safety marker, a package recipe drops the module or its signed manifest, the AppImage claims system registration, or IBus support is removed.
