...
4. **Perf budgets.** Watch `runtime-perf-samples.jsonl` against thresholds; if a surface (chat streaming, foil rendering) threatens a budget, file a `FIX` on that lane with the numbers — budgets do not move to accommodate regressions.
