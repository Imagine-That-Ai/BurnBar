...
| Latest provider-ingestion continuation | `d8607f4d2f` and `567c152058` close model-authorized native audio attachments across the shared policy, Linux picker, daemon MIME/capability gate, and `input_audio` transport. The follow-up OMP slice adds `OMPParser`, registers it in the macOS and Linux parser maps, changes the generated 33-provider catalog from 27 to 28 local parsers (OMP uses the Pi-compatible nested JSONL contract), and adds exact-token/cache/transcript regression coverage. OpenClaude remains explicitly unavailable until its canonical session fixture contract is committed. These are source-level parity improvements; installed signed-candidate scans, production cloud trust, physical-device flows, and the strict **0/40 + 0/7** ledger remain open. |
...
A typed factory map covers all 28 local-parser identities, and an absent
...
the same 28 local parser identities in its daemon default registry, with
