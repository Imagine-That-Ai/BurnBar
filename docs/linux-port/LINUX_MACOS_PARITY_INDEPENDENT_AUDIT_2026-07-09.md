...
| Latest provider-ingestion continuation | `d8607f4d2f` and `567c152058` close model-authorized native audio attachments across the shared policy, Linux picker, daemon MIME/capability gate, and `input_audio` transport. The OMP slice adds `OMPParser`, and the OpenClaude follow-up reuses the Claude-compatible project JSONL parser with a provider-specific identity/cache namespace. The generated 33-provider catalog now has 29 local parsers and 4 API/native-ledger sources, with exact-token/cache/transcript regression coverage for both additions. These are source-level parity improvements; installed signed-candidate scans, production cloud trust, physical-device flows, and the strict **0/40 + 0/7** ledger remain open. |
