...
| LNX-CHAT-CITATIONS-APPROVALS-001 | Implemented in source; installed proof blocked | Bounded citation normalization/source focus and daemon-issued approval IDs with approve/reject/cancel single-flight handling; attachment upload/metadata, reconnect/visibility, functional options, pop-out boundary, daemon-authoritative full-history export, validated resume, and config-derived backend availability are now integrated in `0d8ee32526`, `618c7286b9`, and `e0451afa5e`; 75-file/648-test aggregate and focused daemon chat contracts pass | Run unloaded-history/reconnect/pop-out/backend catalog flows against an installed Linux daemon; re-upload staged attachments after daemon restart; compare with macOS behavior |
