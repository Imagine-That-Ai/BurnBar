...
`72afa18f7e`; the latest installed package is bound to that exact source head.
...
- The ARM64 VM validated the supported Swift-less staged-payload path with
  ...
  rebuilt at `72afa18f7e` with native GStreamer linkage while reusing the previously clean verified daemon
