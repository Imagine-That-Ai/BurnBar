...
| Native shell | Single-instance/background launch, typed deep links, live tray facts/actions, compact status window, source-level freedesktop notification actions, and user XDG login start are implemented | Certify rich/icon-only GNOME, KDE, X11, wlroots, accessibility, notification, cloud inline-reply, provider OAuth return, and lifecycle rows |
...
| P-26 | Tray and native shell | Rich live menu-bar status, quick switch, chat, quota, update state | Live cost/tokens, quota floor, provider count, freshness, compact status window, dashboard/chat/provider/update/reconnect/login-start/quit actions exist; account quick switch, host-loss behavior, and installed matrix remain | Near parity | High |
| P-27 | Notifications/deep links | Actionable notifications, OAuth return, global commands, login start | Single-instance validated deep links, membership return, background launch, XDG login start, source-level freedesktop notification actions, and daemon-backed panic shortcut exist; cloud agent-reply listener/inline reply, provider OAuth callback contract, and installed shortcut/desktop matrix remain | Partial | High |
...
compact status window, and source-level freedesktop notification action
primitive implemented; installed certification, cloud inline reply, and provider
...
canonical autostart entry. Direct ARM64 GNOME X11 evidence now proves hidden
...
freedesktop notification server, or desktop-matrix certification. See
...
- **Difference:** macOS provides a rich menu-bar experience with live cost,
  ...
  and background launch. It still lacks account quick switch, cloud agent-reply
  inline reply, installed notification certification, and a provider OAuth
  callback matrix.
...
- **Recommended solution:** certify the implemented tray, compact status
  ...
  cloud agent-reply listener/inline reply, dedicated OAuth callback validation,
  and portal/global shortcut coverage with X11 fallback.
- **Priority:** **High**.
...
| LNX-NATIVE-001 | Partially implemented | Single-instance launch, typed deep links, live tray facts/actions, compact status window, source-level freedesktop notification actions, and XDG login start | Cloud inline reply, provider OAuth return, and installed desktop/notification matrix |
...
| LNX-NATIVE-001 | LNX-CAP-001 | Single-instance/background launch, typed navigation/membership deep links, live tray facts/actions, compact status window, source-level freedesktop notification actions, and XDG startup are implemented; add cloud inline reply, provider OAuth return, and exact-candidate matrix certification | Repeated native workflows pass on GNOME/KDE/wlroots, rich and icon-only hosts, accessibility, notification, and lifecycle rows |
...
| 4 | **Daily-use native foundation** | In progress; onboarding, bounded event refresh, single-instance/deep-link, live-tray, compact status, source-level notifications, XDG login-start, and account-auth foundations implemented; native Linux App Check attestation, membership/cloud/device depth, cloud inline reply, provider OAuth return, and installed matrix remain | LNX-EVT-001, LNX-ONB-001, LNX-AUTH-DEPLOY-001, LNX-APPCHECK-001, LNX-MEMBERSHIP-001, LNX-SYNC-001, LNX-DEVICE-001, LNX-NATIVE-001, LNX-UPD-001, LNX-CAT-001, LNX-DIFF-001 | Setup, deployed auth, data freshness, alerts/tray, update lifecycle, and current provider diff green |
