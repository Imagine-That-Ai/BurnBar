...
| P21 insights workspace | PR #1669 + integration `5ddd81245d` | Provenance-labeled brief now sits above a selectable three-pane canvas/library/inspector workspace with trend, provider, model, and cache widgets, bounded evidence audit disclosure, refresh, and a chat follow-up composer. Citations, daemon-backed qualitative analysis, and persisted canvas state remain open. |
...
| P14 exact chat threads | PR #1684 + integration PR #1691 + `0d8ee32526` | Canonical encrypted thread list/get/search, idempotent append, older-message pagination, strict Tauri decoding, durable send ordering, loaded-transcript JSON/Markdown export, exact model/thinking selection, daemon-owned bounded attachment refs, citations, and approval actions are integrated. Composer uploads bounded text documents through the daemon and persists path-free metadata; reconnect/visibility handling, functional options, and browser/Tauri pop-out boundaries are explicit. Uploaded bytes remain process-lived and require re-upload after daemon restart; unloaded-history export/resume and remaining backends remain open. |
...
| Integrated Linux validation | PR #1691 plus commits through `9a527310f9`, current 2026-07-13 | Replayed the current P07/P11/P13/P14/P16/P17/P19/P20/P21/P22/P23/P24/P25/P27/P28/P29/P30/P31/P35/P39/P40 source stack: **74 Vitest files / 621 tests**, TypeScript, production bundle verification, axe route audit, differential-oracle tests 6/6, Tauri Rust **87/87**, RPC canon/formatting/suppression checks, focused chat Swift contract/service tests, and the OpenBurnBarDaemon target build. The Linux-only native notification/Secret Service/KWallet paths require a Linux host for live capability and runtime receipts; the macOS host's recovery test bundle is blocked by its existing missing SQLCipher.framework packaging. This is engineering evidence only, not installed-product certification. |
...
| LNX-INSIGHT-001-FOLLOWON | Implemented in source; installed proof blocked | `5ddd81245d` adds a three-pane canvas/library/inspector, selectable evidence widgets, bounded audit disclosure, refresh, and chat follow-up handoff above the provenance-labeled brief; focused Insights and axe tests pass | Add daemon-backed citations/qualitative comparison and persisted canvas state, then certify the installed workflow |
