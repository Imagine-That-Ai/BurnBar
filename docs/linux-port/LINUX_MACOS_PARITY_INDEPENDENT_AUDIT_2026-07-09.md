...
| P40 data/privacy | PR #1672, `4cdc505537` | Daemon-backed telemetry/privacy/cloud-sync writes with pending/error states, explicit unavailable destructive/recovery rows, and a scoped daemon-owned proxy-route-log retention action with count/refresh/two-step clear/error handling; account/local deletion, encrypted export, and backend erasure receipts remain open. |
...
| Integrated Linux validation | PR #1691 plus commits through `b012a53a6c`, `5fdcccabfa`, `4cdc505537`, `dda966ff3a`, and `573184422d`, current 2026-07-13 | Replayed the current P07/P11/P13/P14/P16/P17/P19/P20/P21/P22/P23/P24/P25/P27/P28/P29/P30/P31/P35/P39/P40 source stack: **76 Vitest files / 666 tests**, TypeScript, production bundle verification, axe route audit, differential-oracle tests 6/6, parity-validator/matrix contracts 41/43 (2 Linux-only skips), Tauri Rust **88/88**, pinned Linux arm64 native **277/277 Swift selectors** (14 core + 25 security + 238 daemon), RPC canon/formatting/suppression checks, focused chat/activity/recovery Swift contract/service tests, text-expansion mutation ordering, matrix evidence identity binding, isolated SwiftPM scratch state, and the OpenBurnBarDaemon target build. `573184422d` also hardens accepted single-instance IPC streams against inherited nonblocking-mode races (20/20 targeted repeats). The Linux-only native notification/Secret Service/KWallet paths require a Linux host for live capability and runtime receipts; the macOS host's recovery test bundle is blocked by its existing missing SQLCipher.framework packaging. This is engineering evidence only, not installed-product certification. |
...
| LNX-CHAT-CITATIONS-APPROVALS-001 | Implemented in source; installed proof blocked | Bounded citation normalization/source focus and daemon-issued approval IDs with approve/reject/cancel single-flight handling; attachment upload/metadata, path-free attachment metadata in JSON/Markdown export, reconnect/visibility, functional options, pop-out boundary, daemon-authoritative full-history JSON/Markdown export, strict unloaded-page traversal, validated resume, and config-derived backend availability are now integrated in `0d8ee32526`, `618c7286b9`, `e0451afa5e`, `dd705ca1dd`, and `5fdcccabfa`; focused chat lane: 6 files / 44 tests plus app TypeScript pass | Run unloaded-history/reconnect/pop-out/backend catalog flows against an installed Linux daemon; re-upload staged attachments after daemon restart; compare with macOS behavior |
...
| LNX-PRIVACY-ROUTE-RETENTION-001 | Implemented in source; installed proof blocked | `4cdc505537` adds daemon-backed local proxy-route-log count/refresh and explicit two-step clear/cancel/error state in Data & Privacy; copy explicitly excludes transcripts, credentials, and account data; 20 SettingsSurface tests and app TypeScript pass | Exercise count refresh, confirm/cancel, clear success/failure, daemon restart, locked keyring, and ensure unrelated transcript/credential/account data remains intact |
