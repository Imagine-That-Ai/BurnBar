...
`3258104c1a`; the latest installed package is bound to that exact source head.
...
- `106b15855b` retains the last good mission snapshot during transient refresh
  ...
  transient refresh errors while preserving visible-row exports.
- Source gates pass: 91 frontend files / 861 tests, TypeScript, production
...
- The ARM64 VM validated the supported Swift-less staged-payload path with
  ...
  rebuilt at `3258104c1a` while reusing the previously clean verified daemon
...
| P-21 | Insights | Editorial brief, evidence, citations, follow-ups, comparison, audit | Provenance-labeled brief plus selectable canvas/library/inspector workspace, normalized trend/provider/model/cache widgets, bounded audit disclosure, refresh, chat follow-up handoff, and macOS-style three-scope Compare mode with per-column provenance and source-loss fail-closed behavior are implemented. The exact-head arm64 package is installed in the non-certifying VM receipt `current-b590d5a77-media-settings-onboarding-arm64`; daemon-backed citations, qualitative comparison against macOS, persisted canvas state, and signed/cross-device proof remain open | Partial | Medium |
