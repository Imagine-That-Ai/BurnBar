...
| P29 text expansion | PR #1663 + integration PR #1691 + `6c76df084f` + `1ddc8bc33a` + `274f67fba0` | Daemon-owned AES-GCM sealed snapshots use native Secret Service/KWallet key custody, consent RPC, owner-only permissions, corruption/tamper fail-closed behavior, and in-app-only Composer expansion without renderer localStorage/global capture. Live renderer controls now remain disabled with explicit degraded copy until daemon storage hydration succeeds, including missing-storage and error paths. `274f67fba0` adds a bounded, explicitly opted-in, signed IBus/Fcitx engine manifest/registration gate with owner/path/permission/session checks and secure-field policy; it still does not claim live external expansion, secure-field hooks outside the app, or sync. |
...
| Integrated Linux validation | PR #1691 plus commits through `b012a53a6c`, `5fdcccabfa`, `4cdc505537`, `dda966ff3a`, `573184422d`, `7ae2412143`, `1ddc8bc33a`, `be375903f0`, `274f67fba0`, and `ea82fe5140`, current 2026-07-14 | Replayed the current P07/P11/P12/P13/P14/P16/P17/P19/P20/P21/P22/P23/P24/P25/P27/P28/P29/P30/P31/P35/P39/P40 source stack: **78 Vitest files / 684 tests**, TypeScript, production bundle verification (303 modules), axe route audit, differential-oracle tests 6/6, parity-validator/matrix contracts 41/43 (2 Linux-only skips), Tauri Rust **90/90**, pinned Linux arm64 native **277/277 Swift selectors** (14 core + 25 security + 238 daemon), RPC canon/formatting/suppression checks, focused chat/activity/recovery Swift contract/service tests, text-expansion storage-readiness and signed-engine boundary tests, quota account-switching Settings tests, matrix evidence identity binding, isolated SwiftPM scratch state, and the OpenBurnBarDaemon target build. `573184422d` hardens accepted single-instance IPC streams against inherited nonblocking-mode races (20/20 targeted repeats), `7ae2412143` rejects unverified Activity replay/resume identities, `274f67fba0` adds signed external-engine registration checks, and `ea82fe5140` adds fail-closed Wayland portal probing plus the X11 pet companion contract. The Linux-only native notification/Secret Service/KWallet/input-method/portal paths require a Linux host for live capability and runtime receipts; the macOS host's recovery test bundle is blocked by its existing missing SQLCipher.framework packaging. This is engineering evidence only, not installed-product certification. |
...
| LNX-TEXT-001 | LNX-SEC-001, LNX-EVT-001 | **Implemented in source:** app Composer integration and encrypted persistence/safe consent are complete; `6c76df084f` adds bounded IBus/Fcitx reachability and `274f67fba0` adds an explicitly opted-in, signed external-engine registration gate with owner/path/permission/session checks, without keyboard/clipboard/surrounding-text capture. Live engine execution, secure-field hooks, sync, and conflict resolution remain required | Secure-field and desktop/input-method matrix green with a registered engine; unsupported or uninspectable contexts stay disabled |
