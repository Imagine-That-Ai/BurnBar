...
| Remediation evidence | Latest parity hardening and installed arm64 candidate reach `5e0fc0e82` (with the WebKit startup fallback in `6321897d4e`). The running Ubuntu 24.04.4 GNOME/X11 UTM guest has the exact-head DEB installed; its non-certifying live receipt is `evidence/mission-002-reanchor/vm-e2e/current-5e0fc0e82-insights-autostart-arm64/live-installed-receipt.json`. It records daemon/desktop health, GStreamer, Secret Service, IBus, autostart packaging, and current source gates. The package is unsigned; AT-SPI route capture and cross-device proof remain open. The earlier settings/media receipt under `current-992ef5c580-settings-index-arm64/` and the preceding media-gst/UI receipts remain historical. The clean parity-ledger validation is bound to checkpoint `073c2aba45`. Release `29664085758` and Nightly `29660228199` remain historical engineering evidence. Full certification remains intentionally blocked because the strict product and environment receipts are still missing. |
...
`5e0fc0e82` arm64 package is installed; its daemon is enabled and healthy, the
...
full Linux desktop run is **83 files / 785 tests**; TypeScript, production
...
`192.168.64.5`; the exact `5e0fc0e82` package is installed and authenticated
...
The current source slice is integrated at `5e0fc0e82`: Linux Insights has a
...
| P-21 | Insights | Editorial brief, evidence, citations, follow-ups, comparison, audit | Provenance-labeled brief plus selectable canvas/library/inspector workspace, normalized trend/provider/model/cache widgets, bounded audit disclosure, refresh, chat follow-up handoff, and macOS-style three-scope Compare mode with per-column provenance and source-loss fail-closed behavior are implemented. The exact-head arm64 package is installed in the non-certifying VM receipt `current-5e0fc0e82-insights-autostart-arm64`; daemon-backed citations, qualitative comparison against macOS, persisted canvas state, and signed/cross-device proof remain open | Partial | Medium |
...
| P-24 | Settings | 16 searchable tabs with deep links and writable state | 16-tab searchable inventory and deep routes are wired. General Settings Indexing & Search exposes a fixture-safe daemon-backed **Index project** action through `databaseIndexProject` (`992ef5c580`) plus a secure Linux-native **Launch at login** preference with owner-checked XDG persistence and unavailable-source fail-closed behavior (`5e0fc0e82`). The exact-head package is installed in the non-certifying VM receipt `current-5e0fc0e82-insights-autostart-arm64`; deeper per-tab backend writes and signed/live proof remain open | Partial | Medium |
