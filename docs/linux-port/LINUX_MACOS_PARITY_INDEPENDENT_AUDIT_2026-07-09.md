...
attestation. A real Firebase Web app ID, TPM quote backend,
...
| Linux App Check | Durable five-minute challenge, atomic replay defense, pinned signed-verdict boundary, fixed 30-minute TTL, daemon memory-only acquisition/cache, hash-only ticket reservation, exact ingress claim, streamed evidence upload, receipt-native mint evidence, root broker/native deb-rpm packaging, root-owned enrollment-state binding, and the 16 MiB evidence ceiling are source-implemented; mint kill switch defaults off | Provision the Firebase Web app; implement real TPM quote collection, complete IMA evidence and remote verification/revocation, deploy ingress/verifier, run negative vectors, and certify the installed matrix; Linux remains `linux_lower_trust` |
...
- [ ] Provision the real Firebase Web app ID; implement TPM quote collection and
