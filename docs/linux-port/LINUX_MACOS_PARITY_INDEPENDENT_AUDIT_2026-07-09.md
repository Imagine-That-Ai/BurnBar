...
| Audit field | Value |
|---|---|
| Date | Baseline audit: 2026-07-09; current-candidate source audit through 2026-08-10 UTC |
...
| Current candidate checkpoint | Audited implementation baseline `8e706f5d9b2cd4c09a4a197f60f8d8c04bda6416` on `codex/linux-macos-parity-completion-20260809` was clean before the non-certifying audit/copy correction and remains unpushed. The complete parity preflight passed **46/46** in **413.83 seconds**, and the focused registry/runtime/Fcitx/P-30/P-40/readiness packet passed **29/29**. The candidate tree packages a real native Fcitx5 addon, registers P-30 proof ownership, includes pet chat/file-drop/catalog/atlas behavior, exposes daemon-owned privacy retention and account erasure, and uses stable shell-readiness selectors. These are source and harness results only: the post-audit exact candidate has not yet run in the X31 Linux VM, is not signed/promoted, and the strict ledger correctly remains **0/40 product requirements and 0/7 environments**. |
...
| Historical safety hardening | `9c58df421d` requires explicit signed-feed verification and fresh metadata before Linux install, rollback, or download guidance is enabled; missing metadata fails closed and leaves restart guidance available. `0c243ec95a` originally added an honest Fcitx5 source-only capability contract. That Fcitx status is **superseded** by `ac0ce21685`, which builds and packages a real native addon with its own signed-manifest identity, and `386dde168b`, which refreshes the proof documentation. Live exact-candidate feed/package/IME execution and the strict **0/40 product + 0/7 environment** ledger remain open. |
...
| Latest provider-ingestion continuation | `d8607f4d2f` and `567c152058` close model-authorized native audio attachments across the shared policy, Linux picker, daemon MIME/capability gate, and `input_audio` transport. The OMP slice adds `OMPParser`, and `b66a1c55c8` adds OpenClaude local ingestion by reusing the Claude-compatible project JSONL parser with a provider-specific identity/cache namespace. The generated 33-provider catalog now has 29 local parsers and 4 API/native-ledger sources, with exact-token/cache/transcript regression coverage for both additions. These are source-level parity improvements; installed signed-candidate scans, production cloud trust, physical-device flows, and the strict **0/40 + 0/7** ledger remain open. |
| Current-head source receipt | `linux-source-tests-2026-07-24.json` records **87/87** focused Linux release/evidence and P-39 differential contract tests passing at `264b1c6278`; the receipt is explicitly non-certifying and covers no product rows. |
| Latest Linux quota transport hardening | `163703900c` adds daemon-owned provider CLI execution with concurrent stdout/stderr draining, a bounded 1 MiB output limit, and an 8-second timeout; `126bd1777b` fixes overflow handling so a noisy provider cannot leave a pipe unread and deadlock before termination. Linux-only XCTest coverage now exercises oversized output and a wedged provider. `swiftc -parse` and the focused Node contract suite pass on this host; the Linux XCTest receipt still awaits the Ubuntu/UTM lane, so this does not change the strict **0/40 + 0/7** certification count. |
...
### Historical Linux Fcitx5 package capability contract — 2026-07-23

> **Superseded on 2026-08-10:** this section records the earlier source-only
> checkpoint. `ac0ce21685` now builds and packages a native Fcitx5 addon with a
> separate signed-manifest identity, and `386dde168b` updates the installed
> proof contract. Live exact-candidate IME/keyring/secure-field execution is
...
This historical slice closed the package-capability ambiguity without claiming
runtime parity. The native addon has since been implemented and packaged; live
secure-field/consent execution and the Wayland/X11 receipt matrix remain open.
The strict ledger remains **0/40 product** and **0/7 environment**.
...
A typed factory map covers all 29 local-parser identities, and an absent
...
| P-29 | Text expansion | Global, secure-field-aware expansion, persistence, sync, previews | Registered candidate-bound proof drives a package-owned signed text engine through real GTK free-form and password fields, proving normal expansion, secure-field denial before write, daemon-owned AES-GCM persistence with Secret Service/KWallet custody, cancellation/kill/restart behavior, AT-SPI, screenshots, provenance, and exact prior-engine/state restoration. `ac0ce21685` adds the native Fcitx5 addon beside the existing IBus engine, with separate package paths and signed-manifest identity; standalone AppImage system registration remains unavailable by design. Live signed-candidate IBus/Fcitx5/keyring execution across the support matrix remains open | Partial | High |
| P-30 | Pet companion | Animated ambient overlay, click-through, summon, selection, interactions | Typed runtime-manifest probe, contained draggable fallback, accessible summon/focus/status, selection/clear controls, shared 114-entry catalog, bounded GLB/atlas readers, selected-form rendering, daemon-backed companion chat, and bounded file-drop staging are source-integrated. `3b652f9b9e` adds bounded pointer/mouse drag and Arrow/Home keyboard repositioning with focus metadata and announcements for the Wayland-safe contained substitute. `5c3caab2e` adds the X11-only `Ctrl+Alt+Super+P` summon route and `2b85f1431` exposes `aria-keyshortcuts`. `3acc18fad2` adds the registered candidate-bound installed proof package: X11 must prove the native child, always-on-top state, global summon, and explicit click-through restoration; Wayland must prove the contained fallback without an overlay claim; both prove selection/clear, pointer/keyboard movement, AT-SPI, relaunch, package identity, distinct screenshots, and exact restoration. The macOS offscreen 3D thumbnail pipeline, persona/local-floor behavior, and exact-candidate seven-environment execution remain open | Partial | High |
...
| P-40 | Data and Privacy | Vault/export/deletion/retention/recovery/consent/telemetry/panic workflows | Daemon-backed telemetry/privacy/cloud-sync consent, allowlisted inventory, preview/execute deletion, selected-scope encrypted export, bounded age/size retention, trusted-device-gated cloud export, a typed account-erasure path with partial-failure retry, and separate encrypted recovery-bundle controls are implemented. Historical Ubuntu 24.04 GNOME/X11 aarch64 RPC proof exists, but the current `8e706f5d9b` candidate has not been executed in the VM. Trusted-device backend deletion receipts, recovery propagation, and all seven current-candidate environment rows remain open | Partial | High |
...
- **Difference:** macOS provides global, accessibility-aware expansion with
  ...
  ships explicitly consented, signed IBus and native Fcitx5 integrations with
  trigger-only daemon expansion and secure-field denial. Sync and installed
  cross-desktop proof remain open.
...
- **Recommended solution:** retain the daemon boundary and explicit consent,
  finish the IBus/Fcitx5 keyring, secure-field, and desktop matrix receipts,
  preserve separate signed identities for both engines, and complete
  conflict-aware sync without adding global capture.
- **Priority:** **High**.
...
**Implementation update (2026-08-04, legacy-form continuation):** the same
catalog contract now merges the macOS `Resources/Pets` definitions, producing
114 unique entries with 112 GLB-backed forms and three atlas forms (two
atlas-only and one hybrid). Linux reads the selected atlas image through a
bounded `pet_atlas_read` command and renders the shared row/frame geometry in a
Canvas2D runtime, including reduced-motion behavior and logical-state aliases.
...
| LNX-PRIVACY-ROUTE-RETENTION-001 | Implemented in source; current-candidate proof blocked | `4cdc505537` plus `9598c0b9e8` add daemon-backed local-store inventory and exact preview/execute deletion for the allowlisted proxy-route log and encrypted text-expansion store. `7c8a214ce6` and `825e081bda` add selected-scope encrypted export with authenticated PBKDF2/AES-GCM envelope, bounded payloads, owner-only output, race/path/permission checks, typed daemon/Tauri wiring, passphrase clearing, and Settings controls. `cba9266277` adds complete bounded age/size retention for both stores. The account-erasure slice adds trusted-device-gated server authority, exact confirmation, redacted receipts, and retry for partial completion. `8131b51aec` adds the portal-backed native diagnostics save destination; Settings/bridge and Core/daemon contract tests pass | Exercise inventory, scope selection, preview expiry, confirm/cancel, clear/export/retention success and failure, restart, locked keyring, tamper/permission rejection, native save destination, trusted-device account erasure, backend receipts, and recovery propagation on the exact candidate |
...
| LNX-PET-001-FOLLOWON | Implemented in source; native overlay proof blocked | `9a527310f9` adds contained summon/focus/status and selection/clear controls with typed capability states; `ea82fe5140` adds a Tauri X11-only companion child with explicit focus and click-through toggles; `3b652f9b9e` adds bounded pointer/mouse drag and Arrow/Home keyboard repositioning; `5c3caab2e` adds the X11-only `Ctrl+Alt+Super+P` summon event and `2b85f1431` exposes accessibility metadata. The current candidate also includes daemon-backed companion chat, bounded file-drop staging, the shared 114-entry avatar catalog, selected-avatar persistence, GLB/atlas readers, and legacy atlas animation. `318d3430fd` keeps Wayland and unknown sessions fail-closed and contained | Certify X11 child/summon/click-through and Wayland contained behavior, plus chat/drop/catalog/atlas, GPU, focus, keyboard, scaling, reduced motion, restart, and multi-monitor behavior on the exact candidate |
...
| LNX-TEXT-001 | LNX-SEC-001, LNX-EVT-001 | **Implemented in source:** app Composer integration and encrypted persistence/safe consent are complete; `6c76df084f` adds bounded IBus/Fcitx reachability, `274f67fba0` adds an explicitly opted-in signed registration gate, `9598c0b9e8` adds daemon lifecycle status/start/stop RPCs with owner/path/permission/session checks, and `ac0ce21685` adds the native packaged Fcitx5 addon with separate signed-manifest identity, without keyboard/clipboard/surrounding-text capture. Live engine execution, secure-field hooks, sync, and conflict resolution remain required | Secure-field and desktop/input-method matrix green with both package-owned engines; unsupported, unregistered, AppImage, or uninspectable contexts stay disabled |
