...
| Audit field | Value |
|---|---|
| Date | Baseline audit: 2026-07-09; remediation evidence through 2026-07-20 UTC |
| Gold standard | OpenBurnBar for macOS |
| Linux target | `apps/linux-desktop` plus the shared OpenBurnBar daemon |
| Baseline checkout | `windows/liquid-glass-kernel-reskin` at `18836ae40a` |
| Current source proof | Source verification extends through `1854a46704`; the latest exact-native checkpoint remains `22fd7dfad9`. Frontend tests pass at 94 files / 910 tests, Tauri Rust at 133/133, and the broad Linux-port Node suite at 642 cases / 637 passed / 5 environment skips / 0 failures; TypeScript, ESLint, the production-bundle verifier, Cargo formatting, the Swift file-size budget, and the core target-membership budget also pass. Ten product requirements have registered producers and validators, and an isolated current-commit collector authenticates all ten owners with 30/30 executions passing. The other 30 requirements are not registered, and none of these source results replace installed proof. The exact DEB build/install and installed hash proof remain bound to `22fd7dfad9`: [`evidence/parity-audit-2026-07-10/linux-arm64-current-22fd7dfad9-exact-native-2026-07-20.json`](evidence/parity-audit-2026-07-10/linux-arm64-current-22fd7dfad9-exact-native-2026-07-20.json). |
...
The current source gate at `1854a46704` is **94 frontend files / 910 tests**,
...
Cargo-formatting, Swift file-size, and core target-membership checks. The exact
...
The fail-closed proof system has registered candidate-bound ownership for ten
requirements (`P-01`, `P-02`, `P-03`, `P-04`, `P-05`, `P-31`, `P-34`, `P-37`,
`P-38`, and `P-40`). An isolated collector rebuilt their exact source inputs
and passed all thirty producer, materializer, and validator executions. This is
**10/40 ownership-ready**, or **25% of the full requirement set**; the other
thirty requirements still need registered proof ownership.
...
| P-12 | Quota | Provider quotas, histories, account switching, alerts | Strong read surface with provider credential-slot account switching, explicit auto-routing reset, preferred-slot persistence, and per-slot quota/status labels. `6b707d24b8` retains the last catalog through transient failure. `b76b67e8bc` adds daemon-confirmed failover policy mutation with exact readback/rollback, and `8d9dba71fe` pins the macOS canonical modes. The exact ARM64 route passes AT-SPI; populated live switching/drain, thresholds, cloud profiles, and switching-lag telemetry remain open | Partial | Medium |
| P-13 | Onboarding | Provider connection, scan, permissions, chat engine, recovery, completion gates | Daemon-owned state, required-step gates, restart recovery, Secret Service/XDG/privacy readback, catalog-backed credential setup, and first-data readback are implemented. `21a13a6792` adds the real native cloud-auth state machine for start, polling, device approval, cancel, denial, expiry, retry, active verification, fixture, and unavailable outcomes. Production OAuth configuration, provider scan depth, portal consent, tray, and update completion remain open | Partial | High |
...
| P-27 | Notifications/deep links | Actionable notifications, OAuth return, global commands, login start | Native startup/deep-link handoff, tray/background/XDG registration, strict OAuth/membership parsing, owner-checked single-instance forwarding, freedesktop actions, and bounded bootstrap queues are implemented. `5d10b95bc8` extends the fail-closed protocol to typed provider/model destinations; the current exact ARM64 install proved a live second-instance provider route. Broader global-shortcut/compositor integration remains open | Partial | High |
...
| P-36 | Visual/interaction polish | Consistent components, responsive density, animations, native affordances | Nonblack installed route captures exist. `bfd0eefea9` fixes the WebKitGTK native-control palette after live QA found light selects with low-contrast text; multi-environment visual/regression coverage and remaining raw diagnostics polish remain open | Partial | Medium |
...
- **Implementation notes:** unavailable controls must explain the native
  ...
  config RPC (`3004da3b72`), and deeper per-tab daemon writes plus installed
  proof remain open.
...
| P07/P09/P11/P13/P14/P16/P17/P18/P19/P20/P22/P25/P27/P28/P29/P30/P31/P40 implementation slices | In review | Existing slices plus `0fd663a3f5`/`5d10b95bc8` typed provider/model navigation, `b74bc6d068` verified Activity source resolution, `21a13a6792` native cloud-auth onboarding states, and `bfd0eefea9` Linux-native control-color alignment are source-integrated. The typed provider destination is installed and AT-SPI proven in the current exact ARM64 receipt | Merge/rebase onto the release head, then prove remaining backends/binary attachments, populated Activity replay/resolution, production OAuth/portal consent, trusted-device mobile execution, cross-device memory, database recovery, mission health/history, global shortcut breadth, Linux keyring/IBus/Fcitx, secure fields, native pet adapters, and destructive privacy contracts |
