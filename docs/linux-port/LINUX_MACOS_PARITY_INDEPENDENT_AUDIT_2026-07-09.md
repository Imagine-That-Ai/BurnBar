...
| Audit field | Value |
|---|---|
| Date | Baseline audit: 2026-07-09; remediation evidence through 2026-07-21 UTC |
...
| Latest continuation delta | `96e021053b` raises the shared activity snapshot migration window from the legacy 80 conversations to a bounded 10,000-session window and adds a 10k-session/100-project regression, preserving older inferred projects. `92d87249ea` and `6a87c4ccc4` make P-39 producer evidence attest the real OS, architecture, kernel release, Node/Swift runtimes, resolved Swift executable digest, locale/timezone/feature flags, and the checked-in product version; resolver/workflow tests reject mismatched or synthetic execution metadata. The Xcode project now includes the previously omitted usage-ingestion, trusted-device, CloudSync daemon sources, and the daemon-test `OpenBurnBarInsights` framework dependency. `4bf26e57ee` qualifies the app-level OpenCode parser in the cockpit test. `2298e68f49` plus `058ff3c049` make Linux parser registration derive from the generated ingestion catalog with a fail-closed typed factory map, using the existing Codex parser symbol and preventing daemon-side catalog drift. `a6f3ec2b0f` centralizes macOS quota adapter coverage across all 19 declared quota providers, including explicit unavailable states; `18231a4a40` preserves Linux quota signal provenance instead of labeling local artifacts as provider/API data. `5000b5200b` plus `7bb470cfac` make the differential oracle reject duplicate JSON keys and non-canonical generated timestamps. `fb3afed33b` plus `faa50c0e7a` normalize notification settings readback and reject malformed fractional values. `df0822290b` requires daemon readback confirmation before replacing the provider catalog after a preferred-slot mutation. `708260e4f4` makes P-21 Insights evidence reject non-canonical timestamps, with mutation coverage passing 3/3. `b46f16c6ed` rechecks daemon chat-backend capability immediately before durable send, failing closed on stale or disabled selections; focused chat tests pass 50/50. The Linux quota RPC now reports canonical provider adapter coverage with explicit unavailable entries, and its focused tests pass 4/4. These commits reduce source-level gaps but do not create live signed or cross-environment receipts. |
