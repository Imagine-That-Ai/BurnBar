...
| P29 text expansion | PR #1663 + integration PR #1691 + `6c76df084f` + `1ddc8bc33a` | Daemon-owned AES-GCM sealed snapshots use native Secret Service/KWallet key custody, consent RPC, owner-only permissions, corruption/tamper fail-closed behavior, and in-app-only Composer expansion without renderer localStorage/global capture. Live renderer controls now remain disabled with explicit degraded copy until daemon storage hydration succeeds, including missing-storage and error paths. A bounded daemon probe now reports IBus/Fcitx reachability, desktop session type, registration state, and secure-field policy without reading keyboard, clipboard, or surrounding text. The signed external engine, secure-field hooks outside the app, and sync remain open. |
...
| Integrated Linux validation | PR #1691 plus commits through `b012a53a6c`, `5fdcccabfa`, `4cdc505537`, `dda966ff3a`, `573184422d`, `7ae2412143`, `1ddc8bc33a`, and `be375903f0`, current 2026-07-13 | Replayed the current P07/P11/P12/P13/P14/P16/P17/P19/P20/P21/P22/P23/P24/P25/P27/P28/P29/P30/P31/P35/P39/P40 source stack: **77 Vitest files / 674 tests**, TypeScript, production bundle verification (302 modules), axe route audit, differential-oracle tests 6/6, parity-validator/matrix contracts 41/43 (2 Linux-only skips), Tauri Rust **88/88**, pinned Linux arm64 native **277/277 Swift selectors** (14 core + 25 security + 238 daemon), RPC canon/formatting/suppression checks, focused chat/activity/recovery Swift contract/service tests, text-expansion storage-readiness fail-closed tests (20/20), quota account-switching Settings tests (23/23), matrix evidence identity binding, isolated SwiftPM scratch state, and the OpenBurnBarDaemon target build. `573184422d` hardens accepted single-instance IPC streams against inherited nonblocking-mode races (20/20 targeted repeats), while `7ae2412143` rejects unverified Activity replay/resume identities. The Linux-only native notification/Secret Service/KWallet paths require a Linux host for live capability and runtime receipts; the macOS host's recovery test bundle is blocked by its existing missing SQLCipher.framework packaging. This is engineering evidence only, not installed-product certification. |
