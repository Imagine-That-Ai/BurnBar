...
| P40 data/privacy | PR #1672, `4cdc505537`, `9598c0b9e8` | Daemon-backed telemetry/privacy/cloud-sync writes with pending/error states, plus a daemon-owned, allowlisted local-store inventory and preview/execute deletion contract wired through typed RPC/Tauri and a confirmation UI for the proxy-route log and encrypted text-expansion store. Scope, owner/perms, expiry, fingerprints, exact confirmation, idempotent deletion, and fail-closed errors are enforced; account erasure, full export, retention policy, recovery-key workflows, and backend erasure receipts remain open. |
...
| Integrated Linux validation | PR #1691 plus commits through `b012a53a6c`, `5fdcccabfa`, `4cdc505537`, `dda966ff3a`, `573184422d`, `7ae2412143`, `1ddc8bc33a`, `be375903f0`, `274f67fba0`, `ea82fe5140`, and `9598c0b9e8`, current 2026-07-14 | Replayed the current P07/P11/P12/P13/P14/P16/P17/P19/P20/P21/P22/P23/P24/P25/P27/P28/P29/P30/P31/P35/P39/P40 source stack: **78 Vitest files / 685 tests**, TypeScript, production bundle verification (303 modules), isolated Pet/text-expansion UI tests **19/19**, changed Settings/bridge tests **106/106**, differential-oracle tests 6/6, parity-validator/matrix contracts 41/43 (2 Linux-only skips), Tauri Rust **90/90**, pinned Linux arm64 native **277/277 Swift selectors** (14 core + 25 security + 238 daemon), RPC canon/formatting/suppression checks, focused chat/activity/recovery Swift contract/service tests, text-expansion storage-readiness and signed-engine lifecycle boundary tests, Mission Control replay/tombstone tests, and privacy inventory/preview/execute contract tests. The full 78-file Vitest run was attempted but host contention caused unrelated 5-second timeouts and one unhandled media fixture error; it is not treated as a green release gate. The Linux-only native notification/Secret Service/KWallet/input-method/portal paths require a Linux host for live capability and runtime receipts; the macOS host's recovery test bundle is blocked by its existing missing SQLCipher.framework packaging. This is engineering evidence only, not installed-product certification. |
...
| LNX-PRIVACY-ROUTE-RETENTION-001 | Implemented in source; installed proof blocked | `4cdc505537` plus `9598c0b9e8` add daemon-backed local-store inventory and exact preview/execute deletion for the allowlisted proxy-route log and encrypted text-expansion store. The control binds a five-minute scoped token to owner/perms/fingerprints, requires exact confirmation, uses idempotent unlink, and explicitly excludes transcripts, credentials, and account data; Settings/bridge and daemon contract tests pass | Exercise inventory, scope selection, preview expiry, confirm/cancel, clear success/failure, restart, locked keyring, tamper/permission rejection, and ensure unrelated transcript/credential/account data remains intact |
