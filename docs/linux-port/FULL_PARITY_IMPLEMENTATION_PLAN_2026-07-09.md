...
> `6d71bc8aff`, `7a8391acb5`, and `b2bcfa4b3f` add and compose the daemon-owned
> Firebase cloud-replica gateway: strict HTTPS host/path allowlisting,
> daemon-resolved Auth/App Check credentials, bounded callable envelopes,
> idempotent transactional compare-and-merge handlers, monotonic pull cursors,
> and redacted RPC runtime wiring. Focused validation is green: Functions
> TypeScript build, Vitest 3/3, ESLint, and the Linux gateway Swift target
> 131 tests with one Docker-only environment skip. These source slices reduce
> implementation gaps but do not promote the strict 0/40 or 0/7 certification
> counters; production credentials/deployment and signed installed/device
> evidence remain the release-critical dependency.
>
> **Continuation update (2026-07-21, follow-on slices):** `5a37e88f8e` now
> registers all 27 available local parser identities on Linux, including Aider,
> Cursor SQLite, OpenCode SQLite, Pi Agent, OpenClaw, Ollama, Junie, and
> Z.ai/MiniMax model filters, with seven focused parser tests and explicit
...
> native secret custody continue across approved locked/unavailable stores
> while preserving fail-closed trust refusal. `c8b9a9ec4d` adds a
> portal-backed PipeWire-to-Opus Mercury audio engine/adapter and fail-closed
> native linking. The cloud scheduler suite passes 13/13, parser tests pass
> 7/7, keyring fallback tests pass 3/3, and the media crate tests pass 2/2;
> Linux-only media XCTest execution still requires the Linux VM/native
...
> injected hosts and an explicit staging allowlist. `8776a196f6` adds Linux
> Settings controls for daemon-owned encrypted cloud sync and metadata privacy
> consent, including fail-closed save/readback/error states and explicit
> macOS-only scope copy. The attached physical iPad ran the focused
> `iPadNavigationUITests/testiPadSettingsTabs_countAndNoDaemon` selector at this
> head: **1/1 passed, 0 failures, xcodebuild exit 0**. The receipt is
> [`evidence/parity-audit-2026-07-10/ipad-navigation-focused-current-2026-07-21.json`](evidence/parity-audit-2026-07-10/ipad-navigation-focused-current-2026-07-21.json).
> This remains non-certifying: Linux enrollment/approval, production auth,
> signed packaging, and the seven-environment matrix are still open.

> **Continuation update (2026-07-21, backend closeout):** `24e1bae875` and
> `cb118e647f` add the bounded P-16 daemon contracts for authorized Iroh
> remote reads and credential escrow. Remote reads require a live route tuple
> and request-bound trusted-device authorization before invoking the daemon
> reader; responses are capped at 512 KiB. Credential escrow validates the
> target key fingerprint/version, reads provider credentials only through the
> daemon authority, and returns an ECIES envelope authenticated with metadata
> AAD; plaintext is neither persisted nor exposed to the renderer. `d814f85f49`
> closes the P-08 call-seal source gap: the canonical CallInvite now carries an
> optional request-bound media seal, Linux rejects unsealed calls before
> PipeWire capture, and accepted calls start/stop portal-backed outbound Opus
> capture with AEAD-sealed `media.audio.out` frames. Focused Hermes protocol
> tests pass 18/18, Linux Settings tests pass 37/37, changed Linux Swift files
> parse for the Linux target, and a clean macOS daemon build passes. Linux
> XCTest execution, production callable wiring, signed packaging, live trusted-
> device approval, native audio output receipts, and the seven-environment
> matrix remain explicit dependencies for strict parity certification. Native
> inbound playback is now implemented in source; only its Linux runtime proof
> remains.

> **CI/release contract closeout (2026-07-21):** `d2300d2197` fixes the Linux
> release validator's signer-phase false positive and keeps the AUR launcher
> byte-identical to the canonical daemon launcher. `06274ccc40` repairs the
> generated Functions authorization catalog, returns the hand-maintained
> schema surface below its budget, removes the final empty catch, and fixes
> Ruff's AT-SPI/UTC findings. Local validation is green for Functions lint,
> build, 1,378 unit tests (four expected skips), security 10/10 plus BOLA
> 84/84, schema drift, empty-catch budget, Ruff, Linux release validation, and
> diff hygiene. These are CI and source-quality gates; they do not advance the
> strict 0/40 product or 0/7 environment certification counters.

> **Continuation update (2026-07-21, trusted-device and playback integration):**
> `3bda513f65` adds daemon-owned trusted-device list/approve/revoke contracts,
> bounded redacted validation, capability classification, and unavailable-by-
> default composition. `bcf0b1a7bf` adds native inbound Opus playback through
> the shared GStreamer crate (`opusdec` -> `autoaudiosink`), a C FFI and Swift
> lifecycle adapter, capability probing, route teardown, and fail-closed
> cooldown on sink or packet failure. `ffcae66d4b` wires trusted-device
> operations through the Tauri bridge and Devices & Sync Settings, adds
> decoder/bridge tests, and makes the runtime catalog's external text-engine
> evaluator explicit instead of allowing an unknown evaluator. `cc9ad63f7a`
> adds the bounded native HTTP adapter for the existing Firebase
> `listLinuxAppCheckDevices`/approve/revoke callables, with companion-owned
> credentials and nonce/action-proof injection; `b90945e411` validates the
> manager session before asking the companion to mint proof; Linux still cannot
> mint proof locally. Validation is green at 112/112 focused frontend tests, 135/135
> Tauri Rust tests, Hermes 18/18, production bundle verification, Swift daemon
> build, Linux-target Swift parsing, and the no-GStreamer media test suite.
> The Linux guest's GStreamer-enabled XCTest/runtime, companion/Iroh provider
> injection, and live callable credentials remain external dependencies; strict
> certification stays 0/40 and 0/7.

> **Continuation update (2026-07-21, source-graph and migration closeout):**
> `96e021053b` raises the bounded activity export window from 80 to 10,000
> sessions and adds a 10k-session/100-project regression so older inferred
> projects are not silently dropped during migration. The native Xcode project
> now includes `BurnBarLocalUsageIngestionService`, trusted-device management,
> all four CloudSync runtime sources, and the missing `OpenBurnBarInsights`
> framework dependency for the daemon test target. The undefined-symbol failure
> is gone; the focused test now aborts inside Xcode 27's Swift package graph
> synchronizer (`NSMutableArray insertObjects:atIndexes` count mismatch) before
> test execution. This is an IDE/package-graph blocker, not a migration-logic
> failure.
> `92d87249ea` and `6a87c4ccc4` make P-39 producer evidence attest the real
> runtime/platform metadata and current package version, with the selected
> P-39, resolver, differential, and workflow checks at 81/81. No live signed or
> same-clean-commit cross-platform receipt exists yet.

> **Continuation update (2026-07-21, macOS migration verification):**
> `4bf26e57ee` qualifies the app-level OpenCode parser in the cockpit test,
> removing a duplicate-symbol ambiguity between the app and core parser types.
> With the daemon test target's Insights dependency wired, the authoritative
> macOS xcresult passes the 10,000-session project migration test **1/1**.
> This closes the local migration-source/test gap; signed Linux runtime and
> cross-environment certification remain open.

> **Continuation update (2026-07-21, provider catalog closeout):**
> `2298e68f49` makes the Linux local-usage daemon derive parser membership and
> ordering from the generated `AgentProviderIngestionCatalog`, using a typed
> factory map that fails closed if a catalog local-parser entry lacks an
> implementation. The exact catalog-membership regression and provider parity
> checks pass 12/12; Swift syntax parsing and daemon compilation pass after
> `058ff3c049` corrected the factory to the existing `CodexParser`. The focused
> test binary still cannot load `SQLCipher.framework` in this macOS SwiftPM
> layout, so runtime execution remains open. API/quota/cloud coverage,
> installed corpus proof, signed packaging, and the seven-environment matrix
> remain explicit dependencies for strict certification.

> **Continuation update (2026-07-21, source hardening):** `a6f3ec2b0f`
> centralizes the macOS quota refresh dispatch in a shared 19-provider registry:
> 17 live adapters plus explicit unavailable entries for `cursorAgent` and
> `openBurnBar`, with 3/3 focused tests. `18231a4a40` maps Linux quota signal
> tiers to explicit API-backed, local-artifact, cached, or unavailable
> provenance; its focused tests pass 4/4 and existing quota snapshot tests pass
> 7/7. `5000b5200b` plus `7bb470cfac` make the differential oracle reject
> duplicate JSON object keys and non-canonical generated timestamps; the
> targeted P-39/differential/workflow suite passes 84/84. `fb3afed33b` and
> `faa50c0e7a` harden notification settings readback with defaults, bounds,
> de-duplication, and fractional-value rejection. `df0822290b` requires daemon
> readback confirmation before a preferred-slot mutation replaces the provider
> catalog. These source changes do not supply production API/cloud credentials,
> signed packaging, or live environment receipts.

> **Continuation update (2026-07-21, final source slices):** `708260e4f4`
> hardens the P-21 Insights proof validator to require canonical UTC
> millisecond timestamps; the focused mutation suite passes 3/3. `b46f16c6ed`
> rechecks daemon-owned chat backend capability immediately before durable
> append/stream and fails closed on stale or disabled selections; focused chat
> tests pass 50/50 and TypeScript typecheck is green. `09fd63e4e6` exposes the
> shared 19-provider quota adapter coverage through the Linux RPC (17 live,
> two explicitly unavailable) and fails closed for missing registry entries;
> focused coverage passes 4/4 and the daemon builds. These are source-level
> parity and reliability improvements. Production API/cloud credentials,
> signed packaging, cross-device approval, and the seven environment receipts
> remain the release-critical dependencies.

> **Continuation update (2026-07-21, reliability and live-device slice):**
> `8ede809724` validates Linux quota snapshot identity, provenance shape,
> future timestamps, unavailable-source shape, and finite bucket values before
> materialization; focused validator tests pass 5/5 and existing quota
> snapshot regressions pass 7/7. `50d139047b` rejects stale overlapping
> Activity list/search responses, with Activity coverage 22/22. `44a5864b0d`
> and `7017227ac8` require daemon catalog routing capability before advertising
> or sending through a chat backend; focused capability/send coverage passes
> 52/52 and 33/33. A connected physical iPad passed the focused settings
> selector 1/1 with xcodebuild exit 0; the receipt is recorded under
> `evidence/parity-audit-2026-07-10/ipad-navigation-focused-current-source-wave-2026-07-21.json`.
> The device result is non-certifying because the dependency build began from
> a dirty source wave and Linux enrollment/approval plus the UTM guest remain
> unproven.
...
> **Continuation update (2026-07-21, account export):** `530c51c259` adds the
> missing Linux cloud-account export path. A trusted-device/App Check authority
> remains daemon-owned; the renderer supplies only an optional domain scope and
> destination, and receives only a bounded receipt after the daemon validates
> and writes the server envelope with owner-only permissions. The Linux UI now
> exposes the action in Data & Privacy while older/fixture shells remain
> unavailable. Focused Settings/bridge coverage is **112/112**, Tauri Rust is
> **136/136**, the full Linux frontend suite is **957/957**, and the daemon
> production build passes. This closes a source-level P-40 gap, but production
> cloud deployment, trusted-device approval, signed artifacts, and live Linux
> evidence remain dependencies for strict certification.

> **Continuation update (2026-07-21, shell/keyring reliability):** `2bb7bbfe56`
> hardens the packaged IBus worker so a missing daemon socket, missing CLI, or
> bounded lookup timeout fails closed without terminating the input-method
> process; Python self-test and the focused Node contract suite pass 9/9.
> `6087fe6660` preserves typed native notification actions during a primary
> cold launch, with 11/11 focused single-instance tests and 137/137 Tauri
> library tests. `160c271057` makes the Linux KWallet setup probe report real
> available/blocked state from `kwallet-query` plus the D-Bus session, and the
> focused Linux security suite passes 33/33. These close source-level P-05,
...
### Post-main exact-native checkpoint — 2026-07-20 UTC

`6bf0708eec` reconciles the Linux parity branch with current `main` across the
frontend, Swift package graph, Android application startup, Functions, and CI
workflows. The integrated source passes 94 frontend files / 910 tests, Tauri
Rust 133/133, workflow verifier tests 28/28, Linux Swift verifier tests 18/18,
TypeScript, production-bundle verification, and production Linux daemon and CLI
builds. `8a17df54d2` avoids Swift 6.1 whole-module lookup ambiguity for explicit
