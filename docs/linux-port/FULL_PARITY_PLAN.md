...
| Desktop integration | `AppDelegate+StatusItem.swift`, `SwarmWallpaperRuntime.swift`, `Hotkey.swift` | Tauri tray exposes dashboard, Summary, Providers, Mercury, Quick Switch, chat, usage, updates, settings, health, refresh, reconnect, and quit actions. | Missing macOS-style live menu-bar popover depth, global hotkeys, desktop wallpaper hosting, and live wallpaper panels. |
