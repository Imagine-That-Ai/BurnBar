...
`native-global-panic-shortcut.json`, `native-login-start-relogin.json`,
`native-login-start-autostart-exec.txt`, `tray-host-loss-recovery.json`,
`tray-host-loss-recovery-menu-event.txt`, and `tray-recovered-menu-actions.json`.
