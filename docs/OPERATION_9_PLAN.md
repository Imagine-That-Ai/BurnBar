...
**Source of truth for findings:** `DILIGENCE_REPORT_2026-07-14.md` (repo root) + the six lane reports in this session. Every packet below cites its originating finding.
...
- **A1.** Trigger production deploys (`deploy-production.yml`, `deploy-cloud-run.yml` workflow_dispatch) — from the exact `refs/tags/<tag>` ref (tag-bound credential guard).
...
- **Wave 3 (day ~14+, inside the program):** P-ARCH-1b (quota promotion flip to `rust` + legacy deletion — REQUIRED for program completion, user-confirmed; gated on the evidence evaluator returning `ready`) · P-ARCH-2 (mission enforcement flip; mechanism ships in Wave 1–2, default-flip is ledger-tracked unless the divergence window is already clean by day 14).
...
- **Verify:** `git tag -l 'preserve/*'` empty; `.github/workflows/preserve-tag-leak-tripwire.yml` still present as backstop; attempt a dry `git push --tags` and confirm the guard refuses.
...
- **Change (choose the smaller correct fix, document which):** either (a) make the indexer consume the existing but unused `ParserCheckpointStore` / `CheckpointAwareParser` so a byte-offset resume avoids full re-parse on append; or (b) if (a) is a multi-platform cache-schema change too large for this program, add an explicit bound: only re-parse files whose signature changed AND cap per-tick parse work, and surface the cost. The prior lane flagged the checkpoint machinery exists with tests but has zero production callers — wiring it is the intended fix.
...
- **P-ARCH-1b (day ~14, Wave 3 — REQUIRED for program completion):** run `scripts/ops/export-domain-core-promotion-evidence.mjs`, then `scripts/ci/evaluate-domain-core-promotion.mjs --evidence <path>` (policy floors: consumers `apple`+`windows`, **≥14d coverage, ≥10k samples, ≤500bps p95 regression**). On exit 0 (`ready`): flip quota to `rust` on apple+windows and delete the legacy quota parsers per `docs/SHARED_RUST_DOMAIN_CORE_ROADMAP.md` (`ClaudeStatuslineQuotaParser.cs`, `CodexUsageQuotaParser.cs` C# path and the Swift legacy path it shadows).
- **Invariant:** never flip without the evaluator returning `ready` — if evidence fails a floor at day 14 (e.g. <10k samples), the fix is more soak time or more beta devices, never a policy edit. `rust` mode must fail closed (no silent legacy fallback). Legacy deletion happens in the same PR train as the flip so no zombie path survives. Fix the Android cloudvault env-var naming drift (`OPENBURNBAR_CLOUDVAULT_DOMAIN_MODE` vs canonical `OPENBURNBAR_DOMAIN_CORE_CLOUDVAULT_MODE`) while in these files.
...
- **Done:** evaluator `ready` + quota flipped to `rust` on apple+windows + legacy deleted. This is a program exit criterion, not a ledger item.
...
- Wave 3: **quota promoted to `rust` on apple+windows with legacy deleted (P-ARCH-1b — user-confirmed hard requirement)**; mission `.enforce` mode implemented and tested behind its flag (default-flip may remain ledger-tracked if the divergence window isn't clean by then).
