...
### Local database (GRDB and SQLCipher)

- **Default posture:** The on-disk file at `~/Library/Application Support/OpenBurnBar/OpenBurnBar.sqlite` is opened through GRDB with encryption default-on for new installs. Release builds link SQLCipher 4.16.0 through the vendored `SQLCipher.swift` package and define `SQLITE_HAS_CODEC` for the app and daemon targets.
- **Fail-closed at-rest encryption:** When encryption is requested, `DatabaseEncryptionService` applies the Keychain-held passphrase through GRDB's SQLCipher API on every connection, immediately verifies non-empty `PRAGMA cipher_version`, and aborts startup if the linked codec is missing. A generated key is returned only after `SecItemAdd` succeeds; Keychain persistence failure throws a typed `keychainPersistenceFailed` error before SQLCipher can create a database with an unrecoverable, unpersisted key.
- **Legacy plaintext migration:** Existing plaintext SQLite files are migrated on first encrypted launch with SQLCipher `sqlcipher_export`, then atomically replaced only after the encrypted replacement no longer has the plaintext SQLite header. If migration or codec verification fails, the app rejects plaintext fallback instead of opening the store unencrypted.
...
- **App Check (Firestore and Storage):** The native apps initialize App Check before Firebase. **Production** projects must **enforce** App Check for **Cloud Firestore** and **Firebase Storage** in the Firebase console so traffic without a valid attestation is rejected; Auth alone is not a substitute (see [FIREBASE_APP_CHECK_ENFORCEMENT.md](FIREBASE_APP_CHECK_ENFORCEMENT.md)).
...
| `ops/computer_use_session_daily_rollups/**` | Deny-all (`if false`) | Server only |
| `ops/media_budget_status/**` | Any signed-in user (Mercury budget envelope) | Server only |
| `ops/media_session_daily_rollups/**` | Deny-all (`if false`) | Server only |

Retired 2026-09-22: the former `burnbarOperator` custom claim never had a mint path (no Admin SDK issuance, no inventory, no revocation), so operator reads were already deny-all in practice. The claim was removed from `firestore.rules` and these reads are now explicit `if false`. Operator telemetry flows via hourly Cloud Functions + BigQuery export, never via client reads. Do not reintroduce an operator claim without a minted, inventoried, revocable issuance path.
...
| **Sandbox JWS aimed at production (or vice versa)** | The verifier may decode via the alternate Apple environment when configured (`autoFallbackEnvironment`), but reconciliation requires the decoded `Environment` to match the configured App Store environment before any entitlement write. Production defaults to `Production`; sandbox use must be explicitly configured. |
