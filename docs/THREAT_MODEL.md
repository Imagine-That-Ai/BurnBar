...
- **Default (and current reality):** The on-disk file at `~/Library/Application Support/OpenBurnBar/OpenBurnBar.sqlite` is a standard SQLite database opened via GRDB. **The local DB is plaintext on disk today, protected by macOS file permissions** (owner-only, in the unsandboxed app's container). A same-user process or anyone with filesystem access reads it directly; treat the on-disk file as readable until at-rest encryption ships.
- **At-rest encryption (pending the SQLCipher codec vendoring — tracked):** The encrypted path is designed to use **SQLCipher** (intended to link through the SPM `GRDB-SQLCipher` package in `project.yml`, aligned with the daemon’s GRDB pin), applying a `PRAGMA key` on every connection with key material held in the Keychain (`DatabaseEncryptionService`), and checking `PRAGMA cipher_version` so it refuses a silent plaintext path when encryption is requested but the library is not SQLCipher. **This codec is not yet vendored, so the encrypted path is not active in shipped builds** — do not state or imply that the local database is encrypted at rest until the codec is vendored and the toggle is wired end-to-end. The recovery-bundle design below describes that pending path, not a current guarantee.
