...
| Stop or panic is mistaken for retroactive JavaScript rollback | Stop aborts bridge waiting, advances the local work generation, rejects late completion, prevents queued/new actions, and revokes daemon authority. | JavaScript already executing in a page or isolated world cannot be forcibly killed or undone. Page effects completed before Stop remain and must be reflected in audit and recovery UX. |
...
run. Pending Safari work and App Group transport objects are invalidated, bridge
waiting is aborted, and stale completions are rejected. JavaScript already
executing in the page or isolated world cannot be forcibly killed or undone, and
page effects completed before the halt remain. Allowed, rejected, failed, and
aborted actions retain `safari.*` audit entries without storing provider keys or
raw screenshots in the audit chain.
