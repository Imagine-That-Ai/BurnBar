# Firebase App Check enforcement for Cloud Firestore and Storage
...
**Request metrics (before you enforce):** [Monitor App Check request metrics](https://firebase.google.com/docs/app-check/monitor-metrics) for Firestore and Storage until **verified** requests account for essentially all legitimate app traffic, then click **Enforce**. Enforcement can take up to about 15 minutes to apply everywhere.

**Older app builds** without the App Check integration will start failing against Firestore and Storage once enforcement is on—coordinate with release cadence.
...
7. Open **App Check** → **APIs** for **Cloud Firestore** and **Firebase Storage** → review **metrics** (verified vs outdated/unknown/invalid).
8. When ready, **Enforce** App Check for **Cloud Firestore** and **Firebase Storage**, then confirm both services report `ENFORCED`.
9. Smoke-test: sign in, enable cloud sync, upload a profile avatar, and confirm
   no `PERMISSION_DENIED` / App Check rejection from Firestore or Firebase
   Storage in logs.
