...
4. **CI and local debug:** Register a [debug provider](https://firebase.google.com/docs/app-check/ios/debug-provider) token in App Check for simulator/CI runs. The same token string must be what you inject via `FIREBASE_APP_CHECK_DEBUG_TOKEN` (GitHub Actions secret) and/or `FirebaseAppCheckDebugToken` in your local **gitignored** `GoogleService-Info.plist` (see [GoogleService-Info.plist.example](../AgentLens/Resources/GoogleService-Info.plist.example), [RELEASE_MACOS.md](RELEASE_MACOS.md) and `scripts/ci/inject-firebase-config.sh`). **Never commit** the real token in the repo. Physical iPhone/iPad Debug installs intentionally fall back to DeviceCheck when no debug token is configured, so local device testing keeps working after Firestore App Check enforcement without requiring the App Attest entitlement on development profiles.
