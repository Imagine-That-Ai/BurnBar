...
- **Ollama Cloud** scrapes `ollama.com/settings` HTML using Chrome cookies (same pattern as CodexBar). Falls back to `.unavailable` when no cookies exist.
...
| 2 | `app.factory.ai/settings/billing` HTML scrape | Plan name, tokens used, limits | Personal accounts with Chrome cookies |
...
- **Personal account gap:** Billing API returns 403. Scraper needs Chrome cookies. `FactoryLoginHelper` WKWebView flow exists as fallback.
...
- **Cloud:** Scrapes `ollama.com/settings` HTML with Chrome cookies — session %, weekly %, plan name
- **Without cloud cookies:** Cloud models detected via `:cloud` suffix, quota `.unavailable` with link
- **Chrome decryption:** PBKDF2 + AES-128-CBC, shared with Factory via `ChromeCookieReader`
...
- **Auth:** JWT Bearer token (from Chrome cookies or `KIMI_AUTH_TOKEN` env var)
...
|------|----------|-----------------|
| **CodexBar** (`steipete/CodexBar`) | `CursorStatusProbe.swift`, `FactoryStatusProbe.swift`, `OllamaUsageFetcher.swift`, `OllamaUsageParser.swift`, `WarpUsageFetcher.swift`, `CopilotStatusProbe.swift` | Cursor: `WorkosCursorSessionToken` + `cursor.com/api/usage-summary`. Factory: browser cookies + billing API + WorkOS OAuth. Ollama Cloud: `ollama.com/settings` HTML scrape. Warp: GraphQL with `wk-` keys. SweetCookieKit for cross-browser cookie reading. |
...
|---------|--------|-----------|
| Factory personal account billing | Plan limits unavailable without Chrome cookies | Use WKWebView login or sign into `app.factory.ai` in Chrome |
| Ollama Cloud cookie extraction | Cloud usage needs Chrome decryption at runtime | Works in-app (keychain entitlements); CLI blocked by macOS sandbox |
| Chrome v147 `v10` format | Cookie values use `v10`-prefixed AES-128-CBC | Code handles this; keychain access is the gate |
