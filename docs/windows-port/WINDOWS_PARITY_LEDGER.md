...
**Updated:** 2026-07-13 (F1 ledger green — 47 Real / 0 Deferred / 0 Blocked)
**Finish line:** **F2 True 1:1** remains the named **100% parity** endpoint (`finish_line: F2_True_1to1`). The current 47-row result is the scoped F1 Ship Peer ledger gate; WPD-0009 still requires F2 production composition and evidence before claiming full macOS parity. Operational host steps (Authenticode private key, required GH check flip, physical TPM claim) remain outside the in-repo ledger gate.
