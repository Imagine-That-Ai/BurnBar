...
- ~~`windows/native/` is an **empty skeleton** — the Rust↔managed FFI shim (iroh, burnbar-remote) does not exist; crates build in CI but nothing binds them.~~ **CLOSED 2026-07-06 (Wave 1 item 4), FFI-008 enforcement implemented 2026-08-09:** `windows/native/` carries the real shim — `OpenBurnBar.Native` (hardened absolute-path loader + graceful `NativeShimUnavailableException`), `OpenBurnBar.Native.BurnBarRemote` + `OpenBurnBar.Native.Iroh` (availability-gated facades over the committed uniffi-bindgen-cs bindings for BOTH crates, per WPD-0001), `windows/tests/native` (29 tests: locator/surface/shape on any host + real-FFI loopback), and a codegen-drift gate (`scripts/windows-port/check-csharp-binding-drift.sh` + `csharp-binding-drift.yml`). `pr-windows-full.yml` now builds both locked MSVC cdylibs natively on x64 and ARM64, requires them, executes the loopbacks, and permits only two named live/manual skips. FFI-008 is proven only by a passing exact-candidate hosted run on both architectures.
...
| 16 | Iroh P2P + BurnBarRemote (Rust) | `windows/native/` shim LANDED 2026-07-06; 29-test suite and fail-closed x64/ARM64 MSVC build, stage, load, loopback, exact-domain-core identity, and named-skip enforcement implemented 2026-08-09 | Gate implemented; exact hosted x64 + ARM64 pass required for FFI-008 proof |
...
4. ~~Build the `windows/native/` FFI shim binding the already-built iroh + burnbar-remote crates.~~
   **DONE 2026-07-06:** `OpenBurnBar.Native{,.BurnBarRemote,.Iroh}` + `windows/tests/native` registered
   ...
   Real FFI loopback is proven on macOS (29/29 with cargo-built dylibs; optional hosts retain 16 pass
   + 13 skip without them). FFI-008 enforcement is implemented in `pr-windows-full.yml`: both pinned,
   locked Rust crates build and stage on native x64 and ARM64, `OPENBURNBAR_REQUIRE_NATIVE_SHIMS=1`
   makes absence/load failure fatal, and the TRX gate permits only two named live/manual skips.
   Closure still requires both legs to pass for the exact candidate.
