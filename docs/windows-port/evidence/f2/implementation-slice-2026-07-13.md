...
relative reference paths. Rust parser tests remain green; the full language
matrix and a live Windows LSP host remain separate evidence gates.
...
the focused chat runtime suite is 24/24. The F1 parity ledger remains the
machine-scanned 46-row source/product gate.
