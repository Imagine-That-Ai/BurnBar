...
The 407 MB candidate archive and raw full-tree manifest are intentionally not
committed. A raw candidate inventory can preserve private host identifiers in
historical path names. The committed export script reproduces both artifacts;
`provenance-summary.json` retains the original commit, tree, archive SHA-256,
manifest SHA-256, manifest payload SHA-256, and file count without republishing
