...
| 2 | Gateway model catalog + model health | `OpenBurnBarHTTPGatewayServer+ModelCatalog.swift`, `GatewayModelCatalogSource.swift`, `OpenBurnBarLiveModelCatalog.swift`, `BurnBarGatewayModelHealthStore.swift` | **SUB-DONE** (failure-driven live health) | The production catalog exposes route eligibility plus active health state. `ModelRouteHealthStore` applies the macOS transient/rate-limit/auth/quota cooldown policy per provider/account/format/model, persists metadata only, and drives selection/failover, expiry, and success recovery. Proactive local-model discovery remains provider-specific row 7 work. | `docs/windows-port/evidence/f2/gateway-model-health.md` |
...
| 21 | Policy engine (run/tool approval) | `OpenBurnBarPolicyEngine.swift` | **SUB-DONE** (decision core) | The production authenticated companion plane exposes bounded, side-effect-free `policy.evaluate` with the same risk matrix, approval descriptors, retryability, progress detection, and model-requested-approval behavior. Tool execution and approval persistence/resolution remain separately gated. | `docs/windows-port/evidence/f2/policy-engine-production-composition.md` |
...
1-4, 6, 7, 14, 20, 21, and 25 are WPD-0009 F2 promotions):

| Disposition | Rows | Count |
|---|---|---|
| C#-substituted-already (SUB-DONE) | 1, 2, 3, 4, 5, 6, 7, 9, 11, 12, 13, 14, 15, 20, 21, 23, 24, 25, 27 | **19** |
| C#-substitute-to-build (SUB-BUILD) | 26 (Wave 4), 30 (Wave 3), 31 (Wave 3) | **3** |
| v1.1-deferred (DEFER; Swift-engine-reuse remains a revive path for row 8 and the row 7 CLI remainder) | 8, 16, 18, 19, 22, 29, 32, 33 | **8** |
