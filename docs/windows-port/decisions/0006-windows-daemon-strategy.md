...
| 7 | Provider executors (Anthropic, Codex, FactoryDroid, Ollama-native, OpenAI-compatible bridges) | `OpenBurnBarAnthropicProviderExecutor.swift`, `BurnBarCodexProviderExecutor.swift`, `FactoryDroidProviderExecutor.swift`, `OpenBurnBarProviderExecutor.swift` + `+AnthropicMessagesBridge/+OllamaNative/+ResponsesConversion.swift`, `OllamaCloudModelRoutingPolicy.swift`, `OpenBurnBarProviderCredentialNormalizer.swift` | **SUB-DONE** (HTTP transports) | `HttpModelCompletionExecutor` production-composes OpenAI-compatible and Anthropic Messages transports. `OllamaNativeProviderAdapter` now adds `/api/chat` request/response conversion for buffered and NDJSON streaming text/tools, JSON schema and reasoning options, exact usage, and truncated-stream rejection. Codex CLI, Factory Droid CLI, and proactive discovery remain. | `docs/windows-port/evidence/f2/ollama-native-provider-transport.md` |
