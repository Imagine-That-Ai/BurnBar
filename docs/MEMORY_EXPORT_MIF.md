...
`d456c4678648a772e00eb027e9bf6e55c8c49b50` (the document's seventh pass, audit B
F-2), sha256
`810fc38bee7ee91676819b7526938aacfe4f4e5a27fad8a1da15b6db9516b1aa`, MIF v1
minor 2, copied byte for byte with no local edit — and asserts that digest, as
D-0021 ruling 5 requires of each side. Every re-vendor move since Q-24 has been
additive (Q-26's closed hold vocabulary, Q-30's reason/project members, and now
`record_project`'s identity pair under `dependentRequired` plus two `if`/`then`
branches — a constraint rather than a member, which BB-E's inputs-only project
record satisfies vacuously and the evaluator already implements). A produced
bundle plus one record of every emitted type validates against the new copy out
of process (`jsonschema` 4.26.0, draft 2020-12) with **0 failing assertions over
20 instances**. The two copies had drifted inside one
...
| re-vendor | schema at HEAD | **fixed twice** — at `e591e7c8e` (Q-30, `1107c3ec…`) for the second review, and re-vendored at the end of the third at `d456c467` (`810fc38b…`), the document's seventh pass. Pin and provenance updated; in-process suite green; out-of-process validation 0 failures over 20 instances |
