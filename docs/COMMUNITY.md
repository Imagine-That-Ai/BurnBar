...
`COMMUNITY_KILL_SWITCH` / `community_kill_switch` hard-disable joins, profile updates, Looking Glass exports, and aggregation while preserving owner reads and cleanup paths. `COMMUNITY_PUBLIC_READS_ENABLED` / `community_public_reads_enabled` controls the `/ops/community_status/state/current` envelope that Firestore rules require before public leaderboard reads. Looking Glass exports default to JSONL and accept `format: "parquet"` for typed Parquet bundles with `sessionId`, `recordedAt`, and a raw `json` column.
...
1. Set `COMMUNITY_PUBLIC_READS_ENABLED=false` or the matching Remote Config value so Firestore rules fail closed for `community_leaderboards`.
...
4. Re-enable public reads (`COMMUNITY_PUBLIC_READS_ENABLED=true` or Remote Config) only after `npm run community:canary` passes against known threshold/live docs.

### Kill switch

- `COMMUNITY_KILL_SWITCH=true` or `community_kill_switch=true` disables joins, profile updates, Looking Glass exports, and aggregation.
