...
- Pixel Clock hardware buttons are detected through durable AWTRIX sentinel apps: `openburnbar_btn_right`, `openburnbar_btn_select`, and `openburnbar_btn_left`. They use a short visual duration but a long lifetime, and the controller republishes them during normal heartbeats so Left/Right continues to land on a detectable app instead of reloading the same `openburnbar0` page. AWTRIX auto-rotation is disabled while physical navigation remains unblocked, so sentinel pages are reached by button presses rather than idle carousel cycling.
- `SmartDisplayRepairCoordinator` owns the one-click recovery path for both display families. It routes Nest Hub repair through the cast bridge and Pixel Clock repair through `preparePixelClock`, then returns a shared `SmartDisplayDeviceRepairStatus` with phase, message, optional proof, and timestamps.
...
- Provider glyphs are 8×8 stencils generated from the bundled brand assets by `scripts/generate-pixel-clock-logos.py` into `PixelClockProviderLogoAssets.generated.swift` (and ported verbatim to `website/scripts/lib/pixel-clock-presenter.mjs`). Supported marks: Claude, Codex, Copilot, MiniMax, MiMo, Z.ai, Factory, Cursor, Warp, Ollama, Kimi, DeepSeek, OpenCode, Antigravity, and **Grok (xAI)** — any provider without a curated stencil falls back to a colored monogram.
...
- `SmartDisplayActionsListener` consumes mobile actions and marks them `completed` or `failed`. `smart_display_repair`, `nest_hub_repair`, and `pixel_clock_repair` stream proof-oriented status payloads back onto the action document so iPhone and iPad can show the same result as macOS. For `pixel_clock_prepare`, it writes the Mac-side setup fields back onto the same action document, including `setupSSID` when an AWTRIX setup network is visible, so iPhone and iPad can show exact next steps instead of a generic permissions or timeout error.
...
- Nest Hub repair is considered successful only after the Mac bridge observes the receiver polling `/state.json` after a cast/recast. A successful Cast command without a follow-up poll is not enough to mark the action healthy.
...
- `SmartHubStore.repairNestHub()` and `SmartHubStore.repairSmartDisplays()` use longer action timeouts than simple test commands because discovery, bridge recovery, recast, and proof polling can legitimately take multiple minutes on a recovering LAN.
