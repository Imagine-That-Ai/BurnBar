...
|---|---|---|---|---|---|
| **Menu bar readout + resizable tray popover** | Status-bar brand mark showing today's spend/tokens; left-click opens a resizable tray. Quotas, insights, summary, providers, Mercury, chat, and quick-switch share one persisted layout (order, hide, collapse, relative weight, min/max). | shipped | free | obvious | core |
...
| **Menu bar popover — customizable tray sections** | Once onboarding is dismissed, the popover shows pinned chrome (header, update banner, cloud whisper, action bar) plus a user-sized tray: quotas, insights, summary, providers, Mercury, chat, quick-switch. Hidden sections leave no gap; collapsed sections are a labeled strip. | shipped | ? | obvious |  |
