...
| **K3/K4 completion (Core god-module decomposition)** | **✅ DONE — see [`docs/CORE_DECOMPOSITION_PROGRAM.md`](CORE_DECOMPOSITION_PROGRAM.md) § FINAL close-out. Whole-program composition proven on `core-decomp/wave4-base`; Core main target floored 353 files/95,667 LOC → 16 files/1,610 LOC.** |
| **K3 — LogParsers + Quota** | ✅ DONE. `OpenBurnBarLogParsers` (P-12 #1627) + `OpenBurnBarQuota` (the K3 Quota redo, P-13 #1652); unblocked by extracting the 325-LOC SQLite reader first (`OpenBurnBarSQLiteReader`, P-01 #1573) so Quota/LogParsers extract independently. |
| **K4 — `OpenBurnBarUI` product** | ✅ DONE. All `Views/` + theme/design-system + PixelClock/ShareCardRenderer/LiveActivity moved to `OpenBurnBarUI` (130 files/33,710 LOC) across P-16a–f (#1644, #1650, #1656, #1666, #1675, #1678); the "Views" exclude is deleted (K4 complete). |
| **Daemon repoint → UI-free Engine** | ✅ DONE. daemon/CLI link the UI-free `OpenBurnBarEngine` umbrella (P-18 #1664 + app-side Xcode-target fix); `check-core-umbrella-imports-budget.sh` reports daemon/Widget/Keyboard = 0 umbrella imports, and `check-engine-closure-ui-purity.sh` proves the Engine closure (9 targets) is zero-SwiftUI/AppKit. The most-privileged binaries gain NO path to the UI monolith. |
