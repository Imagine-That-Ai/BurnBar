...
- **Audit export:** Settings → Computer Use → "Export audit log" produces a tar.gz of the JSONL chain + all screenshots, signed with the user's iCloud device certificate. For privacy: defaults to local file save; user must opt into iCloud upload.
