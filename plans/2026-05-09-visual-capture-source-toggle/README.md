...
> **AUDIT CORRECTION #2 — 2026-05-09 (Windsurf → Devin):**  `devin.ai/blog/windsurf-is-now-devin-desktop` (June 2, 2026) + `cognition.com/blog/windsurf` (acquisition of Windsurf IP/product/trademark/brand) confirm **Windsurf is now Devin Desktop** — backwards-compatible, same Electron app, download at `devin.ai/download`. Plan updated: Windsurf row marked **LEGACY — deprecated, toggle now lives under Devin**; Devin row is canonical for new work; migration must map old `windsurf` usage rows to `devin-desktop` where applicable.
...
* **Which CLIs have a desktop twin?** **Audit-corrected 2026-05-09 (#2 Windsurf→Devin)** — Confirmed `Both` set = **Codex (`codex-cli` ↔ `codex-desktop`), Claude (`claude-code` ↔ Claude Desktop), Cursor (`cursor-agent` ↔ Cursor IDE), Factory (`factory-droid` ↔ Factory Desktop `factory.ai/product/desktop`), MiniMax (`minimax-cli` ↔ MiniMax Agent/Code `agent.minimax.io/download`), Z.ai (`zai-cli` ↔ ZCode Desktop `zcode.z.ai`), Devin (`devin CLI` ↔ Devin Desktop `devin.ai/desktop` — *successor to Windsurf as of June 2 2026*), Hermes (CLI ↔ Dashboard TUI), Warp (CLI ↔ Warp Terminal), OpenCode (CLI ↔ TUI), Ollama (service ↔ Desktop)**. **Windsurf is LEGACY — deprecated, now Devin Desktop** (`devin.ai/blog/windsurf-is-now-devin-desktop`, `cognition.com/blog/windsurf`). CLI-only / plugin-only = **Cline, Kilo Code, Roo Code, Augment, Junie** are **IDE plugins, not standalone `.app`s** (VS Code/JetBrains host — NO toggle), plus Antigravity, Gemini CLI, Kimi, Copilot CLI, Aider, Goose, OpenClaw, Prime Agent, Muse, etc. → 11 active Both providers (12 minus legacy Windsurf).
...
| **Windsurf** (`windsurf`) — **LEGACY — deprecated June 2, 2026, now Devin Desktop** (`devin.ai/blog/windsurf-is-now-devin-desktop`) | `~/.windsurf/...` (legacy) | **No longer offered** — use **Devin Desktop** (`/Applications/Devin.app`, `devin.ai/download`) | `.ide` → deprecated | ⚠️ legacy probe only — **NO new toggle** — existing configs migrated to Devin | **NO — deprecated; see Devin** |
...
**Scope:**
* Extend `OpenBurnBarCore/Sources/OpenBurnBarKernel/Resources/catalog.json` with per-provider `visualSurfaces: ["cli","desktop"]` or `["cli"]` — **audit-corrected #2:** `Both` = codex, claude, cursor, factory, minimax, z.ai, devin (canonical, replaces windsurf), hermes, warp, opencode, ollama (**11 active**); `LEGACY` = windsurf → devin-desktop mapping; `CLI-only` = antigravity, gemini, kimi, copilot, aider, etc.; `Plugin-only (NO toggle)` = cline, kilo, roo, augment, junie.
...
AgentLens/Services/Settings/Stores/VisualCapturePreferences.swift (B)  // NEW — Both set now 11 active (12 minus legacy Windsurf → Devin)
