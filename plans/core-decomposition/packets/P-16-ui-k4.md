...
| P-16b2 | `Views/Insights/Verdict/` (6) — trailing sub-packet (the card-prescribed 6k relief valve; Verdict is bidirectionally decoupled from Insights-root views, uses only the now-in-UI `UnifiedDesignSystem`) | 6 | 926 | **CONSOLIDATED → PR #1675** (shipped in the P-16e Swarm+Verdict+Share slice; `import OpenBurnBarInsights` ×6 for the Insight/Verdict models) |
...
| P-16e | `Services/Insights/Share/InsightShareCardRenderer.swift` (1) — the ONLY UI-straggler left after P-16d; the SharedModels/renderers cluster shipped in P-16d, `UIModeTheme.swift` + `AgentInsights/AgentInsightsViewModel.swift` in P-16b. Rides the `"Services/Insights/Share"` exclude (drop that entry). | 1 | ~0.2k | **PR #1675** (base p-16d) — bundled with P-16b2 (Verdict) + the Swarm-cluster half of P-16f into ONE dependency-closed slice: `Views/Swarm*` (11) + `Views/Insights/Verdict/` (6) + this renderer (1) = 18 files / 5,417 LOC. Drops the `"Services/Insights/Share"` exclude. |
| P-16f | `Views/` **root** (26) — the LAST sub-packet; MUST split ≥2 (over the 6k cap). Cut by compile-closure into the **`Views/Swarm*` cluster (11)** and the **non-Swarm root (15)**; Swarm rides PR #1675 (leaf; `BurnBarLogoFormationView` in the non-Swarm half depends on the public `SwarmGlyphSampler`), the non-Swarm 15 ship in **PR #1678** which deletes `"Views"` from `openBurnBarCoreExcludes` entirely (baseline 15→0). | 26 | ~9.4k | **Swarm-half → PR #1675 · non-Swarm-half → PR #1678** |
...
### P-16e + P-16f consolidation note (executed 2026-07-13, PRs #1675 + #1678) — Views-root final sweep, K4 COMPLETE
The orchestrator packet "P-16e" carried the broad scope *Views root + Share renderer +
AgentInsightsViewModel*. Against the live tree that reduces to: the `Views/Swarm*` cluster + the
remaining `Views/Insights/Verdict/` subtree (card P-16b2, still unshipped) + the Share renderer
(card P-16e) + the rest of the `Views/` root (card P-16f). `AgentInsightsViewModel.swift` was
already pulled forward by P-16b. Rather than 4 micro-PRs (Share=1, Verdict=6, Swarm=11, root=15),
this run shipped **two dependency-closed slices under the 6k cap**, and this card is updated to map
them onto the canonical P-16b2/P-16e/P-16f rows:
