...
final class PhoneControlReceiverTests: XCTestCase {
    ...
    @MainActor
    func testAgentContextTargetReceiverAcceptsLegacyNilSessionForActivePeer() async throws {
        let privateKey = Curve25519.Signing.PrivateKey()
        let peerNodeId = "android-phone-copilot"
        let validator = isolatedPhoneControlAuthorityValidator()
        validator.registerPeer(nodeId: peerNodeId, publicKey: privateKey.publicKey)
    ...
    @MainActor
    func testAgentContextTargetReceiverRejectsWhenNoActivePhonePeerOwnsControl() async throws {
        let privateKey = Curve25519.Signing.PrivateKey()
        let peerNodeId = "android-phone-copilot"
        let validator = isolatedPhoneControlAuthorityValidator()
        validator.registerPeer(nodeId: peerNodeId, publicKey: privateKey.publicKey)
    ...
    @MainActor
    func testAgentContextTargetReceiverRejectsNonActivePhonePeer() async throws {
        let attackerKey = Curve25519.Signing.PrivateKey()
        let attackerPeerNodeId = "android-phone-other"
        let activePeerNodeId = "android-phone-active"
        let validator = isolatedPhoneControlAuthorityValidator()
    ...
    @MainActor
    func testSystemPermissionReceiverRejectsMissingSession() async throws {
        let privateKey = Curve25519.Signing.PrivateKey()
        let peerNodeId = "android-phone-system-permission"
        let validator = isolatedPhoneControlAuthorityValidator()
        validator.registerPeer(nodeId: peerNodeId, publicKey: privateKey.publicKey)
