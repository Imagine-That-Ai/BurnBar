...
final class CheckpointTests: XCTestCase {
    ...
    func test_checkpoint_doesNotAdvance_whenCommitFails() async throws {
        ...
        let initialCheckpoint = try await checkpointStore.fetchCheckpoint(for: .claudeCode)
        ...
        let afterRollbackCheckpoint = try await checkpointStore.fetchCheckpoint(for: .claudeCode)
        XCTAssertNil(afterRollbackCheckpoint, "Checkpoint must not advance on rollback")
    }

    func test_checkpoint_doesNotAdvance_whenTransaction_throwsBeforeCommit() async throws {
        // Given: a DataStore with checkpoint store
        let queue = try DatabaseQueue()
        let store = try DataStore(databaseQueue: queue, runMigrations: true, refreshOnInit: false)
        let checkpointStore = ParserCheckpointStore(dbQueue: queue)
        ...
        // Verify no checkpoint exists initially
        let initialCheckpoint = try await checkpointStore.fetchCheckpoint(for: .factory)
        XCTAssertNil(initialCheckpoint, "No checkpoint should exist initially")
        ...
        // Then: no checkpoint should exist (not advanced on failure)
        let afterFailureCheckpoint = try await checkpointStore.fetchCheckpoint(for: .factory)
        XCTAssertNil(afterFailureCheckpoint, "Checkpoint must not advance when transaction fails before commit")
    }

    func test_checkpoint_advances_onlyAfterSuccessfulCommit() async throws {
        // Given: a DataStore with checkpoint store
        let queue = try DatabaseQueue()
        let store = try DataStore(databaseQueue: queue, runMigrations: true, refreshOnInit: false)
        let checkpointStore = ParserCheckpointStore(dbQueue: queue)
        ...
        // Verify no checkpoint exists initially
        let initialCheckpoint = try await checkpointStore.fetchCheckpoint(for: .cursor)
        XCTAssertNil(initialCheckpoint, "No checkpoint should exist initially")
        ...
        let afterCommitCheckpoint = try await checkpointStore.fetchCheckpoint(for: .cursor)
    ...
    func test_checkpoint_idempotent_commitIsIdempotent() async throws {
        // Given: a DataStore with checkpoint store
        let queue = try DatabaseQueue()
        let store = try DataStore(databaseQueue: queue, runMigrations: true, refreshOnInit: false)
        let checkpointStore = ParserCheckpointStore(dbQueue: queue)
        ...
        let afterFirstCommit = try await checkpointStore.fetchCheckpoint(for: .kimi)
    ...
    func test_resume_findsExistingCheckpoint() async throws {
        ...
        let hasCheckpoint = await wrapper.hasCheckpoint()
        XCTAssertTrue(hasCheckpoint, "Wrapper should detect existing checkpoint")
    }

    func test_resume_noCheckpoint_meansFreshStart() async throws {
        ...
        let hasCheckpoint = await wrapper.hasCheckpoint()
        XCTAssertFalse(hasCheckpoint, "Wrapper should detect no checkpoint")
    }

    func test_resume_gapFreeAfterInterruption() async throws {
        ...
        let allUsages = try await queue.read { db in
        ...
        let checkpoint = try await checkpointStore.fetchCheckpoint(for: .claudeCode)
        XCTAssertEqual(checkpoint?.checkpointToken, "v1:batch2:20")
    }

    func test_resume_duplicateFreeOnReingest() async throws {
    ...
    func test_clearCheckpoint_forcesFullReprocess() async throws {
        ...
        let checkpointBefore = try await checkpointStore.fetchCheckpoint(for: .factory)
        ...
        try await checkpointStore.clearCheckpoint(for: .factory)

        // Then: checkpoint is gone and usage remains
        let checkpointAfter = try await checkpointStore.fetchCheckpoint(for: .factory)
        XCTAssertNil(checkpointAfter, "Checkpoint should be cleared")

        // Usage should still exist (data not lost)
        let usageAfter = try await queue.read { db in
    ...
    func test_clearAllCheckpoints_fullReset() async throws {
        ...
        try await checkpointStore.clearAllCheckpoints()

        // Then: all checkpoints are gone
        let allCheckpoints = try await checkpointStore.fetchAllCheckpoints()
        XCTAssertTrue(allCheckpoints.isEmpty, "All checkpoints should be cleared")
    }

    func test_recoveryAfterCorruption_noMissingRows() async throws {
        ...
        try await checkpointStore.clearCheckpoint(for: .claudeCode)
    ...
    func test_atomicVisibility_noPartialState_onFailure() async throws {
        // Given: a DataStore with checkpoint store
        let queue = try DatabaseQueue()
        _ = try DataStore(databaseQueue: queue, runMigrations: true, refreshOnInit: false)
        let checkpointStore = ParserCheckpointStore(dbQueue: queue)
    ...
    func test_atomicVisibility_fullState_afterCommit() async throws {
        // Given: a DataStore with checkpoint store
        let queue = try DatabaseQueue()
        _ = try DataStore(databaseQueue: queue, runMigrations: true, refreshOnInit: false)
        let checkpointStore = ParserCheckpointStore(dbQueue: queue)
    ...
    func test_atomicVisibility_rollbackClearsInMemoryState() async throws {
    ...
    func test_atomicVisibility_multipleCommits_advancesCheckpointEachTime() async throws {
        ...
        let checkpoint = try await checkpointStore.fetchCheckpoint(for: .kimi)
