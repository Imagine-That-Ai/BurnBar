...
final class ModelPurposeClassifierTests: XCTestCase {
    ...
    func test_surfaceSignalPreservesOriginalCaseAcrossPorts() {
        let signals = ClassifierSignals(
            fileExtensions: nil,
            model: nil,
            appSurface: "Terminal",
            hasCodeExecution: nil,
            hasErrorOutput: nil,
            hasSearchResults: nil,
            hasMultiStepPlanning: nil,
            keywords: nil
        )

        let result = ModelPurposeClassifier.classifyPurpose(signals)

        XCTAssertTrue(result.contributingSignals.contains("surface:Terminal"))
    }
