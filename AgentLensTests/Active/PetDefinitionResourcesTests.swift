...
/// Staged-resource Lab tests: the 17 PetDefinition suites that need Imagine
/// Pets `Models/` (GLBs, SceneKit renders) in the test bundle. Gated on
/// `OPENBURNBAR_LAB_STAGED_RESOURCES`; skipped otherwise. The catalog-only
/// remainder lives in PetDefinitionTests.
final class PetDefinitionResourcesTests: XCTestCase {

    override func setUpWithError() throws {
        try super.setUpWithError()
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["OPENBURNBAR_LAB_STAGED_RESOURCES"] == "1",
            "requires staged Imagine Pets Models/ resources (Lab lane stages them)"
        )
    }
