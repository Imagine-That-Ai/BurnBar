...
final class DashboardChatOverlayTests: XCTestCase {
    ...
    ) throws -> (overlay: DashboardChatOverlay, binding: Binding<Bool>) {
    ...
    func test_inspectsWithoutCrashing() throws {
        let (view, _) = try makeOverlay(isOpen: true)
        XCTAssertNoThrow(try view.inspect())
    }

    func test_closedStateRenders() throws {
        let (view, _) = try makeOverlay(isOpen: false)
    ...
    func test_testTriggerOpen() throws {
        let (view, binding) = try makeOverlay(isOpen: false)
        view.testTriggerOpen()
        XCTAssertTrue(binding.wrappedValue)
    }

    func test_testTriggerClose() throws {
        let (view, binding) = try makeOverlay(isOpen: true)
