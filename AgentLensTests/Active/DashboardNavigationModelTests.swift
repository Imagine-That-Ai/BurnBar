...
final class DashboardNavigationModelTests: XCTestCase {
    ...
    func test_sidebarRouteOrder_agentsMode() {
        ...
        XCTAssertEqual(order, [.overview, .insights, .chat, .quota, .memoryReview, .provider(.cursor)])
    }
    ...
    func test_sidebarRouteOrder_modelsMode() {
        ...
        XCTAssertEqual(order, [.overview, .insights, .chat, .quota, .memoryReview, .model("gpt-4")])
    }
}
