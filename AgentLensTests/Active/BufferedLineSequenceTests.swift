...
final class BufferedLineSequenceTests: XCTestCase {
    ...
    func test_singleLine_noTrailingNewline() {
        let url = write("hello", to: "single.txt")
        let lines = readLines(from: url)
        XCTAssertEqual(lines, ["hello"])
    }

    func test_singleLine_withTrailingNewline() {
        let url = write("hello\n", to: "single_nl.txt")
        let lines = readLines(from: url)
        XCTAssertEqual(lines, ["hello"])
    }
    ...
    func test_lineSpanningChunkBoundary() {
    ...
    func test_equivalenceWithOldImplementation() {
