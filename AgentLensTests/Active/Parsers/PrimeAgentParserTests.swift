...
final class PrimeAgentParserTests: XCTestCase {
    ...
    func testExplicitZeroCostWithTokensFallsBackToCatalogPricing() async throws {
        let dir = try makeTempDir()
        defer { try? FileManager.default.removeItem(at: dir) }
        // Prime often records `cost.total == 0` when models.json has no price for the
        // routed backend. Prefer catalog pricing when tokens are non-zero so usage
        // dashboards do not show a free turn for billed traffic.
        let content = [
            sessionEnvelope(id: "sess-free"),
            assistantMessage(model: "gpt-4o", input: 1000, output: 500, cost: 0.0, text: "zero cost with tokens")
        ].joined(separator: "\n")
        _ = try writeSessionFile(dir: dir, content: content)
        let result = try await PrimeAgentParser(logDirectoryOverride: dir.path).parse()
        let usage = try XCTUnwrap(result.usages.first)
        XCTAssertGreaterThan(usage.costUSD, 0, "explicit 0 with tokens should fall back to catalog pricing")
    }
