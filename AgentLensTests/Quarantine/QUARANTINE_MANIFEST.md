...
**2026-07-01 freshness re-date:** At the time, the two `Skipped-with-issue` (Active `XCTSkip`) rows carried real, still-open blockers, so they were legitimately re-dated rather than reclassified — they were *not* documentation-only `LegacyReference` entries. `test_circuitBreaker_halfOpenToClosed_recovery` still needed the fake CloudSync gateway to surface retry failures into the circuit breaker; `testMakeConfigurationWithKey_reportsCipherVersion` still needed a release-build (SQLCipher `PRAGMA cipher_version`) CI lane. Both Target Dates moved from May 2026 to **2026-09-30** so `scripts/ci/check-quarantine-freshness.sh` (R-GH8) could track a live milestone instead of a stale, already-overdue one. The blockers, not the calendar, gated revival.

**2026-07-06 CloudSync revival:** `test_circuitBreaker_halfOpenToClosed_recovery` now runs in Active with a fake-gateway batch-commit fault seam and zero-delay retry policy override. The test drives a retryable Firestore failure through `UsageSyncService`, proves the breaker opens, advances breaker time deterministically, then proves the half-open probe closes the breaker and uploads the pending row.
...
| `test_circuitBreaker_halfOpenToClosed_recovery` | Done | ~~Stale contract~~ **Revived 2026-07-06** → `AgentLensTests/Active/OfflineOnlineMergeTests.swift` | CloudSync | Offline/Online Merge | — | Done |
