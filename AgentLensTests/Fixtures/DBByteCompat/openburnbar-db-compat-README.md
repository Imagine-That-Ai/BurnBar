...
| File | What it is |
| ---- | ---------- |
| `openburnbar-db-compat-v61.sqlcipher` | A **real Mac-produced** SQLCipher database, migrated through the **live** `OpenBurnBarDatabase` migrator to `v61_usage_memory` and seeded with the canonical FTS corpus. Genuinely encrypted (no plaintext SQLite header). |
...
cp /tmp/obb-db-compat-out/openburnbar-db-compat-v61.sqlcipher \
