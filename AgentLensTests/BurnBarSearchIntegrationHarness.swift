...
enum BurnBarSearchIntegrationHarnessError: LocalizedError {
...
@MainActor
final class BurnBarSearchIntegrationHarness {
    struct FileRoots {
    ...
    let clock: BurnBarFakeClock
    let embedder: BurnBarFakeEmbedder
    let queryEmbedder: BurnBarFakeQueryEmbedder
    ...
    init(
        ...
        embedderSeed: String = "burnbar-harness-seed-v1",
        embedderVersionTag: String = "harness-v1",
        embedderDimensions: Int = 96
    ) throws {
        self.fileManager = fileManager
        self.sharedAccessContext = sharedAccessContext
        self.clock = BurnBarFakeClock(now: initialTime)
        self.embedder = BurnBarFakeEmbedder(
            dimensions: embedderDimensions,
            versionTag: embedderVersionTag,
            seed: embedderSeed
        )
        self.queryEmbedder = BurnBarFakeQueryEmbedder(embedder: embedder)

        let root = fileManager.temporaryDirectory
            .appendingPathComponent("BurnBarSearchHarness-\(name)-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        rootURL = root

        let dbDirectory = root.appendingPathComponent("db", isDirectory: true)
        try fileManager.createDirectory(at: dbDirectory, withIntermediateDirectories: true)
        databaseURL = dbDirectory.appendingPathComponent("burnbar-search-harness.sqlite", isDirectory: false)
    ...
    ) -> (service: ArtifactDiscoveryService, settings: BurnBarHarnessArtifactDiscoverySettings) {
    ...
    func makeConversationFixture(
        ...
        BurnBarSearchFixtureBuilder.conversation(
            id: id,
            provider: provider,
    ...
    func makeSkillArtifactFixture(
        ...
        BurnBarSearchFixtureBuilder.skillArtifact(
            id: id,
    ...
    func makeSharedArtifactFixture(
        ...
        BurnBarSearchFixtureBuilder.sharedArtifact(
            id: id,
    ...
    func grantSharedReadAccess(
        ...
        let permission = BurnBarSearchFixtureBuilder.sharedArtifactPermission(
    ...
    func writeFixtureFile(
        ...
        guard normalizedRelativePath.isEmpty == false else {
            throw BurnBarSearchIntegrationHarnessError.invalidRelativePath(relativePath)
        }
    ...
    func enqueueConversationProjection(
        ...
        guard let conversation = try dataStore.fetchConversation(id: conversationID) else {
            throw BurnBarSearchIntegrationHarnessError.missingConversation(conversationID)
        }
...
@MainActor
final class BurnBarHarnessArtifactDiscoverySettings: ArtifactDiscoverySettingsProviding {
...
enum BurnBarSearchFixtureBuilder {
...
enum BurnBarFakeEmbedderError: LocalizedError {
    case forced(String)
...
final class BurnBarFakeEmbedder: ChunkEmbeddingProviding {
    private let deterministicEmbedder: DeterministicFakeEmbeddingProvider
    var failAll = false
    var failingSubstrings: Set<String> = []
    var forcedErrorMessage = "Forced BurnBar fake embedder failure."
    ...
    init(
        ...
        seed: String = "burnbar-harness-seed-v1"
    ) {
...
@MainActor
final class BurnBarFakeQueryEmbedder: QueryEmbeddingProviding {
    private let embedder: BurnBarFakeEmbedder

    init(embedder: BurnBarFakeEmbedder) {
