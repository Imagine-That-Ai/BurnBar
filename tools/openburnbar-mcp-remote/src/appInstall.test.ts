...
function infoPlist(version: string, build: string): string {
  return [
    ...
    "<string>com.openburnbar.app</string>",
    "</dict></plist>",
    ""
  ].join("\n");
}

function writeBundle(root: string, version: string, build: string): string {
  const app = join(root, APP_BUNDLE_NAME);
  mkdirSync(join(app, "Contents"), { recursive: true });
  writeFileSync(join(app, "Contents", "Info.plist"), infoPlist(version, build));
...
function harness(bytes: Buffer, release: MacOSReleaseFeed, key: TestKey, installed?: { version: string; build: string }) {
  ...
  if (installed) {
    writeBundle(applicationsDir, installed.version, installed.build);
  }
  ...
  const deps: Partial<AppInstallDeps> = {
    ...
    run: async (command, args): Promise<CommandResult> => {
      ...
      if (command === "/usr/bin/hdiutil" && args[0] === "attach") {
        writeBundle(mountPoint, release.version, release.build);
...
test("default feed URL is the desktop updater URL and is not a pinned old version", () => {
  ...
  assert.equal(SU_PUBLIC_ED_KEY_BASE64, "613YSraDEJ54LKsfpqbYhyzYnfYRg7z4QwiEJfoy0TI=");
...
test("package is 0.1.1 and never downloads the Mac app during npm install", () => {
...
test("app install fetches the feed version and copies to Applications", async () => {
  ...
  try {
    ...
    assert.deepEqual(installed, { version: "1.0.35", build: "135" });
...
test("CLI app install on this host is macOS-only and does not print a pinned marketing version", async () => {
  const result = await spawnCli(["app", "install"]);
  assert.equal(result.status, 2);
  assert.match(result.stderr, /only supports macOS/);
  assert.doesNotMatch(result.stdout + result.stderr, /1\.0\.29/);
  assert.doesNotMatch(result.stdout + result.stderr, /1\.0\.35/);
});
