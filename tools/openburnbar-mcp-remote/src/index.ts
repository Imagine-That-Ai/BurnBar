...
`);
  ...
  process.stdout.write("Usage: openburnbar mcp <serve|install|doctor|login> [--stdin|--env] | memory <install|run|sync> | resume <sessionId>|--query <memory> [--as <harness>] [--model <model>] [--print|--copy|--open|--spawn] | obbresume <memory> [--as <harness>] | OBB Resume <memory>\n");
}
