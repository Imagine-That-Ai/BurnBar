...
function codexConfig(): string {
  return [
    ...
    "# server. The shim reads the bearer from macOS Keychain. Plaintext env",
    `# token sources require ${tokenEnvVar} plus the explicit insecure-source opt-in.`,
    "# Run `openburnbar mcp login <bearer>` once first.",
