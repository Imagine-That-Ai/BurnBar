...
import { parseTspStringEnum } from "./parse-tsp-enum.mjs";
...
{
  // daemon-rpc: the method catalog derives from the .tsp canon (single
  // source). One parsed table feeds all three language templates, so the
  // emits cannot drift from each other — only from the canon, which
  // check-tsp-canon.mjs guards.
  const rpcMethods = parseTspStringEnum(join(root, "typespec/domains/daemon-rpc.tsp"), "RpcMethod");
  const tsUnion = rpcMethods.map((m) => `  | ${JSON.stringify(m.value)}`).join("\n");
  const tsArray = rpcMethods.map((m) => `  ${JSON.stringify(m.value)},`).join("\n");
  const swiftCases = rpcMethods
    .map((m) => [...m.docs.map((d) => `    /// ${d}`), `    case ${m.name} = ${JSON.stringify(m.value)}`].join("\n"))
    .join("\n");
  const kotlinConsts = rpcMethods.map((m) => {
    const screaming = toScreamingSnake(m.name);
    return { screaming, line: `    const val ${screaming} = ${JSON.stringify(m.value)}` };
  });
  const screamingSet = new Set(kotlinConsts.map((c) => c.screaming));
  if (screamingSet.size !== kotlinConsts.length) {
    throw new Error("Screaming-snake collision in RPC method names; disambiguate the .tsp member names");
  }
  domains["daemon-rpc"] = {
    models: {
      RpcMethod: {
        ts: `/** Daemon RPC method wire ids. Source: RpcMethod in typespec/domains/daemon-rpc.tsp. */\nexport type RpcMethod =\n${tsUnion};\n\nexport const RPC_METHOD_IDS: readonly RpcMethod[] = [\n${tsArray}\n];`,
        swift: `public enum BurnBarRPCMethod: String, Codable, CaseIterable, Hashable, Sendable {\n${swiftCases}\n}`,
        kotlin: `/** Daemon RPC method wire ids. Source: RpcMethod in typespec/domains/daemon-rpc.tsp. */\n@Keep\nobject DaemonRpcMethods {\n${kotlinConsts.map((c) => c.line).join("\n")}\n\n    val ALL: List<String> = listOf(\n${[...screamingSet].map((s) => `        ${s},`).join("\n")}\n    )\n}`,
      },
    },
  };
}
...
function emitKotlin(domainId, models) {
  // The RPC catalog is wire ids, not Firestore docs — no Firestore imports.
