...
ANDROID_DIFF_COVERAGE_ALLOWLIST_JSON="$(cat <<'JSON'
{
  ...
  "android/app/src/main/java/com/openburnbar/ui/hermes/HermesRichBubble.kt": "Compose bubble rendering surface built on remember/inlineContent; local JVM coverage cannot prove recomposition, while HermesAtomParser is JVM-covered by HermesAtomParserTest and the streaming/finalized rendering paths are exercised by the instrumented HermesRichBubbleTest suite.",
