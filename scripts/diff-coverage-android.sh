...
ANDROID_DIFF_COVERAGE_ALLOWLIST_JSON="$(cat <<'JSON'
{
  ...
  "android/app/src/main/java/com/openburnbar/ui/tokens/PensieveTokens.kt": "Generated by packages/design-tokens (Style Dictionary) and drift-guarded byte-for-byte against the pipeline output by tokens.test.mjs; string constants only with no executable lines, so JaCoCo line evidence is not meaningful."
}
JSON
)"
