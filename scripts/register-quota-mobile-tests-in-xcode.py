...
PATH_IN_PROJ = (
    "QuotaStoreOrderTests.swift"  # inside the OpenBurnBarMobileTests group (which has path = OpenBurnBarMobileTests)
)
...
def main() -> int:
    ...
    phase_line = f"\t\t\t\t{build_file_id} /* {PATH_IN_PROJ} in Sources */,"
    group_child_line = f"\t\t\t\t{file_ref_id} /* {PATH_IN_PROJ} */,"
    ...
    pattern = re.compile(
        r"("
        + re.escape(MOBILE_TESTS_SOURCES_PHASE)
    ...
    pattern_group = re.compile(
        r"("
        + re.escape(MOBILE_TESTS_GROUP_ID)
        + r"\s*/\*\s*OpenBurnBarMobileTests\s*\*/\s*=\s*\{[^}]*?children\s*=\s*\(\n)(.*?)(\n\s*\);)",
        re.DOTALL,
    )
