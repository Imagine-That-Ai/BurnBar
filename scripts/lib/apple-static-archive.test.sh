  ...
  libtool)
    ...
    case "${OPENBURNBAR_STATIC_ARCHIVE_TEST_DIAGNOSTIC:-none}" in
      ...
      warning)
        ...
        ;;
      executable-warning)
        printf "/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/libtool: warning: '%s(empty.o)' has no symbols\n" "$archive" >&2
        ;;
      file)
        printf 'libtool: file: %s(empty.o) has no symbols\n' "$archive" >&2
        ;;
      executable-file)
        printf '/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/libtool: file: %s(empty.o) has no symbols\n' "$archive" >&2
        ;;
      unrelated)
        printf 'libtool: file: %s(empty.o) has no symbols\n' "$archive" >&2
...
assert_accepted_diagnostic executable-warning
assert_accepted_diagnostic file
assert_accepted_diagnostic executable-file
assert_rejected_diagnostic \
  unrelated \
