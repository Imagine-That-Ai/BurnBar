...
const candidateRunnerScript = readFileSync(
  join(SCRIPT_DIR, "windows-port/run-candidate-evidence.ps1"),
  "utf8",
);
const vmValidateScript = readFileSync(
  join(SCRIPT_DIR, "windows-port/vm-validate.ps1"),
  "utf8",
);
...
assert.match(
  runnerScript,
  /isProtectedSecretEnvelope/,
  "runner must distinguish encrypted protected-secret envelopes from plaintext secret findings",
);
assert.match(
  runnerScript,
  /\^obb-storage-evidence-/,
  "runner must not classify its own deterministic storage-evidence paths as opaque secrets",
);
assert.match(
  runnerScript,
  /\^obb-storage-\[0-9a-f\]\{32\}\$/,
  "runner must not classify storage test temp directory identifiers as opaque secrets",
);
for (const [name, script] of [
  ["foundation host runner", runnerScript],
  ["candidate evidence runner", candidateRunnerScript],
  ["VM validator", vmValidateScript],
]) {
  assert.match(
    script,
    /Get-CimInstance Win32_OperatingSystem/,
    `${name} must resolve platform from OS architecture before process architecture`,
  );
  assert.match(
    script,
    /OSArchitecture -match 'ARM'/,
    `${name} must choose ARM64 on ARM64 Windows hosts`,
  );
}
assert.match(
  runnerScript,
  /Interactive profile cleanup failed/,
  "runner must remove disposable encrypted profile state before packaging evidence",
);
assert.match(
  runnerScript,
  /Remove-Item[^\n]+\$interactiveProfilesDir[^\n]+-ErrorAction Stop/,
  "runner must fail closed while removing scenario-local protected storage",
);
assert.match(
  runnerScript,
