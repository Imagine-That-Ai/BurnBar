...
FUNCTIONS_SYNC_VENDOR_DIR="${ROOT_DIR}/functions-sync/vendor/openburnbar/domain-core-wasm"
...
if [[ "${MODE}" == "--check" ]]; then
  ...
  compare_trees_exactly "Sync Node Wasm" "${FUNCTIONS_SYNC_VENDOR_DIR}" "${NODE_STAGING_DIR}"
...
rm -rf "${FUNCTIONS_SYNC_VENDOR_DIR}"
mkdir -p "$(dirname "${FUNCTIONS_SYNC_VENDOR_DIR}")"
cp -R "${NODE_STAGING_DIR}" "${FUNCTIONS_SYNC_VENDOR_DIR}"
rm -rf "${MCP_REMOTE_VENDOR_DIR}"
mkdir -p "$(dirname "${MCP_REMOTE_VENDOR_DIR}")"
cp -R "${NODE_STAGING_DIR}" "${MCP_REMOTE_VENDOR_DIR}"
log "wrote ${VENDOR_DIR}, ${FUNCTIONS_VENDOR_DIR}, ${FUNCTIONS_SYNC_VENDOR_DIR}, and ${MCP_REMOTE_VENDOR_DIR}"
