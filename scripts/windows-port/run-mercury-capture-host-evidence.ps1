<#
.SYNOPSIS
    Build and run the Windows Mercury capture-buffer certification.

.DESCRIPTION
    Must run in the signed-in interactive desktop session. Builds the shipping
    ARM64 WinUI app and exact-candidate host harness, captures a visible window
    through the production Mercury WGC source, verifies real nonblank BGRA bytes,
    writes a PNG, and proves uncertified transport encoding fails closed.
#>
[CmdletBinding()]
param(
...
if ([System.Diagnostics.Process]::GetCurrentProcess().SessionId -eq 0) {
    throw 'Mercury capture host evidence must run in the signed-in interactive desktop session, not Session 0.'
}
...
$harnessProject = Join-Path $repo 'windows\tests\mercury-capture-host\OpenBurnBar.Mercury.Capture.HostHarness\OpenBurnBar.Mercury.Capture.HostHarness.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Mercury capture host harness build failed with exit code $LASTEXITCODE." }

    & dotnet run --project $harnessProject -c Release -p:Platform=ARM64 -r win-arm64 --no-build -- --output $hostOutput
    if ($LASTEXITCODE -ne 0) { throw "Mercury capture host harness failed with exit code $LASTEXITCODE." }
}
finally {
    Pop-Location
}

$summaryPath = Join-Path $hostOutput 'mercury-capture-host-summary.json'
$capturePath = Join-Path $hostOutput 'mercury-capture-wgc.png'
...
if (-not $summary.Passed) { throw 'Mercury capture host summary did not pass.' }
...
$receipt = [ordered]@{
    schema = 'openburnbar.windows.mercury-capture-host-evidence.v1'
    ...
    hostSummary = [ordered]@{
        fileName = 'host/mercury-capture-host-summary.json'
        sha256 = Get-Sha256 $summaryPath
        checks = $summary.Checks.Count
        passed = $summary.Passed
        captureSha256 = $summary.CaptureSha256
    }
    wgcCapture = [ordered]@{
        fileName = 'host/mercury-capture-wgc.png'
        sha256 = Get-Sha256 $capturePath
        size = (Get-Item -LiteralPath $capturePath).Length
        width = $summary.CaptureWidth
        height = $summary.CaptureHeight
    }
}

$receiptPath = Join-Path $output 'mercury-capture-host-receipt.json'
Write-JsonFile $receiptPath $receipt
Write-Host "Mercury capture host evidence passed: $receiptPath" -ForegroundColor Green
