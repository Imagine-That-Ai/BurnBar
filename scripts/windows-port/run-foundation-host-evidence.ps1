...
function Resolve-Platform {
    try {
        ...
        if ($os.OSArchitecture -match 'ARM') { return 'ARM64' }
        if ($os.OSArchitecture -match '64') { return 'x64' }
        if ($os.OSArchitecture -match '32' -or $os.OSArchitecture -match 'x86') { return 'x86' }
    }
    catch {
...
function Test-SecretLeaks([string] $Root) {
    ...
    foreach ($file in $files) {
        ...
        $isProtectedSecretEnvelope = $file.FullName -match '[\\/]protected-secrets[\\/][^\\/]+\.secret\.json$'
        ...
        foreach ($match in $highEntropy.Matches($text)) {
            $value = $match.Value
            if (-not $isProtectedSecretEnvelope -and -not (Test-KnownDiagnosticEntropyToken $value) -and (Measure-Entropy $value) -ge 4.2) {
...
function Test-KnownDiagnosticEntropyToken([string] $Value) {
    if ($Value -match '(?i)REDACTED') { return $true }
    if ($Value -match '(?i)^obb-storage-evidence-') { return $true }
    if ($Value -match '(?i)^obb-storage-[0-9a-f]{32}$') { return $true }
    if ($Value -match '(?i)^openburnbar-foundation-evidence-[0-9a-f]{7,40}$') { return $true }
    return $false
}
...
$steps.Add((Invoke-LoggedProcess 'foundation-process-tool-build' 'dotnet' @(
    'build',
    'windows\tests\foundation-host-evidence\OpenBurnBar.FoundationHostEvidence.csproj',
    '--configuration', 'Debug'
)))
$toolExe = Get-ChildItem -LiteralPath (Join-Path $RepoRoot 'windows\tests\foundation-host-evidence\bin\Debug') -Filter 'OpenBurnBar.FoundationHostEvidence.exe' -Recurse |
    Sort-Object LastWriteTimeUtc -Descending |
...
# Scenario profiles contain disposable SQLCipher databases and DPAPI ciphertext.
# They are setup state, not review artifacts; fail closed if they cannot be removed.
$interactiveProfilesDir = Join-Path $interactiveDir 'profiles'
if (Test-Path -LiteralPath $interactiveProfilesDir) {
    Remove-Item -LiteralPath $interactiveProfilesDir -Recurse -Force -ErrorAction Stop
}
if (Test-Path -LiteralPath $interactiveProfilesDir) {
    throw "Interactive profile cleanup failed: $interactiveProfilesDir"
}
