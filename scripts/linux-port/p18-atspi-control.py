#!/usr/bin/env python3
"""Bounded AT-SPI reader for the installed P-18 memory-review flow."""
...
def walk(root: Any, limit: int = 5000):
    ...
    if queue:
        raise RuntimeError("AT-SPI tree exceeded the P-18 node budget")
...
def snapshot(root: Any, pyatspi: Any) -> dict[str, Any]:
    ...
    if len(rows) < 10:
        raise RuntimeError("installed memory review exposed too few AT-SPI nodes")
    return {
        "producer": "openburnbar-p18-atspi-control-v1",
...
def main() -> None:
    ...
    parser.add_argument("--mode", required=True, choices=("snapshot", "activate"))
    ...
    if args.mode == "snapshot":
        ...
        result = {"producer": "openburnbar-p18-atspi-control-v1", "activation": activate(find(app, args.name, True))}
