...
test('release workflow isolates the signer from mutable build tools and the network', () => {
  ...
  // Dedicated sign-* jobs may receive the private key before packaging.
  // The mutable build-architecture job body before the isolated signer must
  // stay key-free so package compilation cannot exfiltrate it.
  const buildJobStart = workflow.indexOf('\n  build-architecture:');
  assert.ok(buildJobStart > 0 && buildJobStart < signerStart);
  const unsignedArchitectureBuild = workflow.slice(buildJobStart, signerStart);
  assert.doesNotMatch(unsignedArchitectureBuild, /OPENBURNBAR_LINUX_ED25519_PRIVATE_KEY_PEM/u);
  for (const marker of [
