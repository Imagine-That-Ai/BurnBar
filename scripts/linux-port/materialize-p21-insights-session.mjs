...
function copy(repoRoot, sourceRoot, outputRoot, name, environmentId, label) {
  ...
  return artifactRecord(repoRoot, destination, "P-21", environmentId, label);
}

export function materializeP21InsightsSession(
  options,
  {
  ...
  if (!expected) throw new Error("P-21 requires a supported Linux environment");
  ...
  const signature = copy(
    ...
    options.environmentId,
    "P-21 installed manifest signature",
  );
  if (
    ...
    throw new Error(
      "P-21 installed attestation changed during materialization",
    );
  }
  const marker = parseJson(
    fs.readFileSync(path.join(repoRoot, copied["insights-marker.json"].path)),
    "P-21 marker",
  );
  const daemon = parseJson(
    fs.readFileSync(
      path.join(repoRoot, copied["insights-daemon-transcript.json"].path),
    ),
    "P-21 daemon transcript",
  );
  const ui = parseJson(
    fs.readFileSync(
      path.join(repoRoot, copied["insights-ui-transcript.json"].path),
    ),
    "P-21 UI transcript",
  );
  const times = [
  ...
  if (times.length === 0 || times.some((time) => !Number.isFinite(time)))
    throw new Error("P-21 evidence timestamps are invalid");
  const document = {
    schemaVersion: 1,
    id: "openburnbar-linux-p21-installed-insights-session-v1",
    requirementId: "P-21",
    ...
    marker,
    evidence: {
      daemonTranscript: copied["insights-daemon-transcript.json"],
      uiTranscript: copied["insights-ui-transcript.json"],
      initialScreenshot: copied["insights-initial.png"],
      compareScreenshot: copied["insights-compare.png"],
      restartScreenshot: copied["insights-restart.png"],
      sourceLossScreenshot: copied["insights-source-loss.png"],
    },
  };
  validateP21InstalledSession(
    document,
  ...
  const output = path.join(outputRoot, P21_SESSION_FILENAME);
...
if (
  ...
  try {
    process.stdout.write(
      `${JSON.stringify({ output: materializeP21InsightsSession(args(process.argv.slice(2))).output })}\n`,
    );
  } catch (error) {
    process.stderr.write(
      `P-21 insights session materialization failed: ${error.message}\n`,
    );
    process.exitCode = 1;
  }
}
