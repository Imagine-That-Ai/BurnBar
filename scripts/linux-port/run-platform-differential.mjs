...
export class DifferentialJsonError extends Error {}

/**
 * Parse JSON after checking the object-key uniqueness rule that JSON.parse
 * otherwise silently weakens (the last duplicate wins). Evidence is an
 * attestation input, so accepting duplicate keys would make the bytes being
 * compared ambiguous and could hide a platform mutation.
 */
export function parseJsonNoDuplicateKeys(text) {
  if (typeof text !== 'string') throw new DifferentialJsonError('JSON input must be text');
  let index = 0;

  const fail = (message) => {
    throw new DifferentialJsonError(`${message} at byte ${index}`);
  };
  ...
  const parseValue = () => {
    ...
    if (token === '{') {
      ...
      while (index < text.length) {
        ...
        keys.add(key);
        skipWhitespace();
        if (text[index] !== ':') fail('expected a colon after a JSON object key');
        index += 1;
        parseValue();
        skipWhitespace();
        if (text[index] === '}') {
        ...
        if (text[index] !== ',') fail('expected a comma between JSON object members');
        index += 1;
      }
    ...
    if (token === '[') {
      ...
      while (index < text.length) {
        ...
        if (text[index] !== ',') fail('expected a comma between JSON array values');
        index += 1;
      }
  ...
  try {
    ...
    throw new DifferentialJsonError(`invalid JSON: ${error.message}`);
  }
}
