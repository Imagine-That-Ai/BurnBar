...
export function captureP23ProviderWorkspaceProof(
  options,
  ...
  if (
    ...
    throw new Error("P-23 invocation binding is invalid");
  ...
  if (resolveHead(rootInfo.root) !== options.targetHead)
    throw new Error("P-23 target HEAD does not match checkout HEAD");
  ...
  const proof = buildP23Proof({
    session: validated.document,
    source,
    collectedAt: now().toISOString(),
    daemonEvents: validated.daemonEvents,
    liveCredentialRoutes: validated.liveCredentialRoutes,
    lifecycleMutations: validated.lifecycleMutations,
  });
  ...
  validateP23Proof({
    ...binding,
    snapshot: readRegularSnapshot(rootInfo.root, record.path, "P-23 proof"),
  });
  atomicWriteJson(registration, {
    ...
    artifacts: [
      { role: P23_PROOF_ROLE, path: `feature-artifacts/${P23_PROOF_FILENAME}` },
    ],
  });
...
if (
  ...
  try {
    const result = captureP23ProviderWorkspaceProof(
    ...
    process.stderr.write(
      `P-23 provider-workspace proof capture failed: ${error.message}\n`,
    );
    process.exitCode = 1;
  }
}
