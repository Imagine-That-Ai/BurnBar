import { P17_PROOF_ROLE, validateP17Proof } from '../lib/p17-activity-proof.mjs';
import { result, validateRequirementContext } from './lib.mjs';

export async function validateProductRequirement(context) {
  const validated = validateRequirementContext(context, ['aggregate-product-proof-closure', P17_PROOF_ROLE]);
  const rows = validated.proofs.get(P17_PROOF_ROLE);
  if (!Array.isArray(rows) || rows.length !== 1) throw new Error(`${P17_PROOF_ROLE} proof must occur exactly once`);
  const proof = validateP17Proof({
