import { P10_PROOF_ROLE, validateP10Proof } from '../lib/p10-dashboard-layout-proof.mjs';
import { result, validateRequirementContext } from './lib.mjs';

export async function validateProductRequirement(context) {
  const validated = validateRequirementContext(context, ['aggregate-product-proof-closure', P10_PROOF_ROLE]);
  const rows = validated.proofs.get(P10_PROOF_ROLE);
  if (!Array.isArray(rows) || rows.length !== 1) throw new Error(`${P10_PROOF_ROLE} proof must occur exactly once`);
  const proof = validateP10Proof({
