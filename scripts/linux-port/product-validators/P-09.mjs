import { P09_PROOF_ROLE, validateP09Proof } from '../lib/p09-navigation-shell-proof.mjs';
import { result, validateRequirementContext } from './lib.mjs';

export async function validateProductRequirement(context) {
  const validated = validateRequirementContext(context, ['aggregate-product-proof-closure', P09_PROOF_ROLE]);
  const rows = validated.proofs.get(P09_PROOF_ROLE);
  if (!Array.isArray(rows) || rows.length !== 1) throw new Error(`${P09_PROOF_ROLE} proof must occur exactly once`);
  const proof = validateP09Proof({
