...
export async function validateProductRequirement(context) {
  ...
  const rows = validated.proofs.get(P29_PROOF_ROLE);
  if (!Array.isArray(rows) || rows.length !== 1)
    throw new Error(`${P29_PROOF_ROLE} proof must occur exactly once`);
  const proof = validateP29Proof({
    ...
    manifestSignatureSha256:
      validated.closure.packageManifestSignature.sha256,
  });
  const artifacts = [
    ...validated.artifacts,
