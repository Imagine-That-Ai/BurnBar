import { P11_PROOF_ROLE, validateP11Proof } from '../lib/p11-usage-ingestion-proof.mjs';
import { result, validateRequirementContext } from './lib.mjs';

export async function validateProductRequirement(context) {
  const validated = validateRequirementContext(context, ['aggregate-product-proof-closure', P11_PROOF_ROLE]);
  const rows = validated.proofs.get(P11_PROOF_ROLE);
  if (!Array.isArray(rows) || rows.length !== 1) throw new Error(`${P11_PROOF_ROLE} proof must occur exactly once`);
  const proof = validateP11Proof({
