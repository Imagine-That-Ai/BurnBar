...
if [[ "${1:-}" == "desktop-inner" ]]; then
  ...
  for index in "${!route_names[@]}"; do
    ...
    printf '%s\t%s\t%s\t%s\n' "$route" "$(basename "$screenshot")" "$(basename "$xwininfo_route")" "$current_window_id" >>"$route_tsv"
  done

  node - "$route_tsv" "$out_dir/packaged-route-session-transcript.json" "$out_dir/daemon-route-transcript.json" "$out_dir/daemon-session-oracle.json" <<'NODE'
const fs = require('fs');
const [tsvPath, outPath, daemonRoutePath, oraclePath] = process.argv.slice(2);
...
const routes = fs.readFileSync(tsvPath, 'utf8')
  ...
  .filter(Boolean)
  .map((line, index) => {
    const [route, screenshot, xwininfo, windowId] = line.split('\t');
    return {
      index,
      route,
      screenshot,
      xwininfo,
      windowId,
      action: index === 0 ? 'ctrl+shift+Home' : 'ctrl+shift+Right',
...
const payload = {
  ...
  surface: 'installed-tauri-deb-xvfb-xfce',
  missionWorktree: process.env.OB_MISSION_WORKTREE_HOST || '/workspace',
  ...
  daemonOracle: oracle ? {
    ...
    missionWorktree: oracle.missionWorktree,
    missionWorktreeHost: oracle.missionWorktreeHost
  } : null,
  routes
};
fs.writeFileSync(outPath, JSON.stringify(payload, null, 2) + '\n');
fs.writeFileSync(daemonRoutePath, JSON.stringify({
  ...payload,
  validatorGuidance: 'Routes were reached in the installed packaged Tauri .deb under the CI compositor profile; daemon rows are constrained by daemon-session-oracle.json.'
}, null, 2) + '\n');
...
if [[ -f "$out_dir/daemon-shell-session.log" ]]; then cat "$out_dir/daemon-shell-session.log"; else cat "$out_dir/daemon-socket-gui-session.log"; fi
