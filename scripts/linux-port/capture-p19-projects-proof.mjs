...
export function captureP19ProjectsProof(
  options,
  ...
  if (
    ...
    throw new Error("P-19 invocation binding is invalid");
  ...
  if (resolveHead(rootInfo.root) !== options.targetHead)
    throw new Error("P-19 target HEAD does not match checkout HEAD");
  ...
  const proof = buildP19Proof({
    session: validated.document,
    source,
    collectedAt: now().toISOString(),
    associatedReferences: validated.associatedReferences,
    events: validated.events,
    tombstoneRejections: validated.tombstoneRejections,
  });
  ...
  validateP19Proof({
    ...binding,
    snapshot: readRegularSnapshot(rootInfo.root, record.path, "P-19 proof"),
  });
  atomicWriteJson(registration, {
    ...
    artifacts: [
      { role: P19_PROOF_ROLE, path: `feature-artifacts/${P19_PROOF_FILENAME}` },
    ],
  });
...
if (
  ...
  try {
    const result = captureP19ProjectsProof(args(process.argv.slice(2)));
    process.stdout.write(
      `${JSON.stringify({ output: result.output, registration: result.registration })}\n`,
    );
  } catch (error) {
    process.stderr.write(
      `P-19 projects proof capture failed: ${error.message}\n`,
    );
    process.exitCode = 1;
  }
}
