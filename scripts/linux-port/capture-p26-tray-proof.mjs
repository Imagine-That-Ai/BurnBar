...
export function captureP26TrayProof(
  options,
  ...
  if (
    ...
    throw new Error("P-26 invocation binding is invalid");
  ...
  if (resolveHead(rootInfo.root) !== options.targetHead)
    throw new Error("P-26 target HEAD does not match checkout HEAD");
  ...
  const proof = buildP26Proof({
    session: validated.document,
    source,
    collectedAt: now().toISOString(),
    actionCount: validated.actionCount,
  ...
  validateP26Proof({
    ...binding,
    snapshot: readRegularSnapshot(rootInfo.root, record.path, "P-26 proof"),
  });
  atomicWriteJson(registration, {
    ...
    artifacts: [
      { role: P26_PROOF_ROLE, path: `feature-artifacts/${P26_PROOF_FILENAME}` },
    ],
  });
...
if (
  ...
  try {
    const result = captureP26TrayProof(args(process.argv.slice(2)));
    process.stdout.write(
      `${JSON.stringify({ output: result.output, registration: result.registration })}\n`,
    );
  } catch (error) {
    process.stderr.write(`P-26 tray proof capture failed: ${error.message}\n`);
    process.exitCode = 1;
  }
}
