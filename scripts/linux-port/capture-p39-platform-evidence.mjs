...
function commandOutput(runner, command, args, options = {}) {
  ...
  if (result.error || result.status !== 0) {
    ...
    throw new Error(`${command} runtime probe failed: ${reason}`);
  }
  return (result.stdout ?? '').trim();
}

function resolveExecutable(runner, command, repoRoot) {
...
function readCurrentProductVersion(repoRoot) {
  const packagePath = path.join(repoRoot, 'apps/linux-desktop/package.json');
  let packageDocument;
  try {
    packageDocument = JSON.parse(fs.readFileSync(packagePath, 'utf8'));
  } catch (error) {
    throw new Error(`P-39 current product version could not be read: ${error.message}`);
  }
  if (!VERSION.test(packageDocument?.version ?? '')) {
    throw new Error('P-39 current product version is not a strict release version');
  }
  return packageDocument.version;
}

/**
 * Capture the non-domain inputs that make a parser run reproducible. These
 * values are attested in each producer artifact and deliberately ignored only
 * at the differential comparison path because macOS and Linux use different
 * native runtimes. Missing probes fail closed; a generated fixture cannot
 * supply this attestation.
 */
export function collectRuntimeAttestation({
  repoRoot,
  platform,
  ...
  if (runtimePlatform !== expectedRuntime) {
    throw new Error(`P-39 ${platform} runtime attestation cannot run on ${runtimePlatform}`);
  }
  const swift = resolveExecutable(runner, 'swift', repoRoot);
  const swiftVersion = commandOutput(runner, 'swift', ['--version'], { cwd: repoRoot });
  if (!swiftVersion) throw new Error('swift runtime probe returned an empty version');
  const locale = String(environment.LC_ALL || environment.LC_CTYPE || environment.LANG || 'C');
  const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC';
  return {
    schemaVersion: EXECUTION_SCHEMA_VERSION,
    platform,
    productVersion: readCurrentProductVersion(repoRoot),
    os: { platform: runtimePlatform, architecture: runtimeArchitecture, release: String(runtimeRelease) },
    runtime: { node: process.versions.node, swiftVersion },
    executable: swift,
    locale,
    timeZone,
    featureFlags: []
...
function validateExecutionAttestation(execution, platform = null) {
  exactObject(execution, [
    'executable', 'featureFlags', 'locale', 'os', 'platform', 'productVersion', 'runtime', 'schemaVersion', 'timeZone'
  ], 'P-39 producer execution attestation');
  exactObject(execution.executable, ['name', 'sha256'], 'P-39 producer executable');
  exactObject(execution.os, ['architecture', 'platform', 'release'], 'P-39 producer OS');
  exactObject(execution.runtime, ['node', 'swiftVersion'], 'P-39 producer runtime');
  if (execution.schemaVersion !== EXECUTION_SCHEMA_VERSION
      || !['macos', 'linux'].includes(execution.platform)
      || (platform !== null && execution.platform !== platform)
      || !VERSION.test(execution.productVersion ?? '')
      || execution.os.platform !== (execution.platform === 'macos' ? 'darwin' : 'linux')
      || typeof execution.os.architecture !== 'string' || execution.os.architecture.length === 0
      || typeof execution.os.release !== 'string' || execution.os.release.length === 0
      || !SHA256.test(execution.executable.sha256 ?? '')
      || execution.executable.name !== 'swift'
      || typeof execution.runtime.node !== 'string' || execution.runtime.node.length === 0
      || typeof execution.runtime.swiftVersion !== 'string' || execution.runtime.swiftVersion.length === 0
      || typeof execution.locale !== 'string' || execution.locale.length === 0
      || typeof execution.timeZone !== 'string' || execution.timeZone.length === 0
      || !Array.isArray(execution.featureFlags)
      || execution.featureFlags.some((flag) => typeof flag !== 'string')) {
    throw new Error('P-39 producer execution attestation is incomplete or platform-mismatched');
  }
  const normalizedFlags = [...new Set(execution.featureFlags)].sort();
  if (normalizedFlags.length !== execution.featureFlags.length
      || normalizedFlags.some((flag, index) => flag !== execution.featureFlags[index])) {
    throw new Error('P-39 producer feature flags must be a unique sorted list');
  }
  return execution;
}

export function validateP39ProducerArtifact(document, { repoRoot = null, platform = null } = {}) {
  exactObject(document?.payload, ['contract', 'corpus', 'execution', 'observed', 'parserOutput'], 'P-39 producer payload');
  ...
  validateExecutionAttestation(document.payload.execution, platform);
  exactObject(
...
export function captureP39PlatformEvidence({
  ...
  const execution = runtimeAttestation
    ? runtimeAttestation({ repoRoot: repository, platform, runtimePlatform, runtimeArchitecture, runtimeRelease })
    : collectRuntimeAttestation({ repoRoot: repository, platform, runtimePlatform, runtimeArchitecture, runtimeRelease });
  if (execution.productVersion !== version) {
    throw new Error(`P-39 current product version ${execution.productVersion} does not match candidate version ${version}`);
  }
  ...
  validateP39ProducerArtifact(document, { repoRoot: repository, platform });
