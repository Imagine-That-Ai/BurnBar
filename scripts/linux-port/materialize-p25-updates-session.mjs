...
const RAW_FILES = Object.freeze([
  "updates-package-lifecycle.json",
  "updates-available.json",
  "updates-current.json",
  "updates-error.json",
  "updates-restart.json",
...
function copy(
  ...
  return artifactRecord(repoRoot, destination, "P-25", environmentId, label);
}

export function materializeP25UpdatesSession(
  options,
  {
  ...
  if (!expected) throw new Error("P-25 requires a supported Linux environment");
  ...
  const signature = copy(
    ...
    options.environmentId,
    "P-25 installed manifest signature",
  );
  if (
    ...
    throw new Error(
      "P-25 installed attestation changed during materialization",
    );
  }
  const marker = {
    ...
    candidateVersion: options.packageVersion,
    previousVersion: options.previousVersion,
    packageChannel: options.packageChannel,
  };
  const times = ["available", "current", "error", "restart"].map((phase) =>
    Date.parse(
      parseJson(
        fs.readFileSync(
          path.join(repoRoot, copied[`updates-${phase}.json`].path),
        ),
        `P-25 ${phase} phase`,
      ).capturedAt,
    ),
  ...
  if (times.length === 0 || times.some((time) => !Number.isFinite(time)))
    throw new Error("P-25 evidence timestamps are invalid");
  const document = {
    schemaVersion: 1,
    id: "openburnbar-linux-p25-installed-updates-session-v1",
    requirementId: "P-25",
    ...
    marker,
    evidence: {
      lifecycle: copied["updates-package-lifecycle.json"],
      availablePhase: copied["updates-available.json"],
      currentPhase: copied["updates-current.json"],
      errorPhase: copied["updates-error.json"],
      restartPhase: copied["updates-restart.json"],
      availableScreenshot: copied["updates-available.png"],
      currentScreenshot: copied["updates-current.png"],
      errorScreenshot: copied["updates-error.png"],
      restartScreenshot: copied["updates-restart.png"],
    },
  };
  validateP25InstalledSession(
    document,
  ...
  const output = path.join(outputRoot, P25_SESSION_FILENAME);
...
if (
  ...
  try {
    process.stdout.write(
      `${JSON.stringify({ output: materializeP25UpdatesSession(args(process.argv.slice(2))).output })}\n`,
    );
  } catch (error) {
    process.stderr.write(
      `P-25 updates session materialization failed: ${error.message}\n`,
    );
    process.exitCode = 1;
  }
}
