...
test('ownership-ready fixture blocks exactly the 37 incomplete requirement lanes', async (t) => {
  ...
  assert.equal(captured.document.summary.validatorCount, 3);
  assert.equal(captured.document.summary.captureCount, 3);
  assert.equal(captured.document.summary.materializerCount, 3);
  assert.equal(captured.document.summary.readyCount, 3);
  assert.deepEqual(
    captured.document.requirements.filter((row) => !row.ready).map((row) => row.requirementId),
    REQUIREMENT_IDS.filter((id) => !['P-02', 'P-05', 'P-40'].includes(id))
  );
