...
export function captureP20MissionsProof(
  options,
  ...
  if (
    ...
    throw new Error("P-20 invocation binding is invalid");
  ...
  if (resolveHead(rootInfo.root) !== options.targetHead)
    throw new Error("P-20 target HEAD does not match checkout HEAD");
  ...
  const proof = buildP20Proof({
    session: validated.document,
    source,
    collectedAt: now().toISOString(),
    daemonEvents: validated.daemonEvents,
    historyEntries: validated.historyEntries,
    uiStates: validated.uiStates,
  });
  ...
  validateP20Proof({
    ...binding,
    snapshot: readRegularSnapshot(rootInfo.root, record.path, "P-20 proof"),
  });
  atomicWriteJson(registration, {
    ...
    artifacts: [
      { role: P20_PROOF_ROLE, path: `feature-artifacts/${P20_PROOF_FILENAME}` },
    ],
  });
...
if (
  ...
  try {
    const result = captureP20MissionsProof(args(process.argv.slice(2)));
    process.stdout.write(
      `${JSON.stringify({ output: result.output, registration: result.registration })}\n`,
    );
  } catch (error) {
    process.stderr.write(
      `P-20 missions proof capture failed: ${error.message}\n`,
    );
    process.exitCode = 1;
  }
}
