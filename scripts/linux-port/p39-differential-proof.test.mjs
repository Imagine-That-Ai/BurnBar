...
test('P-39 proof validation rejects a substituted proof binding or source bytes', () => {
  ...
  try {
    const result = capture(subject);
    const proofPath = fs.realpathSync(path.join(subject.inputRoot, P39_PROOF_FILENAME));
...
test('P-39 proof validation requires canonical UTC millisecond timestamps', () => {
  const subject = fixture();
  try {
    const result = capture(subject);
    const proofPath = fs.realpathSync(path.join(subject.inputRoot, P39_PROOF_FILENAME));
    ...
    const repository = fs.realpathSync(subject.root);
    const validate = () => {
      const snapshot = readRegularSnapshot(repository, path.relative(repository, proofPath), 'P-39 proof');
      return validateP39DifferentialProof({
        repoRoot: subject.root,
        snapshot,
        targetHead: HEAD,
        environmentId: ENVIRONMENT,
    ...
    for (const generatedAt of ['2026-07-21T12:00:00Z', '2026-07-21T12:00:00.000-05:00', 'not-a-date']) {
      proof.generatedAt = generatedAt;
      fs.writeFileSync(proofPath, `${JSON.stringify(proof)}\n`, 'utf8');
      assert.throws(validate, /not invocation-bound/u, generatedAt);
    }
    proof.generatedAt = result.document.generatedAt;
    fs.writeFileSync(proofPath, `${JSON.stringify(proof)}\n`, 'utf8');
...
test('P-39 only permits reviewed platform-only volatile paths', () => {
  const subject = fixture();
  try {
    assert.throws(
      () => capture(subject, { ignoredPaths: ['$.payload.parserOutput'] }),
      /approved platform-only divergence/u
    );
  } finally {
    fs.rmSync(subject.root, { recursive: true, force: true });
  }
