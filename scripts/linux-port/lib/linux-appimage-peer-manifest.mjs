...
export function createLinuxAppImagePeerManifest({
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) throw new Error(`AppImage GUI executable is not a regular file: ${executablePath}`);
  const manifest = {
    ...
    executableSHA256: sha256Bytes(fs.readFileSync(executablePath))
  };
