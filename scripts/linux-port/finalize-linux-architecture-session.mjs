...
const lifecycle = {
  ...
  update: archLifecycle?.update ?? { status: 'blocked', reason: archLifecycleFailure ?? 'Arch update evidence is missing.' },
  rollback: archLifecycle?.rollback ?? { status: 'blocked', reason: archLifecycleFailure ?? 'Arch rollback evidence is missing.' },
  dataPreservation: archLifecycle?.dataPreservation
    ?? { status: 'blocked', reason: archLifecycleFailure ?? 'Arch data-preservation evidence is missing.' }
};
const blockers = [];
