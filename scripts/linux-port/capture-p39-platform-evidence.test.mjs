...
function execution(platform = 'linux') {
  return {
    schemaVersion: 1,
    platform,
    productVersion: '1.2.3',
    os: {
      platform: platform === 'macos' ? 'darwin' : 'linux',
      architecture: 'x86_64',
      release: 'test-kernel'
    },
    runtime: { node: '22.0.0', swiftVersion: 'Swift version 6.0 (test)' },
    executable: { name: 'swift', sha256: 'c'.repeat(64) },
    locale: 'C',
    timeZone: 'UTC',
    featureFlags: []
  };
...
test('producer emits candidate-bound normalized observed output and report', () => {
  ...
  try {
    ...
    const result = captureP39PlatformEvidence({
      ...
      runtimeAttestation: () => execution('linux'),
      resolveHead: () => HEAD,
      runProducer: () => fakeRun()
    });
...
test('producer rejects generated bytes whose attested digest is substituted', () => {
  const subject = fixture();
  try {
    const output = path.join(subject.root, 'out/linux-platform-evidence.json');
    assert.throws(() => captureP39PlatformEvidence({
      repoRoot: subject.root,
      platform: 'linux',
      output,
      candidateClosure: subject.candidateClosure,
      ...
      runtimeAttestation: () => execution('linux'),
      resolveHead: () => HEAD,
      runProducer: () => ({ ...fakeRun(), generatedSha256: 'f'.repeat(64) })
    }), /generated bytes are not hash-bound/u);
...
test('producer validation rejects a substituted normalized digest', () => {
  const subject = fixture();
  try {
    const result = captureP39PlatformEvidence({
      repoRoot: subject.root,
      platform: 'linux',
      output: path.join(subject.root, 'out/linux-platform-evidence.json'),
      candidateClosure: subject.candidateClosure,
      targetHead: HEAD,
      candidateRunId: RUN_ID,
      candidateArtifactDigest: DIGEST,
    ...
    tampered.payload.observed.normalizedSha256 = 'f'.repeat(64);
    assert.throws(
      () => validateP39ProducerArtifact(tampered, { repoRoot: subject.root }),
...
test('producer requires a platform-bound execution attestation', () => {
  const subject = fixture();
  try {
    const base = {
      repoRoot: subject.root,
    ...
    assert.throws(
      () => captureP39PlatformEvidence({ ...base, runtimeAttestation: () => execution('macos') }),
      /platform-mismatched/u
    );
    const result = captureP39PlatformEvidence({ ...base, runtimeAttestation: () => execution('linux') });
    assert.throws(
      () => validateP39ProducerArtifact(result.document, { repoRoot: subject.root, platform: 'macos' }),
      /platform-mismatched/u
    );
    const tampered = structuredClone(result.document);
    delete tampered.payload.execution;
    assert.throws(() => validateP39ProducerArtifact(tampered), /fields must be exactly/u);
  } finally {
    fs.rmSync(subject.root, { recursive: true, force: true });
  }
});

test('runtime attestation hashes the resolved Swift executable and records host inputs', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'openburnbar-p39-runtime-'));
  const swift = path.join(root, 'swift');
  try {
    fs.mkdirSync(path.join(root, 'apps/linux-desktop'), { recursive: true });
    fs.writeFileSync(path.join(root, 'apps/linux-desktop/package.json'), '{"version":"0.1.1"}\n');
    fs.writeFileSync(swift, '#!/bin/sh\necho test\n', { mode: 0o700 });
    const attestation = collectRuntimeAttestation({
      repoRoot: root,
      platform: 'linux',
      runtimePlatform: 'linux',
      runtimeArchitecture: 'x86_64',
      runtimeRelease: 'test-kernel',
      environment: { LANG: 'C' },
      runner: (command) => command === 'which'
        ? { status: 0, stdout: `${swift}\n`, stderr: '' }
        : { status: 0, stdout: 'Swift version 6.0 (test)\n', stderr: '' }
    });
    assert.equal(attestation.platform, 'linux');
    assert.equal(attestation.productVersion, '0.1.1');
    assert.equal(attestation.os.platform, 'linux');
    assert.equal(attestation.executable.name, 'swift');
    assert.equal(attestation.executable.sha256, crypto.createHash('sha256').update(fs.readFileSync(swift)).digest('hex'));
    assert.equal(attestation.runtime.swiftVersion, 'Swift version 6.0 (test)');
  } finally {
