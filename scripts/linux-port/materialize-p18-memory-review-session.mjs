...
const RAW_FILES = Object.freeze([
  "memory-marker.json",
  "memory-daemon-transcript.json",
  "memory-ui-transcript.json",
  "memory-initial.png",
  "memory-restart.png",
]);
...
function copy(repoRoot, sourceRoot, outputRoot, name, environmentId, label) {
  ...
  return artifactRecord(repoRoot, destination, "P-18", environmentId, label);
}

export function materializeP18MemoryReviewSession(
  options,
  {
  ...
  if (!expected) throw new Error("P-18 requires a supported Linux environment");
  ...
  const signature = copy(
    ...
    options.environmentId,
    "P-18 installed manifest signature",
  );
  if (
    ...
    throw new Error(
      "P-18 installed attestation changed during materialization",
    );
  }
  const marker = parseJson(
    fs.readFileSync(path.join(repoRoot, copied["memory-marker.json"].path)),
    "P-18 marker",
  );
  const daemon = parseJson(
    fs.readFileSync(
      path.join(repoRoot, copied["memory-daemon-transcript.json"].path),
    ),
    "P-18 daemon transcript",
  );
  const ui = parseJson(
    fs.readFileSync(
      path.join(repoRoot, copied["memory-ui-transcript.json"].path),
    ),
    "P-18 UI transcript",
  );
  const times = [
  ...
  if (times.length === 0 || times.some((time) => !Number.isFinite(time)))
    throw new Error("P-18 evidence timestamps are invalid");
  const document = {
    schemaVersion: 1,
    id: "openburnbar-linux-p18-installed-memory-review-session-v1",
    requirementId: "P-18",
    ...
    marker,
    evidence: {
      daemonTranscript: copied["memory-daemon-transcript.json"],
      uiTranscript: copied["memory-ui-transcript.json"],
      initialScreenshot: copied["memory-initial.png"],
      restartScreenshot: copied["memory-restart.png"],
    },
  };
  validateP18InstalledSession(
    document,
  ...
  const output = path.join(outputRoot, P18_SESSION_FILENAME);
...
if (
  ...
  try {
    process.stdout.write(
      `${JSON.stringify({ output: materializeP18MemoryReviewSession(args(process.argv.slice(2))).output })}\n`,
    );
  } catch (error) {
    process.stderr.write(
      `P-18 memory-review session materialization failed: ${error.message}\n`,
    );
    process.exitCode = 1;
  }
}
