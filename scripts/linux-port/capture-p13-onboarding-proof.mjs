...
export function captureP13OnboardingProof(
  options,
  ...
  if (
    ...
    throw new Error("P-13 invocation binding is invalid");
  ...
  if (resolveHead(rootInfo.root) !== options.targetHead)
    throw new Error("P-13 target HEAD does not match checkout HEAD");
  ...
  validateP13Proof({
    ...binding,
    snapshot: readRegularSnapshot(rootInfo.root, record.path, "P-13 proof"),
  });
  atomicWriteJson(registration, {
    ...
    artifacts: [
      { role: P13_PROOF_ROLE, path: `feature-artifacts/${P13_PROOF_FILENAME}` },
    ],
  });
...
if (
  ...
  try {
    const result = captureP13OnboardingProof(args(process.argv.slice(2)));
    process.stdout.write(
      `${JSON.stringify({ output: result.output, registration: result.registration })}\n`,
    );
  } catch (error) {
    process.stderr.write(
      `P-13 onboarding proof capture failed: ${error.message}\n`,
    );
    process.exitCode = 1;
  }
}
