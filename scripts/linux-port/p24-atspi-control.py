#!/usr/bin/env python3
"""Bounded AT-SPI controller for the installed P-24 Settings flow."""
...
def walk(root: Any, limit: int = 6000):
    ...
    if queue:
        raise RuntimeError("AT-SPI tree exceeded the P-24 node budget")
...
def activate(node: Any) -> dict[str, str]:
    ...
    preferred = ("press", "click", "activate", "toggle", "select", "jump")
...
def set_text(node: Any, value: str) -> dict[str, str]:
    try:
        ...
        raise RuntimeError(f"AT-SPI text edit failed: {safe(getattr(node, 'name', ''))}") from error
...
def focus(node: Any) -> None:
    try:
        if not node.queryComponent().grabFocus():
            raise RuntimeError("focus request was rejected")
    except Exception as error:
        raise RuntimeError(f"AT-SPI focus failed: {safe(getattr(node, 'name', ''))}") from error
...
def snapshot(root: Any, pyatspi: Any) -> dict[str, Any]:
    ...
    for node in walk(root):
        ...
        if name or node_actions or any(state in node_states for state in ("checked", "selected", "focused")):
            rows.append({"name": name, "role": role(node), "actions": node_actions, "states": node_states})
    if len(rows) < 10:
        raise RuntimeError("installed Settings exposed too few AT-SPI nodes")
    return {
        "producer": "openburnbar-p24-atspi-control-v1",
...
def main() -> None:
    ...
    parser.add_argument(
        "--mode",
        required=True,
        choices=("snapshot", "activate", "set-text", "audit-tab"),
    )
    ...
    if args.mode == "snapshot":
        ...
        result = {"producer": "openburnbar-p24-atspi-control-v1", "activation": activate(find(app, args.name, True))}
    elif args.mode == "set-text":
        if not args.name or args.value is None:
            raise RuntimeError("--name and --value are required")
        result = {"producer": "openburnbar-p24-atspi-control-v1", "edit": set_text(find(app, args.name), args.value)}
    else:
        if not args.name or args.value is None:
            raise RuntimeError("--name and --value are required")
        edit = set_text(find(app, args.name), args.value)
        time.sleep(0.2)
        tab = find(app, args.value, True)
