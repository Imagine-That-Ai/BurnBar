...
function execution(platform) {
  return {
    ...
    os: { platform: platform === 'macos' ? 'darwin' : 'linux', architecture: 'x86_64', release: 'test-kernel' },
...
function fixture() {
  ...
  for (const [platform, filename] of Object.entries(P39_PLATFORM_INPUT_FILENAMES)) {
    fs.writeFileSync(path.join(inputRoot, filename), `${JSON.stringify(artifact({ execution: execution(platform) }))}\n`);
  }
