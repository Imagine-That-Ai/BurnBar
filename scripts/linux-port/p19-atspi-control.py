#!/usr/bin/env python3
"""Bounded AT-SPI reader for the installed P-19 projects flow."""
...
def walk(root: Any, limit: int = 5000):
    ...
    if queue:
        raise RuntimeError("AT-SPI tree exceeded the P-19 node budget")
...
def snapshot(root: Any, pyatspi: Any) -> dict[str, Any]:
    ...
    if len(rows) < 10:
        raise RuntimeError("installed projects exposed too few AT-SPI nodes")
    return {
        "producer": "openburnbar-p19-atspi-control-v1",
...
def main() -> None:
    ...
    if args.mode == "snapshot":
        ...
        result = {"producer": "openburnbar-p19-atspi-control-v1", "activation": activate(find(app, args.name, True))}
    else:
