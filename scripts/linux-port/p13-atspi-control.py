#!/usr/bin/env python3
"""Bounded AT-SPI reader and actuator for the installed P-13 onboarding flow."""
...
def walk(root: Any, limit: int = 5000):
    ...
    if queue:
        raise RuntimeError("AT-SPI tree exceeded the P-13 node budget")
...
def set_text(node: Any, value: str) -> dict[str, str]:
    try:
        ...
        raise RuntimeError(f"AT-SPI text update failed: {safe(getattr(node, 'name', ''))}: {error}") from error
    return {"name": safe(getattr(node, "name", "")), "role": role(node), "action": "set-text"}
...
def snapshot(root: Any, pyatspi: Any) -> dict[str, Any]:
    ...
    if len(rows) < 10:
        raise RuntimeError("installed onboarding exposed too few AT-SPI nodes")
    return {
...
def main() -> None:
    ...
    if args.mode == "snapshot":
        ...
        result = {"producer": "openburnbar-p13-atspi-control-v1", "activation": activate(find(app, args.name, True))}
    else:
        if not args.name or args.value is None:
            raise RuntimeError("--name and --value are required")
        result = {
            "producer": "openburnbar-p13-atspi-control-v1",
            "activation": set_text(find(app, args.name), args.value),
        }
