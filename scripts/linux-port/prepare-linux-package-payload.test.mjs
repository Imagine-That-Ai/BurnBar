...
test('payload staging copies daemon, Swift tree, and SQLCipher SONAME', () => {
  ...
  const textExpansionManifest = path.join(root, 'text-expansion-engine.json');
  ...
  fs.writeFileSync(textExpansionManifest, '{"schemaVersion":2}\n');
  ...
  assert.equal(fs.readFileSync(report.textExpansionManifest, 'utf8'), '{"schemaVersion":2}\n');
  assert.equal(fs.statSync(report.textExpansionManifest).mode & 0o777, 0o644);
