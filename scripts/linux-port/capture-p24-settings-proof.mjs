...
export function captureP24SettingsProof(
  options,
  ...
  if (
    ...
    throw new Error("P-24 invocation binding is invalid");
  ...
  if (resolveHead(rootInfo.root) !== options.targetHead)
    throw new Error("P-24 target HEAD does not match checkout HEAD");
  ...
  validateP24Proof({
    ...binding,
    snapshot: readRegularSnapshot(rootInfo.root, record.path, "P-24 proof"),
  });
  atomicWriteJson(registration, {
    ...
    artifacts: [
      { role: P24_PROOF_ROLE, path: `feature-artifacts/${P24_PROOF_FILENAME}` },
    ],
  });
...
if (
  ...
  try {
    const result = captureP24SettingsProof(args(process.argv.slice(2)));
    process.stdout.write(
      `${JSON.stringify({ output: result.output, registration: result.registration })}\n`,
    );
  } catch (error) {
    process.stderr.write(
      `P-24 Settings proof capture failed: ${error.message}\n`,
    );
    process.exitCode = 1;
  }
}
