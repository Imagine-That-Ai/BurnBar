#!/usr/bin/env python3
"""Bounded AT-SPI controller for the installed P-21 Insights flow."""
...
def walk(root: Any, limit: int = 5000):
    ...
    if queue:
        raise RuntimeError("AT-SPI tree exceeded the P-21 node budget")
...
def snapshot(root: Any, pyatspi: Any) -> dict[str, Any]:
    ...
    if len(rows) < 20:
        raise RuntimeError("installed Insights exposed too few AT-SPI nodes")
    return {
        "producer": "openburnbar-p21-atspi-control-v1",
...
def main() -> None:
    ...
    if args.mode == "snapshot":
        ...
        result = {"producer": "openburnbar-p21-atspi-control-v1", "activation": activate(find(app, args.name, True))}
