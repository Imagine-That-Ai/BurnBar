...
function trustedSystemExecutable(executable, label) {
  const root = fs.realpathSync('/usr');
  const resolved = fs.realpathSync(executable);
  const stat = fs.statSync(resolved);
  assert(resolved.startsWith(`${root}${path.sep}`) && stat.isFile() && stat.uid === 0
    && (stat.mode & 0o111) !== 0 && (stat.mode & 0o022) === 0,
  `P-31 ${label} must resolve to a root-owned non-writable /usr executable`);
  for (let parent = path.dirname(resolved);; parent = path.dirname(parent)) {
    const parentStat = fs.statSync(parent);
    assert(parentStat.isDirectory() && parentStat.uid === 0 && (parentStat.mode & 0o022) === 0,
      `P-31 ${label} system path contains an untrusted directory`);
...
function trustedSystemTool(command) {
  ...
  return trustedSystemExecutable(executable, command);
}
...
function webdriver(environment) {
  ...
  return {
    async start() {
      ...
      trustedSystemExecutable('/usr/bin/WebKitWebDriver', 'WebKitWebDriver');
