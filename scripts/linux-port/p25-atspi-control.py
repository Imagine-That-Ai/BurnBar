#!/usr/bin/env python3
"""Bounded AT-SPI controller for the installed P-25 updates flow."""
...
def walk(root: Any, limit: int = 5000):
    ...
    if queue:
        raise RuntimeError("AT-SPI tree exceeded the P-25 node budget")
...
def snapshot(root: Any, pyatspi: Any) -> dict[str, Any]:
    ...
    if len(rows) < 20:
        raise RuntimeError("installed Updates surface exposed too few AT-SPI nodes")
    return {
        "producer": "openburnbar-p25-atspi-control-v1",
...
def main() -> None:
    ...
    if args.mode == "snapshot":
        ...
        result = {"producer": "openburnbar-p25-atspi-control-v1", "activation": activate(find(app, args.name, True))}
    else:
        if not args.name or args.value is None:
            raise RuntimeError("--name and --value are required")
        result = {"producer": "openburnbar-p25-atspi-control-v1", "text": set_text(find(app, args.name), args.value)}
