...
export function captureP22DatabaseProof(
  options,
  ...
  if (
    ...
    throw new Error("P-22 invocation binding is invalid");
  ...
  if (resolveHead(rootInfo.root) !== options.targetHead)
    throw new Error("P-22 target HEAD does not match checkout HEAD");
  ...
  const proof = buildP22Proof({
    session: validated.document,
    source,
    collectedAt: now().toISOString(),
    daemonEvents: validated.daemonEvents,
    indexedFiles: validated.indexedFiles,
    failClosedMutations: validated.failClosedMutations,
  });
  ...
  validateP22Proof({
    ...binding,
    snapshot: readRegularSnapshot(rootInfo.root, record.path, "P-22 proof"),
  });
  atomicWriteJson(registration, {
    ...
    artifacts: [
      { role: P22_PROOF_ROLE, path: `feature-artifacts/${P22_PROOF_FILENAME}` },
    ],
  });
...
if (
  ...
  try {
    const result = captureP22DatabaseProof(args(process.argv.slice(2)));
    process.stdout.write(
      `${JSON.stringify({ output: result.output, registration: result.registration })}\n`,
    );
  } catch (error) {
    process.stderr.write(
      `P-22 database proof capture failed: ${error.message}\n`,
    );
    process.exitCode = 1;
  }
}
