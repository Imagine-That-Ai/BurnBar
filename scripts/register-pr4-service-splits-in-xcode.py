...
def source_phase_build_ids(contents: str, phase_id: str) -> set[str]:
    ...
    if not match:
        raise RuntimeError(f"mac Sources phase {phase_id} not found")
...
def remove_root_monolith_refs(contents: str) -> str:
    refs_by_path = resolved_file_refs_by_path(contents)
    for path in REMOVE_ROOT_PATHS:
        name = Path(path).name
        for file_ref_id in refs_by_path.get(normalize_path(path), []):
            build_file_ids = build_file_ids_for_file_ref(contents, file_ref_id)
            contents = re.sub(
                rf"^\s*{re.escape(file_ref_id)} /\* {re.escape(name)} \*/ = "
            ...
            for build_file_id in build_file_ids:
                contents = re.sub(
                    rf"^\s*{re.escape(build_file_id)} /\* {re.escape(name)} in Sources \*/ = "
                    rf"\{{isa = PBXBuildFile; fileRef = {re.escape(file_ref_id)} /\* {re.escape(name)} \*/; \}};\n",
                    "",
                    contents,
                    flags=re.MULTILINE,
                )
