...
#     computeruse / tests) is computed ONLY from windows/<area>/, so growth in one
#     area cannot move another area's counter.
#
# Fails CI if EITHER:
...
#   - a Windows source file lives OUTSIDE the eight documented sub-trees.
...
AREAS = ("app", "pal", "native", "storage", "particles", "pretext", "computeruse", "tests")
...
if stray:
    ...
    print(
        "Place Windows source under windows/{app,pal,native,storage,particles,pretext,computeruse,tests}/ (see windows/README.md).",
        file=sys.stderr,
    )
