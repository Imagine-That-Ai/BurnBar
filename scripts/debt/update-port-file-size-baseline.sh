#!/usr/bin/env bash
# Regenerate the port (Kotlin/Rust/TS) file-size baseline. Run this after
# decomposing a giant port file so the budget ratchets DOWN. The target line
# count is preserved; pass --target N to retighten the threshold itself.
set -euo pipefail
