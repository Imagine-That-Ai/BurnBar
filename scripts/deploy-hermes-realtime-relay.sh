...
HOSTED_RELAY_PRODUCT_IDS="${HOSTED_RELAY_PRODUCT_IDS:-${HOSTED_QUOTA_PRODUCT_ID:-com.openburnbar.hostedQuotaSync.cloud.monthly,com.openburnbar.hostedQuotaSync.monthly,com.openburnbar.pro.monthly}}"
ENFORCE_APP_CHECK="${ENFORCE_APP_CHECK:-true}"
# Default-on: reject revoked/disabled-user Firebase ID tokens at the relay edge.
VERIFY_REVOKED_ID_TOKENS="${VERIFY_REVOKED_ID_TOKENS:-true}"
...
case "${DEPLOY_PROFILE}" in
  staging-cheap)
    ...
    ;;
  prod-safe)
    # >=2 so a single instance failing does not drop every live relay socket
    # (the relay holds long-lived WebSocket connections; min-instances=1 was a SPOF).
...
OPENBURNBAR_SOURCE_COMMIT="${OPENBURNBAR_SOURCE_COMMIT:-$(git -C "${ROOT_DIR}" rev-parse HEAD)}"
