...
//    including js-yaml and Python's yaml.safe_load -- expand anchors happily, so
//    local validation passes while production silently disables every update job
//    in the file. The blast radius is the whole config, not the one entry. This
//    check exists because that exact mistake shipped in review and was caught
//    only by reading the Dependabot issue tracker.
//
