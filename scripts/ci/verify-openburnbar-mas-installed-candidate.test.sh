...
assert value["status"] == "installed-receipt-file-observed"
assert value["receiptFilePresent"] is True
assert value["storeReceiptCertification"] == "HOLD"
assert value["receiptCryptographicVerification"] == "HOLD-unavailable"
assert value["appStoreConnectProcessingEvidence"]["processingReceiptSha256"] == hashlib.sha256(
    processing.read_bytes()
).hexdigest()
assert value["appStoreConnectProcessingEvidence"]["packageSha256"] == "d" * 64
PY
...
echo "PASS: installed MAS lifecycle command binds signature/profile plus opaque receipt-file presence/hash to exact App Store Connect processing evidence without claiming cryptographic store-receipt validity."
