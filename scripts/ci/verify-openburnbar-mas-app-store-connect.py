...
def load_artifact_receipt(
    ...
    *,
    ...
    if path.is_symlink() or not path.is_file():
        raise ValueError(f"MAS archive/export receipt must be a real file: {path}")
    ...
    try:
        ...
        raise ValueError(f"MAS archive/export receipt is invalid JSON: {error}") from error
    if not isinstance(value, dict):
        raise ValueError("MAS archive/export receipt must be a JSON object")
    if (
        value.get("schemaVersion") != 1
        or value.get("platform") != "MAC_OS"
        or value.get("status") != "archive-export-verified"
    ):
        raise ValueError("MAS archive/export receipt schema/status is unsupported")
    if value.get("candidate") != {
        "commit": candidate_commit,
        "tree": candidate_tree,
    }:
        raise ValueError("MAS archive/export receipt candidate identity does not match")
    ...
    expected_artifacts = {
        "archiveTreeSha256": sha256_tree(archive),
        "archiveHostAppTreeSha256": sha256_tree(archive_app),
        "archiveSafariExtensionTreeSha256": sha256_tree(archive_appex),
        "exportedHostAppTreeSha256": sha256_tree(app),
        "exportedSafariExtensionTreeSha256": sha256_tree(appex),
        "packageSha256": sha256_file(pkg),
        "packageSize": pkg.stat().st_size,
    }
    mismatches = {
        key: {"expected": expected, "actual": artifacts.get(key)}
...
def create_receipt(args: argparse.Namespace) -> dict[str, Any]:
    ...
    artifact_evidence = load_artifact_receipt(
...
def parse_args() -> argparse.Namespace:
    ...
    preflight_parser = subparsers.add_parser(
        "upload-preflight",
        help="validate receipt-bound MAS artifacts before upload",
    )
    preflight_parser.add_argument("--artifact-receipt", required=True, type=Path)
    preflight_parser.add_argument("--candidate-commit", required=True)
    preflight_parser.add_argument("--candidate-tree", required=True)
    preflight_parser.add_argument("--team-id", required=True)
    preflight_parser.add_argument("--version", required=True)
    preflight_parser.add_argument("--build", required=True)
    preflight_parser.add_argument("--archive", required=True, type=Path)
    preflight_parser.add_argument("--app", required=True, type=Path)
    preflight_parser.add_argument("--pkg", required=True, type=Path)
    preflight_parser.add_argument("--artifact-verifier", required=True, type=Path)
    ...
    receipt_parser.add_argument("--team-id", required=True)
    receipt_parser.add_argument("--artifact-receipt", required=True, type=Path)
    ...
    try:
        if args.command == "upload":
            ...
            if (
                ...
                raise ValueError("upload preflight requires the canonical MAS artifact verifier")
            result = load_artifact_receipt(
                args.artifact_receipt,
                archive=args.archive,
                app=args.app,
                pkg=args.pkg,
            ...
            run_artifact_verifier(
                ...
                build=args.build,
                pkg=args.pkg,
                label="pre-upload canonical artifact verification",
            )
        else:
