...
if [[ ! -f "OpenBurnBarCore/Sources/OpenBurnBarKernelModels/Budget/BudgetGate.swift" ]]; then
  echo "FAIL: OpenBurnBarCore/Sources/OpenBurnBarKernelModels/Budget/BudgetGate.swift is missing." >&2
...
for seam in "${seam_files[@]}"; do
  ...
  if [[ "$lines" -gt "$MAX_SEAM_LINES" ]]; then
    ...
    echo "      Shared gate behavior belongs in OpenBurnBarCore/Sources/OpenBurnBarKernelModels/Budget/BudgetGate.swift." >&2
    fail=1
  fi
done
