...
r="$(new_repo)"
printf 'log("eslint-disable-next-line -- fake native description"); // eslint-disable-next-line no-console\n' >"${r}/a.ts"
assert_exit 1 "${r}" "string-literal eslint native description does not justify"
r="$(new_repo)"
printf 'const text = "eslint-disable-next-line -- this is not a directive";\n' >"${r}/a.ts"
assert_exit 0 "${r}" "string-literal eslint text is not scanned as a directive"
r="$(new_repo)"
