...
ROBOTS
    ...
    if [[ "$relative" == "robots.txt" && "${MOCK_ROBOTS_ROGUE_PREFIX:-}" == "1" ]]; then
      printf '# unexpected preamble\n' > "$body"
      cat "$MOCK_RUNTIME_ROOT/$relative" >> "$body"
      printf '200'
      exit 0
    fi
...
rm -f "$health"
if (cd "$ROOT" && MOCK_ROBOTS_ROGUE_PREFIX=1 run_smoke >/dev/null 2>&1); then
  echo "FAIL: arbitrary robots prefix was accepted" >&2
  exit 1
fi
[[ ! -e "$health" ]] || { echo "FAIL: arbitrary robots prefix produced health evidence" >&2; exit 1; }
