...
// blocks that installed protoc and nothing else. Anyone adding a new macOS
// Swift job by copying a neighbour would reintroduce exactly that gap, and it
// would only show up as a confusing cargo panic on one half of the fleet. So:
//
 ...
 * Restricting to `jobs:` matters: script filenames also appear in `on.*.paths`
 * filters, and treating those as builds produced six false positives.
 */
...
for (const file of files) {
  ...
  for (const job of jobsOf(source)) {
    ...
    if (/brew install protobuf/.test(job.text) && !job.text.includes(ACTION)) {
      problems.push(
    ...
    // Only count an *invocation*. These filenames also appear in
    // hashFiles(...) cache keys and in `on.*.paths` filters; treating a cache
    // key as the build step made the ordering check below fire on eight
    // correctly-ordered jobs.
    const jobLines = job.text.split("\n");
    ...
    jobLines.forEach((line, i) => {
      ...
      // `./scripts/x.sh`, `bash scripts/x.sh`, `sh scripts/x.sh`, or a
      // VAR=1 ./scripts/x.sh continuation.
    ...
    const triggers = FFI_BUILD_SCRIPTS.filter((s) =>
      invocationLines.some((i) => jobLines[i].includes(s)),
    );
    checked += 1;

    const actionLine = jobLines.findIndex((l) => l.includes(ACTION));
    if (actionLine === -1) {
      problems.push(
    ...
    if (actionLine > Math.min(...invocationLines)) {
      problems.push(
        `${path} job "${job.name}": uses ${ACTION} but only AFTER the FFI build step ` +
          `(preflight at job line ${actionLine + 1}, first build at ${Math.min(...invocationLines) + 1}). ` +
