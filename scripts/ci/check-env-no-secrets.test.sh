...
for fixture in stripe webhook private-key google-key secret-var service-account-json; do
  ...
  case "$fixture" in
    ...
    private-key) printf 'APNS_AUTH_KEY=-----BEGIN PRIVATE KEY-----\\n' >> "$bad" ;;
    google-key) printf '%s%s\n' 'FIREBASE_WEB_API_KEY=AIza' '1234567890abcdef1234567890abc' >> "$bad" ;;
    secret-var) echo "PERPLEXITY_API_KEY=not-allowed-here" >> "$bad" ;;
    service-account-json) echo 'SERVICE_ACCOUNT={"private_key":"-----BEGIN PRIVATE KEY-----"}' >> "$bad" ;;
  esac
