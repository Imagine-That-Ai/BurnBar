...
// caused the outage, and does NOT fire on shapes that are already correct.
...
// A correct job: cache key mentions the build script (must be ignored), the
// preflight precedes the real invocation, and an on.*.paths filter also names it.
...
test("FAILS on the protobuf-only preflight in codeql-pr.yml's one-liner form", () => {
  // This is the exact variant that escaped the #1969 sweep: differently named
  // step, `if ...; then brew install protobuf; fi` collapsed onto one line.
  const root = scaffold({
