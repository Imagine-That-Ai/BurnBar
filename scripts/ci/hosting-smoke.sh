...
const cloudflareContentSignalsPreamble = [
...
// Cloudflare may prepend its exact legal content-signals preamble before the
// managed block. Do not accept arbitrary bytes before that block.
if (
  beginIndex !== cloudflareContentSignalsPreamble.length ||
  !managedPrefix.startsWith(cloudflareContentSignalsPreamble) ||
  endIndex < beginIndex ||
