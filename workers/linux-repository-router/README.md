...
The public preview route serves only safe apt/RPM paths below the named
content-addressed snapshot and needs no activation pointer. Release automation
can therefore verify the first snapshot before installing the serving routes.
The runtime role check also rejects cross-surface requests if a route is ever
misconfigured.
...
drill then reactivates the newer candidate with `promote`.
