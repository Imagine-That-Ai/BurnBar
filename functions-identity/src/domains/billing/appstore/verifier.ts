...
import type { AppStoreConfig, AppStoreEnvironment } from "@openburnbar/functions-shared/types.js";
...
export function loadAppleRootCertificates(): Buffer[] {
  ...
  for (const { name, fingerprintHex } of ROOT_CERT_FILES) {
    ...
    const srcPath = join(process.cwd(), "src", "domains", "billing", "appstore", "certs", name);
