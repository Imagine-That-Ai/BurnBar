...
} from "@openburnbar/functions-shared/appCheckAttestation.js";
...
import { PHONE_CONTROL_ESCROW_PLATFORMS } from "@openburnbar/functions-shared/callables/computerUseSecurityCodecs.js";
import { requireTrustedDeviceActionProof, requireTrustedEscrowDevice } from "@openburnbar/functions-shared/callables/computerUseSecurityFirestore.js";
import {
...
import { checkPublicHttpEndpointRateLimit } from "@openburnbar/functions-shared/callables/publicRateLimit.js";
import { boundedTrimmedString } from "@openburnbar/functions-shared/shared/validators.js";
import {
  LINUX_APP_CHECK_DEVICE_COLLECTION,
  LINUX_APP_CHECK_REJECTION_REASON,
  deviceRef,
  requireApprovedLinuxAppCheckIrohHost,
} from "@openburnbar/functions-shared/shared/linuxAppCheckHosts.js";

export { LINUX_APP_CHECK_REJECTION_REASON, requireApprovedLinuxAppCheckIrohHost };
...
function isLinuxDeviceTrustState(value: unknown): value is LinuxDeviceTrustState {
...
export async function consumeLinuxAppCheckChallenge(args: {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (!deviceSnapshot.exists) {
      throw new HttpsError("permission-denied", "Linux App Check device is not registered.", {
    ...
    if (trustState !== "approved") {
      throw new HttpsError("permission-denied", "Linux App Check device is not eligible for challenge consumption.", {
        reason:
          trustState === "revoked"
    ...
    if (deviceSnapshot.get("appId") !== args.appId || deviceSnapshot.get("deviceId") !== args.deviceId) {
      throw new HttpsError("permission-denied", "Linux App Check device record does not match the challenge.", {
  ...
  async (
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (existing.exists) {
        ...
        if (!isLinuxDeviceTrustState(existingState)) {
...
export const issueLinuxAppCheckChallenge = onCallProduction(
  ...
  async (request: CallableRequest<{ appId?: unknown; deviceId?: unknown }>) => {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (trustState !== "approved") {
        throw new HttpsError("permission-denied", "Linux App Check device is not eligible for challenges.", {
          reason:
            trustState === "revoked"
              ? LINUX_APP_CHECK_REJECTION_REASON.revoked
              : LINUX_APP_CHECK_REJECTION_REASON.invalidTrustState,
      ...
      if (device.get("appId") !== appId || device.get("deviceId") !== deviceId) {
        throw new HttpsError("permission-denied", "Linux App Check device record does not match the request.", {
          reason: LINUX_APP_CHECK_REJECTION_REASON.recordMismatch,
        });
      }
...
export const listLinuxAppCheckDevices = onCallProduction(
  ...
  async (request: CallableRequest<{ approverDeviceId?: unknown }>) => {
    ...
    const pendingSnapshots = await collection
      ...
      .get();
    const newestSnapshots =
      pendingSnapshots.docs.length < MAX_LISTED_LINUX_DEVICES
    ...
    const devices = [...snapshots.values()]
      .map((snapshot) =>
