...
///
/// `body` carries the full multi-paragraph reply (LLM-generated when
/// a `userKey`-tier gateway is selected; deterministic and explicitly
/// labelled when the local rule engine is the only available
/// answerer). `groundedPoints` are short bullet attestations the UI
/// renders as data-grounded chips beneath the body — they prove the
/// answer was computed from the digest, not hallucinated.
public struct InsightBriefingAnswer: Codable, Hashable, Sendable {
    ...
    public enum Source: String, Codable, Hashable, Sendable {
        /// Generated by an `InsightModelGateway` (Claude/OpenAI/etc.)
        /// using the privacy-bounded digest as grounding context.
        case modelGateway
        /// Generated deterministically by the local rule engine —
        /// shown when the user is in privacy mode, when no remote
        /// gateway is configured, or when the gateway call failed and
        /// we degraded gracefully so the user still gets a reply.
        case localRules
    }

    public var question: String
    public var body: String
    public var groundedPoints: [String]
    public var citations: [InsightCitation]
    ...
    /// should surface a "Showing local fallback" hint so the user
    /// knows the remote engine wasn't reachable for this turn.
    ...
    public init(
        ...
        self.question = question
        self.body = body
        self.groundedPoints = groundedPoints
        self.citations = citations
        self.source = source
