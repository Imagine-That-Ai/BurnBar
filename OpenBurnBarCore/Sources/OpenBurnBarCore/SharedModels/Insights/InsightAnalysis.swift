...
public struct InsightBriefingAnswer: Codable, Hashable, Sendable, Identifiable {
    ...
    public enum Source: String, Codable, Hashable, Sendable {
        /// Generated by a user-owned `InsightModelGateway` (Claude /
        /// OpenAI / Hermes / Ollama / Pi / OpenAI-compatible etc.)
        /// using the privacy-bounded digest as context.
        case modelGateway
        /// Generated by the BurnBar-hosted fallback (OpenRouter →
        /// MiniMax) when the user has no user-owned LLM configured
        /// or none are reachable. Surfaced honestly so the user can
        /// see the answer used our hosted route, not their own.
        case hostedFallback
        /// Generated deterministically by the local rule engine —
        /// shown only when every other route (user-owned and
        /// hosted) is unreachable, or when privacy mode forbids
        /// egress. Always disclosed as a non-LLM data summary.
        case localRules
    }
    ...
    /// Sentinel `modelDisplayName` the orchestrator stamps when a
    /// hosted-route attempt was paywalled. The UI matches on this
    /// exact string to swap the "Connect your own model" CTA for
    /// the "Upgrade to BurnBar Pro" CTA.
    public static let subscriptionRequiredDisplayName = "BurnBar Pro required"
...
public extension InsightMissionCandidate {
    var launchMissionKind: String {
        dispatchMetadata["missionKind"] ?? {
            switch lens {
            case .accretion: return "accretive"
            case .diligence: return "diligence"
            case .techDebt: return "debt"
            case .routing: return "provider_routing"
            case .quota: return "cost_efficiency"
            case .focus: return "project_focus"
            }
        }()
    }

    var launchQuestion: InsightFollowUpQuestion {
        let criteria = acceptanceCriteria.prefix(4).map { "- \($0)" }.joined(separator: "\n")
        let evidenceLabels = evidence.prefix(6).map(\.label).joined(separator: ", ")
        return InsightFollowUpQuestion(
            question: """
            Launch this recommended \(launchMissionKind) mission from the current Intelligence Brief.
            ...
            Acceptance criteria:
            \(criteria.isEmpty ? "- Define acceptance criteria from the brief evidence." : criteria)
            Evidence: \(evidenceLabels.isEmpty ? "Use the current brief citations and findings." : evidenceLabels)
            """,
            rationale: "Launches a recommended mission candidate with its brief context."
        )
    }
}
