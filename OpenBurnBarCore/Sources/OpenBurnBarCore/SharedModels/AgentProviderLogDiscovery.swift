...
public enum AgentProviderLogDiscovery {
    ...
    public static func filePattern(for provider: AgentProvider) -> String {
        switch provider {
        case .factory, .claudeCode, .copilot, .aider, .zai, .minimax, .forgeDev, .hermes, .piAgent, .cursorAgent, .openClaw, .openClaude:
            return "*.jsonl"
        case .cursor:
            return "*.db"
        case .openAI:
    ...
    public static func resolveLogSource(
        ...
        let expanded = OpenBurnBarLinuxPaths.expandTildeInPath(logical, homeDirectory: homeDirectory)
