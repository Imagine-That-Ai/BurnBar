...
//   • Full Linux graph: GRDB-SQLCipher's `CSQLite` system module, so encrypted
//     data and read-only parser code share one runtime SQLite implementation.
//   • Windows and the dependency-minimal Linux daemon boundary: the vendored
//     `OpenBurnBarCoreCSQLite` amalgamation target.
...
#else
#error("No SQLite backend available: SQLite3, CSQLite, and OpenBurnBarCoreCSQLite are all unavailable.")
#endif
import Foundation
