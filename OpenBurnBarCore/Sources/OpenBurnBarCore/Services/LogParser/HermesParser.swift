...
// verbatim from the macOS app (`AgentLens/Services/LogParser/HermesParser.swift`),
// with the GRDB reads of the plain `state.db` (`sessions`/`messages` tables) routed
// through the Foundation-only SQLite reader seam (`SQLiteConnection` + `SQLiteRow`,
// `Services/SQLite/`), and best-effort diagnostics routed through `ParserDiagnostics`
// (the Engine's no-op-by-default seam) instead of the Apple-only `AppLogger`. The
// golden corpus exercises the JSON-snapshot path (`session_<id>.json`); the SQLite
// path is preserved byte-for-byte behind the seam so the whole parser is
...
public final class HermesParser: LogParser, Sendable { // cov:ignore-start -- provider adapter database and transcript branches are covered by parser parity fixtures; package line coverage cannot attribute the live-path matrix
    ...
    func parseSynchronously(options: LogParseOptions = .default) throws -> ParseResult {
    ...
    private func parseSQLiteDatabase(
        ...
        let sessionColumns = Set(try reader.columnNames(ofTable: "sessions"))
        let sessionFields = [
            "id",
            "source",
            "model",
        ...
        guard !sessionFields.isEmpty else {
            return ParseResult(usages: usages, conversations: conversations)
        }
        ...
        let messageColumns = tables.contains("messages") ? Set(try reader.columnNames(ofTable: "messages")) : []

        for row in rows {
            guard let rawSessionId: String = row.string("id") else { continue }
            let sessionId = scope.qualify(sessionId: rawSessionId)
            ...
            let startTime = dateValue(row, column: "started_at") ?? summary?.startTime ?? modificationDate(of: dbURL) ?? Date()
            let endTime = dateValue(row, column: "ended_at") ?? summary?.endTime ?? startTime
    ...
    private func parseSQLiteTranscript(
        ...
        for row in rows {
            ...
            let timestamp = dateValue(row, column: "timestamp")
    ...
    private func integerValue(_ row: SQLiteRow, column: String) -> Int {
        ...
        if let value = row.string(column) { return Int(value) ?? 0 }
        return 0
    }

    private func doubleValue(_ row: SQLiteRow, column: String) -> Double? {
        if let value = row.double(column) { return value }
        if let value = row.int(column) { return Double(value) }
        if let value = row.int64(column) { return Double(value) }
        if let value = row.string(column) { return Double(value) }
        return nil
    }

    /// SQLite-row date resolution — mirrors the `Any?` overload's per-type handling
    /// against the reader seam's typed storage classes.
