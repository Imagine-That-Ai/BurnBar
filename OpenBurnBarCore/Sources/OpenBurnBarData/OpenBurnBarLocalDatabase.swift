...
public final class OpenBurnBarLocalDatabase: @unchecked Sendable {
        ...
        try pool.write { db in
            try db.execute(
                ...
                INSERT INTO search_documents (
                    id, sourceKind, sourceID, projectName, provider, title,
                    subtitle, bodyPreview, metadataJSON, updatedAt, indexedAt,
                    visibilityScope, sourceDeviceID
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(id) DO UPDATE SET
            ...
            for chunk in fixture.chunks {
                try db.execute(
                    ...
                    INSERT INTO search_chunks (
                        id, documentID, sourceKind, sourceID, ordinal, sectionPath,
                        text, tokenCount, startOffset, endOffset, contentHash, createdAt, updatedAt
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
            ...
            try db.execute(
                sql: """
                INSERT INTO embedding_models (id, provider, model, dimension, distanceMetric, createdAt, updatedAt)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ...
            try db.execute(
                ...
                INSERT INTO embedding_versions (
                    id, modelID, version, configJSON, isActive, createdAt, updatedAt
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(id) DO NOTHING
                """,
                arguments: [
            ...
            try db.execute(
                ...
                INSERT INTO vector_index_snapshots (
                    embeddingVersionID, backendID, snapshotPath, metadataJSON,
                    vectorCount, state, errorCode, errorMessage, createdAt, updatedAt, lastBuiltAt
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    public func vectorSnapshotMetadata() throws -> [String: String] {
        let rows = try rawRows(
            sql: """
            SELECT embeddingVersionID, backendID, metadataJSON, vectorCount, state
...
public struct OpenBurnBarSearchFixtureChunk: Equatable, Sendable {
    ...
    func chunkArguments(documentID: String) -> StatementArguments {
        [
            ...
            0,
            text.utf8.count,
            OpenBurnBarSchemaHasher.sha256Hex(Data(text.utf8)),
