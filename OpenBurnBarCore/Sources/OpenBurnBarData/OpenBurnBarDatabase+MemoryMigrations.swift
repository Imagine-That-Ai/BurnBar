...
extension OpenBurnBarDatabase {
    static func registerChatMemoryAuthorityMigration(on migrator: inout DatabaseMigrator) {
        ...
        migrator.registerMigration("v54_provider_quota_snapshots") { db in
            try db.execute(
                sql: """
            ...
            try db.execute(
                sql: """
                CREATE UNIQUE INDEX IF NOT EXISTS provider_quota_snapshots_identity_idx
                ON provider_quota_snapshots(providerID, source, sourceID, period)
                """
            )
            try db.execute(
                sql: """
                CREATE INDEX IF NOT EXISTS provider_quota_snapshots_provider_time_idx
                ON provider_quota_snapshots(providerID, fetchedAt)
                """
            )
            try db.execute(
                sql: """
                CREATE INDEX IF NOT EXISTS provider_quota_snapshots_reset_idx
