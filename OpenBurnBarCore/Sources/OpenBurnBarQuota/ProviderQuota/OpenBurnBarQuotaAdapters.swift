    ...
    public func fetch(context: ProviderQuotaAdapterContext) async throws -> ProviderQuotaSnapshot {
        ...
        return ProviderQuotaSnapshot(
            ...
            statusMessage: buckets.isEmpty
                ...
                : "OpenCode uses exact local SQLite spend for the 5-hour bucket and CLI history for "
                    + "7-day/monthly plan pressure. OpenCode exposes no hosted per-account quota API, so this "
                    + "estimate covers every OpenCode Go subscription signed in on this Mac rather than one "
                    + "account. Per-subscription spend is still attributed separately wherever BurnBar routes "
                    + "the traffic.",
            buckets: buckets
        )
    }
