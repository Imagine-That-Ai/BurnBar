...
// NOTE: `SwitcherProfileStoreAdapter` (the GRDB-free profile-store protocol) and
// its `InMemorySwitcherProfileStoreAdapter` moved DOWN into the cross-platform Kernel
// (OpenBurnBarKernel/SwitcherProfileStoreAdapter.swift) in core-decomposition P-18 so
// the daemon repoint reaches the protocol via Engine without linking this Apple-only,
// AppKit-importing LaunchServices target. Both use only Kernel types
// (`SwitcherProfileRecord`, `ProviderID`, `Locked`) — a wholesale relocation. This
// file (`SwitcherBrowserLaunchService`) still conforms/consumes them via Kernel.
