...
public final class BurnBarSafariNativeBridgeController: @unchecked Sendable {
    ...
    private struct LearningMutationPayload: Codable, Hashable, Sendable {
        ...
        var resolvedProposalId: String? {
            switch (itemId, proposalId) {
            case (.some(let item), .some(let proposal)) where item == proposal:
                return item
            case (.some(let item), .none):
                return item
            case (.none, .some(let proposal)):
                return proposal
            default:
                return nil
            }
    ...
    private func executePopupAction(
        ...
        case .agentic:
            try validatePageTurnPayload(payload, handoff: false)
        ...
        case .handoff:
            try validatePageTurnPayload(payload, handoff: true)
            let decoded: HandoffPayload = try decodeExactWebPayload(
                payload,
                required: [
                    "safariSessionId", "prompt", "agentId", "pageContext",
                    "screenshot", "tabId"
            ...
            try validatePageTurn(decoded)
            let response: BurnBarSafariHandoffResponse = try sendTyped(
                method: .safariHandoff,
                request: BurnBarSafariHandoffRequest(
                    safariSessionId: decoded.safariSessionId,
                    targetHarness: decoded.agentId,
                    prompt: decoded.prompt,
                    pageState: decoded.pageContext.pageState,
                    readableMarkdown: decoded.pageContext.markdown,
                    accessibilitySnapshot: decoded.pageContext.snapshot,
                    screenshotJPEG: try Self.screenshotJPEGData(decoded.screenshot),
                    screenshotWidth: decoded.screenshot.width,
                    screenshotHeight: decoded.screenshot.height,
                    screenshotTruncated: decoded.screenshot.truncated
                ),
                response: BurnBarSafariHandoffResponse.self
            )
            output = .object([
                "activeRunId": .string(response.runId),
                "runId": .string(response.runId),
                "phase": .string(response.phase.rawValue),
                "launched": .bool(response.launched),
        ...
        case .learningOptIn:
            let decoded: LearningOptInPayload = try decodeExactWebPayload(
                payload,
                required: ["safariSessionId"],
                allowed: ["safariSessionId", "consentVersion"]
            )
            _ = try requireAttachedLearningSession(decoded.safariSessionId)
            let response: BurnBarSafariLearningStateResponse = try sendTyped(
                method: .learningOptIn,
                request: BurnBarSafariLearningOptInRequest(
                    consentVersion: decoded.consentVersion ?? 1
                ),
                response: BurnBarSafariLearningStateResponse.self
            )
            output = try BurnBarSafariNativeBridgeCodec.encodeWebValue(response)

        case .learningOptOut:
            let decoded: LearningOptOutPayload = try decodeExactWebPayload(
                payload,
                required: ["safariSessionId"],
                allowed: ["safariSessionId", "deleteLearnedProfile"]
            )
            _ = try requireAttachedLearningSession(decoded.safariSessionId)
            let response: BurnBarSafariLearningStateResponse = try sendTyped(
                method: .learningOptOut,
                request: BurnBarSafariLearningOptOutRequest(
                    deleteLearnedProfile: decoded.deleteLearnedProfile ?? true
                ),
                response: BurnBarSafariLearningStateResponse.self
            )
            output = try BurnBarSafariNativeBridgeCodec.encodeWebValue(response)
        ...
        case .learningRecall:
            let decoded: LearningRecallPayload = try decodeExactWebPayload(
                payload,
                required: ["safariSessionId", "query"],
                allowed: ["safariSessionId", "query", "limit"]
            )
            _ = try requireAttachedLearningSession(decoded.safariSessionId)
            let query = decoded.query.trimmingCharacters(
                in: .whitespacesAndNewlines
            )
            let limit = decoded.limit ?? 8
            guard query.isEmpty == false,
                  query.utf8.count <= 2_048,
                  (1...20).contains(limit) else {
                throw BurnBarSafariBridgeFailure(
                    code: "invalid_learning_recall",
                    message: "Safari learning recall requires a bounded query and result limit."
                )
            }
            let response: BurnBarSafariLearningRecallResponse = try sendTyped(
                method: .learningRecall,
                request: BurnBarSafariLearningRecallRequest(
                    query: query,
                    limit: limit
                ),
                response: BurnBarSafariLearningRecallResponse.self
            )
            guard response.untrustedContext?.utf8.count ?? 0 <= 16 * 1_024 else {
                throw BurnBarSafariBridgeFailure(
                    code: "learning_recall_too_large",
                    message: "Safari learning recall exceeded its browser context limit."
                )
            }
            output = try BurnBarSafariNativeBridgeCodec.encodeWebValue(response)

        case .learningPropose:
            let decoded: LearningCorrectionPayload = try decodeExactWebPayload(
                payload,
                required: [
                    "safariSessionId", "correctionId", "correction", "tabId",
                    "url"
                ],
                allowed: [
                    "safariSessionId", "correctionId", "correction", "tabId",
            ...
            let correction = decoded.correction.trimmingCharacters(
                in: .whitespacesAndNewlines
            )
            guard Self.validLearningIdentifier(decoded.correctionId),
                  correction.utf8.count >= 8,
                  correction.utf8.count <= 4 * 1_024,
                  decoded.tabId >= 0,
                ...
                throw BurnBarSafariBridgeFailure(
                    code: "invalid_learning_correction",
                    message: "Safari corrections must describe the currently attached page."
                )
            }
            let response: BurnBarSafariLearningProposalResponse = try sendTyped(
                method: .learningPropose,
                request: BurnBarSafariLearningProposalRequest(
                    observation: BurnBarSafariLearningObservation(
                        observationId: "safari-correction:\(decoded.correctionId)",
                        safariSessionId: decoded.safariSessionId,
        ...
        case .learningUpdate:
            let decoded: LearningUpdatePayload = try decodeExactWebPayload(
                payload,
                required: [
                    "safariSessionId", "proposalId", "expectedVersion", "title",
                    "content"
                ],
                allowed: [
                    "safariSessionId", "proposalId", "expectedVersion", "title",
            ...
            _ = try requireAttachedLearningSession(decoded.safariSessionId)
            guard Self.validLearningIdentifier(decoded.proposalId),
                  decoded.expectedVersion > 0,
                  decoded.title.trimmingCharacters(
                      in: .whitespacesAndNewlines
                  ).isEmpty == false,
                  decoded.title.utf8.count <= 512,
                  decoded.content.trimmingCharacters(
                      in: .whitespacesAndNewlines
                  ).isEmpty == false,
                  decoded.content.utf8.count <= 64 * 1_024 else {
                throw BurnBarSafariBridgeFailure(
                    code: "invalid_learning_update",
                    message: "Safari learning edits require a bounded current version, title, and content."
                )
            }
            let response: BurnBarSafariLearningProposalResponse = try sendTyped(
                method: .learningUpdate,
                request: BurnBarSafariLearningUpdateRequest(
        ...
        case .learningApprove, .learningReject:
            let decoded: LearningMutationPayload = try decodeExactWebPayload(
                payload,
                required: ["safariSessionId"],
                allowed: [
                    "safariSessionId", "itemId", "proposalId", "expectedVersion"
                ]
            )
            guard let proposalID = decoded.resolvedProposalId,
                  let version = decoded.expectedVersion,
                  version > 0 else {
                throw BurnBarSafariBridgeFailure(
                    code: "learning_version_required",
                    message: "Learning review requires an exact proposal version."
                )
            }
            _ = try requireAttachedLearningSession(decoded.safariSessionId)
            guard Self.validLearningIdentifier(proposalID) else {
                throw BurnBarSafariBridgeFailure(
                    code: "invalid_learning_item",
                    message: "Learning review requires a valid proposal identifier."
                )
            }
            let response: BurnBarSafariLearningProposalResponse = try sendTyped(
                method: action == .learningApprove ? .learningApprove : .learningReject,
                request: BurnBarSafariLearningMutationRequest(
        ...
        case .learningForget:
            ...
            guard let proposalID = decoded.resolvedProposalId else {
                throw BurnBarSafariBridgeFailure(
                    code: "invalid_learning_item",
                    message: "Learning deletion requires a proposal identifier."
                )
            }
            _ = try requireAttachedLearningSession(decoded.safariSessionId)
            guard Self.validLearningIdentifier(proposalID),
                  decoded.expectedVersion.map({ $0 > 0 }) ?? true else {
                throw BurnBarSafariBridgeFailure(
                    code: "invalid_learning_item",
                    message: "Learning deletion requires a valid proposal version."
                )
            }
            let response: BurnBarSafariLearningStateResponse = try sendTyped(
                method: .learningForget,
                request: BurnBarSafariLearningForgetRequest(
                    proposalId: proposalID,
                    expectedVersion: decoded.expectedVersion
                ),
                response: BurnBarSafariLearningStateResponse.self
        ...
        case .learningRollback:
            let decoded: LearningRollbackPayload = try decodeExactWebPayload(
                payload,
                required: [
                    "safariSessionId", "proposalId", "targetVersion"
                ],
                allowed: [
                    "safariSessionId", "proposalId", "targetVersion"
                ]
            ...
            _ = try requireAttachedLearningSession(decoded.safariSessionId)
            guard Self.validLearningIdentifier(decoded.proposalId),
                  decoded.targetVersion > 0 else {
                throw BurnBarSafariBridgeFailure(
                    code: "invalid_learning_rollback",
                    message: "Safari learning rollback requires a valid proposal and target version."
                )
            }
            let response: BurnBarSafariLearningProposalResponse = try sendTyped(
                method: .learningRollback,
                request: BurnBarSafariLearningRollbackRequest(
                    proposalId: decoded.proposalId,
    ...
    private func makeRunCreateRequest(_ payload: AgenticPayload) throws -> BurnBarRunCreateRequest {
        ...
        metadata[.learningOptedIn] = .bool(payload.learningOptedIn)
    ...
    private func validatePageTurn(_ payload: HandoffPayload) throws {
        try validateTurnCommon(
            safariSessionID: payload.safariSessionId,
            prompt: payload.prompt,
            agentID: payload.agentId,
        ...
        guard payload.prompt.utf8.count
                <= BurnBarSafariHandoffRequest.maximumPromptBytes,
              payload.pageContext.markdown.utf8.count
                <= BurnBarSafariHandoffRequest.maximumMarkdownBytes,
              payload.pageContext.snapshot.utf8.count
                <= BurnBarSafariHandoffRequest.maximumAccessibilitySnapshotBytes,
              payload.screenshot.byteLength
                <= BurnBarSafariHandoffRequest.maximumScreenshotBytes,
              payload.screenshot.source == "viewport" else {
            throw BurnBarSafariBridgeFailure(
                code: "handoff_payload_too_large",
                message: "Safari hand-off context exceeds its private package limits."
            )
        }
    }
    ...
    private func validatePageTurnPayload(
        ...
        if handoff {
            guard payload["url"] == nil, payload["learningOptedIn"] == nil else {
                throw BurnBarSafariBridgeFailure(
                    code: "invalid_bridge_schema",
                    message: "Safari handoff payload contains agentic-only fields."
                )
            }
        }
    }
