...
public final class BurnBarSafariBridgeChunkStore: @unchecked Sendable {
    ...
    public func commit(
        ...
        try withLock {
            ...
            let digest = PlatformCrypto.hexString(Data(hasher.finalize()))
            guard constantTimeSafariDigestsEqual(digest, metadata.sha256) else {
                try? fileManager.removeItem(at: directory)
    ...
    private var profileDirectory: URL {
        trustedRoot
            ...
            .appendingPathComponent(PlatformCrypto.sha256Hex(Data(profileIdentifier.utf8)), isDirectory: true)
    }

    private func transferDirectory(for transferID: String) -> URL {
        profileDirectory
            .appendingPathComponent(PlatformCrypto.sha256Hex(Data(transferID.utf8)), isDirectory: true)
            .standardizedFileURL
    }
