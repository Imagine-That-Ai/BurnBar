...
// swiftlint:disable all reason: generated UniFFI FFI bindings are not hand-maintained Swift style surface.
import Foundation
...
public protocol IrohBlobNodeProtocol : AnyObject {
    ...
    func fetchBlobInternal(ticketText: String, destination: String, expectedSizeBytes: UInt64?) throws  -> BlobTransferStats
     ...
     */
    func fetchBlobWithExpectedSize(ticketText: String, destination: String, expectedSizeBytes: UInt64) throws  -> BlobTransferStats
...
open func fetchBlobInternal(ticketText: String, destination: String, expectedSizeBytes: UInt64?)throws  -> BlobTransferStats {
    return try  FfiConverterTypeBlobTransferStats.lift(try rustCallWithError(FfiConverterTypeIrohFfiError.lift) {
    uniffi_openburnbar_iroh_fn_method_irohblobnode_fetch_blob_internal(self.uniffiClonePointer(),
        FfiConverterString.lower(ticketText),
        FfiConverterString.lower(destination),
        FfiConverterOptionUInt64.lower(expectedSizeBytes),$0
    )
})
}
     ...
     * stream exceeds the receiver's manifest-advertised size.
     */
open func fetchBlobWithExpectedSize(ticketText: String, destination: String, expectedSizeBytes: UInt64)throws  -> BlobTransferStats {
    return try  FfiConverterTypeBlobTransferStats.lift(try rustCallWithError(FfiConverterTypeIrohFfiError.lift) {
    uniffi_openburnbar_iroh_fn_method_irohblobnode_fetch_blob_with_expected_size(self.uniffiClonePointer(),
        FfiConverterString.lower(ticketText),
        FfiConverterString.lower(destination),
        FfiConverterUInt64.lower(expectedSizeBytes),$0
    )
})
}
...
private var initializationResult: InitializationResult = {
    ...
    if (uniffi_openburnbar_iroh_checksum_method_irohblobnode_fetch_blob_internal() != 21118) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_openburnbar_iroh_checksum_method_irohblobnode_fetch_blob_with_expected_size() != 19302) {
        return InitializationResult.apiChecksumMismatch
    }
