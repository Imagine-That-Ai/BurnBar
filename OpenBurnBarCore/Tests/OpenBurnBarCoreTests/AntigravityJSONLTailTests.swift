...
final class AntigravityJSONLTailTests: XCTestCase {
    func testScan_tailsPastStalePrefixInsteadOfReadingTheWholeFile() throws {
        ...
        let scan = try XCTUnwrap(AntigravityQuotaAdapter.scanHistoryLog(
            at: url,
    ...
    func testScan_oldEventsStillCountAsObserved() throws {
        let now = Date()
        let (home, url, fileManager) = try writeLog(events: [now.addingTimeInterval(-6 * 3600)])
        defer { try? fileManager.removeItem(at: home) }
    ...
    func testScan_reconstructsALineLongerThanTheTailChunk() throws {
        ...
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("obb-ag-\(UUID().uuidString)", isDirectory: true)
        let url = home.appendingPathComponent("history.jsonl")
        ...
        let line = "{\"timestamp\":\(ms),\"display\":\"\(padding)\"}\n"
