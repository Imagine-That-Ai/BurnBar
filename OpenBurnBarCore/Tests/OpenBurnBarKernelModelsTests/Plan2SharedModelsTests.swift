import XCTest
// B1: HermesRelayCrypto / PiConnectionMode / PiConnectionStatus are public in
// OpenBurnBarKernelCrypto — plain import, used through the public API.
...
final class Plan2SharedModelsTests: XCTestCase {
    ...
    func test_assistantRuntimeID_caseRawValuesMatchRelayDiscriminator() {
        ...
        XCTAssertEqual(AssistantRuntimeID.junie.rawValue, "junie")
        XCTAssertEqual(
            AssistantRuntimeID.allCases,
            [.hermes, .pi, .codex, .claude, .openClaw, .openClaude, .omp, .droid, .forge, .antigravity, .grok, .cursorAgent, .junie]
        )
    }
