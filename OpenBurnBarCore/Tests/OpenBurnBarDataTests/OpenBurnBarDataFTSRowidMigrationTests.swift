...
/// lines. They cover: (1) the migrator registers v55 as the last migration,
...
final class OpenBurnBarDataFTSRowidMigrationTests: XCTestCase {
    ...
    func test_migrator_latestIdentifier_isV55FTSRowid() {
        XCTAssertEqual(
            OpenBurnBarDatabase.latestMigrationIdentifier,
            "v55_search_chunks_fts_rowid"
        )
        XCTAssertTrue(
            OpenBurnBarDatabase.migrator.migrations.contains("v55_search_chunks_fts_rowid"),
            "registerSearchChunksFTSRowidMigration must be wired into the migrator"
        )
    }
