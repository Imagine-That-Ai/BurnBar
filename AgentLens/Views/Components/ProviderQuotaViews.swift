...
struct ProviderQuotaSettingsSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.lg) {
            Text("""
            Quota reporting stays separate from spend history. OpenBurnBar uses official APIs and reusable login sessions where providers allow it, \
            then falls back to verifiable local signals: Codex session logs, Claude statusline JSON, Factory / Droid token history, and explicit OpenBurnBar login sessions for web quota pages.
            """)
