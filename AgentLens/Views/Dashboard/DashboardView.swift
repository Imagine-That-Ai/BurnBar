...
struct DashboardView: View {
    ...
    @State var showIndexingConsent = false
    @State var showAnalyticsConsent = false
    @State var showCLIConsentSheet = false
    @State var showSessionLogCloudConsent = false
    @State var sessionLogJumpTarget: ConversationJumpTarget?
    @State var sessionLogJumpRequestID = UUID()
    @State var sessionLogJumpTask: Task<Void, Never>?
    ...
    @State var inboxShelf = InboxShelfStore()
    ...
    @State var fleetWatcher: ProviderSessionActivityWatcher?
    ...
    @State var homeInboxModel: InboxModel?
    ...
    static func routeWantsProviderSidebar(_ route: DashboardMainRoute) -> Bool {
        ...
        case .home, .database, .projects, .missions, .sessionLogs, .memoryReview, .inbox, .chat,
             .quota, .controlDeck, .fleet, .recap, .receipts:
            ...
            //
            // Receipts already renders its own two-column split (the stack list
            // plus the slip inspector); a provider rail would make it three.
            return false
        }
    }
