...
struct ReceiptQualityAuditor: Sendable {
    ...
    func auditDeterministic(receipt: ReceiptRecord) -> ReceiptQualityReview {
    ...
    static func gradeForScore(_ score: Double) -> String {
        switch score {
    ...
    private func auditWithLLM(
        ...
        guard let parsed = try? JSONDecoder().decode(LLMOutput.self, from: data) else { return nil } // try?-ok(malformed LLM evaluation defaults to nil)
