...
struct ReceiptAccomplishmentSynthesizer: Sendable {
    ...
    public func gatherGitStats(at path: String?) -> ReceiptGitStats? {
    ...
    public func deriveAchievements(context: SynthesisContext, gitStats: ReceiptGitStats?) -> [ReceiptAchievement] {
    ...
    public func synthesizeDeterministic(context: SynthesisContext, gitStats: ReceiptGitStats?) -> [String] {
        var items: [String] = []
        ...
        if let lastMsg = context.lastAssistantMessage, !lastMsg.isEmpty {
            ...
            if let firstActionable = lines.first(where: { $0.count > 15 && $0.count < 150 }) {
                let cleaned = firstActionable.replacingOccurrences(of: "- ", with: "").replacingOccurrences(of: "*", with: "")
                items.append(cleaned)
            }
        } else if !context.promptSummary.isEmpty {
            items.append("Addressed: \(context.promptSummary)")
        }

        if items.isEmpty {
            items.append("Completed CLI turn in \(context.projectName)")
        }

        return Array(items.prefix(4))
    }

    // MARK: - LLM Accomplishment Synthesis

    private func synthesizeWithLLM(
        context: SynthesisContext,
        gitStats: ReceiptGitStats?,
        ...
        var prompt = """
        Analyze this AI software engineering CLI session and return a concise, honest JSON list of what was ACTUALLY accomplished.
        Ground every bullet in verified deliverables (code changes, tests run, files created). Avoid generic filler.

        Project: \(context.projectName)
        User Prompt / Goal: \(context.promptSummary)
        Files touched: \(context.filesTouched.prefix(10).joined(separator: ", "))
        Tools called: \(context.toolsUsed.prefix(10).joined(separator: ", "))
        """

        if let gitStats {
            prompt += "\nGit evidence: \(gitStats.summary)"
        }

        if let msg = context.lastAssistantMessage, !msg.isEmpty {
            prompt += "\nFinal assistant statement: \(msg.prefix(800))"
        }

        prompt += "\n\nReturn strict JSON with key 'accomplishments' containing an array of 2 to 4 concise strings (max 90 chars each)."
        ...
        if let parsed = try? JSONDecoder().decode(Output.self, from: data), !parsed.accomplishments.isEmpty {
