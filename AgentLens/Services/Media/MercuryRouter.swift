...
private enum MercuryLaneSealingError: LocalizedError, Equatable {
    case refused(reason: MediaFrameAeadNegotiation.SealingDecision.RefusalReason)

    var errorDescription: String? {
        switch self {
        case .refused(let reason):
            return "Mercury lane refused because media frame sealing could not be established: \(reason.rawValue)"
        }
    }
}
