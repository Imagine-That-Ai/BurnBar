    ...
    func handle(document: QueryDocumentSnapshot) async {
        ...
        do {
            ...
            if let wandRoutingSelection {
                ...
                muBackend = resolveBackend(requestedRuntime: muRequestedRuntime, missionKind: muData["missionKind"] as? String)
                let wandRoute = (provider: wandRoutingSelection.provider ?? "unknown", source: wandRoutingSelection.source ?? "unknown")
                logger.info("wand routing selected mission id=\(document.documentID, privacy: .private(mask: .hash)) model=\(muRequestedModelID ?? "auto", privacy: .private) provider=\(wandRoute.provider, privacy: .private) source=\(wandRoute.source, privacy: .private)")
            }
        } catch {
            logger.warning("mission id=\(document.documentID, privacy: .private(mask: .hash)) refused before claim: \(error.localizedDescription, privacy: .private)")
            return
        ...
        @MainActor @Sendable func evaluateAfterExclusiveClaim() async {
            ...
            case let .proceed(grantCeiling):
                let requestedCommands = (data["commandsAllowed"] as? Bool) ?? false
                let requestedEdits = (data["fileEditsAllowed"] as? Bool) ?? false
                if (requestedCommands && grantCeiling.commandsAllowed == false)
                    || (requestedEdits && grantCeiling.fileEditsAllowed == false) {
                    await failAfterTrustedClaim(
                        document: document,
                        backend: backend,
                        message: "Requested grant is wider than the authorized ceiling."
                    )
                    return
                }
                let requestedGrant = MissionApprovalCeiling.grantObject(
                    commandsAllowed: requestedCommands,
                    fileEditsAllowed: requestedEdits
                )
                let approvedGrant = MissionApprovalCeiling.grantObject(
                    commandsAllowed: grantCeiling.commandsAllowed,
                    fileEditsAllowed: grantCeiling.fileEditsAllowed
                )
                if !MissionApprovalCeiling.requestedIsSubset(ceiling: approvedGrant, requested: requestedGrant) {
                    await failAfterTrustedClaim(
                        document: document,
                        backend: backend,
                        message: "Requested grant digest is wider than the approved ceiling."
                    )
                    return
                }
                if let parked = parkedCeilingByRequest[document.documentID],
                   !MissionApprovalCeiling.requestedIsSubset(ceiling: parked.grant, requested: requestedGrant) {
                    await failAfterTrustedClaim(
                        document: document,
                        backend: backend,
                        message: "Requested grant is wider than the redeemed approval ceiling."
                    )
                    return
                }
