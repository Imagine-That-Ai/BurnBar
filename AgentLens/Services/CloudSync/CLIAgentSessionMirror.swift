...
final class CLIAgentSessionMirror: Sendable {
    ...
    func mirror(
        ...
        let documentID = threadID
        let docRef = firestore
            .collection("users").document(uid)
            .collection("cli_sessions").document(documentID)
        do {
            ...
            var legacyPayload = try CLIAgentSessionCodec.encodeSealed(
                record,
            ...
            legacyPayload["source"] = "macos-agentlens"
    ...
    func mirrorArchivedLog(_ conversation: OpenBurnBarCore.ConversationRecord, cloudLogDocumentID: String? = nil) async {
        ...
        let firestore = firestoreProvider()
        let documentID = Self.firestoreDocumentID(for: record)
        let docRef = firestore
            .collection("users").document(uid)
            .collection("cli_sessions").document(documentID)
        do {
