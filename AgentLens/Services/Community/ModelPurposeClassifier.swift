...
enum ModelPurposeClassifier {
    ...
    static func classifyPurpose(
        ...
        if let originalSurface = signals.appSurface {
            let surface = originalSurface.lowercased()
            if let bias = surfaceBias[surface] {
                for (cat, weight) in bias {
                    scores[cat, default: 0] += weight
                }
                contributing.append("surface:\(originalSurface)")
            }
        }
