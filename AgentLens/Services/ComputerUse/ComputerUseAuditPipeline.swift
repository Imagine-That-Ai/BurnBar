...
@MainActor
@dynamicMemberLookup
final class ComputerUseAuditPipeline {
    ...
    func remoteClipboardTimelineSummary(
        ...
        case .error:
            return response.detail ?? action.executableSummary(forApproval: session.macDispatcher.currentScopeContext())
        }
    }

    func phoneControlAttestationRequirement() async -> PhoneControlAttestationRequirement {
        let strict = session.configuration.phoneControlAttestationRequired
    ...
    func scopeContext(for action: ComputerUseAction) -> ComputerUseScopeContext {
        ...
        case .browser(let browser):
            ...
            return session.macDispatcher.currentScopeContext()
    ...
    func accessibilityDeny(for action: ComputerUseAction) -> ComputerUseAccessibilityDenyReason? {
        ...
        return session.macDispatcher.accessibilityDenyReason(at: input)
    }
    ...
    func reserveAuditEntry(
        ...
        let entry = try logger.makeEntry(
            ...
            denyReason: ComputerUseSessionCoordinator.auditReservationSentinel,
    ...
    func captureEvidence(
        ...
        guard let screenshotService = session.screenshotService else { return nil }
        do {
            ...
            session.screenshotEvidenceDataByHash[capture.sha256Hex] = capture.pngData
            return capture
        } catch {
            appendTimeline(
    ...
    func captureData(forHash hash: String?) -> Data? {
        ...
        return session.screenshotEvidenceDataByHash[hash]
    }
    ...
    func appendTimeline(
        ...
        if let cloudMeteringRecorder = session.cloudMeteringRecorder {
    ...
    func appendTimeline(
        ...
        let entry = HermesRealtimeRelayActionLogEntry(
            entryIndex: entryIndex ?? session.actionTimeline.count,
            timestamp: Date(),
            actionKind: kind,
            summary: summary,
            status: status,
        ...
        session.actionTimeline.append(entry)
        if session.actionTimeline.count > 50 {
            session.actionTimeline.removeFirst(session.actionTimeline.count - 50)
        }
    }
    ...
    func emitControlFrame(
        ...
        guard let latestReplySender = session.latestReplySender,
              let latestControlUID = session.latestControlUID,
              let latestControlConnectionID = session.latestControlConnectionID else { return }
    ...
    func sendControlFrame(
        ...
        guard let latestReplySender = session.latestReplySender,
              let latestControlUID = session.latestControlUID,
              let latestControlConnectionID = session.latestControlConnectionID else {
            throw ComputerUseSessionCoordinator.CoordinatorError.missingControlReplySender
        }
        let frame = HermesRealtimeRelayFrame(
            type: type,
            uid: latestControlUID,
    ...
    func cancelPendingApprovals(
        ...
        for (approvalId, continuation) in session.approvalContinuations {
        ...
        session.approvalContinuations.removeAll()
