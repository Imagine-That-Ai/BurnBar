...
/// initializers. The AgentLens chat client and the daemon HTTP gateway both reach
...
final class BudgetEnforcement {
    ...
    /// by `ContextBuilder` so the assistant can answer "where is my spend?" without making
    /// a tool call when the context already has the answer.
    func budgetContextSection() async -> String? {
        ...
        for rule in rules.prefix(8) {
            let used = (try? await gate.ledgerSpend(forRule: rule, reference: now)) ?? 0 // try?-ok(context-prompt display only)
...
/// Stable credential identity for AgentLens-plane requests. Hashes the bearer token so the
...
enum AgentLensCredentialIdentity {
    ...
    private static func hashedSlotID(_ secret: String) -> String {
        // Reuse UsageStore.usagePartitionToken-style prefix for cross-system identity.
        // Cheap stable hash — collisions are acceptable for slot identity since the user
        // labels their credentials.
