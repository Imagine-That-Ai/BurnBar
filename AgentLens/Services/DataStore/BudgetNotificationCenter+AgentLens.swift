...
/// schedules with a `UNTimeIntervalNotificationTrigger` for immediate delivery.
///
...
///
/// The macOS notification backend. Delivery stays here (UNUserNotificationCenter is
/// Apple-only API); the copy/identifier builders live in OpenBurnBarKernel
/// (`BudgetNotificationContent`), shared with the iOS backend. The Analytics tracking
/// calls are macOS-only and stay here.
@MainActor
