...
public final class ReceiptStore: Sendable {
    ...
    public func insert(receipt: ReceiptRecord) async throws {
        ...
        let filesJSON = (try? String(data: encoder.encode(receipt.filesTouched), encoding: .utf8)) ?? "[]"
        let toolsJSON = (try? String(data: encoder.encode(receipt.toolsUsed), encoding: .utf8)) ?? "[]"
        let accomplishmentsJSON = (try? String(data: encoder.encode(receipt.actualAccomplishments), encoding: .utf8)) ?? "[]"
        let qualityJSON = (try? String(data: encoder.encode(receipt.qualityReview), encoding: .utf8)) ?? "{}"
        let achievementsJSON = (try? String(data: encoder.encode(receipt.achievements), encoding: .utf8)) ?? "[]"
        let gitStatsJSON = (try? String(data: encoder.encode(receipt.gitStats), encoding: .utf8)) ?? "{}"
    ...
    public static func decodeReceipt(from row: Row) -> ReceiptRecord? {
        ...
        var qualityReview: ReceiptQualityReview? = nil
        if let qrJSON: String = row["qualityReviewJSON"],
           let data = qrJSON.data(using: .utf8),
           let review = try? decoder.decode(ReceiptQualityReview.self, from: data) {
        ...
        if let achJSON: String = row["achievementsJSON"],
           ...
           let list = try? decoder.decode([ReceiptAchievement].self, from: data) {
            achievements = list
        }

        var gitStats: ReceiptGitStats? = nil
        if let gsJSON: String = row["gitStatsJSON"],
           let data = gsJSON.data(using: .utf8),
           let stats = try? decoder.decode(ReceiptGitStats.self, from: data) {
            gitStats = stats
        }
    ...
    public func updateQualityReview(receiptId: String, review: ReceiptQualityReview) async throws {
        let encoder = JSONEncoder()
        encoder.dateEncodingStrategy = .iso8601
        let reviewJSON = (try? String(data: encoder.encode(review), encoding: .utf8)) ?? "{}"
        try await dbQueue.write { db in
            try db.execute(
