...
struct SourceUpdateChannel {
    ...
    func fetchStatus() async throws -> SourceUpdateStatus {
        ...
        return SourceUpdateStatus(
            ...
            compareURL: try? await compareURL(upstream: upstream, currentSHA: currentSHA, in: sourceRoot)
        )
    }

    func openManualUpdateInTerminal() {
        guard let sourceRoot = try? sourceRootURL() else {
    ...
    private func trackedBranch(in sourceRoot: URL) async throws -> String {
        ...
        if let defaultRef = try? await runGit(["symbolic-ref", "refs/remotes/origin/HEAD"], in: sourceRoot),
