...
enum DirectDownloadUpdateInstaller {
    ...
    private nonisolated static func locateApp(in mountPoint: String) throws -> String {
        ...
        if let contents = try? fileManager.contentsOfDirectory(atPath: mountPoint),
    ...
    /// Sendable box so the concurrent pipe-drain closures can write their result
    /// without capturing a mutable `var` (each box is written by exactly one
    /// closure and read only after `group.wait()`).
    ...
    private nonisolated static func runProcess(_ launchPath: String, _ arguments: [String]) -> ProcessResult {
        ...
        // verbose codesign/hdiutil failure on stderr). Box the results in a
        // Sendable reference so the concurrent closures capture cleanly.
        let outBox = DataBox()
        let errBox = DataBox()
