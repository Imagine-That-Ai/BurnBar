...
| xAI (Grok) — estimated cost, quota `yes`      | Parser `OpenBurnBarCore/.../LogParser/GrokParser.swift` (`~/.grok/sessions/`); quota `XAIQuotaAdapter.swift` (Management API prepaid balance/usage + SuperGrok estimated pacing; no SuperGrok vendor login). CLI login is `~/.grok/auth.json` presence via `GrokCLIAuthFile`. |
...
| One-click install covers **Claude Code, Cursor, Codex CLI** and no other client; everything else is published as "not wired by the installer today" | `AgentLens/Services/CLIBridge/MCPClientWiring.swift` — `MCPClientWiringTarget` is exactly `{claudeCode, cursor, codex}`; `AgentLens/Views/Settings/MCPInstallCard.swift` renders one row per case. Claude Desktop and Hermes are README-only, by hand |
