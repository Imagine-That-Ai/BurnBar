...
all 95 tools it registers — the 69 `burnbar_*` memory, code-intelligence,
...
| Claim                                                                                                                                                                                                                                         | Source                                                                                                                                                                                                                                                                             |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **gate** · "45 MCP tools" (hero, meta description, verification step 1)                                                                                                                                                                       | `tools/openburnbar-mcp/server.py` — `MEMORY_TOOLSET`, counted (45 entries), served when `BURNBAR_MCP_TOOLSET=memory`. The meta description derives it rather than spelling it, because the gate strips tags and would never have seen a hardcoded numeral in an attribute          |
| **gate** · "The same server carries **95** tools in total" (hero, first paragraph)                                                                                                                                                            | `server.py` — every `@mcp.tool()` definition: 69 `burnbar_*` + 26 orchestration. Asserted both ways — the total must be printed as the total, and the `burnbar_*` subset must never be printed as "in total". An earlier revision opened on 63 while § 11 of the same page said 89 |
...
| **editorial** · § 14 "How it works, and what you have to do" — the four-lane matrix (on by default / install / collect / prune) | `website/src/data/memory.ts` — `DUTIES`, `INSTALL_ROUTES`. The long form is `docs/HOW_TO_MEMORY_MCP.md`, which carries the same four answers and the same proofs |
| The engine and its store are created on the **first write**, not at install; `memory_write` is granted by `BURNBAR_MCP_TOOLSET=memory`, and an explicit `…ENABLE_MEMORY_WRITE=false` wins | `tools/openburnbar-mcp/server.py` — `_memory_write_enabled()`, docstring and branch order |
| The daemon's session watcher runs with the daemon, stops without a signed-in vault key, and writes one sentinel per settled session — no transcript read, no extraction, nothing into the local memory store | `OpenBurnBarDaemon/Sources/OpenBurnBarDaemon/PensieveKnowledgeWatcher.swift` — the `vaultKeyProvider` guard at the top of `scan()`, and `signalSessionEnds(in:)`; wired unconditionally in `OpenBurnBarDaemonMain.swift` |
| **gate** · One-click install covers exactly **Claude Code, Cursor, Codex CLI, Factory Droid, Antigravity CLI, Gemini CLI and Muse**; everything else is published as "not wired by the installer today" | `AgentLens/Services/CLIBridge/MCPClientWiring.swift` — `MCPClientWiringTarget`, whose `displayName` and `configURL(for:)` are read case-for-case by invariant 15b of `scripts/test-memory-copy.mjs`; `AgentLens/Views/Settings/MCPInstallCard.swift` renders one row per case. Claude Desktop and Hermes are README-only, by hand. Before that gate existed the page sat one release behind the app: #2555 wired four more clients and `INSTALL_ROUTES` kept claiming three |
| The installer writes `BURNBAR_MCP_TOOLSET=memory` | `MCPClientWiring.swift` — `MCPServerLaunch.toolset` defaults to `"memory"`, written into the JSON `env` block and the Codex `[mcp_servers.*.env]` table |
| **Nothing prunes itself.** `expiresAt` is a read-time filter, not a deletion; `burnbar_forget` is the hard delete; `burnbar_memory_doctor(apply=True)` prunes only orphan bodies and parked supersedes, never a memory | `memory_engine/_util.py` — `_is_expired`, called as a filter in `_read.py`; `memory_engine/_lifecycle.py` — `forget` / `_purge`; `memory_engine/_admin.py` — `doctor`'s `apply` bound, and the same bound in `server.py`'s tool docstring. No scheduler exists anywhere in `memory_engine/` |
...
| `burnbar_team_link_project` writes one team's `teamProjectId` into this checkout's `.openburnbar/project.json`, screens both ids to the shapes the engine reads them back by, refuses EVERY write without `confirm=true` (a first-time link and a re-point alike), and never overwrites a link file it cannot parse | `server.py burnbar_team_link_project` docstring and its denial site; `memory_engine/_namespaces.py link_team_project`; `tests/test_team_two_clone.py` — `test_a_first_time_link_is_refused_without_confirm`, `test_the_link_tool_refuses_to_clobber_a_different_id_without_confirm`, `test_the_link_tool_never_overwrites_a_link_file_it_cannot_parse` |
| A team link counts only where the working tree and git `HEAD` agree on it, per entry: an uncommitted, locally-modified or outside-any-repository entry makes no memory eligible to upload and serves no team row into a session. Both readers enforce it | `AgentLens/Services/CloudSync/TeamMemoryUploadEligibility.swift TeamProjectLink.read`; `memory_engine/_namespaces.py _session_team_links`; `tests/test_team_two_clone.py` — `test_an_unconfirmed_uncommitted_link_makes_nothing_uploadable`, `test_only_a_committed_link_makes_a_project_eligible`, `test_an_uncommitted_link_serves_no_team_row_into_this_session`; `AgentLensTests/Active/TeamMemorySyncTests.swift` — `test_only_a_committed_team_project_link_is_eligible` |
| The doctor's `TEAM_PROJECT_LINK_GAPS` finding reports, per team, the facts this session withholds for `TEAM_PROJECT_NOT_LINKED`, teams this Mac syncs with no entry `HEAD` carries, entries written but never committed (reported apart from unlinked ones, never as linked), and links naming a project none of the team's held facts landed in — counts and ids only, `apply` never writes the link file, and the finding states that linking makes this checkout's approved memories uploadable to the team and that a human must confirm and commit | `memory_engine/_admin.py doctor`; `memory_engine/_namespaces.py team_project_link_report`; `tests/test_team_two_clone.py` — `test_the_doctor_explains_an_empty_team_space_instead_of_leaving_it_silent`, `test_the_doctor_reports_an_uncommitted_link_as_uncommitted_not_as_linked`, `test_the_doctor_never_writes_the_link_file_even_with_apply` |
| Two clones of one repository with different `remote.origin.url` forms derive different engine project ids and the same team document id, because the id derives from the checked-in `teamProjectId` — proved end to end, with the mutation that breaks it | `tests/test_team_two_clone.py`; `docs/security/BurnBar-threat-model.md` audit note |
...
| 45 of the atlas marked as the memory toolset; Castle/Ministry/Bench are never marked as such                                                                                                                                                                                                                                             | `server.py` `MEMORY_TOOLSET` ↔ each entry's `memory` flag, per tool — `MEMORY_TOOLSET` never names a `ministry_*` / `castle_*` / `bench_*` tool, so this also proves none is mismarked                                                                                                                                                                                                                                        |
...
| Claim                                        | Source                                                                                                                                                                                                                                                                                                     |
| -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Current public macOS DMG                     | `website/src/data/site.ts`; `https://github.com/Imagine-That-Ai/BurnBar/releases/download/v1.0.40+repair.41/OpenBurnBar-1.0.40+repair.41-macOS.dmg` — notarized Developer ID GitHub Release asset, build 86 (2026-09-15)                                                                                   |
...
`https://github.com/Imagine-That-Ai/BurnBar/releases/download/v1.0.40+repair.41/OpenBurnBar-1.0.40+repair.41-macOS.dmg`; changing the public DMG URL must
