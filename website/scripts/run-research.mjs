...
async function loadPreviousRundownForDate(dateISO) {
  try {
    const text = await readFile(path.join(OUT_DIR, "latest.json"), "utf8");
    const parsed = JSON.parse(text);
    return parsed?.date && parsed.date < dateISO ? parsed : null;
  } catch {
    return null;
  }
}
...
async function main() {
  ...
  const previousRundown = await loadPreviousRundownForDate(date);
