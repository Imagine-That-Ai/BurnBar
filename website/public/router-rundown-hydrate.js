/**
 * Daily Model Board Rundown — client-side hydrator.
 *
...
(function () {
  ...
  function renderRecommendation(rec) {
    ...
    const lead = el("div", { class: "rankrow__lead" }, [
      ...
      el("div", { class: "rankrow__id" }, [
        ...
        el("span", {
          class: "rankrow__provider mono",
          text:
            rec.providerDisplay +
            (rec.providerFamily ? " · " + rec.providerFamily : "") +
            (rec.tier ? " · " + rec.tier : "")
        })
      ])
    ]);
    const score = el("div", { class: "rankrow__score" }, [
      ...
      el("span", {
        class: "rankrow__cov mono",
        text: "evidence " + pct(rec.score) + " · coverage " + pct(rec.evidenceCoverage) + "%"
      })
    ]);
    const s = rec.signals || {};
    ...
    const chips = el(
      "ul",
      { class: "rankrow__chips" },
      [
        chip("bench", pct(s.benchmarkScore)),
        chip(
          "fresh",
          pct(s.benchmarkFreshness),
          s.benchmarkFreshness != null ? freshnessTone(rec.freshness) : "off"
        ),
        ...
        chip(
          "avail",
          s.availability == null ? "unknown" : s.availability,
          s.availability === "common" ? "on" : s.availability === "limited" ? "warn" : "off"
        ),
    ...
    const rationale = el("div", { class: "rankrow__rationale" }, [
      ...
      el(
        "ul",
        { class: "rankrow__rlist" },
        [rec.selectionReason]
          .concat(rec.explanation || [])
          .filter(Boolean)
      ...
      rec.citations && rec.citations.length > 0
        ? el(
            ...
            rec.citations.map((cite) => {
              ...
              const a = el("a", {
                class: "cite__name",
                href: cite.sourceURL || "/router#sources",
                target: "_blank",
                rel: "noopener noreferrer",
                text: cite.attribution
              });
              cb.appendChild(a);
              const meta = el("div", { class: "cite__meta mono" });
              if (cite.rank != null) meta.appendChild(el("span", { text: "rank " + cite.rank }));
              if (cite.score != null)
                meta.appendChild(el("span", { text: "score " + pct(cite.score) }));
        ...
        : el("p", {
            class: "rankrow__none mono",
            text: "No benchmark snapshot from any active source today."
          })
    ]);
    ...
    if (rec.limitations && rec.limitations.length > 0) {
      body.appendChild(
        el("div", { class: "rankrow__lims" }, [
          el("h4", { class: "rankrow__rh mono", text: "Limitations" }),
          el(
            "ul",
            { class: "rankrow__llist" },
            rec.limitations.map((line) => el("li", { text: line }))
          )
        ])
      );
    }
  ...
  function renderTask(task) {
    ...
    card.appendChild(
      el("ol", { class: "ranks" }, (task.recommendations || []).map(renderRecommendation))
    );
    if (task.rejectedAlternatives && task.rejectedAlternatives.length > 0) {
      ...
      sum.appendChild(
        el("span", {
          class: "rejects__sum",
          text: "Why other candidates didn't make the board pick"
        })
      );
      sum.appendChild(
        el("span", {
          class: "rejects__count mono",
          text: task.rejectedAlternatives.length + " dropped"
        })
      );
      det.appendChild(sum);
      det.appendChild(
        el(
          ...
          task.rejectedAlternatives.map((r) => {
            ...
            d.appendChild(
              el("span", { class: "reject__provider mono", text: " · " + r.providerDisplay })
            );
  ...
  function applyRundown(root, rundown) {
    ...
    if (metaEl) {
      ...
      metaEl.textContent =
        "Generated " +
        (isNaN(d.getTime()) ? rundown.generatedAt : d.toUTCString()) +
        " · schema v" +
        rundown.schemaVersion +
        " · model board · runtime constraints win";
    }
    ...
    if (sources) {
      ...
      for (const source of rundown.sourceStatuses) {
        ...
        li.appendChild(body);
        li.appendChild(
          el("span", { class: "srctag__dot srctag__dot--" + tone, "aria-hidden": "true" })
        );
  ...
  async function hydrate(root) {
    ...
    if (isLocalPreviewHost()) {
      setState(
        root,
        "fallback",
        "local preview · showing build-time fallback (" + (fallbackDate || "—") + ")"
      );
      return;
    }
    try {
      ...
      if (!res.ok) {
        setState(
          root,
          "fallback",
          "live feed unavailable · showing build-time fallback (" + (fallbackDate || "—") + ")"
        );
        return;
      }
      ...
      if (!live || !live.date || !Array.isArray(live.taskRankings)) {
        setState(
          root,
          "fallback",
          "live feed malformed · showing build-time fallback (" + (fallbackDate || "—") + ")"
        );
        return;
      }
      ...
      setState(
        root,
        "fallback",
        "live feed offline · showing build-time fallback (" + (fallbackDate || "—") + ")"
      );
    }
  }
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => {
      document
        .querySelectorAll(".rundown[data-rundown-live-path]")
