...
const collectorOrigin = "https://collect.burnbar.ai";
const collectorEnv = {
...
describe("marketing CSP collector origin", () => {
  ...
  it("accepts only reviewed collector hosts", () => {
    expect(isReviewedCollectorOrigin("https://collect.burnbar.ai")).toBe(true);
    expect(isReviewedCollectorOrigin("https://collect-staging.burnbar.ai")).toBe(true);
  ...
  it("fails closed on a nonempty invalid or non-https collector URL", () => {
    ...
    expect(() =>
      extraCollectorConnectSrc({ PUBLIC_ANALYTICS_COLLECTOR_URL: "http://evil.example/x" })
    ...
    expect(parseCollectorUrl("https://collect.burnbar.ai/v1").origin).toBe(
      "https://collect.burnbar.ai"
    );
  });
