...
- For prebuilt releases (Option 1): nothing else
- For building from source:
  - Xcode 16+ (the full app — the build runs `xcodebuild`; command line tools alone are not enough)
  - Rust via [rustup](https://rustup.rs) (`rustup` + `cargo` build the vendored libsignal FFI)
  - `protoc` (`brew install protobuf` — required by the libsignal build)

`make bootstrap` checks these and tells you exactly what is missing.

## Installation

### Option 1: Download from GitHub Releases (recommended)

1. Go to [Releases](https://github.com/Imagine-That-Ai/BurnBar/releases)
...
```bash
git clone --recursive https://github.com/Imagine-That-Ai/BurnBar.git
cd BurnBar
make bootstrap   # init submodules + build the libsignal FFI XCFramework (slow first run)
make install
```

`--recursive` pulls the `Vendor/libsignal` submodule the build depends on. If you already cloned without it, `make bootstrap` initializes it for you (or run `git submodule update --init --recursive`).
...
git clone --recursive https://github.com/Imagine-That-Ai/BurnBar.git
cd BurnBar
make bootstrap   # the Xcode build needs the libsignal FFI XCFramework too
open OpenBurnBar.xcodeproj
```

Select the **OpenBurnBar** scheme, press **⌘R** to build and run. Debug builds in Xcode use keychain entitlements, so pick your own team under Signing & Capabilities (or see the `DEVELOPMENT_TEAM` note at the top of `project.yml`).

> The Xcode project is generated by [XcodeGen](https://github.com/yonaskolb/XcodeGen): edit `project.yml` and re-run `xcodegen` rather than editing `OpenBurnBar.xcodeproj` by hand.
...
# Run tests (the wrappers prepare the libsignal FFI link flags raw `swift test` misses)
./scripts/test-openburnbar-swift.sh   # OpenBurnBarCore + OpenBurnBarDaemon package tests
./scripts/test-openburnbar-app.sh     # macOS app XCTest target
```
...
```bash
# Everything (Swift packages + macOS app + mobile + Android when configured)
make test

# Or individually — use the scripts/ wrappers, not raw `swift test`
# (they prepare the libsignal FFI linking that raw commands miss):
./scripts/test-openburnbar-swift.sh   # OpenBurnBarCore + OpenBurnBarDaemon package tests
./scripts/test-openburnbar-app.sh     # macOS app XCTest target
...
- **Bug reports:** https://github.com/Imagine-That-Ai/BurnBar/issues
