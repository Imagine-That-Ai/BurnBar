...
| **Reliability / Ops** | 4.5 | Ambitious but messy | Excellent ops *design*; the *operation* is broken — backend deploy plane dead 24 days, proof plane never green. |
...
3. **The proof plane has never been green.** Nightly E2E 0-for-43 all-time; full post-merge harness 0-for-250 (last green 2026-06-19). There is no end-to-end evidence any release candidate works; iOS Mobile XCTests effectively never execute anywhere (PR gate is compile-only, post-merge job always cancelled).
...
6. **Deep-language SAST cadence** — CodeQL PR gate covers only JS+Python; Swift/Kotlin/C#/Rust deep scans depend on the main-branch run, which self-cancels 8/10 times.
...
3. **Get one green end-to-end run** (nightly-e2e or the full harness) so a release candidate is provably healthy once. Resolves the "never been green" diligence finding.
...
- Nightly harness 0-green — **TRUE** (0-for-43 / 0-for-250).
