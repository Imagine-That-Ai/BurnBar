...
import { HERMES_SCHEMA_VERSION, HERMES_PAIRING_TTL_MS, HERMES_MAX_FAILED_PAIRING_ATTEMPTS, writeHermesAuditEvent } from "@openburnbar/functions-shared/shared/accounts.js";
import { assertActiveHostedQuotaEntitlement } from "@openburnbar/functions-shared/shared/entitlements.js";
import { checkHermesRateLimit } from "@openburnbar/functions-shared/shared/providerConnect.js";
...
import type { HermesConnectionDoc, HermesConnectionMode, HermesPairingDoc } from "@openburnbar/functions-shared/types.js";
    ...
    async (
      ...
      try {
        connection = await db.runTransaction(async (tx) => {
          ...
          const displayName =
            boundedTrimmedString(request.data.displayName, "displayName", 80) ?? pairing.displayName ?? "Hermes Host";
