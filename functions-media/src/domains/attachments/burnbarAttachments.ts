...
import { boundedFirestoreDocumentId } from "@openburnbar/functions-shared/callables/computerUseSecurityCodecs.js";
...
const PRO_WITHOUT_MEDIA_SYNC_CAP_BYTES = 200 * 1024 * 1024;
...
function isComposeFile(value: unknown): value is { getMetadata: () => Promise<unknown> } {
  return (
    typeof value === "object" &&
    value !== null &&
...
function metadataRecord(value: unknown): { size?: unknown; generation?: unknown } {
...
export async function combineWithGenerationPin(
  ...
  const combined = await bucket.combine(sources, destination, { ifGenerationMatch });
  const file = Array.isArray(combined) ? combined[0] : combined;
  if (!isComposeFile(file)) {
    throw new HttpsError("internal", "compose did not return an object handle.");
  }
...
const gcsStoragePort: BurnbarStoragePort = {
  async getMetadata(path) {
