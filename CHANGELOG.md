...
### Added
- **Bespoke CLI Session Receipts & Thermal Register Drawer.**
  - **Menu Bar Mini-Flyout Popover**: Whenever an external CLI session closes (Claude Code, Codex, Grok, Cursor, etc.), a transient floating HUD appears beneath the OpenBurnBar menu bar icon with real burn spend, elapsed time, tokens, and verified deliverable summary.
  - **"Actually Accomplished" Punchlist Extraction**: Grounds deliverables directly in git status/diff and tool outputs, synthesizing a concrete, honest punchlist (with zero-token offline deterministic fallback).
  - **Multi-Factor Quality Review & Rubric**: Grades sessions on Goal Completion (40%), Code & Test Rigor (35%), and Efficiency (25%) with letter grades (A+ to D), highlighted wins, and critique notes. Supports on-demand grading directly from the slip and automatic background grading.
  - **Thermal Register Drawer**: Modernized `ReceiptDrawerView` with ticket-stub master list, brand color coding, dynamic facet filters, live aggregate burn stats, and full inspection deck.
  - **Receipts Settings & Retro Audio**: Dedicated preferences panel with popup toggles, notification banners, model selector, and authentic thermal printer / register chink sound effects.
