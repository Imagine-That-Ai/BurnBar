...
final class BurnBarIndexedSearchService: @unchecked Sendable {
    ...
    func search(query: BurnBarSearchQueryRequest) throws -> BurnBarSearchQueryResult {
        ...
        let canDoSemantic = semanticConfig.enabled
    ...
    private func dateRange(from query: BurnBarSearchQueryRequest) -> ClosedRange<Date>? {
        if let s = query.dateRangeStartEpoch, let e = query.dateRangeEndEpoch {
        ...
        return try dbQueue.sync {
            ...
            for raw in cleaned {
                ...
                var sql = """
                    ...
                    WHERE 1 = 1
                    """
                var args: [SQLiteBindValue] = [.text(pattern), .text(pattern), .text(pattern)]
                ...
                if let range = dateRange {
                    sql += " AND unixepoch(c.startTime) >= ? AND unixepoch(c.startTime) <= ?"
