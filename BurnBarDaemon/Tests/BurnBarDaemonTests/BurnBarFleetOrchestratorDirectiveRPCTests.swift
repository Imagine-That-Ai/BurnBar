...
final class BurnBarFleetOrchestratorDirectiveRPCTests: BurnBarFleetOrchestratorRPCTestCase {
    ...
    func testDirectiveRecord_unknownKind_rejectedTyped_noRecord() async throws {
        ...
        XCTAssertEqual(envelope.id, "orch-29a")
        XCTAssertNil(envelope.result)
        // The unknown directive kind is a semantic decode failure that
        // propagates typed as the daemon's -32603 internalError (only a
        // wrong-typed `params` value yields -32602, per the error matrix).
        XCTAssertEqual(envelope.error?.code, -32603, "unknown directive kind is a typed validation error")
    ...
    func testDirectiveRecord_emptyPayload_rejectedTyped_noRecord() async throws {
        ...
        XCTAssertTrue(envelope.error?.message.contains("payload") == true)
