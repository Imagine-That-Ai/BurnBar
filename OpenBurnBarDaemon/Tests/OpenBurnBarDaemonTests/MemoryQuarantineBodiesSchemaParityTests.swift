...
/// `memory_quarantine_bodies` is created in two places against ONE database
/// file: the daemon's `bootstrapSchema` and the one GRDB migrator
/// (`v65_memory_quarantine_bodies` in `OpenBurnBarData`). Wave 2.2 deleted the
/// AgentLens mirror of that migration, so the app runs this same registry.
...
/// two texts agree. If a column were added on one side only, the loser would
...
///
/// So this suite reads both statements from source — applying Swift's own
/// multiline-literal rule, so what it compares is the string each compiler
/// builds, not the indentation each file happens to use — proves the deleted
/// AgentLens mirror stays deleted, and then proves the migrator's statement
/// really is a no-op on a store the daemon bootstrapped.
final class MemoryQuarantineBodiesSchemaParityTests: XCTestCase {
    ...
    /// The v65 DDL lives in exactly one file now.
    ...
    /// The whole point of the suite: the single migrator and the daemon's
    /// bootstrap create the same table, character for character.
    func testTheMigratorDDLIsByteEqualToTheDaemonBootstrapDDL() throws {
        let daemonTable = try tableStatement(inFileAt: Self.daemonBootstrapPath)
        let migratorTable = try tableStatement(inFileAt: Self.dataMigrationPath)
        XCTAssertEqual(
            daemonTable,
            migratorTable,
            """
            `memory_quarantine_bodies` is created by both the daemon bootstrap \
            and the single GRDB migrator against one database file. The two \
    ...
    /// The companion index. The two files differ in ONE respect — the migrator
    /// wraps the statement across two lines and the daemon keeps it on one — so
    /// this asserts the SQL, not the line breaks. A column or a name changing on
    /// one side still fails it.
    func testTheMigratorIndexDDLMatchesTheDaemonBootstrapIndex() throws {
        let daemonIndex = try indexStatement(inFileAt: Self.daemonBootstrapPath)
        let migratorIndex = try indexStatement(inFileAt: Self.dataMigrationPath)
        XCTAssertEqual(
            Self.collapsingWhitespace(daemonIndex),
            Self.collapsingWhitespace(migratorIndex),
            "the quarantine-bodies project index differs between the daemon bootstrap and the single migrator"
        )
    }

    // MARK: - Behaviour on a real store

    /// Source equality would still let the migrator's statement fail against a
    /// store the daemon created, so run it there: bootstrap a real store, park a
    /// real quarantined agent-lane body in it, then execute the migrator's text
    /// twice. One table, one index, no error, and the parked body untouched.
    func testTheMigratorIsANoOpOnADaemonBootstrappedStore() throws {
        let migratorTable = try tableStatement(inFileAt: Self.dataMigrationPath)
        let migratorIndex = try indexStatement(inFileAt: Self.dataMigrationPath)
        ...
        try execute(sql: migratorTable, on: fixture.database)
        try execute(sql: migratorIndex, on: fixture.database)
        try execute(sql: migratorTable, on: fixture.database)
        try execute(sql: migratorIndex, on: fixture.database)
        ...
        XCTAssertEqual(
            ...
            "the migrator's DDL must find the daemon's objects, not create second ones"
        )
        XCTAssertEqual(
            try queryStrings(
        ...
        XCTAssertEqual(
            ...
            [body],
            "and the quarantined body the review inbox reads survived the migrator's run"
        )
    }
    ...
    /// single migrator, so try both shapes — the one-line form first, because a
    ...
    private func execute(sql: String, on database: URL) throws {
        ...
        XCTAssertEqual(status, SQLITE_OK, "executing the migrator's DDL failed: \(message)")
    }
