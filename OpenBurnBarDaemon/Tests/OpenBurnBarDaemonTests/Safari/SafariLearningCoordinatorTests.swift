...
private final class LearningFixture {
    ...
    func coordinator(
        ...
        let resolvedMemoryWriter = memoryWriter ?? memory.map { memory in
            { request in await memory.write(request) }
        }
        let resolvedMemoryForgetter = memoryForgetter ?? memory.map { memory in
            { request in await memory.forget(request) }
        }
