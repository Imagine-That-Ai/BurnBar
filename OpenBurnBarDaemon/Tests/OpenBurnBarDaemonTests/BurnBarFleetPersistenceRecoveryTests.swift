...
final class BurnBarFleetPersistenceRecoveryTests: XCTestCase {
    ...
    func testStore_rebuildWindow_visibleOnFirstPublishedSnapshot_clearsOnNextPersist() async throws {
        ...
        guard case .degraded = health else {
            XCTFail("corrupt store must degrade typed, got \(health)")
            return
        }
        ...
        guard case .degraded(let firstReason) = first.persistenceHealth else {
            XCTFail("first recovery snapshot must stay degraded, got \(first.persistenceHealth)")
            return
        }
    ...
    func testPersister_failedPersist_doesNotAdvanceBaseline_noEventLost() async throws {
        ...
        guard case .degraded(let reason) = failed.persistenceHealth else {
            XCTFail("failed persist must degrade typed, got \(failed.persistenceHealth)")
            return
        }
    ...
    func testStore_schemaMarkerWithMissingV1Tables_rebuildsTyped() throws {
        ...
        guard case .degraded(let reason) = health else {
            XCTFail("partial old schema must degrade typed, got \(health)")
            return
        }
    ...
    func testStore_missingDatabaseWithLastGoodFile_rebuildsTyped() throws {
        ...
        guard case .degraded(let reason) = health else {
            XCTFail("missing store with a last-good file must degrade typed")
            return
        }
    ...
    func testStore_missingDatabaseWithoutSnapshot_rebuildsTypedUsingInitializationMarker() throws {
        ...
        let health = try reopened.open()
        guard case .degraded(let reason) = health else {
            XCTFail("deleted initialized store without a snapshot must degrade typed")
            return
        }
        XCTAssertTrue(reason.contains("rebuilt"), "unexpected reason: \(reason)")
        XCTAssertEqual(reopened.recoveryGeneration, 1)
    ...
    func testPersister_malformedSnapshotPayload_rebuildsTypedAndResetsBaseline() throws {
        ...
        guard case .degraded(let reason) = persister.persistenceHealth() else {
            XCTFail("malformed snapshot must trigger typed rebuild health")
            return
        }
        XCTAssertTrue(reason.contains("rebuilt"), "unexpected reason: \(reason)")
        ...
        guard case .degraded = recovery.persistenceHealth else {
            XCTFail("first post-rebuild snapshot must disclose recovery")
            return
        }
    ...
    func testControlStore_malformedStatePayload_rebuildsTypedAndClearsDesignation() async throws {
        ...
        XCTAssertEqual(try store.orchestratorStateRowCount(), 0)
        guard case .degraded(let reason) = persister.persistenceHealth() else {
            XCTFail("malformed control state must trigger typed rebuild health")
            return
        }
        XCTAssertTrue(reason.contains("rebuilt"), "unexpected reason: \(reason)")
    }
    ...
    func testStore_semanticallyCorruptSnapshot_rebuildsBeforeTransitionDerivation() throws {
        ...
        XCTAssertThrowsError(try reopened.latestSnapshot()) { error in
            guard case BurnBarFleetPersistenceError.semanticSnapshotInvalid = error else {
                XCTFail("expected typed semantic corruption, got \(error)")
                return
            }
        }
        ...
        XCTAssertNil(persister.lastPersisted)
        guard case .degraded(let reason) = persister.persistenceHealth() else {
            XCTFail("semantic corruption must degrade persistence")
            return
        }
        XCTAssertTrue(reason.contains("rebuilt"))
    }
    ...
    func testControlStore_malformedDirectivePayload_isTypedDegradationNotStaleCount() async throws {
        ...
        do {
            ...
            guard case BurnBarFleetControlError.storeUnavailable = error else {
                XCTFail("expected typed directive degradation, got \(error)")
                return
            }
        }
        guard case .degraded(let reason) = reopened.currentHealth() else {
            XCTFail("malformed directive must rebuild the store")
            return
        }
        XCTAssertTrue(reason.contains("rebuilt"))
    }
