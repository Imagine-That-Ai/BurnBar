...
final class BurnBarDaemonLinuxAuthSocketTests: XCTestCase {
    ...
    func testCloudSyncRPCsUseDaemonOwnedRuntimeAndRedactReplicaMaterial() async throws {
        let gateway = LinuxCloudSyncSocketGateway()
        let engine = try LinuxCloudReplicaEngine(
            database: try DatabaseQueue(),
            gateway: gateway,
            deviceID: "linux-socket-test",
            nowMillis: { 8_000 }
        )
        let vaultKey = Data(repeating: 0x44, count: 32)
        let runtime = LinuxCloudSyncRuntime(
            engine: engine,
            identityProvider: { "private-firebase-uid" },
            vaultKeyProvider: { vaultKey }
        )
        let socketPath = makeSocketPath("cloud-sync")
        let server = makeServer(
            socketPath: socketPath,
            authority: makeAuthority(backend: LinuxAuthSocketSecretBackend(refreshToken: refreshToken)),
            capabilityProfile: .full,
        ...
        let policyRequest = BurnBarRPCRequestEnvelopeWithParams(
            id: "cloud-policy",
            method: BurnBarRPCMethod.linuxCloudSyncPolicyUpdate,
            authToken: authToken,
        ...
        let policyData = try roundTrip(try JSONEncoder().encode(policyRequest), socketPath: socketPath)
        let policy = try decode(
            BurnBarRPCResponseEnvelope<BurnBarLinuxCloudSyncStatusResponse>.self,
            from: policyData
        )
        XCTAssertNil(policy.error)
        XCTAssertEqual(policy.result?.enabledDomains, ["usage"])

        _ = try await engine.stageUpdate(
            uid: "private-firebase-uid",
            domain: .usage,
            recordID: "usage-private",
            plaintext: Data("private-usage-payload".utf8),
            vaultKey: vaultKey
        )
        let runRequest = BurnBarRPCRequestEnvelopeWithParams(
            id: "cloud-run",
            method: BurnBarRPCMethod.linuxCloudSyncRun,
            authToken: authToken,
            params: BurnBarLinuxCloudSyncRunRequest()
        )
        let runData = try roundTrip(try JSONEncoder().encode(runRequest), socketPath: socketPath)
        let run = try decode(
            BurnBarRPCResponseEnvelope<BurnBarLinuxCloudSyncRunResponse>.self,
            from: runData
        )
        XCTAssertNil(run.error)
        XCTAssertEqual(run.result?.pushedCount, 1)
        XCTAssertEqual(run.result?.status.pendingMutationCount, 0)

        let statusData = try roundTrip(
            try encodeEnvelope(id: "cloud-status", method: .linuxCloudSyncStatus),
            socketPath: socketPath
        )
        let status = try decode(
            BurnBarRPCResponseEnvelope<BurnBarLinuxCloudSyncStatusResponse>.self,
        ...
        XCTAssertNil(status.error)
        XCTAssertEqual(status.result?.phase, "ready")

        for payload in [policyData, runData, statusData] {
            let text = String(decoding: payload, as: UTF8.self)
            XCTAssertFalse(text.contains("private-firebase-uid"))
    ...
    private func makeServer(
        ...
        cloudSyncRuntime: LinuxCloudSyncRuntime? = nil
    ) -> BurnBarDaemonServer {
        BurnBarDaemonServer(
            ...
            linuxCloudSyncRuntime: cloudSyncRuntime
        )
    }
...
private actor LinuxCloudSyncSocketGateway: LinuxCloudReplicaEngine.Gateway {
    private var mutations: [LinuxCloudReplicaEngine.OutboundMutation] = []
    ...
    func push(
        ...
        return .init(
            acknowledgedMutationIDs: mutations.map(\.mutationID),
            authoritativeReplicas: mutations.map(\.replica)
        )
    }
    ...
    func pull(
        ...
        .init(replicas: [], nextCursor: cursor)
    }

    func pushedCount() -> Int { mutations.count }
}
