...
final class BurnBarLinuxPrivacyServiceTests: XCTestCase {
    ...
    func testCloudAccountExportWriteIsBoundedOwnerOnlyAndReturnsReceiptOnly() async throws {
        let destination = directory.appendingPathComponent("account-export.json")
        let payload = Data(#"{"ok":true,"schemaVersion":2,"domains":[{"id":"usage","encryptionTier":"server_readable"}]}"#.utf8)
        let service = BurnBarLinuxPrivacyService(supportDirectory: directory)

        let receipt = try await service.writePlaintextExport(payload, destinationPath: destination.path)

        XCTAssertEqual(receipt.destinationPath, destination.path)
        XCTAssertEqual(receipt.byteCount, Int64(payload.count))
        let attributes = try FileManager.default.attributesOfItem(atPath: destination.path)
        let mode = (attributes[.posixPermissions] as? NSNumber)?.intValue
        XCTAssertEqual(mode, 0o600)
    ...
    func testCloudAccountExportWriteRejectsExistingDestination() async throws {
        let destination = directory.appendingPathComponent("account-export.json")
        try Data("existing".utf8).write(to: destination)
        try setPrivate(destination)
        let service = BurnBarLinuxPrivacyService(supportDirectory: directory)

        await XCTAssertThrowsErrorAsync(
            try await service.writePlaintextExport(Data("new".utf8), destinationPath: destination.path)
        ) {
