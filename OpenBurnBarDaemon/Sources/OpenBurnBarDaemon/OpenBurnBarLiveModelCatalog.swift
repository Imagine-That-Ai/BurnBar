...
struct BurnBarLiveModelCatalogDiscoverySnapshot: Sendable {
    fileprivate let refreshesByAccount: [BurnBarLiveModelCatalog.AccountDiscoveryKey: BurnBarLiveModelCatalog.LiveRefreshResult]
}
...
public struct BurnBarLiveModelCatalog: Sendable {
    ...
    func discoverySnapshot(now: Date = Date()) async throws -> BurnBarLiveModelCatalogDiscoverySnapshot {
        let contexts = try await accountRefreshContexts(now: now)
        let liveRefreshes = await liveRefreshes(for: contexts)
        return BurnBarLiveModelCatalogDiscoverySnapshot(
            refreshesByAccount: Dictionary(uniqueKeysWithValues: contexts.compactMap { context in
                liveRefreshes[context.index].map { (context.discoveryKey, $0) }
            })
        )
    }
    ...
    func snapshot(
        ...
        return makeSnapshot(
            now: now,
            contexts: contexts,
            liveRefreshes: Dictionary(uniqueKeysWithValues: contexts.compactMap { context in
                discovery.refreshesByAccount[context.discoveryKey].map { (context.index, $0) }
            })
        )
    }
