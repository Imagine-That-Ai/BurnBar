...
public final class BurnBarTextExpansionService: @unchecked Sendable {
    ...
    private static let defaultKeyID = "text-expansion-v1"
    ...
    private actor LinuxTextExpansionEngineRuntime {
        ...
        func expand(
            ...
            let engineResult = try await session.expand(
                trigger: trigger,
    ...
    public init(
        ...
        keyNamespace: String? = ProcessInfo.processInfo.environment["OPENBURNBAR_TEXT_EXPANSION_KEY_NAMESPACE"]
    ) {
        self.fileURL = fileURL
        self.secretStore = secretStore
        self.logger = logger
        self.encoder = Self.makeEncoder()
        self.keyID = Self.resolveKeyID(namespace: keyNamespace)
    ...
    public func updateConsent(
        ...
        if response.consent.systemIMEEnabled != true {
            #if os(Linux)
            _ = await linuxEngineRuntime.stop(timeoutMillis: 500)
            #endif
        }
        return response
    }
    ...
    public func startExternalEngine(
        ...
        guard consent?.inAppOnly == true,
              consent?.systemIMEEnabled == true,
              consent?.declinedGlobalCapture == true else {
    ...
    public func expandExternalEngine(
        ...
        let replacement = try withLock {
            try readSnapshot().snippets.first(where: {
                $0.deletedAt == nil && $0.isEnabled && $0.trigger == canonical
            })?.body
        }
...
#if os(Linux)
        guard let keyID else { throw ServiceError.keyStorageUnavailable }
        do {
    ...
    private static func resolveKeyID(namespace: String?) -> String? {
        guard let namespace = namespace?.trimmingCharacters(in: .whitespacesAndNewlines),
              !namespace.isEmpty else {
            return defaultKeyID
        }
        guard namespace.range(of: "^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$", options: .regularExpression) != nil else {
            return nil
        }
        return "\(defaultKeyID).\(namespace)"
    }
