...
public actor BurnBarLinuxPrivacyService {
    ...
    public struct PlaintextExportReceipt: Codable, Hashable, Sendable {
        public let destinationPath: String
        public let byteCount: Int64

        public init(destinationPath: String, byteCount: Int64) {
    ...
    /// Persist a daemon-owned cloud export using the same destination and
    /// ownership checks as local privacy bundles. The caller supplies bytes
    /// returned by the trusted-device cloud authority; the renderer never
    /// receives those bytes.
    public func writePlaintextExport(
        _ data: Data,
        destinationPath: String
    ) throws -> PlaintextExportReceipt {
        guard data.count <= 8 * 1024 * 1024 else { throw ServiceError.exportTooLarge }
        let destination = try validatedExportDestination(destinationPath)
        do {
            try data.write(to: destination, options: .atomic)
            try fileManager.setAttributes([.posixPermissions: 0o600], ofItemAtPath: destination.path)
            guard isSafeExportFile(destination) else { throw ServiceError.unsafeFile }
        } catch let error as ServiceError {
            try? fileManager.removeItem(at: destination)
            throw error
        } catch {
        ...
        return PlaintextExportReceipt(destinationPath: destination.path, byteCount: Int64(data.count))
    }
    ...
    private func isSafeExportFile(_ path: URL) -> Bool {
        guard let metadata = lstatMetadata(at: path), metadata.isRegular,
              metadata.ownerUID == geteuid(), metadata.mode & 0o077 == 0 else { return false }
        return pathHasNoSymlinks(path)
    }
