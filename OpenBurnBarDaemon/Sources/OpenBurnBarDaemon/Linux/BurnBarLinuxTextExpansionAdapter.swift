...
public struct BurnBarLinuxTextExpansionAdapter: Sendable {
    ...
    public func startExternalEngine(
        ...
        guard isValidManifestShape(manifest),
              ...
              executableDigestMatches(manifest),
    ...
    public func typedStatus() -> Status {
        ...
        guard executableDigestMatches(manifest) else {
            return makeStatus(
                state: .blocked,
                backend: backend.rawValue,
                backendPath: discoveredPath,
                ...
                detail: "The registered input-method engine bytes do not match the signed manifest."
            )
        }
    ...
    private static func expansionRequest(
        ...
        guard replacement.map({ $0.utf8.count <= maxExpansionBytes }) ?? true else {
            throw EngineRuntimeError.expansionInvalid
        }
    ...
    public static func signingPayload(for manifest: EngineManifest) -> Data {
        ...
        if manifest.schemaVersion >= 2 {
            lines.append("executable_sha256=\(manifest.executableSha256 ?? "")")
        }
    ...
    private func executableDigestMatches(_ manifest: EngineManifest) -> Bool {
        guard manifest.schemaVersion >= 2 else { return true }
        guard let expected = manifest.executableSha256,
              expected.range(of: "^[a-f0-9]{64}$", options: .regularExpression) != nil,
              let bytes = try? Data(
                  contentsOf: URL(fileURLWithPath: manifest.executablePath),
                  options: .mappedIfSafe
              ) else {
            return false
        }
        return PlatformCrypto.sha256Hex(bytes) == expected
    }
