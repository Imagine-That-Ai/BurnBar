...
extension BurnBarDaemonServer {
    func resolveAuthenticatedSafariExternalizedRequestData(
        ...
        guard case .object(let object) = request.params,
    ...
    func handleSafariRPC(
        ...
        do {
            ...
            case .safariHandoff:
                ...
                return safariRPCSuccess(
                    id: request.id,
                    result: try await launchSafariHandoff(params)
                )
    ...
    private func safariUISnapshot(
        ...
        let learning = try await learningCoordinator.timeline()
    ...
    private func launchSafariHandoff(
        _ request: BurnBarSafariHandoffRequest
    ) async throws -> BurnBarSafariHandoffResponse {
        let identity = try await requireAttachedSafariSession(
            request.safariSessionId
        )
        let now = Date()
        let captureAge = now.timeIntervalSince(request.pageState.capturedAt)
        guard request.pageState.isActive,
              request.pageState.isTopFrame,
              request.pageState.tabId >= 0,
              request.pageState.title.utf8.count <= 64 * 1024,
              request.targetHarness.utf8.count <= 128,
              request.prompt.trimmingCharacters(in: .whitespacesAndNewlines)
                .isEmpty == false,
              request.prompt.utf8.count
              ...
              captureAge >= -30,
              captureAge <= 120,
              Self.validSafariHandoffURL(request.pageState.url) else {
            throw BurnBarSafariRPCHandlerError.invalidParams(
                "Safari hand-off context is invalid, stale, or exceeds its bounds."
            )
        }
        ...
        guard activePage.tabId == request.pageState.tabId,
              activePage.url == request.pageState.url,
              activePage.navigationEpoch == request.pageState.navigationEpoch,
              activePage.isActive,
              activePage.isTopFrame,
              activePage.windowId == nil
                || request.pageState.windowId == nil
                || activePage.windowId == request.pageState.windowId else {
            throw BurnBarSafariRPCHandlerError.conflict(
                "The Safari page changed while its hand-off package was being prepared."
            )
        }
        ...
        do {
            ...
            where error.domain == "BurnBarResumeService" {
            throw BurnBarSafariRPCHandlerError.unavailable(
            ...
            throw BurnBarSafariRPCHandlerError.unavailable(
                "The selected agent could not be launched from the private Safari hand-off package."
            )
        }
    ...
    private func safariBootstrapResponse(
        ...
        let availability = try? await learningCoordinator.availability()
        ...
        return BurnBarSafariBootstrapResponse(
            ...
            learningOptedIn: availability?.optedIn ?? false,
