...
struct LinuxIrohControllerDirectoryClient: LinuxIrohControllerDirectoryServing,
    ...
    /// The directory client carries Firebase bearer and App Check credentials.
    /// Keep the production host explicit so a malformed or injected endpoint
    /// cannot turn a directory call into credential exfiltration.
    static let defaultAllowedHosts: Set<String> = [
        "us-central1-burnbar.cloudfunctions.net"
    ]
    ...
    private func call<Payload: Encodable, Result: Decodable>(
        ...
        guard Self.isStrictHTTPSEndpoint(baseURL, allowedHosts: allowedHosts),
    ...
    private static func isStrictHTTPSEndpoint(_ url: URL, allowedHosts: Set<String>) -> Bool {
