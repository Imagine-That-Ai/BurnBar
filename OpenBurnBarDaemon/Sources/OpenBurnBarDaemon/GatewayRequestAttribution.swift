...
public actor SafariGatewayAttributionAuthority {
    ...
    func resolve(headers: [String: String]) async -> GatewayRequestAttributionResolution {
        ...
        guard currentGenerationByBinding[binding] == record.generation else {
            invalidate(record: record, digest: digest)
            return .rejected
        }
        // A replay is scoped to one request correlation. It must fail closed,
        // but it is not evidence that the live session capability itself is
        // corrupt. Revoking the entire rolling lease here would let one
        // duplicate delivery deny every later, fresh request for the attached
        // Safari session.
        guard record.consumedCorrelationIDs.contains(correlationID) == false
        else {
            return .rejected
        }
        ...
        guard isAttached,
              ...
              currentRecord.expiresAt > resolvedAt else {
            if let currentRecord = recordsByDigest[digest],
               currentRecord.generation == record.generation {
                invalidate(record: currentRecord, digest: digest)
            }
        ...
        guard currentRecord.resolvingCorrelationIDs.remove(correlationID)
                != nil else {
            invalidate(record: currentRecord, digest: digest)
            return .rejected
        }
        guard currentRecord.consumedCorrelationIDs.contains(correlationID)
                == false else {
            recordsByDigest[digest] = currentRecord
            return .rejected
        }
        guard currentRecord.consumedCorrelationIDs.count
                < maximumCorrelationIDs else {
            invalidate(record: currentRecord, digest: digest)
            return .rejected
        }
