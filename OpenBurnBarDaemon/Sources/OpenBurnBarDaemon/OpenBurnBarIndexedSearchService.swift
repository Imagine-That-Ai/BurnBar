...
final class BurnBarIndexedSearchService: @unchecked Sendable {
    ...
    struct DaemonVectorIndexSnapshotRecord {
        let embeddingVersionID: String
    ...
    var cachedConversationsHaveDeletedAtColumn: Bool?
    ...
    init(
        ...
        if BurnBarDaemonDatabaseCipher.isEncryptedDatabaseFile(at: databasePath) {
            do {
                try BurnBarDaemonDatabaseCipher.applyKeyIfAvailable(to: handle, key: explicitCipherKey)
            } catch {
