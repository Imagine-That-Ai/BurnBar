...
| 2026-05-17 | Computer Use Phase 8 — Agent Watch reuses Mercury Phase 3 transport with cursor extension | Decision 4 of the master plan. No new transport plumbing; 4 extra bytes after the existing 18-byte `MediaFrame` header (`i16 cursorX`, `i16 cursorY`) prefixed by the new `MediaFrame.Flags.hasCursorMetadata` bit. The plan's draft used bit `0x04` for this flag, but `0x04` is already taken by `.muted` (audio). The wire bit is `0x08`. Old peers ignore the trailing 4 bytes when the flag is absent, so the codec extension is byte-identical for pre-Computer-Use senders. |
| 2026-05-17 | Computer Use Phase 9 — Browser approval sheet uses `mercuryGradient` border with pre-action thumbnail | `ComputerUseApprovalSheet` ships a 320×180 pre-action screenshot above the action summary; three buttons in a row — Reject + Halt (`error` fill), Reject (cancel role), Approve (`green` borderedProminent + keyboard-shortcut default action). Mirrors the Mercury attachment-row aesthetic but raises the bar by showing the screenshot evidence the agent saw at decision time. |
| 2026-05-17 | Computer Use Phase 10 — Trust modes shown as segmented pill in `ComputerUseSessionPanel`; downgrade from phone, upgrade only from Mac | Three-state selection (Manual / Step / Trusted) maps cleanly to the macOS HIG segmented control. The picker's active segment paints with `LinearGradient(.orange, .red)` — same Mercury-adjacent ember gradient used for action borders. The phone-side `ComputerUseTrustModeBadge` sheet enforces downgrade-only — Decision 2's enforcement against a compromised phone elevating itself. |
| 2026-05-17 | Computer Use Phase 11 — Audit chain panel uses `monoTiny` ordinals + status glyphs | `ComputerUseSessionPanel` lists the 10 most recent entries with mono ordinals (`01`, `02`, ...) and SF Symbol glyphs (`circle.fill` executing → `checkmark.circle.fill` completed → `xmark.circle.fill` denied). Mirrors `IntelligenceBriefScreen.kt`'s Editorial Observatory ordinal pattern — auditing is forensic, calls for editorial-voice typography. |
| 2026-05-17 | Computer Use Phase 12 — Phone-side trust-mode downgrade only; upgrade from Mac only | The `AgentWatchView` top-right pill lets the phone-side user drop Trusted → Step or Manual. Upgrades return the user to the Mac. Closes a hypothetical attack where a stolen / compromised phone elevates a Manual-mode session into Trusted-mode and stops requiring approval. |
| 2026-05-17 | Computer Use Phase 13 — Trusted-scope library lives in `ComputerUseSessionPanel`, not in chat | Scopes are infrastructure, not conversation. Chat is the wrong surface for stable per-user configuration; Settings → Computer Use is the long-lived home. The library carries named bundles ("GitHub PR triage", etc.), each composed of one or more `ComputerUseScopeRule` rows with a 24 h / 50-action expiry. |
| 2026-05-20 | Mercury mirror/media SOTA UX Polish Pass | Completed high-fidelity visual and UX updates to iOS media surfaces: color-shifting/breathing radial auroras behind the MacBook avatar; telemetry capsule HUDs with glowing silver-white borders; custom floating HUD alert notification overlays; elegant `StreamStateOverlay` with rotating state indicators and frosted glass backplates; concentric sci-fi lock ripples for touch feedback; buttery-smooth cursor spring glide controls (`interactiveSpring(response: 0.22, dampingFraction: 0.78)`) with glowing neon teal/coral/purple outlines; fine-crosshair trackpads with drag trailing indicator rings and physical haptic feedbacks; and selected glowing halos for translucent control keycaps. |
| 2026-05-21 | Agent Live Stage in chat (iOS/iPadOS) — windowed mirror that auto-opens on session start | First-class "look-over-the-agent's-shoulder" experience: when a Computer Use `sessionId` arrives, an `AgentLiveStage` overlay auto-springs to a 320×180 (compact) / 360×203 (regular) mercury-stroked dock tile in `RootTabView`'s ZStack. Tap promotes to `.split` (top 55% on compact, leading 60% on regular) keeping the chat reachable; pinch-out or the expand chevron promotes to `.maximize` (full-bleed mirror + a 56pt draggable `AgentLiveStageChatPuck` that expands to a 320×420 mercury-stroked floating composer panel). The mirror's passthrough surface routes taps (< 10pt) to `AgentWatchReceiver.tap(normalizedX:y:)` and drags to `scrollDrag(...)`, with a fading "YOU ARE DRIVING" mercury pill that surfaces on first input and after 4-second input lulls. Auto-open chosen over manual entry because (a) tap-to-drive UX requires zero user setup mid-agent-action, (b) the dock tile is small enough to be ignorable but always present when work is happening, (c) panic-halt is one tap away from any phase. A new app-scope `AgentWatchOverlaySingleton` owns the persistent iroh control stream so the live mirror survives tab swaps — `AgentWatchScreen` (You-tab) was rebound as a passive viewer of the same singleton, eliminating double-dial. Session-end uses a 6s grace window so the final action animation and audit-head update finish painting before the dock collapses; grace cancels if a new session arrives mid-window. Hidden under `.cloudStoreChromeVisibilityChanged` notification — entering split/maximize hides the Aurora nav tray automatically. Reuses every existing pipeline: `AgentWatchVideoCoordinator` (extracted to `AgentWatchVideoSurface.swift`) keeps the same `AVSampleBufferDisplayLayer` + `VideoReceivePipeline`; receiver/sender API is identical to `AgentWatchView`; mercury/ember/whimsy color axis unchanged. Covered by `AgentLiveStagePresenterTests` (13 cases — auto-open, grace, transitions, panic, corners, observe-idempotency) and `AgentLiveStageWiringTests` (5 cases — singleton gating, session-id observer wiring). |
...
| 2026-08-18 | Liquid Plasma selectors (Grok Bot D asset) replace the flat chat model menu | The `ChatEngineModelMenu` pill becomes **two living plasma orbs**. **Left, the persona orb:** a morphing plasma sphere with a *face* — ten personas (`PlasmaPersona`), each with its own eye style, palette, tagline, skills and voice line. All ten eye styles are drawn (`PlasmaPersonaFace`) even though the asset only implements five, because five personas falling back to plain dots are not distinguishable at 52pt, which defeats the mascot. Eyes glance on a 5s track and blink on a deliberately non-dividing 5.9s one, so the face never settles into a predictable loop. Selecting a persona prefixes the agent's system prompt; the choice is **per-agent** (the voice you want from a fast local CLI is not the one you want from a reasoning gateway), persisted as a JSON blob under `chatPanel.personaSeats.v1` / `chatPanel.personaRoster.v1` following the `ElderWandSettings` convention rather than minting 12 more defaults keys, gated by `persistsViewState`, and inherited by split panes via `copyPaneConversationControls`. While the agent streams, three staggered droplets (`riseAndPopBubble1…3`, 2.4/3.0/2.6s at 0/0.4/0.9s delays) rise out of the orb and pop — a thinking indicator in the character's own idiom instead of a spinner. **Right, the model orb:** opens the ghostly liquid-glass bubble (`ghostlyBubbleMorph`, 11s) holding a three-rung cascade — **Route › Provider › Model**. Rungs 1 and 2 are constellations of drifting 52pt glass spheres (`PlasmaConstellation`); rung 3 is a list, because routes and providers are a handful of identities recognised by their logo while a model catalog is forty near-identical strings that differ only in their suffix and must be *read*. **Motion is transcribed from the shipped runtime, not approximated:** orb tracks 7s/8s, bubble 11s, glow breath 4s, badge orbit 4.2s, and four `alternate` constellation drifts at 9.2/10.6/11.4/8.8s assigned by the asset's `nth-child(4n+k)` rotation so the field never falls into a visible grid rhythm. Where the bundle's standalone CSS disagrees with the runtime (7.5/8.5/12/3.5s), the runtime wins — it is what the reference demo renders. **Two deliberate departures from the asset, both documented at their source.** (1) *Rung 1 lists BurnBar's real routes, not the asset's ten gateways.* Nine of the ten cannot serve a request here: there is no port 8320 (BurnBar's gateway is 8317), no CLIProxy or VibeProxy listener at all, and OpenRouter/Fireworks/Baseten are provider strings in a billing allowlist rather than routable endpoints. Shipping them would give the user ten orbs of which one works — a picker that lies about where the next token goes, which is the exact failure the ladder exists to fix. The rung keeps the asset's *structure* (a constellation of routing orbs with badge, endpoint and status light) and fills it with the three real local gateways (Hermes :8642, OpenClaw :18789, Pi Agent :8765) plus the direct CLI routes, gateways first because a dead endpoint is the most valuable thing the rung can report. (2) *Five status states, not the asset's two.* The asset models "signed in / not" and offers a fake OAuth card. BurnBar distinguishes `live` / `authRejected` / `offline` / `ready` / `unknown`, each with its own remedy, and an unprobed gateway reports "not probed" rather than "offline" because we only probe the active route — claiming an uncontacted server is down would send the user to restart something that was fine. Auth rejection outranks availability, since a server answering 401 *is* reachable. **Performance:** one `PlasmaClock` per surface at 30fps drives a whole field (twelve orbs used to mean twelve display-link wake-ups and twelve invalidations per frame for one picture); gradients and content are resolved outside the tick; aura blur radius is fixed while scale and opacity breathe, because breathing a blur re-runs an offscreen pass every frame. Idle chrome is *still* — the orbs earn motion by being hovered, opened, or by the agent actually answering — and Reduce Motion resolves to a deterministic time-zero pose rather than a paused arbitrary frame. **Prompt seam:** the persona composes into the trusted `.core` section, because the ten voices are app-authored constants and must survive token pressure; the desktop pet's user-authored voice keeps precedence for that turn and stays wrapped as untrusted style (`PlasmaPersonaPrompt.resolveVoice`). No persona voice may contain instruction-override language, which is asserted per persona. Analytics adds `chat.persona.selected` carrying only the closed app-authored `persona_id`, never the user-typed seat label. **Containment Law preserved:** identity tints paint rims, dots and glows only; the one status line that becomes text uses the semantic warning token, and a new seat picks one of the ten shipped personas rather than authoring a hex. Covered by `PlasmaAssetFidelityTests`, `PlasmaPersonaTests`, `PlasmaRouteTests` and `PlasmaModelSelectorTests` (92 green with `PromptInjectionHardeningTests`). |
| 2026-08-31 | Fluid aurora kernel replaces the memory-consent emblem | The first-run `MemoryConsentSheet` used to open on a flat `Circle()` behind a `brain.head.profile` SF Symbol — a logo asking permission. It now opens on a **living fluid aurora kernel** (`FluidAuroraKernelView`): three luminous ribbons in a BurnBar ramp — **whimsy → mint → lavender → ember** — drifting over a deep ground with a breathing specular core and a hairline rim. Two new house colour stops (`Color.auroraMint` / `Color.auroraLavender`, adaptive light/dark pairs in the `Color.adaptive` dialect) extend the cool end of the brand ramp; whimsy and ember resolve through `DesignSystem.Colors`, so the kernel inherits appearance + Editorial skin for free. The palette is a pure `Sendable` value (`FluidAuroraKernelPalette`) whose knobs are appearance-modulated the same way `PlasmaShade.halo` modulates orb halos: dark sheet = additive glow, cream paper = quieter pearl watermark (no glow headroom on light), Reduce Transparency = near-opaque printed tint. Motion is pure wrapped-phase math (`FluidAuroraMotion`), split from the view the same way `BurnBarKernelMath` splits from the field: sine/cosine tracks on mutually incommensurate periods (37s lead drift / 8.5s counter / 23s breath / 2.9s core / 11s halo) so ribbons never phase-lock and `Float` precision never degrades on long-lived sheets; the triangle-fold design was rejected because it parks each track's *extreme* at phase zero, making the Reduce-Motion still pose the most swung frame in the loop and putting a derivative kink at the wrap. The still pose is `frame(at: 0)` by construction — a fanned three-layer silhouette (sin 0, sin 2π/3, sin 4π/3), not a paused arbitrary frame — mirroring `PlasmaClock.still`'s contract. Driven by one shared `PlasmaClock` at 30fps; the emblem keeps the "Memory" accessibility label the SF Symbol owed VoiceOver. Covered by `FluidAuroraKernelTests` (12 tests: ramp order, light/dark modulation, Reduce Transparency, still pose, phase wrapping, period incommensurability, wrap continuity, value ranges, amplitude scaling, counter-drift wiring). |
