...
/// Unit tests for the pure month-grid math behind the iOS Calendar:
//  month boundaries, locale week starts, row-major ordering, drag-hit math.
final class CalendarMonthGridModelTests: XCTestCase {
    ...
    func test_dayIndex_mapsPointsToRowMajorCells() {
        ...
        XCTAssertEqual(
            CalendarMonthGridModel.dayIndex(
                at: CGPoint(x: width + 40, y: 0), gridWidth: width, rows: rows, cellHeight: 46
            ),
            6
        )
        // Below the last row is out of bounds.
        XCTAssertNil(
            CalendarMonthGridModel.dayIndex(
                at: CGPoint(x: 0, y: 4 * 50), gridWidth: width, rows: rows, cellHeight: 46
            )
        )
        // Negative origins are out of bounds.
        XCTAssertNil(
            CalendarMonthGridModel.dayIndex(
                at: CGPoint(x: -4, y: 0), gridWidth: width, rows: rows, cellHeight: 46
            )
        )
    }
}
