# Agent setup for `BurnBar`

This is a deepsec scanning workspace. Project `BurnBar` was just registered
(target: `..`). Setup is incomplete — `data/BurnBar/INFO.md`
...
2. **Fill in `data/BurnBar/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id BurnBar
pnpm deepsec process --project-id BurnBar
```
