...
class VideoDecoderConfigurationPayloadTest {
    ...
    fun decoderConfigurationPayloadRoundTripsParameterSetsAndSamplePayload() {
        val payload = VideoDecoderConfigurationPayload(
            codec = VideoDecoderConfigurationPayload.Codec.HEVC,
            parameterSets = listOf(byteArrayOf(0x40, 0x01), byteArrayOf(0x42, 0x01, 0x02)),
            samplePayload = lengthPrefixed(byteArrayOf(0x26, 0x01, 0x02)),
        )
    ...
    fun normalizerUnwrapsDecoderConfigurationAndConvertsLengthPrefixedNalsToAnnexB() {
        val payload = VideoDecoderConfigurationPayload(
            codec = VideoDecoderConfigurationPayload.Codec.HEVC,
            parameterSets = listOf(byteArrayOf(0x40), byteArrayOf(0x42)),
            samplePayload = lengthPrefixed(byteArrayOf(0x26, 0x01), byteArrayOf(0x02, 0x03)),
        )
    ...
    fun normalizerLeavesExistingAnnexBPayloadsUnchanged() {
        val annexB = byteArrayOf(0x00, 0x00, 0x00, 0x01, 0x26, 0x01)
    ...
    private fun lengthPrefixed(vararg nals: ByteArray): ByteArray {
        ...
        nals.forEach { nal ->
            ...
            output[offset++] = ((length ushr 8) and 0xFF).toByte()
