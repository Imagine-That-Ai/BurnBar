...
class VideoDecoderConfigurationPayloadTest {
    ...
    fun decoderConfigurationPayloadRoundTripsParameterSetsAndSamplePayload() {
        val payload =
            VideoDecoderConfigurationPayload(
                ...
                parameterSets = listOf(byteArrayOf(VAL_0X40, 0x01), byteArrayOf(VAL_0X42, 0x01, 0x02)),
                samplePayload = lengthPrefixed(byteArrayOf(VAL_0X26, 0x01, 0x02)),
            )
    ...
    fun normalizerUnwrapsDecoderConfigurationAndConvertsLengthPrefixedNalsToAnnexB() {
        val payload =
            VideoDecoderConfigurationPayload(
                codec = VideoDecoderConfigurationPayload.Codec.HEVC,
                parameterSets = listOf(byteArrayOf(VAL_0X40), byteArrayOf(VAL_0X42)),
                samplePayload = lengthPrefixed(byteArrayOf(VAL_0X26, 0x01), byteArrayOf(0x02, VAL_0X03)),
            )
        ...
        assertArrayEquals(
            byteArrayOf(
                0x00, 0x00, 0x00, 0x01, VAL_0X40,
                0x00, 0x00, 0x00, 0x01, VAL_0X42,
                0x00, 0x00, 0x00, 0x01, VAL_0X26, 0x01,
                0x00, 0x00, 0x00, 0x01, 0x02, VAL_0X03,
            ),
            normalized,
        )
    }
    ...
    fun normalizerLeavesExistingAnnexBPayloadsUnchanged() {
        val annexB = byteArrayOf(0x00, 0x00, 0x00, 0x01, VAL_0X26, 0x01)
