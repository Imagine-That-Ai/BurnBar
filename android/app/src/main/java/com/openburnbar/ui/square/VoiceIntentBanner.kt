...
@Composable
internal fun VoiceIntentBannerView(intent: AndroidVoiceIntent, onDismiss: () -> Unit, modifier: Modifier = Modifier) {
            ...
            val sub =
                when (intent) {
                    ...
                    is AndroidVoiceIntent.DispatchMission ->
                        if (intent.runtimeHint != null) {
                            "→ ${intent.runtimeHint}: ${intent.prompt}"
                        } else {
                            intent.prompt
                        }
