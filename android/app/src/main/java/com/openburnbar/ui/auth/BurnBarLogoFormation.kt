...
private fun easeOut(x: Double): Double { val t = x.coerceIn(0.0, 1.0); return 1 - (1 - t).pow(3.0) }
...
private fun hash21(x: Double, y: Double): Double {
    var px = fract(x * 123.34); var py = fract(y * 233.53)
    val dv = px * (px + 23.234) + py * (py + 23.234)
    px += dv; py += dv
    return fract(px * py)
}
private fun vnoise(x: Double, y: Double): Double {
    val ix = floor(x); val iy = floor(y); val fx = x - ix; val fy = y - iy
    val ux = fx * fx * (3 - 2 * fx); val uy = fy * fy * (3 - 2 * fy)
    val a = hash21(ix, iy); val b = hash21(ix + 1, iy); val c = hash21(ix, iy + 1); val d = hash21(ix + 1, iy + 1)
    val ab = a + (b - a) * ux; val cd = c + (d - c) * ux
    return ab + (cd - ab) * uy
}
private fun fbm2(x0: Double, y0: Double): Double {
    var x = x0; var y = y0; var v = 0.0; var amp = 0.5
    repeat(5) { v += amp * vnoise(x, y); val nx = 1.6 * x + 1.2 * y; val ny = -1.2 * x + 1.6 * y; x = nx; y = ny; amp *= 0.5 }
    return v
}

// ---- model ----
private class Dot(
    val target: Offset, val start: Offset, val r: Double, val g: Double, val b: Double,
    val delay: Double, val driftA: Float, val driftF: Double, val driftP: Double, val dotSize: Double,
)
private class GDot(val px: Float, val py: Float, val r: Double, val g: Double, val b: Double)
private class Glyph(
    var x: Float, var y: Float, var vx: Float, var vy: Float,
    var prov: Int, var flash: Double, var formT: Double, var seed: Long,
)

private fun sampleDots(bmp: Bitmap?): List<Dot> {
    bmp ?: return emptyList()
    val w = bmp.width; val h = bmp.height; val grid = 62
    ...
    for (gy in 0 until grid) for (gx in 0 until grid) {
        ...
        if (a < 0.45) continue
        val r = ((c ushr 16) and 0xff) / 255.0; val g = ((c ushr 8) and 0xff) / 255.0; val b = (c and 0xff) / 255.0
        val nx = (gx + 0.5) / grid; val ny = (gy + 0.5) / grid
        raw.add(Triple(Offset(nx.toFloat(), ny.toFloat()), doubleArrayOf(r, g, b), Unit))
    }
    val logoOX = LOGO_CX - LOGO_SIZE / 2; val logoOY = LOGO_CY - LOGO_SIZE / 2
    for ((n, col, _) in raw) {
        val tx = logoOX + n.x * LOGO_SIZE; val ty = logoOY + n.y * LOGO_SIZE
        val ang = rng.nextDouble() * 2 * Math.PI
        val rad = (0.35 + rng.nextDouble() * 0.75) * maxOf(DW, DH).toDouble() * 0.6
        val sx = (LOGO_CX + cos(ang) * rad).toFloat(); val sy = (LOGO_CY + sin(ang) * rad).toFloat()
        val delay = (0.04 + rng.nextDouble() * 0.22 + n.y.toDouble() * 0.06).coerceAtMost(0.30)
        out.add(Dot(Offset(tx, ty), Offset(sx, sy), col[0], col[1], col[2], delay,
...
private fun sampleGlyph(bmp: Bitmap?, grid: Int = 40, maxDots: Int = 220): List<GDot> {
    ...
    for (gy in 0 until grid) for (gx in 0 until grid) {
        ...
        val c = bmp.getPixel(px, py)
        val a = ((c ushr 24) and 0xff) / 255.0
        var r = ((c ushr 16) and 0xff) / 255.0; var g = ((c ushr 8) and 0xff) / 255.0; var b = (c and 0xff) / 255.0
        val lum = 0.299 * r + 0.587 * g + 0.114 * b
        if (a > 0.4 && lum < 0.985) {
            val mx = maxOf(r, maxOf(g, b))
            if (mx < 0.06) { r = 0.92; g = 0.94; b = 0.97 } else if (mx < 0.62) { val s = 0.62 / mx; r = min(1.0, r * s); g = min(1.0, g * s); b = min(1.0, b * s) }
...
private fun scatterOffset(seed: Long, k: Int): Offset {
    ...
    s = s xor (s ushr 33); s *= 0xff51afd7ed558ccduL.toLong(); s = s xor (s ushr 33)
...
private fun roundedPoly(pts: List<Offset>, rad: Float): Path {
    val path = Path(); val n = pts.size
    for (i in 0 until n) {
        val prev = pts[(i - 1 + n) % n]; val cur = pts[i]; val nxt = pts[(i + 1) % n]
        val v1x = cur.x - prev.x; val v1y = cur.y - prev.y; val v2x = nxt.x - cur.x; val v2y = nxt.y - cur.y
        val l1 = maxOf(hypot(v1x, v1y), 0.001f); val l2 = maxOf(hypot(v2x, v2y), 0.001f)
...
private fun lineP(a: Offset, b: Offset): Path = Path().apply { moveTo(a.x, a.y); lineTo(b.x, b.y) }
...
private fun stepSim(t: Double, dt: Double, glyphs: MutableList<Glyph>, n: Int) {
    ...
    val r = 44f
    for (gl in glyphs) {
        gl.x += gl.vx * dt.toFloat(); gl.y += gl.vy * dt.toFloat()
        if (gl.x < 85) { gl.x = 85f; gl.vx = kotlin.math.abs(gl.vx) }
        if (gl.x > DW - 85) { gl.x = DW - 85; gl.vx = -kotlin.math.abs(gl.vx) }
        if (gl.y < 100) { gl.y = 100f; gl.vy = kotlin.math.abs(gl.vy) }
        if (gl.y > 360) { gl.y = 360f; gl.vy = -kotlin.math.abs(gl.vy) }
    ...
    for (i in 0 until glyphs.size - 1) for (j in i + 1 until glyphs.size) {
        val a = glyphs[i]; val b = glyphs[j]
        val dx = b.x - a.x; val dy = b.y - a.y; val d = hypot(dx, dy)
        if (d < r * 2 && d > 0.001f) {
            val nx = dx / d; val ny = dy / d; val overlap = (r * 2 - d) / 2
            a.x -= nx * overlap; a.y -= ny * overlap; b.x += nx * overlap; b.y += ny * overlap
            val pi = a.vx * nx + a.vy * ny; val pj = b.vx * nx + b.vy * ny
            a.vx += (pj - pi) * nx; a.vy += (pj - pi) * ny; b.vx += (pi - pj) * nx; b.vy += (pi - pj) * ny
            if (n > 1) { a.prov = (a.prov + Random.nextInt(1, n)) % n; b.prov = (b.prov + Random.nextInt(1, n)) % n }
            a.formT = 0.0; a.seed = Random.nextLong(); b.formT = 0.0; b.seed = Random.nextLong()
...
fun BurnBarLogoFormation(modifier: Modifier = Modifier) {
    ...
    val providerDots = remember {
        listOf(
            R.drawable.logo_anthropic, R.drawable.logo_open_ai, R.drawable.google_logo, R.drawable.logo_mistral,
            R.drawable.logo_meta, R.drawable.logo_grok, R.drawable.logo_deep_seek, R.drawable.logo_qwen,
    ...
    val glyphs = remember {
        mutableListOf<Glyph>().apply {
            repeat(3) { i ->
                add(Glyph(Random.nextDouble(120.0, (DW - 120).toDouble()).toFloat(), Random.nextDouble(115.0, 345.0).toFloat(),
                    Random.nextDouble(-14.0, 14.0).toFloat(), Random.nextDouble(-14.0, 14.0).toFloat(),
                    (i * 3) % providerCount, 0.0, 0.0, Random.nextLong()))
            }
        }
    }
    ...
    LaunchedEffect(Unit) {
        ...
        while (true) {
            withFrameNanos { now ->
                if (last == 0L) last = now
                val dt = ((now - last).toDouble() / 1e9).coerceAtMost(0.05); last = now
                t += dt
    ...
    Canvas(modifier = modifier) {
        ...
        val ox = (size.width - DW * s) / 2f; val oy = (size.height - DH * s) / 2f
        translate(ox, oy) {
...
private fun DrawScope.drawFormation(
    t: Double, glyphs: List<Glyph>, dots: List<Dot>, providerDots: List<List<GDot>>,
    logoImg: ImageBitmap?, oilBmp: Bitmap, oilPx: IntArray,
) {
    val p = min(t / 5.0, 1.0)
    ...
    for (d in dots) {
        ...
        val x = lerp(fx, d.target.x, local); val y = lerp(fy, d.target.y, local)
        val appear = smooth(p / 0.12)
        val alpha = appear * (1 - solidFade * 0.92) * (0.45 + 0.55 * local)
        if (alpha <= 0.01) continue
        val warm = local
        val cr = 0.55 + (d.r - 0.55) * warm; val cg = 0.40 + (d.g - 0.40) * warm; val cb = 0.45 + (d.b - 0.45) * warm
    ...
    if (glass > 0.001) {
        // contact shadow
        drawOval(Color.Black.copy(alpha = 0.5f * glass.toFloat()),
            topLeft = Offset(CUBE_CX - CUBE_S * 0.85f, pts.F.y + CUBE_S * 0.1f - CUBE_S * 0.23f),
            size = Size(CUBE_S * 1.7f, CUBE_S * 0.46f))
        clipPath(sil) {
            drawPath(cubeRight(pts), hx("0C0D14")); drawPath(cubeRight(pts), hx("171924").copy(alpha = glass.toFloat()))
        ...
        if (glyphAppear > 0.001 && providerDots.isNotEmpty()) {
            for (gl in glyphs) {
                ...
                for ((k, d) in gd.withIndex()) {
                    val tx = gl.x + d.px * 98f; val ty = gl.y + d.py * 98f
                    val off = scatterOffset(gl.seed, k)
                    val sx = gl.x + off.x * 1.5f; val sy = gl.y + off.y * 1.5f
                    val x = sx + (tx - sx) * f.toFloat(); val y = sy + (ty - sy) * f.toFloat()
        ...
        clipPath(sil) {
            val topEdge = Path().apply { moveTo(pts.D.x, pts.D.y); lineTo(pts.C.x, pts.C.y); lineTo(pts.B.x, pts.B.y) }
...
private fun DrawScope.drawImageFitted(img: ImageBitmap, center: Offset, size: Float, alpha: Float) {
    val iw = img.width.toFloat(); val ih = img.height.toFloat(); val sc = size / maxOf(iw, ih)
    val w = iw * sc; val h = ih * sc
    drawImage(img, dstOffset = IntOffset((center.x - w / 2).toInt(), (center.y - h / 2).toInt()),
        dstSize = IntSize(w.toInt(), h.toInt()), alpha = alpha)
}

private fun computeOil(t: Double, bmp: Bitmap, px: IntArray) {
    val n = 68
    for (yy in 0 until n) for (xx in 0 until n) {
        val ux = (xx + 0.5) / n; val uy = (yy + 0.5) / n
        val pxn = ux * 3.2; val pyn = uy * 3.2
        val qx = fbm2(pxn + 0.15 * t, pyn + 0.15 * t); val qy = fbm2(pxn + 5.2 + 0.12 * t, pyn + 1.3 + 0.12 * t)
