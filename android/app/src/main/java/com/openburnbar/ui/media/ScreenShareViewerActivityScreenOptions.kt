...
internal fun ScreenShareViewerActivity.screenShareViewerScreenOptions(
    ...
    ScreenShareViewerScreenOptions(
        ...
        selectedDisplayId = ui.activeDisplayId.orEmpty(),
        ...
        onTapNormalized = { x, y, mouseButton, displayId -> sendTapNormalized(x, y, mouseButton, displayId.orEmpty()) },
        onScrollDragNormalized = { x1, y1, x2, y2, displayId -> sendScrollDragNormalized(x1, y1, x2, y2, displayId.orEmpty()) },
        onScrollNormalized = { deltaY, displayId -> sendScrollNormalized(deltaY, displayId.orEmpty()) },
        onPointerMove = { dx, dy -> sendPointerMoveNormalized(dx, dy, ui.activeDisplayId.orEmpty()) },
        onPointerClick = { mouseButton -> sendPointerClickNormalized(mouseButton, ui.activeDisplayId.orEmpty()) },
        onTypeText = { text -> sendTypeTextNormalized(text, ui.activeDisplayId.orEmpty()) },
        onShortcut = { key, modifiers -> sendShortcutNormalized(key, modifiers, ui.activeDisplayId.orEmpty()) },
        onPanic = { sendPanicNormalized(ui.activeDisplayId.orEmpty()) },
