...
internal fun communityGeoDisplayLabel(tier: CommunityGeoTier, geoKey: String): String = when (tier) {
...
internal fun communityGeoConfidenceCopy(tier: CommunityGeoTier, geoKey: String): String = when (tier) {
    CommunityGeoTier.CITY ->
        if (geoKey.isBlank() || geoKey == tier.wire) {
...
internal fun peerComparisonSparklineData(percentiles: FirestorePercentileBands, yourTokens: Long): List<Float>? {
    val bands =
        listOf(
            percentiles.p50,
            percentiles.p75,
...
internal fun shouldShowPeerComparisonChart(cohortSize: Long, percentiles: FirestorePercentileBands, yourTokens: Long): Boolean {
