...
class HermesSquareFeatureFlags private constructor(context: Context) {
    ...
    var phaseA: Boolean by mutableStatePref(KEY_PHASE_A)
    var phaseB: Boolean by mutableStatePref(KEY_PHASE_B)
    var phaseC: Boolean by mutableStatePref(KEY_PHASE_C)
    var phaseD: Boolean by mutableStatePref(KEY_PHASE_D)
    ...
    private fun mutableStatePref(key: String) = object : kotlin.properties.ReadWriteProperty<Any?, Boolean> {
        private var cached = mutableStateOf(prefs.getBoolean(key, false))
