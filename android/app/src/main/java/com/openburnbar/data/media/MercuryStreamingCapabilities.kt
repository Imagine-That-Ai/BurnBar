...
object MercuryDatagramCapabilityProbe {
    fun snapshot(maxDatagramSize: Int?): MercuryDatagramCapability = MercuryDatagramCapability(maxPayloadBytes = maxDatagramSize?.takeIf { it > 0 })
...
data class MercuryStreamingCapabilitySnapshot(
    ...
    fun capability(codec: MercuryVideoCodec): MercuryVideoCodecCapability? = codecCapabilities.firstOrNull { it.codec == codec }
...
data class MercuryCodecPolicy(
    ...
    val preferredCodecs: List<MercuryVideoCodec> =
        listOf(
...
object MercuryWireVersionNegotiator {
    fun resolve(local: MercuryMediaFrameVersionSupport, remote: MercuryMediaFrameVersionSupport): MercuryMediaFrameWireVersion =
        if (local.supportsV2 && remote.supportsV2) MercuryMediaFrameWireVersion.V2 else MercuryMediaFrameWireVersion.V1

    fun canSendMetadataV2(local: MercuryMediaFrameVersionSupport, remote: MercuryMediaFrameVersionSupport): Boolean =
        resolve(local, remote) == MercuryMediaFrameWireVersion.V2
}
...
fun MercuryVideoCodecCapability.toWire(): HermesRealtimeRelayVideoCodecCapability = HermesRealtimeRelayVideoCodecCapability(
    codec = codec.toWire(),
    canEncode = canEncode,
    canDecode = canDecode,
...
fun HermesRealtimeRelayVideoCodecCapability.toMercury(): MercuryVideoCodecCapability = MercuryVideoCodecCapability(
...
fun MercuryMediaFrameVersionSupport.toWire(): HermesRealtimeRelayMediaFrameVersionSupport = HermesRealtimeRelayMediaFrameVersionSupport(
    supportsV1 = supportsV1,
    supportsV2 = supportsV2,
)

fun HermesRealtimeRelayMediaFrameVersionSupport.toMercury(): MercuryMediaFrameVersionSupport = MercuryMediaFrameVersionSupport(
    supportsV1 = supportsV1,
    supportsV2 = supportsV2,
)

fun MercuryDatagramCapability.toWire(): HermesRealtimeRelayDatagramCapability = HermesRealtimeRelayDatagramCapability(maxPayloadBytes = maxPayloadBytes)

fun HermesRealtimeRelayDatagramCapability.toMercury(): MercuryDatagramCapability = MercuryDatagramCapability(maxPayloadBytes = maxPayloadBytes)

fun MercuryStreamingCapabilitySnapshot.toWire(): HermesRealtimeRelayStreamingCapabilities = HermesRealtimeRelayStreamingCapabilities(
...
fun HermesRealtimeRelayStreamingCapabilities.toMercury(): MercuryStreamingCapabilitySnapshot = MercuryStreamingCapabilitySnapshot(
