...
data class ChatTilePreferences(
    ...
    fun setSelectedHermesModel(id: String?): ChatTilePreferences = copy(selectedHermesModelOverride = id?.trim()?.takeIf { it.isNotEmpty() })

    fun sanitized(): ChatTilePreferences = if (enabledTiles.isEmpty()) copy(enabledTiles = setOf(AssistantRuntimeID.HERMES)) else this

    fun orderedVisibleTiles(): List<AssistantRuntimeID> = AssistantRuntimeID.values().filter { enabledTiles.contains(it) }

    fun orderedVisibleHermesSubProviders(): List<HermesSubProvider> = HermesSubProvider.values().filter { enabledHermesSubProviders.contains(it) }
    ...
    companion object {
        ...
        private fun extractStringArray(json: String, key: String): List<String> {
            ...
            val arrStart = if (keyIdx >= 0) json.indexOf('[', startIndex = keyIdx) else -1
            val arrEnd = if (arrStart >= 0) json.indexOf(']', startIndex = arrStart) else -1
            val inner =
                if (arrEnd > arrStart) json.substring(arrStart + 1, arrEnd).trim() else ""
        ...
        private fun extractStringValue(json: String, key: String): String? {
            val keyPattern = "\"$key\""
            val keyIdx = json.indexOf(keyPattern)
            val colonIdx = if (keyIdx >= 0) json.indexOf(':', startIndex = keyIdx) else -1
            val valueStart = if (colonIdx >= 0) json.indexOf('"', startIndex = colonIdx + 1) else -1
            ...
            return builder.toString().takeIf { closed }
        }
    }
}
