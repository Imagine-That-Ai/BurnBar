...
internal object FirestoreRollupMerger {
     ...
     */
    fun mergeCollectionDocs(documents: List<DocumentSnapshot>): UsageRollups = mergeWindowDocs(
        documents
    ...
    private fun buildMergedRollups(allDocs: Map<String, UsageRollups>, allTimeDoc: UsageRollups): UsageRollups = UsageRollups(
    ...
    private fun windowCost(allDocs: Map<String, UsageRollups>, key: String, fallback: UsageRollups): Double = allDocs[key]?.costUsd() ?: fallback.costUsd()

    private fun windowTokens(allDocs: Map<String, UsageRollups>, key: String, fallback: UsageRollups): Long = allDocs[key]?.tokens() ?: fallback.tokens()

    private fun windowRequests(allDocs: Map<String, UsageRollups>, key: String, fallback: UsageRollups): Int = allDocs[key]?.requests() ?: fallback.requests()

    private fun UsageRollups.costUsd(): Double = totals["costUsd"] ?: 0.0

    private fun UsageRollups.tokens(): Long = listOfNotNull(
        totals["tokens"],
        today.takeIf { it > 0 },
