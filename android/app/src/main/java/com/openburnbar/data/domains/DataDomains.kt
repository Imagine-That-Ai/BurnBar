...
object DataDomains {
    val all: List<DataDomain> = listOf(
        ...
        DataDomain(
            ...
            encryptionTier = EncryptionTier.SERVER_READABLE, summary = "Your paired Macs, phones, and relays (Hermes, Pi agent, iroh) and which can talk to your account. NOTE: end-to-end relay frames AND the hosted chat gateway are both sealed and never readable by the server — the gateway routes ciphertext per-link and never reads message text, sender names, or attachment file names. The per-agent subscription graph is cloaked behind opaque keyed doc ids.",
            serverSees = listOf("device ids", "pairing metadata", "last seen", "relay routing", "opaque relay key material (public keys)"), deviceOnly = listOf("end-to-end relay + gateway frame contents (message text, sender names, attachment file names — all sealed on-device)"),
