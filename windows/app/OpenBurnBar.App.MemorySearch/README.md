...
Windows TFM, zero P/Invoke, zero UI framework, zero network client — the SAME managed assembly the
WinUI shell links is unit-tested on the macOS authoring host today (`dotnet test`). Mirrors the
proven `OpenBurnBar.App.TextExpansion` / `OpenBurnBar.App.Settings` portable-core pattern.

Tests: `windows/tests/memory-search/` (net10.0 xUnit, 138 tests).
...
## The injectable seams (network lives in bucket B)

The real embedding + LLM network calls are the OS/network adapter and are **deferred to bucket B**.
Everything here rides fakeable seams:

- `IEmbeddingProvider` (chunk + query) — text → `float[]`; deterministic fake ships here.
