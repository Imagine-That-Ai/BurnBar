...
internal sealed class ParserGoldenRoot
{
    ...
    public ParserGoldenFixture[] Fixtures { get; init; } = Array.Empty<ParserGoldenFixture>();
}
...
internal sealed class ParserGoldenFixture
{
    ...
    public ParserContractUsage[] Usages { get; init; } = Array.Empty<ParserContractUsage>();
}
...
public static class ParserContractGolden
{
    ...
    public static ParserContractUsage LoadClaudeCodeSessionUsage()
    {
        ...
        var root = JsonSerializer.Deserialize<ParserGoldenRoot>(json, JsonOptions)
            ?? throw new InvalidOperationException("parser-output-golden.json did not deserialize.");
