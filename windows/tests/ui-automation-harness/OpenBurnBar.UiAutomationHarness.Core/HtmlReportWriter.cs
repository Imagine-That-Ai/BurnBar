...
public static class HtmlReportWriter
{
    public static void Write(string path, UiHarnessRunSummary summary, ArtifactRedactor redactor)
    {
        ...
        html.AppendLine("<h2>Routes</h2><table><thead><tr><th>Route</th><th>Verdict</th><th>Size</th><th>Luma StdDev</th><th>Evidence</th><th>Message</th></tr></thead><tbody>");
        ...
        html.AppendLine("<h2>Input Route Contract</h2><table><thead><tr><th>Action</th><th>Route</th><th>Audit Kind</th><th>Token Required</th></tr></thead><tbody>");
        foreach (InputRouteEvidence input in summary.InputRoutes)
        {
            html.AppendLine($"<tr><td>{Esc(input.ActionKind)}</td><td>{Esc(input.DispatchRoute)}</td><td><code>{Esc(input.AuditKind)}</code></td><td>{input.RequiresCapabilityToken}</td></tr>");
        }
