...
| [`cloudsync/`](cloudsync/) | The **Firestore CloudSync gateway** — the portable, dependency-free `net8.0` C# port of the macOS `AgentLens/Services/CloudSync/*` Firestore seam (unavailable off-Apple, where the Firebase SDK is gone). Ships `OpenBurnBar.CloudSync`: the **14 Firestore model domains** (`Models/Firestore*Models.cs`, parity with `OpenBurnBarCore/.../OpenBurnBarFirestoreModels/*`), the `ICloudSyncGateway` abstraction (parity with `CloudSyncFirestoreGateway.swift`) implemented as a **REST/RPC v1 client** over `documents.get` / `.patch` / `:commit` / `:runQuery` / `.delete` behind an `ICloudSyncHttpTransport` seam, the Firestore **typed-value document codec** + canonical (sorted-key) JSON, the **Firebase callable envelope** (`{data}`/`{result}`/`{error}`, e.g. `mintWindowsAppCheckToken`), a **FakeGateway** (parity with `CloudSyncFirestoreFakeGateway.swift`) that drives the **offline-write-queue + snapshot-listener** state machines, and the **E2EE fail-closed** write guard. The live authenticated Firestore/callable round-trip is dev-host/CI-deferred (there is no Firebase SDK to bind on macOS); the tests prove request SHAPES + response PARSING + byte-parity vectors. | W-CloudSync, G2 |
