...
| [`cloudsync/`](cloudsync/) | The **data & sync / cloud crypto** layer. `OpenBurnBar.CloudSync.Crypto` is the portable (net8.0) C# port of the zero-knowledge **CloudVault E2EE crypto** (parity: `OpenBurnBarCore/.../CloudVaultCrypto.swift` + `apps/console/lib/escrow.ts`): the P-256 ECDH + HKDF-SHA256 + AES-256-GCM **escrow key-wrap** (a vault key wrapped by a trusted native device unwraps here and vice-versa), `sealText`/`sealBlob`/`sealPayload` (AES-256-GCM detached + combined, v1 no-AAD + v2 path-bound AAD, vault-keyed plaintext-HMAC integrity), the symmetric recovery-key wrap, the vault-keyed HMAC hashes, and `vaultKeyID`/SHA-256 — all **fail-closed** (E2EE; the server never sees the vault key or plaintext). AES/HKDF/HMAC/SHA come from the .NET BCL; P-256 from cross-platform BouncyCastle. Proven by `dotnet test` on macOS (`tests/cloudsync`) with **byte-parity in BOTH directions** against a committed Apple-CryptoKit-origin KAT vector (`tools/CloudVaultVectorGen`). | W-Cloud, G2 |
...
must live under one of the ten documented sub-trees above. The per-tree budget
