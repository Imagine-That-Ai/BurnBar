...
| [`integrations/`](integrations/) | The **home-automation integrations** — `integrations/homeassistant/` (portable net8.0 HA REST message model + response/error mapping + entity-state→MediaPlayer Cast projection + webhook-id + blueprint installer + token/config stores behind interfaces; `.Net` = the `HttpClient` transport adapter) and `integrations/smarthub/` (portable net8.0 Nest-Hub bridge route dispatcher + `/state.json` contract + constant-time bridge-token auth + snapshot/display-config models, and the DNS-SD/mDNS record build+parse that replaces Bonjour's opaque bytes; `.Net` = the live `HttpListener` host + `UdpClient` mDNS responder + `NetworkInterface` LAN enumerator). Parity sources: `AgentLens/Services/HomeAssistant/*` + `AgentLens/Services/SmartHub/*BridgeServer` + `LocalNetworkDiscovery`. | W7 |
| [`tests/`](tests/) | **Unit, integration, and parity test projects** for the app, PAL, and native shim. Includes `tests/presentation` (net10.0, xUnit) — the macOS-runnable proof of the SessionLogs + Memory + Switcher view-models: the inbox model, grouping/filter, transcript parser, an end-to-end SessionLogs read through the SQLCipher adapter against the committed byte-compat fixture, and the account-switcher destination routing / grouping / form-validation / add-switch-drain state machine. Also `tests/integrations` (net10.0, xUnit) — the HA REST round-trips against recorded fixtures, the SmartHub bridge route dispatch + `/state.json` contract + auth, the DNS-SD/mDNS record bytes, and real HTTP round-trips through the live `HttpListener` host + `HttpClient` transport on loopback. | W7, W11 |
