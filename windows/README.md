...
| [`cloudsync/`](cloudsync/) | **Cloud-sync + auth clients.** Ships `cloudsync/appcheck/` — the **Windows App Check client** (Phase 2 · AC-011) that consumes the Phase-0 mint backend (`functions/src/callables/windowsAppCheck.ts`). A portable `net8.0` core (`OpenBurnBar.CloudSync.AppCheck`): the `IAttestationProducer` seam + the `MockAttestationProducer` (byte-parity with the server's `signMockAttestation`), the Firebase-callable mint client (request-encode / response-decode / fail-closed classification) behind an `IAppCheckMintTransport` network seam, the `AppCheckToken` TTL/expiry model, and `WindowsAppCheckProvider` (install + TTL-aware refresh + single-flight + fail-closed `X-Firebase-AppCheck` attachment); a `net10.0` HttpClient transport (`OpenBurnBar.CloudSync.AppCheck.Net`); and the `net8.0-windows` real TPM producer (`OpenBurnBar.CloudSync.AppCheck.Windows`: CNG `NCryptCreateClaim`, R14/AC-013/dev-host-deferred). Proven by `dotnet test` on macOS (`tests/cloudsync-appcheck`) incl. the golden mock-MAC parity vectors and a live-loopback `HttpListener` round-trip. | W2, R14 |
