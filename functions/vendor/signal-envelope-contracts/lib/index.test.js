...
import { SIGNAL_AT_REST_ENCRYPTION, SIGNAL_BINDING_AAD_PREFIX, SIGNAL_ENVELOPE_FORMAT_VERSION, SIGNAL_RELAY_KEY_VERSION, SIGNAL_TRANSPORT_ENCRYPTION, bindingToAAD, sanitizeSignalEnvelope, sanitizeSignalEnvelopeForExport, } from "./index.js";
...
const bindingAADFixture = JSON.parse(readFileSync(fileURLToPath(fixtureUrl), "utf8"));
...
test("rejects downgrade, mode confusion, malformed base64, and plaintext siblings", () => {
    ...
    assert.equal(sanitizeSignalEnvelope({
        ...transportEnvelope(),
...
test("sanitizeSignalEnvelopeForExport FAILS CLOSED on a signal-shaped but invalid envelope (no raw plaintext passthrough)", () => {
    ...
    assert.ok(!("INJECTED_PLAINTEXT" in out));
...
test("bindingToAAD produces the deterministic v1 canonical string for transport and at-rest bindings", () => {
    const transport = transportEnvelope().binding;
    assert.equal(bindingToAAD(transport), `${SIGNAL_BINDING_AAD_PREFIX}transport|gateway|uid-1|client-1||||event-1|1`);
    const atRest = atRestEnvelope().binding;
    assert.equal(bindingToAAD(atRest), `${SIGNAL_BINDING_AAD_PREFIX}at-rest|cloudvault|uid-1||session_logs|doc-1|body||1`);
...
test("bindingToAAD keeps absent-optional positions stable as empty segments", () => {
    ...
    const pipeCount = (s) => s.split("|").length - 1;
...
test("bindingToAAD throws fail-closed on a pipe- or CRLF-injected segment", () => {
    for (const injected of ["a|b", "line\r", "line\n", "x|\n"]) {
        assert.throws(() => bindingToAAD({
            uid: injected,
            scope: "gateway",
            mode: "transport",
            formatVersion: SIGNAL_ENVELOPE_FORMAT_VERSION,
        }), /reserved '\|' or CR\/LF/, `injection of ${JSON.stringify(injected)} must throw`);
    }
...
test("bindingToAAD matches the shared cross-language fixture byte-for-byte", () => {
    ...
    for (const vector of bindingAADFixture.vectors) {
        assert.equal(bindingToAAD(vector.binding), vector.expectedAAD, `${vector.name}: TS output must equal the vendored expectedAAD`);
    }
});
test("bindingToAAD normalizes NFD to the same bytes as NFC (cross-platform Unicode parity)", () => {
    ...
    const nfcBinding = { ...nfcVector.binding };
    const nfdBinding = { ...nfcVector.binding, uid: nfdUid };
    ...
    for (const vector of bindingAADFixture.vectors) {
        if (vector.name === "non-ascii-nfc-uid")
            continue;
