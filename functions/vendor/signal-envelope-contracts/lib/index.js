export { isCloudVaultSignalEnvelope, sanitizeCloudVaultSignalEnvelope, } from "./cloudVaultSignalEnvelope.js";
...
function counter(raw) {
    ...
    if (!Number.isSafeInteger(value) || value < 0 || value > SIGNAL_MAX_COUNTER)
        return undefined;
    return value;
}
...
function sanitizeCiphertextLayer(raw) {
    ...
    if (!payloadCiphertextB64 || !payloadAADLabel || schemaVersion === undefined)
        return undefined;
    return { payloadCiphertextB64, payloadAADLabel, schemaVersion };
}
function sanitizeBinding(raw, expected) {
    const record = recordOrUndefined(raw);
    if (!record)
        return undefined;
...
function sanitizeTransportDelivery(raw) {
    const record = recordOrUndefined(raw);
    if (!record || record.scheme !== SIGNAL_TRANSPORT_ENCRYPTION)
        return undefined;
    const signalMessageType = record.signalMessageType === 2 || record.signalMessageType === 3 ? record.signalMessageType : undefined;
...
function sanitizeAtRestDelivery(raw) {
    const record = recordOrUndefined(raw);
    if (!record || record.scheme !== SIGNAL_AT_REST_ENCRYPTION)
        return undefined;
    if (record.contentKeyLength !== 32 || !Array.isArray(record.wraps))
        return undefined;
    if (record.wraps.length < 1 || record.wraps.length > SIGNAL_MAX_RECIPIENT_WRAPS)
        return undefined;
    const wraps = record.wraps.flatMap((item) => {
        const wrap = recordOrUndefined(item);
        if (!wrap)
            return [];
        const recipientKind = wrap.recipientKind === "device" || wrap.recipientKind === "escrow" || wrap.recipientKind === "recovery"
            ? wrap.recipientKind
            : undefined;
        ...
        if (!recipientKind || !recipientIdentityKeyId || !recipientIdentityKeyB64 || !sealedContentKeyB64)
            return [];
...
export function sanitizeSignalEnvelope(raw, expectedMode) {
    const record = recordOrUndefined(raw);
    if (!record)
        return undefined;
    const signalEnvelopeFormatVersion = counter(record.signalEnvelopeFormatVersion);
    const mode = record.mode === "transport" || record.mode === "at-rest" ? record.mode : undefined;
    if (signalEnvelopeFormatVersion !== SIGNAL_ENVELOPE_FORMAT_VERSION ||
        !mode ||
    ...
    if (record.relayEncryption !== expectedEncryption)
        return undefined;
    const rawRelayKeyVersion = record.relayKeyVersion === undefined
        ? undefined
        : typeof record.relayKeyVersion === "number"
            ? Math.floor(record.relayKeyVersion)
            : Number(record.relayKeyVersion);
    if (mode === "transport") {
        if (rawRelayKeyVersion !== SIGNAL_RELAY_KEY_VERSION)
            return undefined;
    }
    ...
    const keyDelivery = mode === "transport" ? sanitizeTransportDelivery(record.keyDelivery) : sanitizeAtRestDelivery(record.keyDelivery);
    const binding = sanitizeBinding(record.binding, { mode, scope: mode === "transport" ? "gateway" : undefined });
    if (!ciphertextLayer || !keyDelivery || !binding)
        return undefined;
    return stripUndefined({
        signalEnvelopeFormatVersion,
        mode,
...
export function isSignalEnvelope(value, expectedMode) {
 ...
 * domain separation.
 */
export function bindingToAAD(binding) {
    const segments = [
        binding.mode,
        binding.scope,
        binding.uid,
...
export function sanitizeSignalEnvelopeForExport(path, envelope) {
...
function collectNestedDrops(path, envelope, dropped) {
    ...
    if (!keyDelivery)
        return;
    const allowed = keyDelivery.scheme === SIGNAL_AT_REST_ENCRYPTION
...
function collectDisallowedKeys(path, data, allowed) {
    const dropped = [];
