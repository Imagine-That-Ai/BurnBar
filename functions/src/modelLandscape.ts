...
export function normalizeArtificialAnalysisModels(payload: unknown, fetchedAt: string): ModelBenchmarkSnapshotDoc[] {
  ...
  return rows.flatMap((row): ModelBenchmarkSnapshotDoc[] => {
    ...
    const codingScore = scoreToUnit(
      asNumber(evaluations?.artificial_analysis_coding_index) ?? asNumber(evaluations?.livecodebench),
    );
    const generalScore = scoreToUnit(
      asNumber(evaluations?.artificial_analysis_intelligence_index) ?? asNumber(evaluations?.mmlu_pro),
    );
    const inputPrice = asNumber(pricing?.price_1m_input_tokens);
    const outputPrice = asNumber(pricing?.price_1m_output_tokens);
    const blended =
      asNumber(pricing?.price_1m_blended_3_to_1) ??
    ...
    const base = {
      ...
      providerID,
      costSignal,
      inputCostPerMtoken: inputPrice,
      outputCostPerMtoken: outputPrice,
      blendedCostPerMtoken: blended,
      latencySignal,
      confidence: 0.8,
...
export function normalizeHuggingFaceLeaderboard(
  ...
  const rows = Array.isArray(payload) ? payload : asArray(asRecord(payload)?.leaderboard ?? asRecord(payload)?.data);
  return rows.flatMap((row): ModelBenchmarkSnapshotDoc[] => {
    const item = asRecord(row);
    if (!item) return [];
...
export function normalizeDesignArenaFixture(payload: unknown, fetchedAt: string): ModelBenchmarkSnapshotDoc[] {
  const rows = Array.isArray(payload) ? payload : asArray(asRecord(payload)?.data ?? asRecord(payload)?.leaderboard);
  return rows.flatMap((row): ModelBenchmarkSnapshotDoc[] => {
    const item = asRecord(row);
    if (!item) return [];
...
export function normalizeDesignArenaModels(payload: unknown, fetchedAt: string): ModelBenchmarkSnapshotDoc[] {
  const root = asRecord(payload);
  const rows = Array.isArray(payload) ? payload : asArray(root?.data ?? root?.models);
  ...
  return rows.flatMap((row): ModelBenchmarkSnapshotDoc[] => {
    ...
    for (const [arena, arenaValue] of Object.entries(rankings)) {
      ...
      for (const [category, metricValue] of Object.entries(arenaRecord)) {
        ...
        const score =
          scoreToUnit(asNumber(metrics.normalizedScore) ?? asNumber(metrics.score)) ??
        ...
        const reliabilitySignal = scoreToUnit(asNumber(metrics.winRate) ?? asNumber(metrics.win_rate));
        const confidence = confidenceToUnit(asNumber(metrics.confidence)) ?? 0.75;

        if (score == null && rank == null && latencySignal == null && reliabilitySignal == null) {
          continue;
        }

        snapshots.push({
          id: stableSnapshotID("design_arena", [modelID, arena, category].join("-"), taskCategory, fetchedAt),
          source: "design_arena",
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (response.status === 429 || response.status >= 500) {
        ...
        const wait =
          Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1000 : baseBackoff * Math.pow(2, attempt);
        if (attempt < retries) {
...
export async function collectModelLandscapeBenchmarks(
  ...
  if (artificialAnalysisKey) {
    try {
      ...
      statuses.push(
        status("artificial_analysis", "error", `Artificial Analysis refresh failed: ${errorMessage(err)}`, fetchedAt),
      );
    }
  } else {
    statuses.push(
  ...
  try {
    ...
    statuses.push(
      status(
        "terminal_bench",
        "error",
        `Terminal-Bench refresh failed: ${errorMessage(err)}`,
        fetchedAt,
        undefined,
        "Terminal-Bench / Hugging Face",
      ),
    ...
    try {
      ...
      statuses.push(
        status(
          "design_arena",
          "error",
          `Design Arena fixture failed: ${errorMessage(err)}`,
          fetchedAt,
          undefined,
          "Design Arena",
        ),
    ...
    try {
      ...
      statuses.push(
        status(
          "manual_fixture",
          "fresh",
          `Normalized ${normalized.length} manual fixture rows.`,
          fetchedAt,
          fetchedAt,
...
export async function writeModelLandscapeBenchmarks(db: Firestore, result: ModelLandscapeRefreshResult): Promise<void> {
