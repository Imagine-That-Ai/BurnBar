 ...
 * Provider webhooks use generous ceilings because the caller identity is the
 * provider's egress IP pool, which can shift during retries.
 */
const PUBLIC_HTTP_ENDPOINT_LIMITS: Record<PublicHttpEndpointName, { windowSeconds: number; maxAttempts: number }> = {
  // Apple App Store Server Notifications V2: signed provider webhook. 120/min
  // per egress IP absorbs normal retry bursts without allowing unbounded abuse.
  appStoreServerNotificationsV2: { windowSeconds: 60, maxAttempts: 120 },
  ...
  // Public read-only JSON endpoint; cached and mostly idempotent.
  latestRouterRundown: { windowSeconds: 60, maxAttempts: 60 },
  // GitHub push webhook: signed provider webhook, modest traffic per repo.
  onKnowledgeRepoPush: { windowSeconds: 60, maxAttempts: 60 },
  ...
  // Stripe webhook: signed provider webhook. 120/min per egress IP covers
  // normal event bursts while capping signature-verification CPU spend.
  stripeBurnBarProWebhook: { windowSeconds: 60, maxAttempts: 120 },
};
...
/**
 * Declared rate limit for a callable. UIDs are the primary identity; when an
 * App Check attestation is present it is folded into the key so a revoked or
 * spoofed app cannot ride on a UID's quota.
 */
...
/**
 * Rate-limit a callable by authenticated identity (UID + App Check when
 * available). This is the callable counterpart to checkPublicHttpEndpointRateLimit
 * and is consumed by onCallProduction's optional rateLimit option.
 */
export async function checkCallableRateLimit(
  ...
  const identity = uid ? `uid:${uid}` : "unauthenticated";
  const keyMaterial = appCheckAppId ? `${identity}:appcheck:${appCheckAppId}` : identity;
  await incrementRateLimit(rateLimitDocId(keyMaterial, limit.action), limit.action, limit);
}
