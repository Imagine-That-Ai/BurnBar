/**
 * Shared fixture for the B-SEC-2 knowledge dedup-hash suites
 * (knowledgeMemoryDedupHash.test.ts and knowledgeMemoryDedupFlagDay.test.ts).
 *
 * Provides the in-memory Firestore double plus the device-side derivation
 * helpers (vault-keyed HMAC dedup hashes, sealed payload builders, callable
 * request builders). Each test file declares its own `vi.mock` blocks and
 * wires `makeDb()` / `FIELD_DELETE` into them via dynamic import so the
 * factories stay hoist-safe.
 *
 * Not a test file on purpose: vitest only collects `src/__tests__/**\/*.test.ts`.
 */
import { createHash, createHmac, hkdfSync } from "node:crypto";

import { runFakeFirestoreTransaction } from "./fakeFirestoreTransaction.js";

// Sentinel used by the firebase-admin/firestore mock's FieldValue.delete().
