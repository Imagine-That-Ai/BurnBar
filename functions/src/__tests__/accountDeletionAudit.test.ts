...
import type { DeleteUserAccountOptions } from "../accountDeletion.js";
...
function baseOptions(
  overrides: Partial<{
    ...
    appendAuditEventRequired: () => Promise<unknown>;
    appendAuditEvent: () => Promise<unknown>;
  }> = {},
  ...
  return {
    ...
    audit: {
      actor: "user:ios",
      domain: "account",
      appendAuditEventRequired: (overrides.appendAuditEventRequired ?? appendAuditEventRequired) as never,
      appendAuditEvent: (overrides.appendAuditEvent ?? appendAuditEvent) as never,
    },
  };
}
