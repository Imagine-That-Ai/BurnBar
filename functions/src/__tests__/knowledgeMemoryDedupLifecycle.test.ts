/**
 * Dedup-v0 flag-day lifecycle — search never serves v0 rows, purge deletes
 * them, and the commit-path cap aggregate excludes legacy v0 rows so a
 * near-cap user can re-ingest. Split from `knowledgeMemoryDedupHash.test.ts`
 * (which keeps the B-SEC-2 side-channel suite) to stay under the max-lines
 * lint floor; the shared Firestore double and derivation helpers live in
 * `knowledgeDedupTestHarness.ts`.
 */
...
import {
  ...
  stored,
  vectorForMutation,
} from "./knowledgeDedupTestHarness.js";
...
// wrappers + aggregate the loop reads. The FIELD_DELETE sentinel and the db
// double come from the shared harness (async factory: vi.mock hoisting).
vi.mock("firebase-admin/firestore", async () => {
  const { FIELD_DELETE } = await import("./knowledgeDedupTestHarness.js");
  return {
