...
function requireId(value: unknown): string {
  ...
  throw new Error("expected attachment id");
}
...
describe("burnbarAttachments", () => {
  ...
  it("finalize meters metadata.size not the client lie", async () => {
    ...
    const path = `users/alice-bola-uid/burnbar_attachments/${requireId(begun)}/final`;
    await memoryStoragePort.mintPutUrl(path, 500, 60);
    await putMemoryObject(path, 500);
    const result = await runFinalize(authed({ id: requireId(begun) }));
  ...
  it("second finalize of same generation returns 200; swapped bytes fail", async () => {
    ...
    const finalPath = `users/alice-bola-uid/burnbar_attachments/${requireId(begun)}/final`;
    await memoryStoragePort.mintPutUrl(finalPath, 64, 60);
    await putMemoryObject(finalPath, 64);
    const first = await runFinalize(authed({ id: requireId(begun) }));
    expect(first).toMatchObject({ ok: true });
    const second = await runFinalize(authed({ id: requireId(begun) }));
    ...
    await expect(runFinalize(authed({ id: requireId(begun) }))).rejects.toMatchObject({
  ...
  it("mint/PUT of a part after finalize is denied", async () => {
    ...
    const finalPath = `users/alice-bola-uid/burnbar_attachments/${requireId(begun)}/final`;
    const partPath = `users/alice-bola-uid/burnbar_attachments/${requireId(begun)}/parts/0`;
    await memoryStoragePort.mintPutUrl(partPath, 64, 60);
    await putMemoryObject(partPath, 64);
    await memoryStoragePort.mintPutUrl(finalPath, 64, 60);
    await putMemoryObject(finalPath, 64);
    await runFinalize(authed({ id: requireId(begun) }));
    await expect(runMint(authed({ id: requireId(begun), partIndex: 0, contentLength: 64 }))).rejects.toMatchObject({
      code: "failed-precondition",
    });
  ...
  it("ticket download meters outbound quota", async () => {
    ...
    await putMemoryObject(finalPath, 64);
    await runFinalize(authed({ id: requireId(begun) }));
    const ticket = await runTicket(authed({ id: requireId(begun) }));
  ...
  it("refuses compose after finalize", async () => {
    ...
    await putMemoryObject(finalPath, 64);
    await runFinalize(authed({ id: requireId(begun) }));
    await expect(runCompose(authed({ id: requireId(begun) }))).rejects.toMatchObject({
      code: "failed-precondition",
    });
  });
});
