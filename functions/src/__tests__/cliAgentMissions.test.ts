...
const hoisted = vi.hoisted(() => {
  ...
  function makeDb() {
    const db = {
      ...
      runTransaction: async (fn: Parameters<typeof runFakeFirestoreTransaction>[0]) => {
...
vi.mock("../callables/computerUseSecurityFirestore.js", () => ({
  requireTrustedDeviceActionProof: vi.fn(async (args: {
    deviceId: string;
...
function requireHostWriteNonce(value: unknown): string {
  if (value && typeof value === "object" && "hostWriteNonce" in value && typeof value.hostWriteNonce === "string") {
    return value.hostWriteNonce;
  }
  throw new Error("expected hostWriteNonce");
}
...
describe("claimCliAgentMission", () => {
  ...
  it("lets exactly one Mac claim; the loser gets failed-precondition", async () => {
    ...
    expect(typeof requireHostWriteNonce(winner)).toBe("string");
...
describe("updateCliAgentMissionStatus", () => {
  ...
  beforeEach(async () => {
    ...
    hostWriteNonce = requireHostWriteNonce(claimed);
  });
...
describe("hostWriteNonce hashing", () => {
  it("stores a hash, not the raw nonce", async () => {
    ...
    const hostWriteNonce = requireHostWriteNonce(claimed);
    expect(doc?.hostWriteNonceHash).toBe(
      createHash("sha256").update(hostWriteNonce, "utf8").digest("hex"),
    );
    expect(doc?.hostWriteNonceHash).not.toBe(hostWriteNonce);
  });
});
...
describe("requireTrustedDeviceActionProof platforms", () => {
  it("create uses phone platforms and claim uses macOS", async () => {
    ...
    const claimCall = mocked.mock.calls.at(-1)?.[0];
