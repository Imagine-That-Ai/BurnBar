...
const V1_OPERATIONS = new Set(["claude_quota", "codex_quota", "cursor_quota", "anthropic_quota"]);
...
function parseSample(raw: unknown, nowMillis: number, index: number): DomainCoreShadowSample {
  ...
  if (raw.schemaVersion === DOMAIN_CORE_SHADOW_SAMPLE_LEGACY_SCHEMA_VERSION) {
    if (raw.domain !== "quota" || typeof raw.consumer !== "string" || !V1_CONSUMERS.has(raw.consumer) || !V1_OPERATIONS.has(raw.operation)) {
