...
export const DOMAIN_CORE_SHADOW_SAMPLE_SCHEMA_VERSION = 2;
export const DOMAIN_CORE_SHADOW_SAMPLE_LEGACY_SCHEMA_VERSION = 1;
...
export type DomainCoreShadowConsumerV2 = "apple" | "android" | "windows" | "console" | "functions";
...
export type DomainCoreShadowSample = DomainCoreShadowSampleV1 | DomainCoreShadowSampleV2;
...
function isConsumerV2(value: unknown): value is DomainCoreShadowConsumerV2 {
  return (
    value === "apple" || value === "android" || value === "windows" || value === "console" || value === "functions"
  );
}

function isDomainV2(value: unknown): value is DomainCoreShadowSampleV2["domain"] {
...
function validV2Identity(
  ...
  return REQUIRED_COVERAGE[domain]?.[slice]?.includes(consumer) === true;
}
...
function parseSample(raw: unknown, nowMillis: number, index: number): DomainCoreShadowSample {
  ...
  if (
    !isDomainV2(raw.domain) ||
    typeof raw.slice !== "string" ||
    !isConsumerV2(raw.consumer) ||
    !validV2Identity(raw.domain, raw.slice, raw.consumer)
  ) {
...
function isFirestore(store: Firestore | DomainCoreShadowStore): store is Firestore {
  return "collection" in store;
}

export async function persistDomainCoreShadowSamples(
  firestore: Firestore | DomainCoreShadowStore,
  samples: DomainCoreShadowSample[],
  nowMillis: number,
): Promise<{ accepted: number; duplicates: number }> {
  const store = isFirestore(firestore) ? domainCoreShadowStore(firestore) : firestore;
  return store.runTransaction(async (transaction) => {
    const refs = samples.map((sample) => store.doc(`${DOMAIN_CORE_SHADOW_COLLECTION}/${sample.sampleId}`));
