...
function seedsForEndpoint(entry) {
  ...
  if (name.includes("HermesGateway") || name.includes("hermesGateway")) {
    if (
      ...
      name.includes("Grant") ||
      name.includes("Oversight") ||
      name.includes("enqueue")
    ) {
    ...
    if (name.includes("approveHermesGatewayDeviceGrant")) {
      push(globalPath(`hermes_gateway_device_sessions/${PROBE.deviceCode}`), {
        uid: BOB,
        expiresAt: "__FUTURE_TIMESTAMP__",
      });
    }
  }
...
const registry = Object.fromEntries(runtimeEntries.map((entry) => [entry.exportedName, seedsForEndpoint(entry)]));
...
function canInlineArray(value) {
  ...
  return (
    value.length === 0 || value.every((item) => item === null || ["string", "number", "boolean"].includes(typeof item))
  );
}
...
function resolveVictimData(data: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(data)) {
    if (value === "__BOB_UID__") {
      resolved[key] = BOB_UID;
    } else if (value === "__FUTURE_TIMESTAMP__") {
      resolved[key] = Timestamp.fromMillis(Date.now() + 60_000);
    } else {
      resolved[key] = value;
    }
  }
