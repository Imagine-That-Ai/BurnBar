...
function signalAtRestEnabledDomains(reg) {
  return [
    ...new Set(
      reg.domains
        .filter((d) => d.sealingScheme === SIGNAL_AT_REST_SCHEME)
        .map((d) => d.id)
...
function signalAtRestRequiredCollections(reg) {
  return [
    ...new Set(
      reg.domains
        .filter((d) => d.sealingScheme === SIGNAL_AT_REST_SCHEME)
        .flatMap((d) => d.signalSealedCollections ?? []),
    ),
  ].sort();
}

export const FUNCTIONS_SIGNAL_AT_REST_PATH = join(
  HERE,
  "..",
  "..",
  "functions",
  "src",
  "generated",
  "signalAtRestRequiredCollections.ts",
);

export function emitFunctionsSignalAtRestRequiredCollections(reg) {
  const enabledDomains = signalAtRestEnabledDomains(reg);
  const collections = signalAtRestRequiredCollections(reg);
  const lines = [
    ...
    "",
    `export const GENERATED_SIGNAL_AT_REST_SCHEME = ${JSON.stringify(SIGNAL_AT_REST_SCHEME)};`,
    "",
    "export const SIGNAL_AT_REST_ENABLED_DOMAINS = " +
      JSON.stringify(enabledDomains, null, 2) +
...
function main() {
  ...
  mkdirSync(dirname(FUNCTIONS_SIGNAL_AT_REST_PATH), { recursive: true });
  writeFileSync(FUNCTIONS_SIGNAL_AT_REST_PATH, emitFunctionsSignalAtRestRequiredCollections(reg));
  ...
  console.log(
    `[data-domains] generated ${Object.keys(files).length} platform files + Functions policy + website trust module for ${reg.domains.length} domains.`
  );
}
