...
export class InMemorySessionStore extends SessionStore {
  ...
  restore(entries: Array<{ name: string; deviceId: number; recordB64: string }>): void {
    for (const entry of entries) {
      this.sessions.set(
        addressKey(ProtocolAddress.new(entry.name, entry.deviceId)),
        SessionRecord.deserialize(Buffer.from(entry.recordB64, 'base64')),
      );
    }
  }
}
...
export class InMemoryPreKeyStore extends PreKeyStore {
  ...
  restore(entries: Array<{ id: number; recordB64: string }>): void {
    for (const entry of entries) {
      this.preKeys.set(
        entry.id,
        PreKeyRecord.deserialize(Buffer.from(entry.recordB64, 'base64')),
      );
    }
  }
}
...
export class InMemorySignedPreKeyStore extends SignedPreKeyStore {
  ...
  restore(entries: Array<{ id: number; recordB64: string }>): void {
    for (const entry of entries) {
      this.signedPreKeys.set(
        entry.id,
        SignedPreKeyRecord.deserialize(Buffer.from(entry.recordB64, 'base64')),
      );
    }
  }
}
