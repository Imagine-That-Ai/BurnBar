...
export class BackdropEngine {
  ...
  /**
   * Native embedders (the macOS/iOS WKWebView backdrop, the Tauri shell) call
   * this when the hosting window's occlusion state changes. `document.hidden`
   * never fires for a window that is merely covered by another window or
   * minimized, so without this hook the rAF loop keeps burning GPU/CPU behind
   * fully occluded windows. Fully stops the loop (not just early-returns) so
   * an occluded backdrop costs ~0; restarting is seamless — same pattern as
