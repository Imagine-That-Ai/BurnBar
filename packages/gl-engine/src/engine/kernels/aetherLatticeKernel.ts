/**
 * Aether Lattice kernel — "Aether": a hybrid of volumetric raymarching and
 * quasicrystal interference. The quasicrystal field (N equally-rotated cosine
 * waves with golden-angle phases) modulates the 3D density of a volumetric
 * medium during raymarch integration. Light scatters through the medium with
 * Beer–Lambert extinction, producing luminous, aperiodic lattice shafts that
 * breathe and spin like a living hologram.
 *
 * Novelty: instead of using fbm noise as the density field (volumetric) or
 * rendering the quasicrystal as a 2D interference pattern (moiré), the QC
 * value drives the 3D density sampled at each march step — a true hybrid.
 *
 * Performance: shipped at 32 steps (proven 60fps envelope on integrated
 * GPUs). For mid-tier Android targeting 30-45fps, reduce MARCH_STEPS to 20.
 * The shell-weight early-out (W_FLOOR) collapses most steps to a cheap scalar
 * evaluation when the QC field is near zero.
 *
 * Pure fragment shader via {@link createShaderKernel}; no per-frame JS.
 * Opts into the shared scroll control block via `controls:["scroll"]` (D2).
 */

import { createShaderKernel } from "../gl/createShaderKernel";
import type { Kernel, KernelId } from "../types";
import { QC_FREQ, QC_GOLDEN, QC_WAVES } from "./quasicrystalWaves";
...
float quasicrystal3D(vec3 pos, float freq, float spin, float scrub){
  float q = 0.0;
  for (int j = 0; j < ${QC_WAVES}; j++){
    float a  = QC_PI * float(j) / float(${QC_WAVES}) + spin;
    vec3  k  = vec3(cos(a), sin(a), 0.0) * freq; // 2D wave basis extended into Z
    float ph = float(j) * ${QC_GOLDEN.toFixed(10)};
...
vec3 renderKernel(vec2 uv, vec2 fragCoord){
  ...
  float freq   = ${QC_FREQ.toFixed(1)} * (0.9 + 0.2 * breath);
...
export function createAetherLatticeKernel(): Kernel {
  return createShaderKernel({
    id: "aether-lattice" as KernelId,
    label: "Aether Lattice",
    body: BODY,
    // D7 opt-in → host pushes D2's shared uScroll / uScrollVel block.
    controls: ["scroll"],
  });
}
