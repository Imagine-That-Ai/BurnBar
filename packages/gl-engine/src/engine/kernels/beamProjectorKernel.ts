/**
 * Beam Projector kernel — "Beacon": a calm, elegant searchlight over a deep
 * night sky. The projector sits dead-still off-canvas at the bottom-center; the
 * beam's aim DRIFTS toward the pointer (heavily eased — it follows, never
 * snaps) so moving the cursor slowly sweeps the cone across the canvas. With no
 * pointer it holds a single near-static beam with the faintest breathe.
 *
 * Layered volumetrics: a soft additive cone (fog in-scatter — the body of the
 * shaft), a small set of steady crepuscular god-rays, a lens bloom at the
 * source, and an fbm fog field that brightens where the beam passes. Dark
 * theme: cold-blue in-scatter over ink, filmic knee; light theme: a pale
 * luminous column that never blows out the pearl.
 *
...
export function createBeamProjectorKernel(): Kernel {
