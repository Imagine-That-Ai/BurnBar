/**
 * Fluid Aurora Gradient kernel — 2026's most widely adopted WebGL background effect.
 *
 * A full-screen fragment shader that paints slow, luminous fluid ribbons
 * advected by low-frequency simplex noise. The effect is "fluid aurora" —
 * the dominant mainstream background trend in 2026 (seen on Framer, Awwwards,
 * Shadertoy hot lists, and every landing-page builder). It is essentially
 * the aurora kernel distilled to its visual essence: domain-warped noise
 * flowing through a palette ramp, with a subtle film-grain dither.
 *
 * Runnable as a single-pass shader via the site's `createShaderKernel` factory.
 */
...
export function createFluidAuroraKernel(): Kernel {
