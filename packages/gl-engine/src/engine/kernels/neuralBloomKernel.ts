/**
 * Neural Bloom kernel — a generative AI-to-shader pipeline background.
 *
 * Research: the Agent 7 session explored text-to-GLSL pipelines (ShaderToy AI,
 * neural style transfer in fragment shaders). The winning approach is a
 * "neural bloom" shader: domain-warped FBM interpreted as a latent feature map,
 * then colourized through a palette-mixing network (implemented as a small
 * multi-layer perceptron in GLSL) to produce organic, ever-shifting colour
 * fields that resemble neural style transfer without needing a texture or
 * external model weights.
 *
 * The shader uses 3-octave FBM as a "latent" field, feeds it through a 2-layer
 * MLP (4 hidden neurons, sigmoid activation) to map spatial noise into
 * palette coordinates, then composites a slow-moving "bloom" of accent colours
 * with a dark/light theme branch. Pointer interaction warps the latent space
 * locally, creating a "neural attention" spot. The MLP is tiny — just a handful
 * of mat4/dot products — so it runs at 60fps on mid-tier Android.
 *
 * Distinct from existing kernels:
 *   - aurora / fluid-aurora: domain-warped FBM ribbons without the MLP palette mapping.
 *   - mesh / blobs-mesh: gradient anchors blended by inverse-square weights.
 *   - retro-plasma: four-sine interference field.
 *   - liquid-lumen: fusing metaball charges.
 *   - cloudfield: raymarched volumetric clouds.
 *   This kernel is the only one that treats FBM as a latent feature map and
 *   passes it through a learned-style palette network, giving it a distinctly
 *   "AI-generated" organic colour behaviour.
 *
 * Single-pass GLSL ES 3.00 via {@link createShaderKernel}; RGBA8, no sim, no
 * float target — same renderable set as aurora / mesh / retro-plasma.
 * Mobile budget: the MLP is 4 hidden units (cheap), FBM is 3 octaves, and the
 * loop unrolls well. Should hold 60fps on Adreno 610 / Mali-G57.
 */
...
export function createNeuralBloomKernel(): Kernel {
