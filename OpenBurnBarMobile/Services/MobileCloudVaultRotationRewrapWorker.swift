...
private protocol MobileCloudVaultRotationBlobClient {
    func beginEncryptedSessionBlobUpload(documentID: String, bodyHash: String, byteCount: Int) async throws -> MobileCloudVaultRotationUploadTicket
    func uploadEncryptedBody(data: Data, ticket: MobileCloudVaultRotationUploadTicket) async throws
    func commitEncryptedSearchIndex(deviceId: String, indexVersion: Int, document: [String: Any], chunks: [[String: Any]]) async throws
...
private struct FirebaseMobileCloudVaultRotationBlobClient: MobileCloudVaultRotationBlobClient {
    ...
    func beginEncryptedSessionBlobUpload(documentID: String, bodyHash: String, byteCount: Int) async throws -> MobileCloudVaultRotationUploadTicket {
        ...
        return MobileCloudVaultRotationUploadTicket(storagePath: storagePath, uploadURL: uploadURL)
    }
...
struct MobileCloudVaultRotationRewrapWorker {
    ...
    private var blobClient: MobileCloudVaultRotationBlobClient = FirebaseMobileCloudVaultRotationBlobClient()
    ...
    private func rewrapSessionLogStorage(
        ...
        while true {
            ...
            for document in snapshot.documents {
                ...
                guard let aad = try? CloudVaultAADContext(uid: uid, collection: "session_logs", docID: document.documentID, field: "sealedBody") else {
                    continue
                }
                let sealedData = try await blobClient.downloadEncryptedBody(storagePath: storagePath)
                ...
                do {
                    ...
                    do {
                        ...
                        NSLog("CloudVault mobile rewrap could not open session blob \(storagePath) with new generation either: \(rotatedOpenError)")
                    }
                    throw oldOpenError
                }
                ...
                let uploadTicket = try await blobClient.beginEncryptedSessionBlobUpload(
                    documentID: document.documentID,
                    bodyHash: newBodyHash,
                    byteCount: resealedData.count
                )
                try await blobClient.uploadEncryptedBody(data: resealedData, ticket: uploadTicket)
                ...
                for (index, chunk) in chunks.enumerated() {
                    let snippet = chunk.replacingOccurrences(of: "\n", with: " ").trimmingCharacters(in: .whitespacesAndNewlines)
                    ...
                    cloudSearchChunks.append([
                        ...
                        "tokenHashes": try CloudVaultCrypto.searchIndexTokenHashes(for: searchText, keyData: newKeyData, limit: Self.cloudSearchChunkTokenHashLimit),
                ...
                try await blobClient.commitEncryptedSearchIndex(
                    deviceId: deviceId,
                ...
                if uploadTicket.storagePath != storagePath {
                    do {
                        try await blobClient.deleteEncryptedSessionBlob(documentID: document.documentID, storagePath: storagePath)
                    } catch {
                        NSLog("CloudVault mobile rewrap could not delete superseded session blob \(storagePath): \(error)")
                    }
                }
                rewrapped += 1
            }

            try await jobRef.collection("checkpoints").document("session_logs_storage").setData([
        ...
        return MobileCloudVaultRotationRewrapProgress(scannedDocuments: 0, rewrappedDocuments: 0, changedFields: 0, scannedStorageBlobs: scanned, rewrappedStorageBlobs: rewrapped)
    }

    private func titlePlaintext(from raw: Any?, uid: String, docID: String, oldKeyData: Data, newKeyData: Data) -> String? {
