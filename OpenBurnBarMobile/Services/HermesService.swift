...
final class HermesService {
    ...
    private func streamCompletion(context: String?, iteration: Int = 0) async throws {
        ...
        do {
            for try await line in stream.lines {
                ...
                for event in HermesStreamingEngine.consumeSSELine(line, eventLines: &eventLines) {
                    streamingEngine.processSSEPayload(event, into: &assistantMessage)
                }
            }
        } catch {
        ...
        if !eventLines.isEmpty {
            streamingEngine.processSSEPayload(eventLines.joined(separator: "\n"), into: &assistantMessage)
        }
    ...
    private func streamRelayCompletion(context: String?, iteration: Int = 0) async throws {
        ...
        do {
            try await relayTransport.sendStreaming(
                ...
                self.streamingEngine.processSSEPayload(event, into: &assistantMessage)
            }
        } catch {
            // The streaming engine's commit throttle can hold back up
            // to ~80ms of streamed text; flush the staged message before
    ...
    // Tolerant JSON value coercion — bodies moved verbatim to
    // `HermesWireValueParsing` so `HermesStreamingEngine` shares them;
    // these wrappers keep the dozens of parse* call sites unchanged.
    ...
    private func tokenCountSourceValue(_ value: Any?) -> HermesTokenCountSource? {
        HermesWireValueParsing.tokenCountSourceValue(value)
    }
