...
//
// This is the iOS sibling of the macOS (`AgentLens`) `EasterEggScene`; the burst
// counts, durations, and physics are mirrored verbatim so every surface feels
// identical.
//
...
extension EasterEggScene {
    ...
    private static func buildBoundaryCoins(
        ...
        var result: [BoundaryCoin] = []
        result.reserveCapacity(count)
        for index in 0..<count {
            let metal: TokenMetal = index % 2 == 0 ? .gold : .silver
            let x: CGFloat = startX + CGFloat(index) * spacing
            result.append(
                BoundaryCoin(
                    symbolID: .coin(metal: metal),
                    x: x,
