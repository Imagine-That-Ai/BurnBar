...
// tracks the month's peak day cost (sqrt-scaled so one monster day doesn't
// wash the rest out), and a strip of up to three provider dots for the
// day's top spenders.
...
struct CalendarDayCell: View {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(spacing: 2.5) {
                    ForEach(Array(topProviders.prefix(3).enumerated()), id: \.offset) { _, provider in
                        Circle()
                            .fill(MobileTheme.Colors.primary(for: provider))
    ...
    /// one monster day wash the rest out (same recipe as the macOS cell).
    private var fillColor: Color {
        if isSelected {
            return accent.opacity(colorScheme == .dark ? 0.34 : 0.24)
        }
        guard cost > 0, peak > 0 else {
            return MobileTheme.Colors.surface.opacity(colorScheme == .dark ? 0.4 : 0.5)
        }
    ...
    private var ringColor: Color {
        ...
        return MobileTheme.Colors.border.opacity(0.28)
    }

    private var numberColor: Color {
        if !isInMonth { return MobileTheme.Colors.textMuted }
        return isToday ? accent : MobileTheme.Colors.textPrimary
    }
