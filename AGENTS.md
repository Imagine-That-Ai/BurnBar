...
- **Windows SharedUi shell:** the Windows app's primary window renders the same React bundle as the Linux app (`apps/linux-desktop` → `vite build --mode windows` → `dist-windows/`) hosted in WebView2; the C# `OpenBurnBar.App.SharedUi` dispatcher serves the `LinuxShellBridge` command surface from in-process stores. Contract + backing tiers: [`docs/windows-port/SHARED_UI_HOST.md`](docs/windows-port/SHARED_UI_HOST.md); tests `dotnet test windows/tests/shared-ui` (macOS-runnable).
