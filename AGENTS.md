...
label event is recorded for audit. The gate reads that label on merge-queue and `pull_request_target` runs alike, and removing the label re-runs the gate so a revoked override cannot keep an earlier green verdict current. The Mac/iOS build (`app-pr-gate.yml`, 53–69 min on macos-26) runs on `merge_group`, `main` pushes, and nightly — never on `pull_request`, and not as a required check. The breaker reuses its `main` verdicts as the post-merge proof.
...
- **Database schema:** SQLite schema reference lives in [`docs/SCHEMA_SQLITE.sql`](docs/SCHEMA_SQLITE.sql) — GENERATED from the live migrator, never hand-edited. After any GRDB migration: `swift run --package-path OpenBurnBarCore OpenBurnBarSchemaExport`, regen the DB byte-compat vector (`DatabaseByteCompatVectorTests` workflow), refresh the parity baseline (`node scripts/check-migrator-parity.mjs --update-baseline`), and extend `scripts/rollback-migration.sh` + the Windows journal. Fast-feedback's `SQLite schema doc drift` step (required via `Fast Feedback Gate`) enforces doc↔source surface equality plus a statement-for-statement lock against the vector (`scripts/ci/verify-sqlite-schema-doc.mjs`); the SwiftPM lane enforces byte equality (`OpenBurnBarSchemaExport --check`).
...
- **Circuit breakers:** Use `functions/src/resilienceHelpers.ts` (`stripeWithResilience`, `firestoreWithResilience`, `pushWithResilience`, `resilientFetch`, etc.). New provider HTTP must use `providerFetch` from `functions/src/providers/httpClient.ts`. The deploy lane (not the merge door) enforces no raw `fetch` in `functions/src` via `bash scripts/ci/verify-resilience-wiring.sh` (`deploy-production.yml`, `verify-ops-readiness.sh`); its regex misses `globalThis.fetch`, so still route every call through `providerFetch`/`resilientFetch`.
...
- **Fast CI:** `.github/workflows/fast-feedback.yml` runs lint + typecheck + unit tests in <5 min on every PR. The full macOS/iOS build (`app-pr-gate.yml`) runs on `merge_group`, `main` pushes, and nightly — never on PRs. Fix fast-feedback failures first.
