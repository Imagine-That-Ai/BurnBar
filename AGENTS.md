...
- **Database schema:** SQLite schema reference lives in [`docs/SCHEMA_SQLITE.sql`](docs/SCHEMA_SQLITE.sql) — hand-maintained, not generated. Update it alongside any GRDB migration: fast-feedback's `SQLite schema doc drift` step (required via `Fast Feedback Gate`) enforces migration-source table coverage plus PCM columns/indexes and the AgentLens↔Core mirror pairs (`scripts/ci/verify-sqlite-schema-doc.mjs`).
