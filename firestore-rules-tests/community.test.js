...
async function main() {
  ...
  await step("owner read allowed but trace write denied without L3 consent", async () => {
    await assertFails(
  ...
  await testEnv.withSecurityRulesDisabled(async (ctx) => {
    const adminDb = ctx.firestore();
    await setDoc(doc(adminDb, "users", aliceUid, "community", "consent"), {
      l2Rankings: "granted",
      l3LookingGlass: "granted",
    });
  });

  await step("owner can create and update looking_glass traces with L3 consent", async () => {
    await assertSucceeds(
      setDoc(doc(aliceDB, "users", aliceUid, "looking_glass_traces", "trace1"), { sessionId: "s" }),
    );
    await assertSucceeds(
      updateDoc(doc(aliceDB, "users", aliceUid, "looking_glass_traces", "trace1"), { sessionId: "s2" }),
    );
    await assertSucceeds(getDoc(doc(aliceDB, "users", aliceUid, "looking_glass_traces", "trace1")));
  });
  ...
  await step("revoked L3 consent blocks trace updates but permits deletion", async () => {
    await assertFails(
      updateDoc(doc(aliceDB, "users", aliceUid, "looking_glass_traces", "trace1"), { sessionId: "s3" }),
    );
    await assertSucceeds(deleteDoc(doc(aliceDB, "users", aliceUid, "looking_glass_traces", "trace1")));
  });
