...
extension Database {
    ...
    public func usePassphrase(_ passphrase: Data) throws {
        let code = passphrase.withUnsafeBytes {
            _sqlite3_key(sqliteConnection, $0.baseAddress, CInt($0.count))
        }
        guard code == SQLITE_OK else {
            throw DatabaseError(resultCode: code, message: String(cString: sqlite3_errmsg(sqliteConnection)))
        }
    ...
    public func changePassphrase(_ passphrase: Data) throws {
        // SQLCipher discourages sqlite3_rekey for large migrations; OpenBurnBar
        // uses sqlcipher_export for plaintext migration and keeps this API for
        // GRDB source compatibility.
        //
        ...
        let code = passphrase.withUnsafeBytes {
            _sqlite3_rekey(sqliteConnection, $0.baseAddress, CInt($0.count))
        }
        guard code == SQLITE_OK else {
            throw DatabaseError(resultCode: code, message: lastErrorMessage)
        }
