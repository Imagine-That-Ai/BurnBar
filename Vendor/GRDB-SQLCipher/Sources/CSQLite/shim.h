...
/// Wrapper around SQLCipher's sqlite3_key() so Swift can call it reliably
/// through the local CSQLite shim target.
static inline int _sqlite3_key(sqlite3 *db, const void *pKey, int nKey) {
    return sqlite3_key(db, pKey, nKey);
}

/// Wrapper around SQLCipher's sqlite3_rekey() so Swift can call it reliably
/// through the local CSQLite shim target.
static inline int _sqlite3_rekey(sqlite3 *db, const void *pKey, int nKey) {
