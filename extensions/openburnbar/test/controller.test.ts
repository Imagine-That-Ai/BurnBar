...
function makeConnectedClient(
  overrides: Partial<ConstructorParameters<typeof OpenBurnBarExtensionController>[0]['client']> = {}
) {
  return {
...
describe('OpenBurnBarExtensionController', () => {
  ...
  it('runs repair and refreshes daemon-backed state', async () => {
    const client = makeConnectedClient({
      health: vi.fn().mockRejectedValueOnce(new Error('socket missing')).mockResolvedValue({
        ok: true,
        daemonVersion: '0.1.0',
        protocolVersion: 1,
        socketPath: '/tmp/openburnbar.sock'
  ...
  it('VAL-EXT-002: reattaches and retries when a run RPC hits a session mismatch', async () => {
    const client = makeConnectedClient({
      createRun: vi
        .fn()
        .mockRejectedValueOnce(new Error("Client session mismatch. Expected 'old-session', received 'session-1'."))
        .mockResolvedValue({
          runID: 'run-new',
          phase: 'awaiting_approval'
        })
  ...
  it('VAL-EXT-003: observer mutation errors with claim-control guidance also trigger claim-and-retry', async () => {
    const createRun = vi
      .fn()
      .mockRejectedValueOnce(new Error('Observer client cannot mutate mission state; claim control and retry.'))
      .mockResolvedValue({
  ...
  it('maps trust-gated workspace refusals into structured daemon tool errors', async () => {
    ...
    const applyPatch = vi
      .fn()
      .mockRejectedValue(
        new OpenBurnBarWorkspaceRpcError('TRUST_REQUIRED', 'Trust this workspace before applying edits.')
      );
    const controller = new OpenBurnBarExtensionController(
      {
        client,
...
describe('buildPanelViewModel', () => {
  it('produces no-workspace state without capability chips', async () => {
    ...
    expect(vm.capabilityChips).toEqual([{ label: 'Open a folder to enable tools', kind: 'warning' }]);
  ...
  it('respects showOpenBurnBarApp host flag', async () => {
    ...
    expect(buildPanelViewModel(controller.snapshot, { showOpenBurnBarApp: true }).showOpenBurnBarApp).toBe(true);
  });
...
describe('VAL-EXT-009: Mission operator actions', () => {
  ...
  it('VAL-EXT-009: answerPendingQuestion calls daemon questionAnswer and refreshes state', async () => {
    ...
    await controller.refresh();
    const result = await controller.answerPendingQuestion('question-1', 'Proceed with the plan', 'proceed', true);
