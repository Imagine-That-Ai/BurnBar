...
export class SafariBackgroundController {
  ...
  private async executeCommand(command: NativeCommand): Promise<unknown> {
    switch (command.action) {
      ...
      case 'abort':
        await this.abortOwnedTabs('The daemon halted this Safari session.');
        return { aborted: true };
      default:
        break;
    }
  ...
  private async abortRun(): Promise<void> {
    ...
    const response = await this.bridge.popupAction('abort', {
      ...(this.snapshot.bridge.activeRunId ? { activeRunId: this.snapshot.bridge.activeRunId } : {}),
      reason: 'user_stop'
    });
    if (!response.accepted) {
      throw new SafariExtensionError('abort_rejected', 'OpenBurnBar did not accept the panic-stop request.');
    }
    this.mutate((state) => {
      state.running = false;
      state.busy = false;
      state.approvals = [];
      this.appendActivityTo(state, 'Stopped. No further Safari actions will run in this session.', 'warning');
    });
  }

  private async abortOwnedTabs(reason: string): Promise<void> {
    const owned = this.ownership.list();
    await Promise.allSettled(
      owned.map(async (entry) => {
        const tab = await this.browserAPI.tabs.get(entry.tabId);
  ...
  private async submitTurn(mode: 'ask' | 'agentic' | 'handoff', prompt: string): Promise<void> {
    ...
    if (mode === 'ask') {
      await this.submitAsk(normalizedPrompt, prepared.agent, prepared.context, prepared.screenshot);
      return;
    }
    const payload =
      mode === 'agentic'
  ...
  private applyPopupActionOutput(output: unknown): void {
    ...
    this.mutate((state) => {
      ...
      if (approvals) {
        state.approvals = approvals.slice(-MAX_APPROVALS);
      }
      if (activity) {
  ...
  private async knownTabs(): Promise<SafariTabSnapshot[]> {
    ...
    return tabs.flatMap((tab) => (typeof tab.id === 'number' ? [this.tabSnapshot(tab)] : []));
  }
  ...
  private async activeNativePage(): Promise<PageState | undefined> {
    const tab = await activeTab(this.browserAPI);
    return tab ? this.pageStateFromTab(tab) : undefined;
  }
  ...
  private clearError(): void {
    this.mutate((state) => {
      if (state.bridge.connection !== 'disconnected') {
