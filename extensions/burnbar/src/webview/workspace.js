...
function updateRailIndicator(section) {
  ...
  if (!railSections || !activeBtn) return;
...
function renderRunsSection(vm) {
  ...
  const allRuns = [];
  if (vm.activeRun) allRuns.push(vm.activeRun);
  if (vm.historyRuns) allRuns.push(...vm.historyRuns);
...
function renderSystemSection(vm) {
  ...
  const connDetail = [];
  if (vm.daemonVersion) connDetail.push("v" + vm.daemonVersion);
  if (vm.hasWorkspace) connDetail.push(vm.isWorkspaceTrusted ? "trusted" : "restricted");
  ...
  if (!vm.showOpenBurnBarApp) openAppBtn.dataset.visible = "false";
...
function renderInspector(vm) {
  ...
  if (run.providerName) addInspRow(identSection, "Provider", run.providerName);
  if (run.modelId) addInspRow(identSection, "Model", run.modelId);
  if (run.updatedAt) addInspRow(identSection, "Updated", relativeTime(run.updatedAt) + " ago");
...
window.addEventListener("message", event => {
  ...
  if (msg.type === "snapshot") {
    ...
    if (msg.section) switchSection(msg.section);
  }
});
