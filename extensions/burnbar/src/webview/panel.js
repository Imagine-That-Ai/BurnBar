...
function render(vm) {
  ...
  if (statusText) {
    if (vm.isConnected) {
      const parts = [];
      if (vm.daemonVersion) parts.push("v" + vm.daemonVersion);
      if (vm.hasWorkspace) parts.push(vm.isWorkspaceTrusted ? "trusted" : "restricted");
  ...
  if (summary) {
    if (activeRun && vm.isConnected) {
      ...
      if (meta) {
        const parts = [];
        if (activeRun.providerName) parts.push(activeRun.providerName);
        if (activeRun.modelId) parts.push(activeRun.modelId);
        if (activeRun.updatedAt) parts.push(relativeTime(activeRun.updatedAt));
