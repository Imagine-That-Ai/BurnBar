...
1. Add a case to the canonical `AgentProvider` enum in `OpenBurnBarCore/Sources/OpenBurnBarKernelModels/SharedModels/AgentProvider.swift` (set `bundledLogoName`, `iconName`, and membership in `swarmGlyphProviders` — test-enforced to cover `allCases`).
